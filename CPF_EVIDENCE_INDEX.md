# 2026-07-20 Evidence freshness 보정

현재 제공된 인덱스는 20260715_01 기준이며 최신 22818f0 완료 근거로 사용할 수 없다.

다음 최신 evidence가 필요하다.

- `work-start`에 기준 SHA `22818f0` 또는 다음 작업 시작 SHA
- `ff4661e..e969641`, `e969641..22818f0`, 통합 diff inventory
- full-test 154/354 주장 재검증
- qualityGate 87 tasks 주장 재검증
- MariaDB full install·재실행·38개 검사
- Gateway→ACC target/DB/header/trace/policy/failure runtime
- BAT JobInstance/Execution/Step/checkpoint/restart/rerun runtime
- channel policy + ADM API/UI 권한·감사
- 94 ledger report/matrix/GAP/evidence consistency
- process/port cleanup
- secret/UTF-8/mojibake/garbage/document reference scan

최신 evidence가 생성되기 전에는 과거 evidence를 완료 승격 근거로 사용하지 않는다.
...
## 추가 필수 evidence — generator/base/logging/XYZ

| evidence ID | 상태 | 필수 내용 |
|---|---|---|
| acc-generator-lifecycle | 필요 | disposable worktree ACC remove→clean→generate→build→runtime→compare |
| generator-minimal-manifest | 필요 | expected/actual generated file manifest와 unexpected file 0건 |
| generator-empty-garbage-scan | 필요 | zero-byte, placeholder, 빈 package, patch-candidates, 중간 결과 0건 |
| generator-version-id | 필요 | platform version·canonical O/S/B execution ID |
| base-hierarchy-scan | 필요 | 전체 controller/service 상속 현황과 예외 allowlist |
| base-hierarchy-runtime | 필요 | AOP/transaction/OpenAPI/Spring bean/runtime 회귀 |
| logging-aspect-inventory | 필요 | module별 `@Aspect` 파일·join point·owner·중복 결과 |
| logging-double-write-test | 필요 | PFW/domain 이중 적재와 masking 원문 유출 0건 |
| xyz-production-exclusion | 필요 | prod inventory/DB/secret/route/menu/dependency 0건 |
| xyz-reference-runtime | 필요 | local/dev build·OpenAPI·runtime sample 성공 |

---

## 추가 필수 evidence — API 사용성·Base Contract·JavaDoc

| evidence ID | 상태 | 필수 내용 |
|---|---|---|
| service-call-basic-api | 필요 | typed client 또는 contract ID 기반 1~3줄 sample |
| service-call-policy-resolution | 필요 | default/profile/domain/transaction/override effective policy |
| service-call-local-remote-parity | 필요 | 동일 typed API의 same-JVM/remote WAS 검증 |
| service-call-no-transport-leak | 필요 | business source에서 URI/HTTP/ResolvedTarget/remote lambda 0건 |
| framework-ergonomics-inventory | 필요 | 모든 공통 capability basic/advanced API 표 |
| base-contract-source-scan | 필요 | 전 계층 canonical base/contract와 예외 allowlist |
| base-contract-runtime | 필요 | bean/AOP/transaction/serialization/OpenAPI 회귀 |
| javadoc-api-coverage | 필요 | public/protected API·SPI coverage와 누락 0건 |
| javadoc-doclint | 필요 | aggregate JavaDoc와 warning/error 0건 |
| javadoc-distribution | 필요 | sourcesJar/javadocJar/offline bundle checksum |
| sample-current-api | 필요 | basic/advanced sample compile·runtime·link 검증 |

---

## 추가 필수 evidence — 3단 확장·Override·Property

| evidence ID | 상태 | 필수 내용 |
|---|---|---|
| extension-hierarchy-inventory | 필요 | 계층별 PFW/Domain/Feature chain과 예외 allowlist |
| extension-hierarchy-generator | 필요 | 신규 domain 생성물의 전체 계층 확인 |
| overload-api-compile-matrix | 필요 | 기본/고급 overload와 Java/Kotlin/Groovy ambiguity 0건 |
| override-template-security | 필요 | hook으로 보안·감사·transaction·masking 우회 불가 |
| extension-spi-test-harness | 필요 | SPI lifecycle/order/failure/default 구현 |
| config-property-inventory | 필요 | 운영 default와 code invariant 분류표 |
| config-korean-comment-scan | 필요 | 목적·기본값·단위·범위·영향·reload 설명 누락 0건 |
| config-metadata-doclint | 필요 | configuration metadata와 IDE description 검증 |
| config-profile-diff | 필요 | profile 중복 설정 0건 또는 승인 예외 |
| config-effective-adm | 필요 | effective value/origin/version/masking/audit 화면/API |
| config-reload-multi-instance | 필요 | invalid rollback과 인스턴스 일관성 |
| secret-property-scan | 필요 | repository secret 원문 0건 |
