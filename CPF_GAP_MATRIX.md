...
| --- | --- | --- | --- | --- |
| 1 | PFW Service Call Engine | 부분 구현 | 실제 HTTP 호출 adapter 통합, retry/circuit 상태 전이, timeout budget, health scheduler 연결 | runtime smoke에서 성공/실패/timeout/retry/circuit 이력 검증 |
...
| 2 | Service Registry UI | 미구현 | ADM 화면에 서비스, endpoint, instance, health, circuit, call history 조회 UX 추가 | 브라우저 또는 정적 UI marker 검증 |
