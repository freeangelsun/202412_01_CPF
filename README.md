...
Spring Batch의 `BATCH_*` 테이블은 JobRepository 원천 실행 메타이고, CPF의 `pfw_batch_*` 테이블은 ADM 관제, 스케줄, 수동 실행, 재수행, 중지, worker heartbeat, ghost 조치, lock, 운영 이력을 위한 운영 메타입니다. 별도 `bat` 모듈은 현재 만들지 않습니다. `pfw`가 공통 실행기를 제공하고 `adm`과 `xyz`가 Spring Batch 의존성과 교육 Job을 통해 E2E 흐름을 검증하는 구조가 기준입니다.
