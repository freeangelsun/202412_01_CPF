...
온라인 API는 `@CpfTransaction`, `TransactionContextFilter`, 표준 거래 헤더, Swagger 공통 헤더, `pfw_transaction_log`/`pfw_transaction_log_detail` 기반 거래 로그 저장까지 구현되어 있습니다. 별도 `pfw_transaction_meta`, 거래 메타 자동 등록, 거래별 로그 정책 override 테이블은 아직 다음 보강 대상입니다.
...
BAT runtime smoke는 정상 Job, heartbeat 진행률 Job, 실패 Job을 순서대로 실행해 Spring Batch JobRepository와 CPF 배치 메타 연결을 확인합니다.
...
- 다음 보강 우선순위는 온라인 거래 메타 자동 등록, 온라인/배치 로그 정책 override, ADM 거래/오류/감사 통합 관제, 브라우저 클릭 검증, broker 실연동 검증 순서입니다.
