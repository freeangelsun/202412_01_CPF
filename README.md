...
| Module | Code | 구분 | 적용 방식 | Responsibility |
|---|---:|---|---|---|
| `cpf-core` | `PFW` | 필수 | 기본 적용 | 기술 공통 Contract, Runtime capability, 확장 SPI |
| `cpf-gateway` | `GWY` | 선택 | 외부 진입점이 필요한 경우 적용 | 외부 진입, 라우팅, 보안 정책과 장애 격리 |
| `cpf-common` | `CMN` | 필수 | 기본 적용 | 고객 업무 공통 기능과 공통 업무 모델 |
| `cpf-admin` | `ADM` | 필수 | 기본 적용 | 플랫폼 운영, 관제, 보안, 감사와 제어 |
| `cpf-biz-admin` | `BZA` | 선택 | 고객 업무 관리자 기능이 필요한 경우 적용 | 고객 업무 관리자 화면과 업무 운영 기능 |
| `cpf-batch` | `BAT` | 선택 | Batch·Worker·Center-Cut이 필요한 경우 적용 | Batch, Scheduler, Agent, Worker와 Center-Cut |
| `cpf-member` | `MBR` | 생성형 | 필요 시 선택 사용·확장 | Generator 표준 구조를 따르는 회원  |
| `cpf-account` | `ACC` | 생성형 | 필요 시 선택 사용·확장 | Generator 표준 구조를 따르는 계좌  |
| `cpf-reference` | `REF` | 선택 | 참조·교육 용도로 필요 시 적용 | 참조 구현과 EDU  |
| `cpf-external` | `EXS` | 생성형 | 필요 시 선택 사용·확장 | Generator 표준 구조를 따르는 대외  |
