...
- ADM 기반 운영 조회·통제·복구
- 설치·배포·관측·백업 도구
- `cpf-starters` — 보안·Kafka 메시징·캐시·관측·회복성·기능 전환·비밀정보를 필요한 만큼만 조립하는 정식 루트 프로젝트군
...
선택하지 않은 제품과 기술 구현이 필수 의존성이나 기동 조건으로 포함되지 않도록 모듈과 스타터 경계를 유지합니다. 현재 루트 경로 `cpf-starters/`에는 `security`, `messaging-kafka`, `cache`, `observability`, `resilience`, `featureflag`, `secret`의 7개 프로젝트가 등록돼 있습니다. Gradle에서는 `:cpf-starter-security`처럼 단수형 논리 프로젝트명을 사용하고, 소스는 `cpf-starters/security`처럼 복수형 물리 경로에 둡니다. 각 스타터는 독립 실행 제품이 아니라 고객 응용과 CPF 실행 모듈에 선택적으로 결합되는 라이브러리입니다. 현재 생성 도구와 루트 빌드는 기능 묶음 의존성을 실제로 펼치지 않으므로 도메인은 필요한 공개 스타터를 개별 선언합니다. 기능 묶음과 통합 스타터는 정식 프로젝트·게시 좌표·생성 도구 해석·시험이 함께 제공된 뒤 사용합니다.
