...
ADM runtime smoke는 거래 메타 scan, 목록, 상세 조회까지 확인하고, 로그 정책 runtime smoke는 `dbLogEnabled=N` override 적용 시 거래 로그 DB 저장이 멈추고 해제 후 다시 저장되는지 확인합니다.
...
- 다음 보강 우선순위는 ADM 거래/오류/감사 통합 관제 고도화, 로그 정책 다중 인스턴스 broker 전파, 브라우저 클릭 검증, broker 실연동 검증, center-cut 기본 구현체 순서입니다.
