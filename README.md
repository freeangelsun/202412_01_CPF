...
| 모듈 | 역할 |
| --- | --- |
| `pfw` | 거래 ID, 표준 거래 헤더, 거래 로그 AOP, 공통 응답/예외, OpenAPI, WebClient, workflow, 보안 메타, 배치 공통 Facade |
...
| `xyz` | 개발자 교육 샘플 API |
| `bizadm` | CPF 적용 프로젝트의 업무/프로젝트 관리자 운영 기본 구현과 교육 샘플, BIZADM 전용 로그인/권한 token |
| `exs` | 대외 연계 주제영역 운영 기본 구현과 교육 샘플, 대외 token/통제 정책/재처리 |
