...
| 모듈 | 책임 |
| --- | --- |
| `pfw` | 프레임워크 코어입니다. 표준 헤더, 거래 ID, 거래/파일 로그, 공통 응답/예외, OpenAPI, HTTP client, 서비스 호출 엔진, broker/file transfer/security/runtime 공통 port, 배치 공통 API, 로그 정책, Trace Boost, PFW 자체 메시지/코드/설정을 제공합니다. |
| `cmn` | 프로젝트 공통 영역입니다. CPF를 적용하는 프로젝트에서 업무 공통 규칙, helper, fixture, EDU 샘플을 확장하는 공간입니다. 기술 engine은 PFW port를 우선합니다. |
