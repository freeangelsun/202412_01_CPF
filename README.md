...
CPF는 Java 21, Spring Boot 3.4 기반의 업무 개발 표준 프레임워크입니다. 신규 업무 모듈이 거래 추적, 표준 응답, 예외 처리, 권한, 감사, 배치, 대외 연계, 공통 업무 기능을 처음부터 다시 만들지 않도록 공통 코드와 운영 기준을 제공합니다.

README는 짧은 진입점입니다. 상세 개발 기준, 관리자 운영 절차, SQL 설치 기준, 기능 구현 상태는 `specs` 아래 HTML 가이드를 기준 문서로 봅니다.

## 핵심 원칙

- `pfw`는 프레임워크 코어 영역입니다. 거래 ID, 거래 헤더, 공통 응답/예외, 거래 로그, OpenAPI 공통 설정, WebClient, Spring Batch 공통 실행 API, 프레임워크 메시지/응답코드/설정 같은 CPF 자체 기능을 제공합니다.
- `cmn`은 프로젝트 공통 영역입니다. CPF를 적용하는 프로젝트에서 필요한 업무 공통 기능을 확장하는 위치입니다. 프로젝트별 채번, 업무 알림 로그, 업무 로그, 파일/전문/MQ 보조 기능은 프로젝트 판단에 따라 `cmn`에서 확장합니다.
- 업무 모듈은 `pfwDB` 테이블을 직접 관리하지 않습니다. 프레임워크 기능은 PFW Java API, Filter, Interceptor, AOP, Facade를 통해 사용합니다.
- 운영 기능은 UI 표시만으로 완료되지 않습니다. 서버 권한 검사, 감사 사유, before/after diff, 민감정보 마스킹, 테스트 증거가 함께 있어야 완료입니다.
- 기능을 추가하거나 변경하면 소스, SQL, Swagger, EDU 샘플, README, HTML 가이드를 함께 현행화합니다.
...
| `mbr` | 회원 업무 예제 | 회원 CRUD, 로그인, refresh token hash, 로그인 이력, 회원 상태/권한 연계 |
| `acc` | 계정 업무 예제 | 계정 조회 연계와 CPF 거래 로그 적용 예제 |
| `exs` | 대외 연계 예제 | 기관/채널/endpoint, token hash, 통제 정책, 송수신 로그, 재처리 요청 |
| `xyz` | EDU 교육 API | CRUD, 표준 응답/오류, 거래 헤더, CMN 공통 기능, 배치, 대외 호출, 전문, 파일, 보안 샘플 |

## DB 소유권

| DB | 소유 영역 | 대표 테이블 |
| --- | --- | --- |
| `pfwDB` | 프레임워크 코어와 운영 메타 | `pfw_transaction_log`, `pfw_message`, `pfw_response_code`, `pfw_config`, `pfw_batch_job`, `pfw_batch_schedule`, `pfw_batch_worker`, `pfw_batch_ghost_event`, `BATCH_*` |
...
| `exsDB` | 대외 연계 예제 | `exs_institution`, `exs_channel`, `exs_endpoint`, `exs_token_store`, `exs_transaction_log`, `exs_message_log`, `exs_retry_log` |

Spring Batch의 `BATCH_*` 테이블은 JobRepository 원천 실행 메타입니다. CPF의 `pfw_batch_*` 테이블은 ADM 관제, 스케줄, 수동 실행, 재수행, 중지, worker heartbeat, ghost 조치, lock, 운영 이력을 위한 운영 메타입니다.
...
주요 헤더는 `X-Transaction-Id`, `X-Trace-Id`, `X-Span-Id`, `X-Channel-Code`, `X-Request-Type`, `X-Client-Version`, `X-Client-Ip`, `X-Idempotency-Key`, `Authorization`입니다. 모든 API에서 전부 필수는 아니지만, 운영 추적과 장애 분석을 위해 가능한 모든 호출에서 전달하는 것을 기본으로 합니다.
...
| `specs/기능_구현_매트릭스.html` | 기능별 구현 상태, 파일 경로, 테스트 증거, 남은 리스크 |
...
- ADM UI 정적 smoke는 메뉴와 API marker 확인입니다. 실제 브라우저 클릭 검증은 자동화 환경과 시나리오가 있을 때 별도로 수행해야 합니다.
