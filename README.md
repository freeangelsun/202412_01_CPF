...
CPF는 Java 21, Spring Boot 3.4 기반의 업무 개발 표준 프레임워크입니다. 신규 업무 모듈이 거래 추적, 표준 응답, 권한, 감사, 배치, 대외연계, 공통 업무 기능을 처음부터 다시 만들지 않도록 기준 코드와 운영 가이드를 제공합니다.

README는 짧은 진입점입니다. 상세 구현 기준, 관리자 운영 절차, SQL 설치, 기능별 구현 상태는 `specs` 아래 HTML 가이드를 기준 문서로 봅니다.
...
- `cmn`은 프로젝트 공통 영역입니다. CPF를 적용하는 프로젝트에서 필요한 업무 공통 기능을 구현하는 곳입니다. 업무 채번, 업무 알림 로그, 업무 로그, 파일/전문/MQ 보조 기능은 프로젝트 판단에 따라 `cmn`에서 확장합니다.
- 업무 모듈은 `pfwDB` 테이블을 직접 관리하지 않습니다. 프레임워크 기능이 필요하면 PFW Java API, Filter, Interceptor, AOP, Facade를 통해 사용합니다.
- 운영 기능은 UI 표시만으로 완료 처리하지 않습니다. 서버 권한검사, 감사 사유, before/after diff, 민감정보 마스킹, 테스트 증거가 함께 있어야 완료입니다.
...
| `exs` | 대외연계 예제 | 기관/채널/endpoint, token hash, 통제 정책, 수신/송신 로그, 재처리 요청 |
...
| `mbrDB` | 회원 업무 예제 | `mbr_member`, `mbr_member_role`, `mbr_login_history`, `mbr_refresh_token` |
| `exsDB` | 대외연계 예제 | `exs_institution`, `exs_channel`, `exs_endpoint`, `exs_token_store`, `exs_transaction_log`, `exs_retry_log` |
...
`smoke-openapi.ps1`는 이미 기동 중인 앱을 대상으로 OpenAPI JSON을 검증합니다. `smoke-adm-runtime.ps1`는 ADM 앱을 직접 기동하고 `/adm/api/health`, OpenAPI, ADM 배치 API, ADM UI 정적 smoke를 검증한 뒤 스크립트가 시작한 프로세스를 종료합니다. 실제 브라우저 클릭 자동화 런타임이 없으면 클릭 검증은 성공으로 처리하지 않고 별도 미검증 상태로 기록합니다.

MariaDB 설치 검증은 `specs/sql/00_all_install_and_smoke.sql`로 수행합니다. 실행하지 않은 검증은 성공으로 보고하지 않고 미검증으로 남깁니다.
...
- ADM UI 정적 smoke는 메뉴와 API marker 확인입니다. 실제 브라우저 클릭 검증은 자동화 런타임과 시나리오가 있을 때 별도로 수행해야 합니다.
