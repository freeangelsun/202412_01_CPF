...
public class PfwBatchScheduleService extends cpf.adm.common.base.AdmBaseService {
    ...
    public PfwBatchScheduleService(@Qualifier("pfwJdbcTemplate") JdbcTemplate pfwJdbcTemplate) {
        this.pfwJdbcTemplate = pfwJdbcTemplate;
    }

    public List<PfwBatchScheduleCandidate> findDueSchedules(LocalDateTime serverNow) {
        List<Map<String, Object>> schedules = pfwJdbcTemplate.queryForList("""
                ...
                FROM pfw_batch_schedule s
                JOIN pfw_batch_job j ON j.job_id = s.job_id
                WHERE s.enabled_yn = 'Y'
                  AND j.use_yn = 'Y'
                ORDER BY s.schedule_id
                """);
        List<PfwBatchScheduleCandidate> dueSchedules = new ArrayList<>();
    ...
    public void updateFireTimes(PfwBatchScheduleCandidate candidate, String requestUser) {
        pfwJdbcTemplate.update("""
                ...
                candidate.nextRunAt(),
                TextUtils.defaultIfBlank(requestUser, "PfwBatchScheduler"),
                candidate.scheduleId());
    }

    private java.util.Optional<PfwBatchScheduleCandidate> buildCandidate(
        ...
        try {
            ...
            return java.util.Optional.of(new PfwBatchScheduleCandidate(
                    scheduleId,
                    jobId,
    ...
    private boolean isBusinessDay(String calendarId, LocalDate businessDate) {
        List<String> rows = pfwJdbcTemplate.queryForList("""
