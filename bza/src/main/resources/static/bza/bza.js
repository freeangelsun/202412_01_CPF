const state={accessToken:sessionStorage.getItem('bza.accessToken'),refreshToken:sessionStorage.getItem('bza.refreshToken'),operator:null,current:'dashboard'};
const menus=[
...
document.addEventListener('DOMContentLoaded',async()=>{bind();if(state.accessToken){try{await loadOperator();showApp();await route(location.hash.slice(1)||'dashboard');}catch{clearSession();}}});
...
async function raw(url,options={}){const response=await fetch(url,options);const text=await response.text();let data={};try{data=text?JSON.parse(text):{};}catch{data={message:text};}if(!response.ok){const error=new Error(data.message||`요청 실패 (${response.status})`);error.status=response.status;throw error;}return data;}

async function route(name){const allowed=visibleMenus();state.current=allowed.some(([id])=>id===name)?name:'dashboard';document.querySelectorAll('[data-route]').forEach(button=>button.classList.toggle('active',button.dataset.route===state.current));const label=allowed.find(([id])=>id===state.current)?.[1]||'대시보드';$('pageTitle').textContent=label;$('pageDescription').textContent='BZA 업무 백오피스 운영';$('content').innerHTML='<div class="empty">조회 중...</div>';try{await loaders[state.current]();}catch(error){$('content').innerHTML=`<div class="error">${escapeHtml(error.message)}</div>`;}}
...
function escapeHtml(input){return value(input).replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));}
function escapeAttribute(input){return String(input??'').replace(/[&<>'"]/g,char=>({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[char]));}
