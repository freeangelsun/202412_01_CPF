...
const required = [
  ...
  ["admInvokeOperation<Record<string, unknown>>('admApprovalRequest'", api],
  ["admInvokeOperation<Record<string, unknown>>('admApprovalRequestDetail'", api],
  ['admInvokeOperation<Record<string, unknown>>(contract.operationId', api],
...
if (api.includes("request(`/adm/api/center-cut/executions/")) {
  throw new Error('Center-Cut mutation must use generated operation consumer, not raw request')
}
