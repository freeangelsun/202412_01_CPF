export const accessMethods: Record<string, any> = {
  ...
  async viewOperatorRaw(operator: any) {
        const reason = window.prompt("연락처 원문 조회 사유를 입력하세요.");
        if (!reason) return;
        this.operatorResult = await this.sendJson(`/adm/api/operators/${encodeURIComponent(operator.operatorId)}/contacts/raw`, "POST", { reason });
      },
  async activateOperator(operator: any) {
        const reason = window.prompt("계정 활성화 사유를 입력하세요.");
        if (!reason) return;
