...
export const observabilityMethods = {
  ...
  async registerLogLevelRule() {
        ...
        const targetId = String(this.logLevelForm.transactionId || this.logLevelForm.businessTransactionId).trim();
        this.logLevelResult = await admApprovalRequest({ data: {
          requestKey: `dynamic-log-register-${crypto.randomUUID()}`,
          actionType: "DYNAMIC_LOG_REGISTER",
          ownerModule: "CPF-PLATFORM-OBSERVABILITY",
          ownerCommand: "DYNAMIC_LOG_REGISTER",
          targetType: "DYNAMIC_LOG_LEVEL",
          targetId,
          payloadSnapshot: JSON.stringify({
            businessTransactionId: this.logLevelForm.businessTransactionId || null,
            transactionId: this.logLevelForm.transactionId || null,
            logLevel: this.logLevelForm.logLevel || "DEBUG",
            ttlSeconds: Number(this.logLevelForm.ttlSeconds)
          }),
          reason: this.logLevelForm.reason
        } });
        this.setMessage("동적 로그 규칙 등록 승인 요청을 생성했습니다. 독립 승인 후 승인 화면에서 실행하세요.");
      },
