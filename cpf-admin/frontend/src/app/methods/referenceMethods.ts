...
export const referenceMethods = {
  ...
  async requestCacheApproval(command: "CACHE_REFRESH" | "CACHE_EVICT_KEY" | "CACHE_EVICT_NAMESPACE" | "CACHE_RECONCILE", targetId: string, payload: Record<string, unknown>) {
        const requestKey = `cache-${command.toLowerCase()}-${crypto.randomUUID()}`;
        const result = generatedData<Record<string, unknown>>(await admApprovalRequest({
          requestKey,
          actionType: command,
          ownerModule: "CPF-DATA-CACHE",
        ...
        this.setMessage(id
          ? `Cache 위험조치 승인 요청 ${id}가 생성되었습니다. 독립 승인 후 승인 화면에서 실행하세요.`
          : "Cache 위험조치 승인 요청이 생성되었습니다. 독립 승인 후 승인 화면에서 실행하세요.");
        return result;
      },
  async refreshCache(target: string) {
        ...
        await this.requestCacheApproval("CACHE_REFRESH", target, { target });
      },
  ...
  async evictCacheKey() {
        ...
        const tenantId = this.cacheControl.tenantId || "DEFAULT";
        await this.requestCacheApproval("CACHE_EVICT_KEY", `${tenantId}:${this.cacheControl.namespace}:${this.cacheControl.key}`, {
          tenantId,
  ...
  async evictCacheNamespace() {
        if (!this.cacheControl.namespace
            ...
            || !this.requireReason(this.cacheReason)) return;
        const tenantId = this.cacheControl.tenantId || "DEFAULT";
        await this.requestCacheApproval("CACHE_EVICT_NAMESPACE", `${tenantId}:${this.cacheControl.namespace}`, {
          tenantId,
          namespace: this.cacheControl.namespace,
          version: Number(this.cacheControl.version)
        });
  ...
  async reconcileCache() {
        ...
        await this.requestCacheApproval("CACHE_RECONCILE", "DURABLE", {});
      },
  messagePayload() {
        return {
  ...
  validateResponseCodeForm() {
        const code = this.responseCodeForm.responseCode || "";
        if (!/^[SE][A-Z]{3}[0-9]{6}$/.test(code)) return "응답코드는 EEDU010001 또는 SEDU000000 형식이어야 합니다.";
