<template>
  <div v-if="open" class="danger-dialog-backdrop" role="presentation" @click.self="cancel">
    <section class="danger-dialog" role="dialog" aria-modal="true" :aria-labelledby="`${dialogId}-title`">
      ...
      <label v-if="approvalRequired">승인 ID <input v-model.trim="form.approvalId" required placeholder="승인된 요청 ID"></label>
...
function submit(){if(validationMessage.value)return;emit("confirm",{reason:form.reason,approvalId:form.approvalId,expectedVersion:props.expectedVersion,idempotencyKey:crypto.randomUUID()});}
</script>
<style scoped>
