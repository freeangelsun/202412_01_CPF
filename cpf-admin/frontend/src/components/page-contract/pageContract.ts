...
const PAGED_GROUPS = new Set(["tracing", "connectivity", "batch", "observability", "change", "reference", "security"]);
const DETAIL_GROUPS = new Set(["tracing", "connectivity", "batch", "observability", "change", "reference", "security"]);
...
export function validateRiskActionReason(contract: AdmCommercialPageContract, action: string, reason: string): string {
  const normalizedReason = reason.trim();
  if (contract.requiresActionConfirmation && normalizedReason.length < 5) {
    throw new Error(`${action} 위험조치 사유는 5자 이상 입력해야 합니다.`);
  }
