...
import {isReferenceType,validateParameter,visible,type ParameterDefinition} from './parameterSchema'
...
function inputType(d:ParameterDefinition){if(d.sensitive||d.type==='SECRET_REFERENCE')return'password';if(['INTEGER','LONG','DECIMAL'].includes(d.type))return'number';if(d.type==='DATE')return'date';if(d.type==='DATETIME')return'datetime-local';return'text'}
</script>
<template><div class="cpf-parameter-form">
 <template v-for="d in ordered" :key="d.name">
  <label v-if="visible(d,modelValue)" :class="{wide:d.type==='JSON_OBJECT',invalid:errors[d.name]}">
   ...
   <div v-else-if="isReferenceType(d.type)" class="reference-input"><input :type="inputType(d)" :value="String(modelValue[d.name]??d.defaultValue??'')" :disabled="disabled" :placeholder="d.placeholder||`${d.referenceType||d.type} Reference`" @input="update(d.name,($event.target as HTMLInputElement).value)"><span>{{d.referenceType||d.type}}</span></div>
   <input v-else :type="inputType(d)" :value="String(modelValue[d.name]??d.defaultValue??'')" :disabled="disabled" :placeholder="d.placeholder" :min="d.minValue??undefined" :max="d.maxValue??undefined" :minlength="d.minLength??undefined" :maxlength="d.maxLength??undefined" @input="update(d.name,($event.target as HTMLInputElement).value)">
