...
export default defineComponent({
  ...
  data(){return{
    ...
    approvalEngine:{
      ...
      idempotencyKey:crypto.randomUUID(),reason:"위험조치 승인 운영"
    }
  }},
  ...
  methods:{
    ...
    async saveApprovalEnginePolicy(){await this.runApprovalEngine(async()=>{const steps=JSON.parse(this.approvalEngine.stepsJson);if(!Array.isArray(steps)||!steps.length)throw new Error("정책 단계 JSON 배열이 필요합니다.");return admInvokeOperation("admApprovalPolicySave",{body:{policyCode:this.approvalEngine.policyCode,policyVersion:this.approvalEngine.policyVersion,policyName:this.approvalEngine.policyName,actionType:this.approvalEngine.actionType,effectiveFrom:this.approvalEngine.effectiveFrom,effectiveTo:this.approvalEngine.effectiveTo||null,enabledYn:this.approvalEngine.enabledYn,selfApprovalAllowedYn:this.approvalEngine.selfApprovalAllowedYn,breakGlassAllowedYn:this.approvalEngine.breakGlassAllowedYn,description:this.approvalEngine.description,steps,reason:this.approvalEngine.reason}});});},
    async createApprovalEngineRequest(){await this.runApprovalEngine(async()=>{JSON.parse(this.approvalEngine.payloadSnapshot);const result=await admInvokeOperation<Record<string,unknown>>("admApprovalRequest",{body:{requestKey:this.approvalEngine.requestKey,policyCode:this.approvalEngine.policyCode||null,policyVersion:this.approvalEngine.policyCode?this.approvalEngine.policyVersion:null,actionType:this.approvalEngine.actionType,ownerModule:this.approvalEngine.ownerModule,ownerCommand:this.approvalEngine.ownerCommand,targetType:this.approvalEngine.targetType,targetId:this.approvalEngine.targetId,payloadSnapshot:this.approvalEngine.payloadSnapshot,expireAt:this.approvalEngine.expireAt||null,reason:this.approvalEngine.reason}});this.approvalEngine.requestId=String(result.requestId??result.id??this.approvalEngine.requestId);return result;});},
