...
export default defineComponent({
  ...
  methods:{
    ...
    async executeApprovalEngineRequest(){if(!this.approvalEngine.requestId){this.approvalEngineError="Request ID가 필요합니다.";return;}if(!window.confirm(`승인 요청 ${this.approvalEngine.requestId}의 Owner Command를 실행하시겠습니까?`))return;await this.runApprovalEngine(async()=>unwrapGeneratedResponse(await admApprovalExecute(Number(this.approvalEngine.requestId),{reason:this.approvalEngine.reason})));},
    async reconcileApprovalEngineRequest(){if(!this.approvalEngine.requestId){this.approvalEngineError="Request ID가 필요합니다.";return;}if(!window.confirm(`UNKNOWN 승인 요청 ${this.approvalEngine.requestId}의 Owner 상태만 대조하시겠습니까? Mutation은 재실행하지 않습니다.`))return;await this.runApprovalEngine(async()=>unwrapGeneratedResponse(await admApprovalReconcile(Number(this.approvalEngine.requestId),{reason:this.approvalEngine.reason})));}
  }
});
</script>
