<template>
  <div class="page-stack">
    <section class="panel">
      <div class="panel-title"><div><h2>승인 정책·요청 엔진</h2><p class="hint">Versioned 정책, 참여자 Snapshot, ALL/ANY/N_OF_M 결정과 Owner Command 실행을 관리합니다.</p></div><div class="actions"><button @click="listApprovalEnginePolicies">정책 목록</button><button @click="loadApprovalEnginePolicy">정책 상세</button></div></div>
      ...
      <div class="actions"><button class="primary" @click="saveApprovalEnginePolicy">정책 Version 저장</button></div>
      <hr>
      <div class="filters">
...
export default defineComponent({
  ...
  methods:{
    ...
    async listApprovalEnginePolicies(){await this.runApprovalEngine(()=>admInvokeOperation("admApprovalPolicies",{query:{actionType:this.approvalEngine.actionType||undefined}}));},
    async loadApprovalEnginePolicy(){if(!this.approvalEngine.policyCode||this.approvalEngine.policyVersion<1){this.approvalEngineError="Policy Code와 Version이 필요합니다.";return;}await this.runApprovalEngine(()=>admInvokeOperation("admApprovalPolicyDetail",{path:{policyCode:this.approvalEngine.policyCode,version:this.approvalEngine.policyVersion}}));},
    async saveApprovalEnginePolicy(){await this.runApprovalEngine(async()=>{const stepsValue=JSON.parse(this.approvalEngine.stepsJson);const steps=stepsValue;if(!Array.isArray(steps)||!steps.length)throw new Error("정책 단계 JSON 배열이 필요합니다.");return admInvokeOperation("admApprovalPolicySave",{body:{policyCode:this.approvalEngine.policyCode,policyVersion:this.approvalEngine.policyVersion,policyName:this.approvalEngine.policyName,actionType:this.approvalEngine.actionType,effectiveFrom:this.approvalEngine.effectiveFrom,effectiveTo:this.approvalEngine.effectiveTo||null,enabledYn:this.approvalEngine.enabledYn,selfApprovalAllowedYn:this.approvalEngine.selfApprovalAllowedYn,breakGlassAllowedYn:this.approvalEngine.breakGlassAllowedYn,description:this.approvalEngine.description,steps,reason:this.approvalEngine.reason}});});},
    async createApprovalEngineRequest(){await this.runApprovalEngine(async()=>{parseStrictJsonObject(this.approvalEngine.payloadSnapshot,"승인 Payload");this.approvalEngine.requestPending=true;const result=await admInvokeOperation<Record<string,unknown>>("admApprovalRequest",{body:{requestKey:this.approvalEngine.requestKey,policyCode:this.approvalEngine.policyCode||null,policyVersion:this.approvalEngine.policyCode?this.approvalEngine.policyVersion:null,actionType:this.approvalEngine.actionType,ownerModule:this.approvalEngine.ownerModule,ownerCommand:this.approvalEngine.ownerCommand,targetType:this.approvalEngine.targetType,targetId:this.approvalEngine.targetId,payloadSnapshot:this.approvalEngine.payloadSnapshot,expireAt:this.approvalEngine.expireAt||null,reason:this.approvalEngine.reason}});this.approvalEngine.requestId=String(result.approvalRequestId??result.requestId??result.id??this.approvalEngine.requestId);this.approvalEngine.requestPending=false;this.approvalEngine.requestKey=crypto.randomUUID();return result;});},
    ...
    async decideApprovalEngineRequest(){if(!this.approvalEngine.requestId){this.approvalEngineError="Request ID가 필요합니다.";return;}await this.runApprovalEngine(async()=>{this.approvalEngine.decisionPending=true;const result=await admInvokeOperation("admApprovalDecision",{path:{id:this.approvalEngine.requestId},body:{action:this.approvalEngine.decision,idempotencyKey:this.approvalEngine.idempotencyKey,reason:this.approvalEngine.reason}});this.approvalEngine.decisionPending=false;this.approvalEngine.idempotencyKey=crypto.randomUUID();return result;});},
