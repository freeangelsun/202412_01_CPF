...
export default defineComponent({
  ...
  methods:{
    ...
    async listApprovalEnginePolicies(){await this.runApprovalEngine(()=>admApprovalPolicies({query:{actionType:this.approvalEngine.actionType||undefined}}));},
    async loadApprovalEnginePolicy(){if(!this.approvalEngine.policyCode||this.approvalEngine.policyVersion<1){this.approvalEngineError="Policy Code와 Version이 필요합니다.";return;}await this.runApprovalEngine(()=>admApprovalPolicyDetail({path:{policyCode:this.approvalEngine.policyCode,version:this.approvalEngine.policyVersion}}));},
    async saveApprovalEnginePolicy(){await this.runApprovalEngine(async()=>{const stepsValue=JSON.parse(this.approvalEngine.stepsJson);const steps=stepsValue;if(!Array.isArray(steps)||!steps.length)throw new Error("정책 단계 JSON 배열이 필요합니다.");return admApprovalPolicySave({data:{policyCode:this.approvalEngine.policyCode,policyVersion:this.approvalEngine.policyVersion,policyName:this.approvalEngine.policyName,actionType:this.approvalEngine.actionType,effectiveFrom:this.approvalEngine.effectiveFrom,effectiveTo:this.approvalEngine.effectiveTo||null,enabledYn:this.approvalEngine.enabledYn,selfApprovalAllowedYn:this.approvalEngine.selfApprovalAllowedYn,breakGlassAllowedYn:this.approvalEngine.breakGlassAllowedYn,description:this.approvalEngine.description,steps,reason:this.approvalEngine.reason}});});},
    async createApprovalEngineRequest(){await this.runApprovalEngine(async()=>{parseStrictJsonObject(this.approvalEngine.payloadSnapshot,"승인 Payload");this.approvalEngine.requestPending=true;const result=await admApprovalRequest<Record<string,unknown>>({data:{requestKey:this.approvalEngine.requestKey,policyCode:this.approvalEngine.policyCode||null,policyVersion:this.approvalEngine.policyCode?this.approvalEngine.policyVersion:null,actionType:this.approvalEngine.actionType,ownerModule:this.approvalEngine.ownerModule,ownerCommand:this.approvalEngine.ownerCommand,targetType:this.approvalEngine.targetType,targetId:this.approvalEngine.targetId,payloadSnapshot:this.approvalEngine.payloadSnapshot,expireAt:this.approvalEngine.expireAt||null,reason:this.approvalEngine.reason}});this.approvalEngine.requestId=String(result.approvalRequestId??result.requestId??result.id??this.approvalEngine.requestId);this.approvalEngine.requestPending=false;this.approvalEngine.requestKey=crypto.randomUUID();return result;});},
    async loadApprovalEngineRequest(){if(!this.approvalEngine.requestId){this.approvalEngineError="Request ID가 필요합니다.";return;}await this.runApprovalEngine(()=>admApprovalRequestDetail({path:{id:this.approvalEngine.requestId}}));},
    async decideApprovalEngineRequest(){if(!this.approvalEngine.requestId){this.approvalEngineError="Request ID가 필요합니다.";return;}await this.runApprovalEngine(async()=>{this.approvalEngine.decisionPending=true;const result=await admApprovalDecision({path:{id:this.approvalEngine.requestId},data:{action:this.approvalEngine.decision,idempotencyKey:this.approvalEngine.idempotencyKey,reason:this.approvalEngine.reason}});this.approvalEngine.decisionPending=false;this.approvalEngine.idempotencyKey=crypto.randomUUID();return result;});},
    async executeApprovalEngineRequest(){if(!this.approvalEngine.requestId){this.approvalEngineError="Request ID가 필요합니다.";return;}if(!window.confirm(`승인 요청 ${this.approvalEngine.requestId}의 Owner Command를 실행하시겠습니까?`))return;await this.runApprovalEngine(()=>admApprovalExecute({path:{id:this.approvalEngine.requestId},query:{reason:this.approvalEngine.reason}}));},
    async reconcileApprovalEngineRequest(){if(!this.approvalEngine.requestId){this.approvalEngineError="Request ID가 필요합니다.";return;}if(!window.confirm(`UNKNOWN 승인 요청 ${this.approvalEngine.requestId}의 Owner 상태만 대조하시겠습니까? Mutation은 재실행하지 않습니다.`))return;await this.runApprovalEngine(()=>admApprovalReconcile({path:{id:this.approvalEngine.requestId},query:{reason:this.approvalEngine.reason}}));}
  }
});
</script>
