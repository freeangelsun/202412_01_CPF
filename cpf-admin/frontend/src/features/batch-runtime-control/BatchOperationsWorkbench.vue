...
async function executeAction(command:{reason:string;approvalId:string;expectedVersion?:number;idempotencyKey:string}){if(!pendingAction.value)return;actionSubmitting.value=true;failure.value=null;try{const id=selectedIdentity.value;switch(pendingAction.value){case"retry":await retryExecution(id,command);break;case"stop":await stopExecution(id,command);break;case"run":await runJob(id,command);break;case"schedule-enable":await setScheduleEnabled(id,true,command);break;case"schedule-disable":await setScheduleEnabled(id,false,command);break;case"scheduler-run":await runSchedulerOnce(command);break;case"release-lock":await releaseLock({...command,lockKey:id});break;case"ghost-fail":await actGhostExecution(id,{...command,actionType:"FAIL"});break;case"ghost-abandon":await actGhostExecution(id,{...command,actionType:"ABANDON"});break;case"unknown-resolve":await resolveUnknownResult(id,"CONFIRMED_FAILURE",command);break;case"runtime-approved":await submitBatchRuntimeCommand({approvalRequestId:command.approvalId,reason:command.reason});break;}pendingAction.value=null;await load();}catch(error){failure.value=classifyAdmFailure(error);}finally{actionSubmitting.value=false;}}
