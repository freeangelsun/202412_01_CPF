<template>
  <section class="cpf-page batch-workbench" :data-workbench-mode="mode">
    <header class="cpf-page-heading">
      ...
      <div class="heading-actions"><button v-if="mode==='topology'" class="danger ghost" type="button" @click="prepareGlobalAction('runtime-approved')">승인 Runtime Command 실행</button><span class="freshness">{{ fetchedAt ? `조회 ${formatDate(fetchedAt)}` : '미조회' }}</span><button class="ghost" type="button" @click="load">새로고침</button></div>
    </header>
...
import {
  ...
  retryExecution, runJob, runSchedulerOnce, setScheduleEnabled, simulateSchedule, stopExecution, submitBatchRuntimeCommand,
...
const actionConfigs:Record<string,ActionConfig>={
 ...
 "runtime-approved":{id:"runtime-approved",label:"승인 Runtime Command 실행",title:"승인 완료 Runtime Command를 실행하시겠습니까?",description:"Target·Command·Requester는 immutable 승인 Snapshot에서 복원되며 현재 인증 사용자는 승인 실행자로 검증됩니다.",risk:"CRITICAL",approvalRequired:true,expectedVersionRequired:false,confirmLabel:"승인 명령 실행"}
};
...
async function executeAction(command:{reason:string;approvalId:string;expectedVersion?:number;idempotencyKey:string}){if(!pendingAction.value)return;actionSubmitting.value=true;failure.value=null;try{const id=selectedIdentity.value;switch(pendingAction.value){case"retry":await retryExecution(id,command);break;case"stop":await stopExecution(id,command);break;case"run":await runJob(id,command);break;case"schedule-enable":await setScheduleEnabled(id,true,command);break;case"schedule-disable":await setScheduleEnabled(id,false,command);break;case"scheduler-run":await runSchedulerOnce(command);break;case"release-lock":await releaseLock({...command,lockKey:id});break;case"ghost-fail":await actGhostExecution(id,{...command,actionType:"FAIL"});break;case"ghost-abandon":await actGhostExecution(id,{...command,actionType:"ABANDON"});break;case"unknown-resolve":await resolveUnknownResult(id,"CONFIRMED_FAILED",command);break;case"runtime-approved":await submitBatchRuntimeCommand({approvalRequestId:command.approvalId,reason:command.reason});break;}pendingAction.value=null;await load();}catch(error){failure.value=classifyAdmFailure(error);}finally{actionSubmitting.value=false;}}
