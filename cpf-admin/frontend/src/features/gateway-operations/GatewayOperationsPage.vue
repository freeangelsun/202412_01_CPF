...
export default defineComponent({
  ...
  methods:{
    ...
    async loadCapability(){try{const c=await this.getJson("/adm/api/gateway-registry/capability");this.capability=c||{};const catalog=c?.catalog||{};this.protocols=Array.isArray(catalog.protocols)?catalog.protocols:[];this.loadBalances=Array.isArray(catalog.loadBalancePolicies)?catalog.loadBalancePolicies:[];this.connectionTestTypes=Array.isArray(catalog.connectionTestTypes)?catalog.connectionTestTypes:[];if(this.connectionTestTypes.length&&!this.connectionTestTypes.includes(this.testType))this.testType=this.connectionTestTypes[0]}catch(e:any){this.capability={installed:false,available:false,status:"UNAVAILABLE",reason:e?.message||"Gateway Capability 조회 실패"};this.errorMessage=this.capability.reason}},
    ...
    async requestConnectionTest(){if(!this.capability.available){this.errorMessage="Gateway Control Plane Capability를 사용할 수 없습니다.";return}if(!this.selectedBindingId||this.testReason.trim().length<5){this.errorMessage="Binding과 5자 이상의 시험 사유가 필요합니다.";return;}this.testSubmitting=true;this.errorMessage="";try{const operationId=crypto.randomUUID();await this.sendJson(`/adm/api/gateway-registry/bindings/${encodeURIComponent(this.selectedBindingId)}/connection-tests`,"POST",{operationId,bindingId:this.selectedBindingId,testType:this.testType,reason:this.testReason,payloadHash:"",expiresAt:new Date(Date.now()+10*60*1000).toISOString(),requestedBy:undefined});await this.loadBindingDetails(this.selectedBindingId)}catch(e:any){this.errorMessage=e?.message||"연결시험 요청 실패"}finally{this.testSubmitting=false}},
