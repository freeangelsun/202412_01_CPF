...
import {
  admGatewayCancelConnectionTest,
  admGatewayChangeBindingState,
  admGatewayDeleteBinding,
  admGatewayDeleteServerGroup,
...
export default defineComponent({
  ...
  methods:{
    ...
    async validateBinding(){const f=this.bindingForm;if(!f)return;try{await admGatewayChangeBindingState({path:{id:f.bindingId},data:{operationId:crypto.randomUUID(),bindingId:f.bindingId,targetState:"VALIDATED",expectedVersion:Number(f.version||0),approvalId:"",reason:f.reason}});await this.loadAll()}catch(e:any){this.errorMessage=e?.message||"Binding 검증 전환 실패"}},
    async deleteGroup(){const f=this.groupForm;if(!f)return;try{await admGatewayDeleteServerGroup({path:{id:f.serverGroupId},data:{operationId:crypto.randomUUID(),expectedVersion:Number(f.version||0),reason:f.reason}});this.groupForm=null;await this.loadAll()}catch(e:any){this.errorMessage=e?.message||"Server Group 폐기 요청 실패"}},
    async deleteBinding(){const f=this.bindingForm;if(!f)return;try{await admGatewayDeleteBinding({path:{id:f.bindingId},data:{operationId:crypto.randomUUID(),expectedVersion:Number(f.version||0),reason:f.reason}});this.bindingForm=null;await this.loadAll()}catch(e:any){this.errorMessage=e?.message||"Binding 폐기 요청 실패"}},
    ...
    async cancelConnectionTest(){const current=this.connectionTestOperation;if(!current)return;try{this.connectionTestOperation=await admGatewayCancelConnectionTest<AnyRow>({path:{operationId:String(current.operationId)},data:{operationId:current.operationId,expectedVersion:Number(current.version||0),reason:this.testReason}});await this.loadBindingDetails(this.selectedBindingId)}catch(e:any){this.errorMessage=e?.message||"연결시험 취소 실패"}},
    async revalidateConnectionTest(){const current=this.connectionTestOperation;if(!current)return;try{this.connectionTestOperation=await admGatewayRevalidateConnectionTest<AnyRow>({path:{operationId:String(current.operationId)},data:{sourceOperationId:current.operationId,newOperationId:crypto.randomUUID(),payloadHash:"",expiresAt:new Date(Date.now()+10*60*1000).toISOString(),reason:this.testReason}});await this.loadBindingDetails(this.selectedBindingId)}catch(e:any){this.errorMessage=e?.message||"연결시험 재검증 실패"}},
