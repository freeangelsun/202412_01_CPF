<template>
  <section class="panel log-page">
    ...
    <div class="filters">
      ...
      <label>Client ID <input v-model="logSearch.clientId"></label><label>Original Channel <input v-model="logSearch.originalChannel" placeholder="MBR"></label><label>Current Channel <input v-model="logSearch.currentChannel" placeholder="MBR"></label><label>Caller Channel <input v-model="logSearch.callerChannel"></label><label>Target Channel <input v-model="logSearch.targetChannel"></label><label>Target Operation <input v-model="logSearch.targetOperationId"></label><label>도메인 <input v-model="logSearch.domainCode" placeholder="MEMBER"></label><label>Application <input v-model="logSearch.application"></label>
      <label>Module <input v-model="logSearch.moduleId"></label><label>WAS <input v-model="logSearch.wasId"></label><label>Instance <input v-model="logSearch.instanceId"></label><label>Host <input v-model="logSearch.hostName"></label>
      <label>Starter <input v-model="logSearch.starterId" placeholder="cpf-starter-cache-redis"></label><label>Capability <input v-model="logSearch.capabilityId" placeholder="CACHE"></label><label>Provider <input v-model="logSearch.provider" placeholder="REDIS"></label><label>Capability Operation <input v-model="logSearch.capabilityOperation"></label>
      ...
      <label>회원번호 보호검색 <input v-model="logSearch.memberNo"></label><label>고객번호 보호검색 <input v-model="logSearch.customerNo"></label><label>로그 유형 <input v-model="logSearch.logType"></label>
    </div>
    ...
    <div class="table-wrap"><table><thead><tr><th>IDX</th><th>transactionId</th><th>거래명 / URI</th><th>Module / WAS</th><th>Instance / Host</th><th>Channel 흐름</th><th>Target Operation</th><th>HTTP / 응답</th><th>시작</th><th>소요(ms)</th></tr></thead><tbody>
      <tr v-for="item in pagedLogs" :key="item.LOG_IDX||item.logIdx" :class="{selected:selectedLogIdx===(item.LOG_IDX||item.logIdx)}" @click="selectLog(item)"><td>{{ item.LOG_IDX||item.logIdx }}</td><td>{{ item.TRANSACTION_ID||item.transactionId }}</td><td>{{ item.BUSINESS_TRANSACTION_NAME||item.businessTransactionName||'-' }}<small>{{ item.URI||item.uri||'-' }}</small></td><td>{{ item.MODULE_ID||item.moduleId||'-' }} / {{ item.WAS_ID||item.wasId||'-' }}</td><td>{{ item.INSTANCE_ID||item.instanceId||'-' }}<small>{{ item.HOST_NAME||item.hostName||'-' }}</small></td><td>{{ item.ORIGINAL_CHANNEL||item.originalChannel||'-' }} → {{ item.CALLER_CHANNEL||item.callerChannel||'-' }} → {{ item.CURRENT_CHANNEL||item.currentChannel||'-' }} → {{ item.TARGET_CHANNEL||item.targetChannel||'-' }}</td><td>{{ item.TARGET_OPERATION_ID||item.targetOperationId||'-' }}</td><td>{{ item.HTTP_STATUS||item.httpStatus||'-' }} / {{ item.RESPONSE_CODE||item.responseCode||'-' }}</td><td>{{ item.START_TIME||item.startTime||'-' }}</td><td>{{ item.DURATION_MS??item.durationMs??'-' }}</td></tr>
      <tr v-if="!pagedLogs.length"><td colspan="10">조회된 거래 로그가 없습니다.</td></tr></tbody></table></div>
...
export default defineComponent({setup(){return useAdmConsolePage()},name:"LogsPage",components:{StructuredDetails},data(){return{selectedLogIdx:null as any}},computed:{
 ...
 segments():Array<Record<string,string>>{const d=this.detail||{};const failure=String(d.failureStage||d.failure_stage||d.error?.stage||"");const status=String(d.status||d.resultStatus||d.responseCode||"");return [
  {name:"IN",title:`${d.currentChannel||d.CURRENT_CHANNEL||"-"} ${d.method||d.httpMethod||""} ${d.uri||d.URI||""}`,time:String(d.receivedAt||d.startTime||d.START_TIME||""),state:failure==="IN"?"failed":"done",failure:failure==="IN"?status:""},
