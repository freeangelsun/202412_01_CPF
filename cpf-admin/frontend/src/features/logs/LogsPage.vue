<template>
  <section class="panel log-page">
    ...
    <div class="filters">
      ...
      <label>Client ID <input v-model="logSearch.clientId"></label><label>최초 System <input v-model="logSearch.originalSystemCode" placeholder="MBR"></label><label>현재 System <input v-model="logSearch.systemCode" placeholder="MBR"></label><label>Caller System <input v-model="logSearch.callerSystemCode"></label><label>Target System <input v-model="logSearch.targetSystemCode"></label><label>Target Operation <input v-model="logSearch.targetOperationId"></label><label>도메인 <input v-model="logSearch.domainCode" placeholder="MEMBER"></label><label>Application <input v-model="logSearch.application"></label>
    ...
    <div class="table-wrap"><table><thead><tr><th>IDX</th><th>transactionId</th><th>거래명 / URI</th><th>Module / WAS</th><th>Instance / Host</th><th>System 흐름</th><th>Target Operation</th><th>HTTP / 응답</th><th>시작</th><th>소요(ms)</th></tr></thead><tbody>
      <tr v-for="item in pagedLogs" :key="item.LOG_IDX||item.logIdx" :class="{selected:selectedLogIdx===(item.LOG_IDX||item.logIdx)}" @click="selectLog(item)"><td>{{ item.LOG_IDX||item.logIdx }}</td><td>{{ item.TRANSACTION_ID||item.transactionId }}</td><td>{{ item.BUSINESS_TRANSACTION_NAME||item.businessTransactionName||'-' }}<small>{{ item.URI||item.uri||'-' }}</small></td><td>{{ item.MODULE_ID||item.moduleId||'-' }} / {{ item.WAS_ID||item.wasId||'-' }}</td><td>{{ item.INSTANCE_ID||item.instanceId||'-' }}<small>{{ item.HOST_NAME||item.hostName||'-' }}</small></td><td>{{ item.ORIGINAL_SYSTEM_CODE||item.originalSystemCode||'-' }} → {{ item.CALLER_SYSTEM_CODE||item.callerSystemCode||'-' }} → {{ item.SYSTEM_CODE||item.systemCode||'-' }} → {{ item.TARGET_SYSTEM_CODE||item.targetSystemCode||'-' }}</td><td>{{ item.TARGET_OPERATION_ID||item.targetOperationId||'-' }}</td><td>{{ item.HTTP_STATUS||item.httpStatus||'-' }} / {{ item.RESPONSE_CODE||item.responseCode||'-' }}</td><td>{{ item.START_TIME||item.startTime||'-' }}</td><td>{{ item.DURATION_MS??item.durationMs??'-' }}</td></tr>
...
export default defineComponent({setup(){return useAdmConsolePage()},name:"LogsPage",components:{StructuredDetails},data(){return{selectedLogIdx:null as any}},computed:{
 ...
 segments():Array<Record<string,string>>{const d=this.detail||{};const failure=String(d.failureStage||d.failure_stage||d.error?.stage||"");const status=String(d.status||d.resultStatus||d.responseCode||"");return [
  {name:"IN",title:`${d.systemCode||d.SYSTEM_CODE||"-"} ${d.method||d.httpMethod||""} ${d.uri||d.URI||""}`,time:String(d.receivedAt||d.startTime||d.START_TIME||""),state:failure==="IN"?"failed":"done",failure:failure==="IN"?status:""},
