...
describe("maintenance workflow", () => {
  ...
  it("rejects missing target and short reason", () => { expect(() => validateMaintenanceAction({ serviceId: "", endpointCode: "API", instanceId: "I-1", action: "DRAIN", reason: "장애 대응 점검" })).toThrow(/Service/); expect(() => validateMaintenanceAction({ serviceId: "MBR", endpointCode: "API", instanceId: "I-1", action: "RESUME", reason: "짧음" })).toThrow(/8자/); });
