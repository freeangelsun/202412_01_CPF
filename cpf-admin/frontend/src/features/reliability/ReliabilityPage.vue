      ...
      <section class="panel">
        <div class="panel-title">
          ...
          <div class="actions">
            ...
            <button type="button" v-if="canWrite('RELIABILITY')" @click="replayDlq">DLQ 재처리 승인 요청</button>
        ...
        <div class="filters">
          ...
          <label>Server Instance <input v-model="reliabilitySearch.instanceId" type="text"></label>
          ...
          <label>Expected Version <input v-model.number="reliabilityAction.expectedVersion" type="number" min="0"></label>
          <label>확정 상태
            <select v-model="reliabilityAction.targetStatus">
              <option>CONFIRMED_SUCCESS</option>
              <option>CONFIRMED_FAILED</option>
