  ...
  <div class="cpf-page">
    ...
    <section v-if="activeTab==='endpoints'" class="cpf-card">
      ...
      <form v-if="endpointForm" class="cpf-form-grid" @submit.prevent="saveEndpoint">
        ...
        <label>Base URL<input v-model.trim="endpointForm.baseUrl" placeholder="서비스 Base URL을 입력하세요"></label>
...
<script lang="ts">
import{defineComponent}from"vue";import{useAdmConsolePage}from"../../app/useAdmConsolePage";import CpfIcon from"../../components/CpfIcon.vue";import{admServiceRegistrySaveService,admServiceRegistrySaveEndpoint,admServiceRegistrySaveInstance}from"../../generated/cpf-api";import{requestServiceInstanceApproval,requestServiceRegistryDeleteApproval}from"../../shared/serviceRegistryApproval";import{validateRegistryDeleteRequest,type ServiceRegistryTargetKind}from"./serviceRegistryWorkflow";
...
export default defineComponent({setup(){return useAdmConsolePage()},name:"ServiceRegistryPage",components:{CpfIcon},data(){return{tabs:[{id:"services",label:"Service"},{id:"endpoints",label:"Endpoint"},{id:"instances",label:"Instance·Health"},{id:"runtime",label:"Routing·Circuit·호출"}],activeTab:"services",serviceForm:null as R|null,endpointForm:null as R|null,instanceForm:null as R|null,localError:"",capabilities:{serviceTypes:[],endpointTypes:[],instanceStatuses:[],instanceCommands:[],environments:[]} as R,stateDialog:{open:false,instance:null as R|null,command:"",reason:"",error:"",busy:false},deleteDialog:{open:false,kind:"service" as ServiceRegistryTargetKind,target:null as R|null,targetId:"",version:0,reason:"",error:"",busy:false}}},computed:{services():R[]{return this.serviceRegistryResult?.services||[]},endpoints():R[]{return this.serviceRegistryResult?.endpoints||[]},instances():R[]{return this.serviceRegistryResult?.instances||[]},health():R[]{return this.serviceRegistryResult?.health||[]},routingPolicies():R[]{return this.serviceRegistryResult?.routingPolicies||[]},circuits():R[]{return this.serviceRegistryResult?.circuits||[]},callHistory():R[]{return this.serviceRegistryResult?.callHistory||[]},unavailableCount():number{return this.instances.filter((r:R)=>!["UP","DEGRADED","RECOVERING"].includes(String(r.status).toUpperCase())).length}},mounted(){this.loadCapabilities();this.loadServiceRegistry()},methods:{
 ...
 async confirmInstanceState(){const r=this.stateDialog.instance;if(!r||this.stateDialog.reason.trim().length<8){this.stateDialog.error="8자 이상의 승인 요청 사유가 필요합니다.";return}this.stateDialog.busy=true;this.stateDialog.error="";try{const approval=await requestServiceInstanceApproval({serviceId:r.serviceId,endpointCode:r.endpointCode,instanceId:r.instanceId,action:this.stateDialog.command,reason:this.stateDialog.reason,expectedVersion:Number(r.version)});this.localError=approval.approvalRequestId?`승인 요청 #${approval.approvalRequestId}이 생성되었습니다. 승인 화면에서 결재·실행하세요.`:"승인 요청이 생성되었습니다. 승인 화면에서 결재·실행하세요.";this.stateDialog.busy=false;this.closeStateDialog();await this.loadServiceRegistry()}catch(e:any){this.stateDialog.busy=false;this.stateDialog.error=e?.message||"Instance 상태 변경 승인 요청 실패"}},
 ...
 async confirmDelete(){const validation=validateRegistryDeleteRequest({kind:this.deleteDialog.kind,targetId:this.deleteDialog.targetId,expectedVersion:this.deleteDialog.version,reason:this.deleteDialog.reason});if(!validation.ok){this.deleteDialog.error="message" in validation?validation.message:"삭제 요청 검증 실패";return}this.deleteDialog.busy=true;this.deleteDialog.error="";try{const approval=await requestServiceRegistryDeleteApproval(this.deleteDialog.kind,this.deleteDialog.targetId,this.deleteDialog.version,this.deleteDialog.reason);this.localError=approval.approvalRequestId?`삭제 승인 요청 #${approval.approvalRequestId}이 생성되었습니다. 독립 승인 후 Owner Command로 실행됩니다.`:"삭제 승인 요청이 생성되었습니다. 독립 승인 후 Owner Command로 실행됩니다.";this.deleteDialog.busy=false;this.closeDeleteDialog();await this.loadServiceRegistry()}catch(e:any){this.deleteDialog.busy=false;this.deleteDialog.error=e?.message||"Service Registry 삭제 승인 요청 실패"}}
}});
</script>
