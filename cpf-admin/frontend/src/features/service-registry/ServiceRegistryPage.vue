<template>
  <div class="cpf-page">
    ...
    <section v-if="activeTab==='services'" class="cpf-card">
      ...
      <div class="table-wrap"><table><thead><tr><th>Service ID</th><th>서비스명</th><th>유형</th><th>Owner</th><th>상태</th><th>Version</th><th>수정시각</th></tr></thead><tbody>
      <tr v-for="r in services" :key="r.serviceId" @click="editService(r)"><td>{{r.serviceId}}</td><td>{{r.serviceName}}</td><td>{{r.serviceType}}</td><td>{{r.ownerModuleCode}}</td><td><span class="cpf-status" :class="r.enabled?'success':'danger'">{{r.enabled?'사용':'중지'}}</span></td><td>{{r.version}}</td><td>{{r.updatedAt}}</td></tr>
      <tr v-if="!services.length"><td colspan="7">조회 결과가 없습니다.</td></tr></tbody></table></div>
      <form v-if="serviceForm" class="cpf-form-grid" @submit.prevent="saveService">
        ...
        <label>유형<select v-model="serviceForm.serviceType"><option>DOMAIN</option><option>PLATFORM</option><option>EXTERNAL</option><option>MONITOR_ONLY</option></select></label>
    ...
    <section v-if="activeTab==='endpoints'" class="cpf-card">
      ...
      <div class="table-wrap"><table><thead><tr><th>Endpoint</th><th>Service</th><th>명칭</th><th>유형</th><th>주소 / Context</th><th>Timeout</th><th>Retry</th><th>Version</th></tr></thead><tbody>
      <tr v-for="r in endpoints" :key="r.endpointCode" @click="editEndpoint(r)"><td>{{r.endpointCode}}</td><td>{{r.serviceId}}</td><td>{{r.endpointName}}</td><td>{{r.endpointType}}</td><td>{{r.baseUrl}}{{r.contextPath}}</td><td>{{r.defaultTimeoutMs}} ms</td><td>{{r.defaultRetryCount}}</td><td>{{r.version}}</td></tr>
      <tr v-if="!endpoints.length"><td colspan="8">조회 결과가 없습니다.</td></tr></tbody></table></div>
    ...
    <section v-if="activeTab==='instances'" class="cpf-card">
      ...
      <form v-if="instanceForm" class="cpf-form-grid" @submit.prevent="saveInstance">
        ...
        <label>환경<select v-model="instanceForm.environmentCode"><option>DEV</option><option>TEST</option><option>PROD</option></select></label>
...
export default defineComponent({name:"ServiceRegistryPage",components:{CpfIcon},mixins:[admConsoleMixin],data(){return{tabs:[{id:"services",label:"Service"},{id:"endpoints",label:"Endpoint"},{id:"instances",label:"Instance·Health"},{id:"runtime",label:"Routing·Circuit·호출"}],activeTab:"services",serviceForm:null as R|null,endpointForm:null as R|null,instanceForm:null as R|null,localError:"",instanceStatuses:["UP","DEGRADED","RECOVERING","DOWN","DRAINING","MAINTENANCE","DISABLED","UNKNOWN","STALE"]}},computed:{services():R[]{return this.serviceRegistryResult?.services||[]},endpoints():R[]{return this.serviceRegistryResult?.endpoints||[]},instances():R[]{return this.serviceRegistryResult?.instances||[]},health():R[]{return this.serviceRegistryResult?.health||[]},routingPolicies():R[]{return this.serviceRegistryResult?.routingPolicies||[]},circuits():R[]{return this.serviceRegistryResult?.circuits||[]},callHistory():R[]{return this.serviceRegistryResult?.callHistory||[]},unavailableCount():number{return this.instances.filter((r:R)=>!["UP","DEGRADED","RECOVERING"].includes(String(r.status).toUpperCase())).length}},mounted(){this.loadServiceRegistry()},methods:{
 ...
 newService(){this.serviceForm={serviceId:"",serviceName:"",serviceType:"DOMAIN",ownerModuleCode:"",description:"",useYn:"Y",version:0,reason:"서비스 레지스트리 등록"};this.activeTab="services"},editService(r:R){if(!this.canWrite('SERVICE_REGISTRY'))return;this.serviceForm={...r,useYn:r.enabled?'Y':'N',reason:"서비스 레지스트리 변경"}},async saveService(){try{const f=this.serviceForm!;await this.sendJson("/adm/api/service-registry/services","POST",{operationId:this.op(),serviceId:f.serviceId,serviceName:f.serviceName,serviceType:f.serviceType,ownerModuleCode:f.ownerModuleCode,description:f.description,useYn:f.useYn,expectedVersion:f.version||0,reason:f.reason,requestedBy:this.operator()});this.serviceForm=null;await this.loadServiceRegistry()}catch(e:any){this.localError=e?.message||"Service 저장 실패"}},
 newEndpoint(){this.endpointForm={endpointCode:"",serviceId:this.serviceRegistrySearch.serviceId||"",endpointName:"",endpointType:"HTTP",baseUrl:"",contextPath:"",defaultTimeoutMs:10000,defaultRetryCount:0,useYn:"Y",version:0,reason:"서비스 Endpoint 등록"};this.activeTab="endpoints"},editEndpoint(r:R){if(!this.canWrite('SERVICE_REGISTRY'))return;this.endpointForm={...r,useYn:r.enabled?'Y':'N',reason:"서비스 Endpoint 변경"}},async saveEndpoint(){try{const f=this.endpointForm!;await this.sendJson("/adm/api/service-registry/endpoints","POST",{operationId:this.op(),endpointCode:f.endpointCode,serviceId:f.serviceId,endpointName:f.endpointName,endpointType:f.endpointType,baseUrl:f.baseUrl,contextPath:f.contextPath,defaultTimeoutMs:f.defaultTimeoutMs,defaultRetryCount:f.defaultRetryCount,useYn:f.useYn,expectedVersion:f.version||0,reason:f.reason,requestedBy:this.operator()});this.endpointForm=null;await this.loadServiceRegistry()}catch(e:any){this.localError=e?.message||"Endpoint 저장 실패"}},
 newInstance(){this.instanceForm={instanceId:"",serviceId:this.serviceRegistrySearch.serviceId||"",endpointCode:this.serviceRegistrySearch.endpointCode||"",instanceName:"",baseUrl:"",hostName:"",portNo:8080,environmentCode:"DEV",zoneCode:"",cellCode:"",weight:100,priorityNo:0,activeYn:"Y",maintenanceYn:"N",drainYn:"N",version:0,reason:"서비스 Instance 등록"};this.activeTab="instances"},editInstance(r:R){if(!this.canWrite('SERVICE_REGISTRY'))return;this.instanceForm={...r,portNo:r.port,priorityNo:r.priority,activeYn:r.active?'Y':'N',maintenanceYn:r.maintenance?'Y':'N',drainYn:r.draining?'Y':'N',reason:"서비스 Instance 변경"}},async saveInstance(){try{const f=this.instanceForm!;await this.sendJson("/adm/api/service-registry/instances","POST",{operationId:this.op(),instanceId:f.instanceId,serviceId:f.serviceId,endpointCode:f.endpointCode,instanceName:f.instanceName,baseUrl:f.baseUrl,hostName:f.hostName,portNo:f.portNo,environmentCode:f.environmentCode,zoneCode:f.zoneCode,cellCode:f.cellCode,weight:f.weight,priorityNo:f.priorityNo,activeYn:f.activeYn,maintenanceYn:f.maintenanceYn,drainYn:f.drainYn,expectedVersion:f.version||0,reason:f.reason,requestedBy:this.operator()});this.instanceForm=null;await this.loadServiceRegistry()}catch(e:any){this.localError=e?.message||"Instance 저장 실패"}},
 async changeInstanceState(r:R,command:string){const reason=window.prompt(`${command} 사유를 입력하세요(5자 이상)`,`${r.instanceId} ${command} 운영 조치`);if(!reason||reason.trim().length<5)return;try{await this.sendJson(`/adm/api/service-registry/services/${encodeURIComponent(r.serviceId)}/endpoints/${encodeURIComponent(r.endpointCode)}/instances/${encodeURIComponent(r.instanceId)}/state`,"POST",{command,reason});await this.loadServiceRegistry()}catch(e:any){this.localError=e?.message||"Instance 상태 변경 실패"}}
}});
</script>
