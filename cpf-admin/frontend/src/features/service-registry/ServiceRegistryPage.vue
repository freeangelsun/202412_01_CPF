...
export default defineComponent({name:"ServiceRegistryPage",components:{CpfIcon},mixins:[admConsoleMixin],data(){return{tabs:[{id:"services",label:"Service"},{id:"endpoints",label:"Endpoint"},{id:"instances",label:"Instance·Health"},{id:"runtime",label:"Routing·Circuit·호출"}],activeTab:"services",serviceForm:null as R|null,endpointForm:null as R|null,instanceForm:null as R|null,localError:"",capabilities:{serviceTypes:[],endpointTypes:[],instanceStatuses:[],instanceCommands:[],environments:[]} as R,stateDialog:{open:false,instance:null as R|null,command:"",reason:"",error:""}}},computed:{services():R[]{return this.serviceRegistryResult?.services||[]},endpoints():R[]{return this.serviceRegistryResult?.endpoints||[]},instances():R[]{return this.serviceRegistryResult?.instances||[]},health():R[]{return this.serviceRegistryResult?.health||[]},routingPolicies():R[]{return this.serviceRegistryResult?.routingPolicies||[]},circuits():R[]{return this.serviceRegistryResult?.circuits||[]},callHistory():R[]{return this.serviceRegistryResult?.callHistory||[]},unavailableCount():number{return this.instances.filter((r:R)=>!["UP","DEGRADED","RECOVERING"].includes(String(r.status).toUpperCase())).length}},mounted(){this.loadCapabilities();this.loadServiceRegistry()},methods:{
 ...
 openStateDialog(r:R,command:string){this.stateDialog={open:true,instance:r,command,reason:`${r.instanceId} ${command} 운영 조치`,error:""};this.changeInstanceState(r,command)},
 async changeInstanceState(r:R,command:string){this.stateDialog={open:true,instance:r,command,reason:`${r.instanceId} ${command} 운영 조치`,error:""}},
 closeStateDialog(){this.stateDialog={open:false,instance:null,command:"",reason:"",error:""}},
 async confirmInstanceState(){const r=this.stateDialog.instance;if(!r||this.stateDialog.reason.trim().length<5){this.stateDialog.error="5자 이상의 조치 사유가 필요합니다.";return}try{await this.sendJson(`/adm/api/service-registry/services/${encodeURIComponent(r.serviceId)}/endpoints/${encodeURIComponent(r.endpointCode)}/instances/${encodeURIComponent(r.instanceId)}/state`,"POST",{operationId:this.op(),command:this.stateDialog.command,expectedVersion:r.version,reason:this.stateDialog.reason});this.closeStateDialog();await this.loadServiceRegistry()}catch(e:any){this.stateDialog.error=e?.message||"Instance 상태 변경 실패"}}
}});
</script>
