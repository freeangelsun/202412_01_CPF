...
describe("ADM BFF 공통 API client", () => {
  ...
  it("JSON object와 JSON string의 중첩 actor alias를 차단한다", async () => {
    await expect(admRawResponse("/adm/api/runtime-control/changes", "POST", { nested: { operatorId: "browser" } })).rejects.toThrow("operatorId");
    await expect(admRawResponse("/adm/api/runtime-control/changes", "POST", '{"nested":{"requestedBy":"browser"}}')).rejects.toThrow("requestedBy");
    await expect(admRawResponse("/adm/api/runtime-control/changes", "POST", "plain text")).rejects.toThrow("raw string body is forbidden");
  });

  it("FormData와 URLSearchParams actor alias를 대소문자와 관계없이 차단한다", async () => {
    const form = new FormData(); form.set("OperatorId", "browser");
    await expect(admRawResponse("/adm/api/runtime-control/changes", "POST", form)).rejects.toThrow("OperatorId");
    const params = new URLSearchParams(); params.set("requestedBy", "browser");
    await expect(admRawResponse("/adm/api/runtime-control/changes", "POST", params)).rejects.toThrow("requestedBy");
  });

  it("일반 문자열 필드는 허용하되 JSON 문자열 내부 actor alias는 차단한다", async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response("{}", { status: 200 }));
    vi.stubGlobal("fetch", fetchMock);
    await admRawResponse("/adm/api/runtime-control/changes", "POST", { reason: "normal operation", nested: { description: "safe" } });
    await expect(admRawResponse("/adm/api/runtime-control/changes", "POST", { payload: '{"operatorId":"browser"}' })).rejects.toThrow("operatorId");
  });
  ...
  it("Blob mutation body를 검증 불가능한 privileged payload로 거부한다", async () => {
    await expect(admRawResponse("/adm/api/runtime-control/changes", "POST", new Blob(["payload"]))).rejects.toThrow("Blob body is forbidden");
  });
