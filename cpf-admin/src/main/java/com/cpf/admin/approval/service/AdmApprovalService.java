...
public class AdmApprovalService extends AdmBaseService {
    ...
    @Autowired
    /** AdmApprovalService 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    /** findPolicies 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    private Map<String,Object> requestApprovalInternal(CreateRequest request,String operatorId,AtomicBoolean replayFlag) {
        ...
        v.put("transactionId",CpfContexts.transactionId());v.put("operatorId",requester);
    ...
    /** ApprovalMutationResult 타입의 역할과 책임을 정의하며 CPF 계약 경계를 명확히 유지한다. */
    ...
    /** CreateRequest 타입의 역할과 책임을 정의하며 CPF 계약 경계를 명확히 유지한다. */
    ...
    public record DecisionRequest(@NotBlank @Size(max=20) String action,
      ...
      @NotBlank @Size(min=8,max=500) String reason,Boolean breakGlass){
        /** DecisionRequest 작업을 CPF 표준 계약에 따라 수행한다. */
