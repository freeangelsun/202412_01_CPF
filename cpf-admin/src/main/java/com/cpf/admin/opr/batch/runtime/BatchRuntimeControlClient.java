...
public class BatchRuntimeControlClient {
    ...
    public CpfDataRow schedulerTriggerReconcileApproved(
            String scheduleId,
        ...
        return row(invoke(
                HttpMethod.POST,
                "/api/v1/batch/scheduler/schedules/" + encode(required(scheduleId, "scheduleId"))
                        + "/triggers/reconcile-unknown/retry",
                request,
                new ApprovalContext(required(approvalRequestId, "approvalRequestId"),
                        required(approvalRequesterId, "approvalRequesterId"))));
    }

    public CpfDataRow schedulerTriggerState(String scheduleId, String scheduledFireAt) {
        return row(invoke(
                HttpMethod.GET,
                "/api/v1/batch/scheduler/schedules/" + encode(required(scheduleId, "scheduleId"))
                        + "/triggers?scheduledFireAt=" + encode(required(scheduledFireAt, "scheduledFireAt")),
                null));
    }
