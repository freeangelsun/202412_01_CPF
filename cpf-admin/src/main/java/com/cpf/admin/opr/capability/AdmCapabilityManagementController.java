...
public class AdmCapabilityManagementController extends AdmBaseController {
    ...
    private static List<String> csv(Map<String,String> map,String key){String text=value(map,key);if(text.isBlank())return List.of();return java.util.Arrays.stream(text.split(",")).map(token -> token.trim()).filter(v->!v.isEmpty()).toList();}
