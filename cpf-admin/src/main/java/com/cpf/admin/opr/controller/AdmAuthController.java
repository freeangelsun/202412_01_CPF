...
import com.cpf.admin.opr.service.AdmOperationPermissionProjectionService;
...
public class AdmAuthController extends com.cpf.admin.common.base.AdmBaseController {
    ...
    private final AdmOperationPermissionProjectionService operationPermissionProjectionService;
    ...
    public AdmAuthController(AdmOperatorService operatorService, AdmSessionService sessionService,
                             ...
                             AdmOperationPermissionProjectionService operationPermissionProjectionService) {
        this.operatorService = operatorService;
        this.sessionService = sessionService;
        this.securityService = securityService;
        this.operationPermissionProjectionService = operationPermissionProjectionService;
    }

    @PostMapping("/login")    @Operation(operationId = "admAuthLogin", summary = "ADM 로그인", description = "운영자를 인증합니다. Browser에는 HttpOnly BFF Session Cookie가 발급되며 내부 인증 토큰은 응답 Body에 노출되지 않습니다.")
    public ResponseEntity<AdmLoginResponse> login(@RequestBody AdmLoginRequest request) {
        ...
        return ResponseEntity.ok(sessionService.issue(
                operator, menus, operatorService.findButtonIdsForRoles(operator.roleIds()),
                operationPermissionProjectionService.findAllowedOperationIds(operator.roleIds())));
    }
    ...
    public ResponseEntity<AdmCurrentSessionResponse> me(HttpServletRequest request) {
        ...
        return ResponseEntity.ok(new AdmCurrentSessionResponse(session.operatorId(), session.roleIds(),
                ...
                operationPermissionProjectionService.findAllowedOperationIds(session.roleIds())));
    }
