...
public class AdmCacheController extends com.cpf.admin.common.base.AdmBaseController {
 ...
 public AdmCacheControlResponse evictKey(@Valid @RequestBody EvictKeyRequest body,HttpServletRequest request){return audited("CACHE_EVICT_KEY",body.namespace()+":"+body.key(),body.reason(),request,(op,
 ...
 public AdmCacheControlResponse evictNamespace(@Valid @RequestBody EvictNamespaceRequest body,HttpServletRequest request){return audited("CACHE_EVICT_NAMESPACE",body.namespace(),body.reason(),request,(op,
 ...
 public AdmCacheControlResponse reconcile(@Valid @RequestBody ControlRequest body,HttpServletRequest request){return audited("CACHE_RECONCILE","DURABLE",body.reason(),request,service::reconcile);}
 private AdmCacheControlResponse audited(String action,String target,String reason,HttpServletRequest request,CacheAction operation){
         String operator=requireOperator(request);
         String requiredReason=audit.requireReason(reason);
         AdmCacheControlResponse result=operation.run(operator, requiredReason);
         audit.record(CpfTransactionContext.transactionId(),operator,action,"cache",target,requiredReason,request.getRemoteAddr());return result;}
 @FunctionalInterface
