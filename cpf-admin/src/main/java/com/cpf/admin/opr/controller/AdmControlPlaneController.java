...
public class AdmControlPlaneController extends com.cpf.admin.common.base.AdmBaseController {
    ...
    @PostMapping("/adm/api/incidents") @CpfOnlineTransaction(id="OADMIC0002",name="ADMIncidentCreate") @Operation(operationId="admIncidentCreateIncident", summary="Incident 생성")
    ...
    @PostMapping("/adm/api/incidents/{incidentId}/status") @CpfOnlineTransaction(id="OADMIC0003",name="ADMIncidentTransition") @Operation(operationId="admIncidentTransitionIncident", summary="Incident 상태 전이")
    ...
    @GetMapping("/adm/api/maintenance/actions") @CpfOnlineTransaction(id="OADMMT0001",name="ADMMaintenanceList") @Operation(operationId="admMaintenanceFindActions", summary="Maintenance 명령 이력")
    ...
    @PostMapping("/adm/api/maintenance/actions") @CpfOnlineTransaction(id="OADMMT0002",name="ADMMaintenanceExecute") @Operation(operationId="admMaintenanceExecuteAction", summary="Service instance Drain/Disable/Resume")
    public ResponseEntity<Map<String,Object>> executeMaintenance(@RequestBody Map<String,Object> body,HttpServletRequest request){String user=operator(request);String
            reason=String.valueOf(body.getOrDefault("reason",""));Map<String,Object> result=service.executeMaintenance(body,user);audit(request,user,"SERVICE_INSTANCE_CONTROL",
            "cpf_service_instance",String.valueOf(body.get("instanceId")),reason,result);return ResponseEntity.ok(result);}
