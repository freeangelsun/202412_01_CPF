...
public class AdmIncidentLifecycleService {
    ...
    public Page<PolicyResponse> findPolicies(int page, int size) {
        int p = page(page); int s = size(size); long total = count("cpf_incident_policy");
        List<PolicyResponse> rows = jdbc.query("""
                SELECT * FROM (
                  SELECT p.*, ROW_NUMBER() OVER (ORDER BY p.policy_id DESC) AS rn
                  FROM cpf_incident_policy p
    ...
    public PolicyResponse savePolicy(Long policyId, PolicySaveRequest request, String operatorId, String clientIp) {
        ...
        if (policyId == null) {
            ...
            int inserted = jdbc.update(c -> {
                PreparedStatement ps = c.prepareStatement("""
                        INSERT INTO cpf_incident_policy
            ...
            int updated = jdbc.update("""
                    UPDATE cpf_incident_policy SET policy_code=?, event_type=?, event_sub_type=?, severity=?,
        ...
        audit.record(CpfTransactionContext.transactionId(), operatorId, "INCIDENT_POLICY_SAVE", "cpf_incident_policy",
    ...
    public Page<IncidentResponse> findIncidents(String status, int page, int size) {
        ...
        long total = normalized == null ? count("cpf_incident") : jdbc.queryForObject(
                "SELECT COUNT(*) FROM cpf_incident WHERE status=?", Long.class, normalized.toUpperCase(Locale.ROOT));
    ...
    public IncidentResponse findIncident(long incidentId) {
        try {
            return jdbc.queryForObject("SELECT * FROM cpf_incident WHERE incident_id=?", (rs,n)->incident(rs), incidentId);
    ...
    public List<TimelineResponse> findTimeline(long incidentId) {
        return jdbc.query("""
                ...
                FROM cpf_incident_timeline WHERE incident_id=? ORDER BY timeline_id
    ...
    public SignalResult ingestSignal(SignalRequest request, String operatorId) {
        ...
        Integer observed = jdbc.queryForObject("""
                SELECT COUNT(*) FROM cpf_incident_signal
    ...
    public IncidentResponse transition(long incidentId, String action, IncidentActionRequest request, String operatorId, String clientIp) {
        ...
        try {
            updated = jdbc.update("""
                    UPDATE cpf_incident SET status=?, active_key=?, escalation_level=escalation_level+?,
        ...
        audit.record(CpfTransactionContext.transactionId(), operatorId, "INCIDENT_"+action, "cpf_incident",
    ...
    public Page<MaintenanceResponse> findMaintenance(int page, int size) {
        int p=page(page), s=size(size); long total=count("cpf_maintenance_window");
        List<MaintenanceResponse> rows=jdbc.query("""
                ...
                FROM cpf_maintenance_window m) x WHERE x.rn>? AND x.rn<=? ORDER BY x.rn
    ...
    public MaintenanceResponse saveMaintenance(Long id, MaintenanceSaveRequest request, String operatorId, String clientIp) {
        ...
        if(id==null){
            ...
            int inserted=jdbc.update(c->{PreparedStatement ps=c.prepareStatement("""
                    INSERT INTO cpf_maintenance_window(maintenance_code,target_type,target_id,starts_at,ends_at,use_yn,version,
            ...
            int updated=jdbc.update("""
                    UPDATE cpf_maintenance_window SET maintenance_code=?,target_type=?,target_id=?,starts_at=?,ends_at=?,use_yn=?,
        ...
        audit.record(CpfTransactionContext.transactionId(),operatorId,"MAINTENANCE_SAVE","cpf_maintenance_window",
    ...
    private long upsertActiveIncident(PolicyResponse policy, SignalRequest request, LocalDateTime occurredAt, int observed, String operatorId) {
        ...
        try {
            ...
            jdbc.update(c->{PreparedStatement ps=c.prepareStatement("""
                    INSERT INTO cpf_incident(policy_id,policy_code,severity,status,title,summary,source_type,source_id,
            ...
            int updated=jdbc.update("""
                    UPDATE cpf_incident SET occurrence_count=occurrence_count+1,last_occurred_at=?,summary=?,version=version+1,
            ...
            return jdbc.queryForObject("SELECT incident_id FROM cpf_incident WHERE active_key=?",Long.class,key);
        }
    }
    ...
    private long insertSignal(long policyId, SignalRequest r, LocalDateTime occurredAt, boolean suppressed, String operatorId){
        KeyHolder h=new GeneratedKeyHolder();jdbc.update(c->{PreparedStatement ps=c.prepareStatement("""
                INSERT INTO cpf_incident_signal(policy_id,source_type,source_id,correlation_id,transaction_id,title,summary,
    ...
    private boolean isMaintenance(String targetType,String targetId,LocalDateTime at){
        Integer count=jdbc.queryForObject("""
                SELECT COUNT(*) FROM cpf_maintenance_window WHERE use_yn='Y' AND starts_at<=? AND ends_at>?
    ...
    private PolicyResponse findPolicy(long id){try{return jdbc.queryForObject("SELECT * FROM cpf_incident_policy WHERE policy_id=?",(rs,n)->new PolicyResponse(
    ...
    private PolicyResponse findPolicyByCode(String code){return jdbc.queryForObject("SELECT * FROM cpf_incident_policy WHERE policy_code=? AND use_yn='Y'",(rs,n)->new PolicyResponse(
            rs.getLong("policy_id"),rs.getString("policy_code"),rs.getString("event_type"),rs.getString("event_sub_type"),
            rs.getString("severity"),rs.getInt("threshold_count"),rs.getInt("window_seconds"),rs.getInt("escalation_minutes"),
            rs.getString("receiver_group"),rs.getString("use_yn"),rs.getLong("version"),rs.getString("created_by"),
            local(rs.getTimestamp("created_at")),rs.getString("updated_by"),local(rs.getTimestamp("updated_at"))),code);}
    private MaintenanceResponse findMaintenanceById(long id){try{return jdbc.queryForObject("SELECT * FROM cpf_maintenance_window WHERE maintenance_id=?",(rs,n)->maintenance(rs),id);}
    ...
    private void insertTimeline(long incidentId,String action,String before,String after,String reason,String approval,String actor){jdbc.update("""
            INSERT INTO cpf_incident_timeline(incident_id,action_type,before_status,after_status,reason,approval_request_id,actor_id,created_at)
    ...
    private CommandReservation reserve(String type,String key,String requestHash,String actor){
        try{KeyHolder h=new GeneratedKeyHolder();jdbc.update(c->{PreparedStatement ps=c.prepareStatement("""
                INSERT INTO cpf_incident_command(command_type,idempotency_key,request_hash,status,created_by,created_at,updated_at)
        ...
        }catch(DuplicateKeyException duplicate){Map<String,Object> row=jdbc.queryForMap("SELECT command_id,request_hash,status,result_ref FROM cpf_incident_command WHERE idempotency_key=?",key);
    ...
    private void completeCommand(long commandId,String resultRef){int
            updated=jdbc.update("UPDATE cpf_incident_command SET status='DONE',result_ref=?,updated_at=CURRENT_TIMESTAMP WHERE command_id=? AND status='RUNNING'",resultRef,commandId);
