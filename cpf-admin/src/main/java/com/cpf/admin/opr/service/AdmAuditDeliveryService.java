...
public class AdmAuditDeliveryService extends AdmBaseService {
    ...
    private String safeTransactionId() {
        try {
            return CpfContexts.transactionId();
        } catch (RuntimeException ex) {
            return "UNAVAILABLE";
        }
    }
    ...
    public record AuditCommand(
        ...
        AuditCommand normalized() {
            return new AuditCommand(
                    CpfStrings.hasText(transactionId) ? transactionId.trim() : CpfContexts.transactionId(),
                    CpfStrings.hasText(traceId) ? traceId.trim() : CpfContexts.traceId(),
