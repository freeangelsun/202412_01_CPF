...
class FeatureFlagApprovalOwnerCommandAdapterTest {
    ...
    private final FeatureFlagApprovalOwnerCommandAdapter adapter=new FeatureFlagApprovalOwnerCommandAdapter(provider(operations),new ObjectMapper());
    ...
    @Test void approvedChangeConvergesToKillSwitch(){doReturn(result(CpfFeatureFlagResult.Source.PROVIDER,7)).doReturn(result(CpfFeatureFlagResult.Source.KILL_SWITCH,8)).when(operations).find("flag.a");var r=adapter.execute(command("req","app"));assertEquals(AdmApprovalExecutionStatus.SUCCEEDED,r.status());verify(operations).setKillSwitch("flag.a",true,"app","approved reason");}
    ...
    /** Feature Flag 는 opt-in 이므로 Adapter 는 ObjectProvider 로 받는다. 테스트도 같은 계약을 쓴다. */
    private static org.springframework.beans.factory.ObjectProvider<CpfFeatureFlagOperations> provider(CpfFeatureFlagOperations value){
        return new org.springframework.beans.factory.ObjectProvider<>(){
            @Override public CpfFeatureFlagOperations getObject(Object... args){return value;}
            @Override public CpfFeatureFlagOperations getObject(){return value;}
            @Override public CpfFeatureFlagOperations getIfAvailable(){return value;}
            @Override public CpfFeatureFlagOperations getIfUnique(){return value;}
        };
    }
}
