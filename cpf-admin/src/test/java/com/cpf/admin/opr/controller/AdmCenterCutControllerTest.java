...
class AdmCenterCutControllerTest {
    ...
    void findJobsDelegatesToOperationService() {
        when(service.findJobs()).thenReturn(List.of(Map.of("centerCutJobId", "CPF_XYZ_CENTER_CUT_SAMPLE_JOB")));

        ResponseEntity<List<Map<String, Object>>> response = controller.findJobs();

        assertThat(response.getBody()).hasSize(1);
        assertThat(response.getBody().get(0)).containsEntry("centerCutJobId", "CPF_XYZ_CENTER_CUT_SAMPLE_JOB");
        verify(service).findJobs();
    }

    @Test
    void findTargetsAcceptsStatusAndLimitFilters() {
        when(service.findTargets("CPF_XYZ_CENTER_CUT_SAMPLE_JOB", "FAILED", 20))
                .thenReturn(List.of(Map.of(
                        "statusCode", "FAILED",
                        "parentTransactionGlobalId", "20260615120000000XYZbatAP010000001",
                        "lastErrorMessage", "교육용 강제 실패")));

        ResponseEntity<List<Map<String, Object>>> response =
                controller.findTargets("CPF_XYZ_CENTER_CUT_SAMPLE_JOB", "FAILED", 20);
        ...
        verify(service).findTargets("CPF_XYZ_CENTER_CUT_SAMPLE_JOB", "FAILED", 20);
    }
