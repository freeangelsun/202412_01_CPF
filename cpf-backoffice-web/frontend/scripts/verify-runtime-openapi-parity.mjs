...
const runtimeInput=String(process.argv[3] || process.env.CPF_MBW_RUNTIME_OPENAPI_FILE || "").trim();
if(!fs.existsSync(sourcePath) || !fs.statSync(sourcePath).isFile()) throw new Error(`source OpenAPI missing: ${sourcePath}`);
if(!runtimeInput) throw new Error("runtime OpenAPI missing: path was not provided");
const runtimePath=path.resolve(runtimeInput);
if(!fs.existsSync(runtimePath) || !fs.statSync(runtimePath).isFile()) throw new Error(`runtime OpenAPI missing: ${runtimePath}`);
...
// CPF canonical module code 는 MBW 다(canonicalize-cpf-openapi.py MODULE_PREFIX).
// 아직 갱신되지 않은 문서의 legacy 표기(BACKOFFICE)도 같은 Public prefix 로 받는다.
const prefix=module==="ADM"?"/adm/api/":(module==="MBW"||module==="BACKOFFICE")?"/api/v1/backoffice/":null;
if(!prefix) throw new Error(`unsupported module: ${module}`);
