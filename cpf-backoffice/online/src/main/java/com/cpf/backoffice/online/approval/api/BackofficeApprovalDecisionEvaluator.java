...
/**
 * MBW 결재 Target/단계의 ALL/ANY/N_OF_M 규칙을 순수 계산합니다.
 *
 ...
 */
public final class BackofficeApprovalDecisionEvaluator {

    private BackofficeApprovalDecisionEvaluator() {
    }
     ...
     */
    public static BackofficeApprovalStepStatus evaluate(
        ...
        if (approvedOrAgreedCount >= threshold) {
            return BackofficeApprovalStepStatus.APPROVED;
        }

        int remaining = participantCount - approvedOrAgreedCount - rejectedCount;
        if (approvedOrAgreedCount + remaining < threshold) {
            return BackofficeApprovalStepStatus.REJECTED;
        }
        return BackofficeApprovalStepStatus.WAITING;
    }
