...
import com.cpf.backoffice.online.approval.spi.BackofficeApprovalDirectoryPort;
...
/**
 * MBW 결재 Policy/Participant Snapshot 정본 adapter.
 *
...
public class BackofficeApprovalPolicyRepository implements BackofficeApprovalDirectoryPort {
    ...
    public BackofficeApprovalPolicyRepository(
    ...
    @Override
    public List<BackofficeApprovalDirectoryEntry> resolve(
            BackofficeApprovalTargetType targetType, String targetCode, Instant effectiveAt) {
        ...
        LinkedHashMap<String,BackofficeApprovalDirectoryEntry> unique = new LinkedHashMap<>();
        for (Map<String,Object> row : jdbc().queryForList(query, p)) {
            ...
            unique.putIfAbsent(employeeNo, new BackofficeApprovalDirectoryEntry(
    ...
    public Optional<BackofficeApprovalDirectoryEntry> findPrimaryAssignment(String employeeNo, Instant at) {
        return resolve(BackofficeApprovalTargetType.EMPLOYEE, employeeNo, at).stream().findFirst();
    }
    ...
    public long insertPolicyApproval(Map<String,Object> values) {
        ...
        if (id == null) throw new IllegalStateException("MBW policy approval 생성 키를 확인할 수 없습니다.");
        return id.longValue();
    }
    ...
    public long insertLine(Map<String,Object> values) {
        ...
        Number id = key.getKey();
        if (id == null) throw new IllegalStateException("MBW approval line 생성 키를 확인할 수 없습니다.");
        return id.longValue();
    }
    ...
    private NamedParameterJdbcTemplate jdbc() {
        ...
        if (jdbc == null) throw new IllegalStateException("MBW datasource가 구성되지 않았습니다.");
        return jdbc;
    }
