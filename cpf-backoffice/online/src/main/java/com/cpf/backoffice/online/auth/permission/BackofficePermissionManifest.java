...
/** MBW Frontend와 API Filter가 공유하는 Menu/Action 권한 정본입니다. */
@Component
public class BackofficePermissionManifest {
    static final String CLASSPATH_LOCATION="cpf/backoffice/backoffice-permission-manifest.json";
    private final Definition definition;
    public BackofficePermissionManifest(ObjectMapper mapper){this.definition=read(mapper);}
    ...
    private Definition read(ObjectMapper mapper){
        try(InputStream in=new ClassPathResource(CLASSPATH_LOCATION).getInputStream()){
            ...
            if(raw.schemaVersion()!=2||raw.menuGroups()==null||raw.menuGroups().isEmpty()||raw.apiResourceGroups()==null||raw.apiResourceGroups().isEmpty())
                throw new IllegalStateException("MBW permission manifest v2 필수 Metadata가 없습니다.");
            ...
            List<ActionRule> rules=Optional.ofNullable(raw.actionRules()).orElse(List.of()).stream().map(r->new ActionRule(normalizeCode(r.method()),normalizePath(r.pathPattern()),normalizeCode(r
                    .actionCode()))).toList();
            return new Definition(raw.schemaVersion(),raw.owner(),raw.sourceProjection(),raw.menuGroups().stream().map(BackofficePermissionManifest::normalizeCode).toList(),Map.copyOf(resources),rules,Map.copyOf(aliases));
        }catch(IOException e){throw new IllegalStateException("MBW permission manifest를 읽을 수 없습니다.",e);}
    }
