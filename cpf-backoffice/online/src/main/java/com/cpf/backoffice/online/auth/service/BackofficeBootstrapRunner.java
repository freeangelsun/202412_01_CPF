...
public final class BackofficeBootstrapRunner implements ApplicationRunner {
    ...
    private final BackofficeBootstrapApprovalRepository approvals;

    public BackofficeBootstrapRunner(
            Environment environment,
            CpfPasswordEncoder passwordService,
            BackofficeAuthRepository authRepository,
            BackofficeBootstrapApprovalRepository approvals) {
    ...
    public void run(ApplicationArguments arguments) throws Exception {
        ...
        try {
            ...
            passwordPath = secureSecretFile(require("cpf.backoffice.bootstrap.password-file"), "PASSWORD");
            ...
            String loginId = require("cpf.backoffice.bootstrap.login-id");
            String operatorName = require("cpf.backoffice.bootstrap.operator-name");
            String roleCode = environment.getProperty("cpf.backoffice.bootstrap.role-code", "MBW_MANAGER").trim();
            ...
            BackofficeAuthRepository.BootstrapResult existing = authRepository.findBootstrapOperation(operationId).orElse(null);
            if (existing != null) {
                requireSameLogin(existing, loginId);
                approvals.reconcileComplete(tokenHash, operationId, existing.adminUserId(), Instant.now());
                log.info("MBW bootstrap reconciled. operationId={}, adminUserId={}", operationId, existing.adminUserId());
                return;
            }
            ...
            if (!claimed) {
                BackofficeBootstrapApprovalRepository.ApprovalState state = approvals.find(tokenHash)
                        .orElseThrow(() -> new SecurityException("MBW_BOOTSTRAP_APPROVAL_NOT_FOUND"));
                existing = authRepository.findBootstrapOperation(operationId).orElse(null);
                if (existing != null) {
                    requireSameLogin(existing, loginId);
                    approvals.reconcileComplete(tokenHash, operationId, existing.adminUserId(), Instant.now());
                    log.info("MBW bootstrap reconciled after claim conflict. operationId={}, adminUserId={}",
                            operationId, existing.adminUserId());
                    return;
                }
                throw new SecurityException("MBW_BOOTSTRAP_APPROVAL_NOT_CLAIMABLE:" + state.status());
            }
            ...
            try {
                ...
                BackofficeAuthRepository.BootstrapResult reconciled = authRepository.findBootstrapOperation(operationId).orElse(null);
                if (reconciled != null) {
                    requireSameLogin(reconciled, loginId);
                    approvals.reconcileComplete(tokenHash, operationId, reconciled.adminUserId(), Instant.now());
                    log.warn("MBW bootstrap result was reconciled after terminal update uncertainty. operationId={}", operationId);
                } else {
            ...
            if (cleanupFailure != null) {
                IllegalStateException cleanupException =
                        new IllegalStateException("MBW_BOOTSTRAP_SECRET_DESTROY_FAILED:" + cleanupFailure);
    ...
    private void rejectLegacyBootstrapProperties() {
        if (environment.getProperty("cpf.backoffice.bootstrap.enabled") != null
                || environment.getProperty("cpf.backoffice.bootstrap.password") != null) {
            throw new IllegalStateException("MBW_BOOTSTRAP_LEGACY_ENABLE_OR_PLAINTEXT_PASSWORD_FORBIDDEN");
        }
    }

    private String environmentFingerprint() {
        String environmentCode = require("cpf.environment.code");
        String approvalScope = require("cpf.backoffice.bootstrap.approval-scope");
    ...
    private String operationId(String fingerprint, String loginId) {
        String configured = environment.getProperty("cpf.backoffice.bootstrap.operation-id");
        String value = configured == null || configured.isBlank()
                ? "MBW-BOOTSTRAP-" + fingerprint.substring(0, 16) + "-" + loginId.toUpperCase(Locale.ROOT)
                : configured.trim();
        if (value.length() > 100 || !value.matches("[A-Za-z0-9._:-]+")) {
            throw new IllegalStateException("MBW_BOOTSTRAP_OPERATION_ID_INVALID");
        }
        return value;
    }
    ...
    private String claimOwnerId() {
        ...
        if (value.length() > 100 || !value.matches("[A-Za-z0-9._:-]+")) {
            throw new IllegalStateException("MBW_BOOTSTRAP_INSTANCE_ID_INVALID");
        }
        return value;
    }

    private long leaseSeconds() {
        String configured = environment.getProperty("cpf.backoffice.bootstrap.claim-lease-seconds", "300");
        try {
            long value = Long.parseLong(configured.trim());
            if (value < 30 || value > 1800) throw new NumberFormatException("out of range");
            return value;
        } catch (NumberFormatException failure) {
            throw new IllegalStateException("MBW_BOOTSTRAP_CLAIM_LEASE_SECONDS_INVALID", failure);
        }
    }
    ...
    private String require(String key) {
        ...
        if (value == null || value.isBlank()) {
            throw new IllegalStateException("MBW_BOOTSTRAP_PROPERTY_REQUIRED:" + key);
        }
        return value.trim();
    }
    ...
    static Path secureSecretFile(String value, String label) throws IOException {
        ...
        if (!Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS) || Files.isSymbolicLink(path)) {
            throw new SecurityException("MBW_BOOTSTRAP_" + label + "_FILE_UNSAFE");
        }
        if (Files.size(path) > 4096) throw new SecurityException("MBW_BOOTSTRAP_SECRET_FILE_TOO_LARGE");
    ...
    private static void verifySecretAcl(Path path, String label) throws IOException {
        if (FileSystems.getDefault().supportedFileAttributeViews().contains("posix")) {
            ...
            if (!permissions.contains(PosixFilePermission.OWNER_READ)
                ...
                throw new SecurityException("MBW_BOOTSTRAP_" + label + "_POSIX_PERMISSION_UNSAFE");
            }
            return;
        }
        ...
        if (aclView == null) {
            throw new SecurityException("MBW_BOOTSTRAP_" + label + "_ACL_UNVERIFIABLE");
        }
        ...
        for (AclEntry entry : aclView.getAcl()) {
            ...
            if (entry.permissions().stream().anyMatch(SENSITIVE_ACL_PERMISSIONS::contains)) {
                throw new SecurityException("MBW_BOOTSTRAP_" + label + "_WINDOWS_ACL_UNSAFE");
            }
        }
    }
    ...
    private static char[] readSecret(Path path, int maximum) throws IOException {
        ...
        if (declaredSize < 1 || declaredSize > 4096) {
            throw new SecurityException("MBW_BOOTSTRAP_SECRET_FILE_SIZE_INVALID");
        }
        ...
        try (InputStream input = Files.newInputStream(path, LinkOption.NOFOLLOW_LINKS)) {
            ...
            while (offset < encoded.length) {
                ...
                if (read < 0) {
                    throw new SecurityException("MBW_BOOTSTRAP_SECRET_FILE_TRUNCATED");
                }
                offset += read;
            }
            if (input.read() != -1) {
                throw new SecurityException("MBW_BOOTSTRAP_SECRET_FILE_CHANGED");
            }
            ...
            if (length == 0 || length > maximum) throw new SecurityException("MBW_BOOTSTRAP_SECRET_INVALID");
    ...
    private static void requireSameLogin(BackofficeAuthRepository.BootstrapResult result, String loginId) {
        if (!result.loginId().equals(loginId)) {
            throw new SecurityException("MBW_BOOTSTRAP_OPERATION_LOGIN_MISMATCH");
        }
    }
    ...
    static void requireStrongPassword(String loginId, char[] password) {
        if (password.length < 14) throw new SecurityException("MBW_BOOTSTRAP_PASSWORD_POLICY_REJECTED");
        boolean upper = false;
        boolean lower = false;
        boolean digit = false;
        ...
        if (categories < 3 || containsIgnoreCase(password, loginId)) {
            throw new SecurityException("MBW_BOOTSTRAP_PASSWORD_POLICY_REJECTED");
        }
    }
