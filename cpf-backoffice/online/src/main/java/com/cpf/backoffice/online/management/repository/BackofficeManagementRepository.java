...
public class BackofficeManagementRepository extends BackofficeBaseRepository {
  ...
  public BackofficeManagementRepository(
  ...
  public List<Map<String, Object>> findOrganizations() {
    return jdbc()
        .queryForList(sql.required("backoffice-repository-find-organizations-01"), Map.of());
  }

  public int saveOrganization(Map<String, ?> values) {
    if (values.get("expectedVersion") == null
        && !organizationExists(String.valueOf(values.get("organizationCode")))) {
  ...
  public boolean organizationExists(String code) {
    Long count =
        jdbc()
            .queryForObject(
                sql.required("backoffice-repository-organization-exists-01"),
  ...
  public boolean wouldCreateOrganizationCycle(String organizationCode, String parentCode) {
    ...
    Long count =
        jdbc()
            .queryForObject(
                sql.required("backoffice-repository-would-create-organization-cycle-01"),
  ...
  public int saveEmployee(Map<String, ?> values) {
    ...
    Long count =
        jdbc()
            .queryForObject(
                sql.required("backoffice-repository-save-employee-03"),
                new MapSqlParameterSource("employeeNo", employeeNo),
                Long.class);
  ...
  public CpfPage<Map<String, Object>> organizationPage(CpfPageRequest page) {
    ...
    Long total =
        jdbc()
            .queryForObject(
                sql.required("backoffice-repository-organization-page-01"),
                new MapSqlParameterSource(),
                Long.class);
    List<Map<String, Object>> rows =
        jdbc().queryForList(sql.required("backoffice-repository-organization-page-02"), q);
    return new CpfPage<>(rows, total == null ? 0 : total, page.page(), page.size());
  }

  public CpfPage<Map<String, Object>> employeePage(
      String organizationCode, String status, CpfPageRequest page) {
    MapSqlParameterSource q =
        new MapSqlParameterSource()
            .addValue("organizationCode", organizationCode)
            .addValue("status", status)
    ...
    Long total =
        jdbc()
            .queryForObject(sql.required("backoffice-repository-employee-page-01"), q, Long.class);
    List<Map<String, Object>> rows =
        jdbc().queryForList(sql.required("backoffice-repository-employee-page-02"), q);
    return new CpfPage<>(rows, total == null ? 0 : total, page.page(), page.size());
  }
  ...
  public List<Map<String, Object>> findEffectivePermissions(String loginId) {
    return jdbc()
        .queryForList(
            sql.required("backoffice-repository-find-effective-permissions-01"),
            new MapSqlParameterSource("loginId", loginId));
  }

  /** 인증된 MBW 로그인 ID와 결재 처리용 직원 번호의 바인딩을 조회합니다. */
  public Optional<String> findEmployeeNoByLoginId(String loginId) {
    return jdbc()
        .queryForList(
            sql.required("backoffice-repository-find-employee-no-by-login-id-01"),
            new MapSqlParameterSource("loginId", loginId))
        .stream()
  ...
  public List<Map<String, Object>> findBusinessAudits(int limit) {
    return jdbc()
        .queryForList(
            sql.required("backoffice-find-business-audits"),
  ...
  private NamedParameterJdbcTemplate jdbc() {
    ...
    if (jdbcTemplate == null) {
      throw new ResponseStatusException(
          HttpStatus.SERVICE_UNAVAILABLE, "MBW DB datasource가 비활성화되어 백오피스 저장소를 사용할 수 없습니다.");
    }
    return jdbcTemplate;
  }
}
