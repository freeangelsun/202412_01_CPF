...
param(
    ...
    [string]$InstanceId='cpf-batch-agent-dev-1',
    [int]$Port=8184,
    [string]$JavaPath='',
    [string]$JarPath='',
    [string]$StateRoot='',
...
if([string]::IsNullOrWhiteSpace($JarPath)){
    $candidates=@(Get-ChildItem -LiteralPath (Join-Path $Root 'cpf-batch/agent/build/libs') -Filter 'cpf-batch-agent-*.jar' -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    if($candidates.Count-ne1){throw "Expected exactly one cpf-batch-agent executable JAR. Build :runtime:batch:agent:bootJar first or pass -JarPath. count=$($candidates.Count)"}
...
$stateDir=Join-Path $StateRoot 'agent'
...
if(Test-Path -LiteralPath $pidFile){
    ...
    if(Get-Process -Id $old -ErrorAction SilentlyContinue){throw "CPF batch agent already running. instance=$InstanceId pid=$old"}
...
if(-not$ready){
    ...
    Remove-Item -LiteralPath $pidFile,$metaFile -Force -ErrorAction SilentlyContinue
    throw "CPF batch agent readiness failed. profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness"
}
Write-Host "CPF_BATCH_AGENT_READY profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness UTF-8"
