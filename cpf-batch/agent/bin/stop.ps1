[CmdletBinding()]
param([string]$InstanceId='cpf-batch-agent-dev-1',[string]$StateRoot='',[int]$TimeoutSeconds=20)
$ErrorActionPreference='Stop';Set-StrictMode -Version Latest
if([string]::IsNullOrWhiteSpace($StateRoot)){$StateRoot=Join-Path ([IO.Path]::GetTempPath()) 'cpf-batch-runtime'}
$stateDir=Join-Path $StateRoot 'agent';$pidFile=Join-Path $stateDir ($InstanceId+'.pid');$metaFile=Join-Path $stateDir ($InstanceId+'.json')
if(-not(Test-Path -LiteralPath $pidFile -PathType Leaf)){Write-Host 'CPF_BATCH_AGENT_STOP already-stopped';return}
...
Write-Host 'CPF_BATCH_AGENT_STOP=PASS'
