...
public class JdbcCenterCutClaimRepository {
    ...
    public Optional<Claim> claimForExecution(
        ...
        return claimCandidate(requiredExecutionId, runner, pool, duration);
    }

    /** 실제 Worker가 모든 RUNNING 실행에서 다음 DB Work Item을 공정하게 Claim합니다. */
    public Optional<Claim> claimNext(String runner,String pool,Duration duration) {
        return claimCandidate(null, runner, pool, duration);
    }

    private Optional<Claim> claimCandidate(
            String requiredExecutionId,String runner,String pool,Duration duration) {
        List<Map<String,Object>> candidates = jdbc.queryForList(
                sql.required("centercut-claim-find-candidates"));
        for(Map<String,Object> row:candidates){
            String executionId=Objects.toString(row.get("center_cut_execution_id"),"");
            if(requiredExecutionId!=null&&!requiredExecutionId.equals(executionId))continue;
    ...
    public void complete(Claim c,String status,String result,String message) {
        ...
        int itemChanged=jdbc.update(sql.required("centercut-item-complete"),
                status,status,status,sanitized,c.itemId());
    ...
    public record Work(long itemId,String executionId,String businessKey,String payload,String centerCutJobId,
                       String transactionId,String segmentId,String parentSegmentId,String handlerKey,String jobCode,
                       int retryCount,int retryLimit) {
        public Work {
            if(retryCount<0||retryLimit<0) throw new IllegalArgumentException("Center-Cut retry values cannot be negative");
        }
    }
