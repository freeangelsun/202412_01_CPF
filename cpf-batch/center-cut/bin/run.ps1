...
param(
    ...
    [string]$InstanceId='cpf-batch-center-cut-dev-1',
    [int]$Port=8183,
    [string]$JavaPath='',
    [string]$JarPath='',
    [string]$StateRoot='',
...
if([string]::IsNullOrWhiteSpace($JarPath)){
    $candidates=@(Get-ChildItem -LiteralPath (Join-Path $Root 'cpf-batch/center-cut/build/libs') -Filter 'cpf-batch-center-cut-*.jar' -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    if($candidates.Count-ne1){throw "Expected exactly one cpf-batch-center-cut executable JAR. Build :runtime:batch:center-cut:bootJar first or pass -JarPath. count=$($candidates.Count)"}
...
$stateDir=Join-Path $StateRoot 'center-cut'
...
if(Test-Path -LiteralPath $pidFile){
    ...
    if(Get-Process -Id $old -ErrorAction SilentlyContinue){throw "CPF batch center-cut already running. instance=$InstanceId pid=$old"}
...
if(-not$ready){
    ...
    Remove-Item -LiteralPath $pidFile,$metaFile -Force -ErrorAction SilentlyContinue
    throw "CPF batch center-cut readiness failed. profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness"
}
Write-Host "CPF_BATCH_CENTER_CUT_READY profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness UTF-8"
