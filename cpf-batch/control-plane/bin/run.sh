...
INSTANCE_ID="${CPF_WAS_ID:-cpf-batch-control-plane-$PROFILE-1}"
PORT="${CPF_PORT:-8180}"
...
if [ -z "$JAR_PATH" ]; then
  set -- "$ROOT"/cpf-batch/control-plane/build/libs/cpf-batch-control-plane-*.jar
  [ "$#" -eq 1 ] && [ -f "$1" ] || { echo 'expected exactly one executable jar; build :runtime:batch:control-plane:bootJar or set CPF_BATCH_JAR' >&2; exit 2; }
  JAR_PATH="$1"
fi
STATE_ROOT="${CPF_RUNTIME_STATE_DIR:-${TMPDIR:-/tmp}/cpf-batch-runtime}"
STATE_DIR="$STATE_ROOT/control-plane"; mkdir -p "$STATE_DIR"
...
i=0; while [ "$i" -lt "${CPF_READINESS_TIMEOUT_SECONDS:-90}" ]; do
  ...
  if curl -fsS --max-time 2 "$READINESS" >/dev/null 2>&1; then echo "CPF_BATCH_CONTROL_PLANE_READY profile=$PROFILE instance=$INSTANCE_ID port=$PORT pid=$pid readiness=$READINESS UTF-8"; exit 0; fi
  i=$((i+1)); sleep 1
done
