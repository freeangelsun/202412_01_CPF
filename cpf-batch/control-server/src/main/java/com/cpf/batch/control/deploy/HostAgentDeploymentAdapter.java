...
/** Host Agent 배포 Adapter. 모든 명령은 안정 command ID로 멱등 처리하고 통신 단절 시 결과를 조회합니다. */
@Component
public final class HostAgentDeploymentAdapter implements DeploymentTargetAdapter {
    private static final Set<String> HOST_AGENT_MODES = Set.of("embedded-bootjar", "external-tomcat-war");
    ...
    public HostAgentDeploymentAdapter(RestClient.Builder builder) { this.builder = builder; }
    ...
    public boolean supports(DeploymentCellManifest.Instance instance, String mode) {
        return instance.agentBaseUrl() != null && !instance.agentBaseUrl().isBlank()
                && mode != null && HOST_AGENT_MODES.contains(mode.toLowerCase(Locale.ROOT));
    }
