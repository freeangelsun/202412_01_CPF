...
public final class CpfBatchKafkaInboundBridge implements CpfBatchInboundHandler {
    ...
    private boolean accept(
        ...
        try {
            CpfBatchContextBundle remoteContext=codec.restoreContext(envelope);
            try (AutoCloseable ignoredCore=CpfContexts.bind(remoteContext.snapshot());
                 AutoCloseable ignoredBatch=CpfBatchRuntimeContexts.bind(remoteContext)) {
                ...
                throw new IllegalStateException("CPF batch context scope close failed", scopeCloseFailure);
            }
            return true;
