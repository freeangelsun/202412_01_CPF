...
param(
    ...
    [string]$InstanceId='cpf-batch-scheduler-dev-1',
    [int]$Port=8181,
    [string]$JavaPath='',
    [string]$JarPath='',
    [string]$StateRoot='',
...
if([string]::IsNullOrWhiteSpace($JarPath)){
    $candidates=@(Get-ChildItem -LiteralPath (Join-Path $Root 'cpf-batch/scheduler/build/libs') -Filter 'cpf-batch-scheduler-*.jar' -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    if($candidates.Count-ne1){throw "Expected exactly one cpf-batch-scheduler executable JAR. Build :runtime:batch:scheduler:bootJar first or pass -JarPath. count=$($candidates.Count)"}
...
$stateDir=Join-Path $StateRoot 'scheduler'
...
if(Test-Path -LiteralPath $pidFile){
    ...
    if(Get-Process -Id $old -ErrorAction SilentlyContinue){throw "CPF batch scheduler already running. instance=$InstanceId pid=$old"}
...
if(-not$ready){
    ...
    Remove-Item -LiteralPath $pidFile,$metaFile -Force -ErrorAction SilentlyContinue
    throw "CPF batch scheduler readiness failed. profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness"
}
Write-Host "CPF_BATCH_SCHEDULER_READY profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness UTF-8"
