 ...
 */
public class BatBatchOperationRepository {
    private final ObjectProvider<JdbcTemplate> jdbcTemplateProvider;
    private final ObjectProvider<DataSource> dataSourceProvider;
    private final CpfFileLogWriter fileLogWriter;

    public BatBatchOperationRepository(
    ...
    public long startExecution(
        ...
        if (executionId == null) {
            throw new IllegalStateException("BAT 배치 실행 ID를 확인할 수 없습니다.");
        }
        tryUpdateExecutionExtendedMetrics(
                executionId,
                BatBatchRuntimeProgress.empty(status),
                springBatchExecutionId,
                workerId,
                user);
        return executionId;
    ...
    public void completeExecution(
        ...
        jdbc().update("""
                ...
                user,
                executionId);
        BatBatchRuntimeProgress progress = counts.toProgress(status, jobExecution);
    ...
    public void updateExecutionRuntime(
            ...
            Long springBatchExecutionId,
            String workerId,
            BatBatchRuntimeProgress progress,
            String requestUser) {
        if (!available() || executionId < 1) {
            return;
        }
        BatBatchRuntimeProgress resolved = progress == null
                ? BatBatchRuntimeProgress.empty("RUNNING")
                : progress;
        String user = defaultIfBlank(requestUser, "CPF_BATCH");
        jdbc().update("""
                UPDATE bat_execution
    ...
    public void recordWorkerHeartbeat(
        ...
        try {
            jdbc().update("""
                    ...
                    ) VALUES (?, ?, ?, ?, ?, ?, 'Y', CURRENT_TIMESTAMP(3), ?, ?, 'BAT 배치 실행기 heartbeat', ?, ?)
                    ON DUPLICATE KEY UPDATE
    ...
    private void upsertStepExecutions(long cpfExecutionId, JobExecution jobExecution, String workerId, String user) {
        for (StepExecution step : jobExecution.getStepExecutions()) {
            BatBatchRuntimeProgress progress = StepCounts.from(step).toProgress(step.getStatus().name(), step);
            upsertStepRuntime(
                    cpfExecutionId,
                    step.getId(),
                    workerId,
    ...
    private void tryUpdateExecutionLogLinkage(long executionId, JobExecution jobExecution, String user) {
        ...
        try {
            String businessDateText = executionContextValue(
                    jobExecution,
                    BatBatchFileLogWriter.CONTEXT_BUSINESS_DATE,
            ...
            jdbc().update("""
                    ...
                    nullableLong(executionContextValue(
                            jobExecution,
                            BatBatchFileLogWriter.CONTEXT_ORIGINAL_JOB_EXECUTION_ID,
                            jobExecution.getJobParameters().getString("originalJobExecutionId"))),
                    executionContextLong(jobExecution, BatBatchFileLogWriter.CONTEXT_RESTART_ATTEMPT, 0L),
                    executionContextValue(jobExecution, BatBatchFileLogWriter.CONTEXT_SEGMENT_ID, null),
                    executionContextValue(jobExecution, BatBatchFileLogWriter.CONTEXT_PARENT_SEGMENT_ID, null),
                    relativePath,
                    user,
                    executionId);
    ...
    private record BatchCounts(long readCount, long writeCount, long skipCount, long failureCount, long retryCount) {
        ...
        BatBatchRuntimeProgress toProgress(String status, JobExecution execution) {
            long processed = writeCount + skipCount + failureCount;
            long total = Math.max(readCount, processed);
            return BatBatchRuntimeProgress.of(
                    total,
                    processed,
                    writeCount,
                    failureCount,
    ...
    private record StepCounts(long readCount, long writeCount, long skipCount, long failureCount, long retryCount) {
        ...
        BatBatchRuntimeProgress toProgress(String status, StepExecution step) {
            long processed = writeCount + skipCount + failureCount;
            long total = Math.max(readCount, processed);
