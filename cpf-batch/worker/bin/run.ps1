...
param(
    ...
    [string]$InstanceId='cpf-batch-worker-dev-1',
    [int]$Port=8182,
    [string]$JavaPath='',
    [string]$JarPath='',
    [string]$StateRoot='',
...
if([string]::IsNullOrWhiteSpace($JarPath)){
    $candidates=@(Get-ChildItem -LiteralPath (Join-Path $Root 'cpf-batch/worker/build/libs') -Filter 'cpf-batch-worker-*.jar' -File -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending)
    if($candidates.Count-ne1){throw "Expected exactly one cpf-batch-worker executable JAR. Build :runtime:batch:worker:bootJar first or pass -JarPath. count=$($candidates.Count)"}
...
$stateDir=Join-Path $StateRoot 'worker'
...
if(Test-Path -LiteralPath $pidFile){
    ...
    if(Get-Process -Id $old -ErrorAction SilentlyContinue){throw "CPF batch worker already running. instance=$InstanceId pid=$old"}
...
if(-not$ready){
    ...
    Remove-Item -LiteralPath $pidFile,$metaFile -Force -ErrorAction SilentlyContinue
    throw "CPF batch worker readiness failed. profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness"
}
Write-Host "CPF_BATCH_WORKER_READY profile=$Profile instance=$InstanceId port=$Port pid=$($p.Id) readiness=$readiness UTF-8"
