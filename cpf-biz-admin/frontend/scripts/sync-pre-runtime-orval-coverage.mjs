...
function requestOptionLines(parameters, hasBody) {
...
function mutationBlock(id, method, route, pathParams, contractParams, paramsType, bodyType) {
  const symbol = pascal(id);
  const pathArgs = pathParams.map(value => `${value.name}: ${value.type}`);
  const paramsRequired = contractParams.some(value => value.required);
  const paramsArg = contractParams.length ? `params${paramsRequired ? "" : "?"}: ${paramsType}` : null;
  ...
  const requestOptions = requestOptionLines(contractParams, Boolean(bodyType));
...
for (const value of missing) {
  ...
  if (value.operation.requestBody) {
    const schema = value.operation.requestBody.content?.["application/json"]?.schema;
  ...
  const generated = value.method === "get"
    ...
    : mutationBlock(value.operation.operationId, value.method, value.route, pathParams, contractParams, paramsType, bodyType);
