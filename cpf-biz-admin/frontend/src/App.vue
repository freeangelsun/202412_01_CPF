...
async function syncRoute():Promise<void>{
  ...
  if(!requested)return;
  const selected=visibleRoutes.value.find(item=>item.id===requested);
  if(!selected)return;
...
<template>
  ...
  <div id="appView" class="app-shell" :class="{'sidebar-open':sidebarOpen}" :hidden="!authenticated">
    ...
    <div class="workspace">
      ...
      <main class="content"><Suspense><router-view /><template #fallback><div class="route-loading">화면을 준비하고 있습니다...</div></template></Suspense><RouteOperationWorkbench :title="activeRoute.label" :operation-ids="activeRoute.expectedOperationIds" /></main>
    </div>
  </div>
