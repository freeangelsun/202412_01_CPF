...
const loginMessage=ref(""); const passwordMessage=ref(""); const passwordOpen=ref(false);
...
function go(routeId:string){location.hash=`#/${routeId}`;}
...
<template>
  ...
  <div id="appView" class="app-shell" :hidden="!authenticated">
    <aside class="sidebar">
      ...
      <nav aria-label="업무 백오피스 메뉴">
        <section v-for="group in routeGroups" :key="group.id" class="nav-group"><p>{{group.label}}</p><button v-for="route in group.items" :key="route.id" type="button" :class="{active:activeRoute.id===route.id}" @click="go(route.id)"><span class="nav-dot"></span><span>{{route.label}}</span></button></section>
      </nav>
    ...
    <div class="workspace">
      <header class="workspace-header"><div><p class="eyebrow">BZA / {{activeRoute.group.toUpperCase()}}</p><h1>{{activeRoute.label}}</h1><p>{{activeRoute.description}}</p></div><div class="toolbar"><button class="ghost" @click="passwordOpen=true">비밀번호 변경</button><button class="primary" @click="navigateFromHash">새로고침</button></div></header>
