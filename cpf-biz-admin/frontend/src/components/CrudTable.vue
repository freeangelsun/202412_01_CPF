...
const props=withDefaults(defineProps<{
  ...
  rawOperation?:(id:string,reason:string)=>Promise<Record<string,unknown>>;
  disableOperation?:(row:Record<string,unknown>,reason:string)=>Promise<unknown>; disableAction?:string;
  createAction?:string;updateAction?:string;
}>(),{paged:true,pageSize:20,rawIdField:"employeeNo",disableAction:"WRITE",createAction:"WRITE",updateAction:"WRITE"});
...
const disableOpen=ref(false),disableReason=ref("업무 기준정보 비활성화"),disableRow=ref<Record<string,unknown>|null>(null),disableConfirmed=ref(false);
const page=ref(0),size=ref(props.pageSize),total=ref(0),editing=ref(false);const form=reactive<Record<string,any>>({});
const canCreate=computed(()=>hasBzaPermission(props.menuCode,props.createAction));
const canUpdate=computed(()=>hasBzaPermission(props.menuCode,props.updateAction));
const canDisable=computed(()=>!!props.disableOperation&&hasBzaPermission(props.menuCode,props.disableAction));
const writable=computed(()=>canCreate.value||canUpdate.value||canDisable.value);
...
async function loadRaw(){
  ...
  try{
    ...
    rawRecord.value=props.rawOperation
      ? await props.rawOperation(id,rawReason.value.trim())
      : await bzaApi<Record<string,unknown>>(endpoint,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({reason:rawReason.value.trim()})});
  }catch(e){rawRecord.value={};rawError.value=e instanceof Error?e.message:String(e);}
  finally{rawLoading.value=false;}
}
function disable(row:Record<string,unknown>){if(!props.disableOperation)return;disableRow.value=row;disableReason.value="업무 기준정보 비활성화";disableConfirmed.value=false;disableOpen.value=true;}
function cancelDisable(){disableOpen.value=false;disableRow.value=null;disableConfirmed.value=false;}
async function confirmDisable(){if(!props.disableOperation||!disableRow.value)return;if(disableReason.value.trim().length<5){error.value="비활성화 사유는 5자 이상이어야 합니다.";return;}if(!disableConfirmed.value)return;try{await props.disableOperation(disableRow.value,disableReason.value.trim());cancelDisable();await load();}catch(e){error.value=e instanceof Error?e.message:String(e);}}
...
<template><section class="card table-card"><div class="card-head"><div><p class="eyebrow">MANAGEMENT</p><h2>{{title}}</h2></div><div class="inline-actions"><span class="count-pill">{{total}}건</span><button v-if="canCreate" class="primary" type="button" @click="open()">등록</button></div></div><p v-if="error" class="error-banner">{{error}}</p><div v-if="loading" class="empty-state">조회 중...</div><div v-else class="table-wrap"><table><thead><tr><th v-for="c in columns" :key="c">{{columnLabel(c)}}</th><th v-if="writable||rawReadable">관리</th></tr></thead><tbody><tr v-for="(row,index) in rows" :key="String(row.id||row.permissionId||row.assignmentId||row.organizationCode||row.employeeNo||index)"><td v-for="c in columns" :key="c">{{row[c]??"-"}}</td><td v-if="writable||rawReadable"><button v-if="canUpdate" type="button" class="ghost" @click="open(row)">수정</button><button v-if="rawReadable" type="button" class="ghost" @click="requestRaw(row)">원문 보기</button><button v-if="canDisable" type="button" class="danger" @click="disable(row)">비활성화</button></td></tr><tr v-if="rows.length===0"><td :colspan="columns.length+((writable||rawReadable)?1:0)" class="empty-cell">조회 결과가 없습니다.</td></tr></tbody></table></div><div v-if="paged" class="pager"><button class="ghost" :disabled="page===0" @click="prev">이전</button><span>{{page+1}} / {{Math.max(totalPages,1)}}</span><button class="ghost" :disabled="(page+1)*size>=total" @click="next">다음</button></div>
...
<dialog :open="disableOpen" class="modal"><form class="modal-card" @submit.prevent="confirmDisable"><div class="card-head"><div><p class="eyebrow">DANGEROUS ACTION</p><h2>비활성화 확인</h2></div><button type="button" class="icon-button" @click="cancelDisable">×</button></div><label><span>감사 사유</span><textarea v-model="disableReason" rows="3" required></textarea></label><label><input v-model="disableConfirmed" type="checkbox"> 대상과 영향 범위를 확인했습니다.</label><div class="dialog-actions"><button type="button" class="ghost" @click="cancelDisable">취소</button><button type="submit" class="danger" :disabled="!disableConfirmed||disableReason.trim().length<5">비활성화</button></div></form></dialog>
</section></template>
