...
async function disable(row:Record<string,unknown>){if(!props.disableOperation)return;const reason=window.prompt("비활성화 사유를 입력하세요.","업무 기준정보 비활성화");if(!reason||reason.trim().length<5)return;try{await props.disableOperation(row,reason.trim());await load();}catch(e){error.value=e instanceof Error?e.message:String(e);}}
