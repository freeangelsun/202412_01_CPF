...
import { configureCpfBffSessionRecovery } from "../../shared/orval-mutator";
import {
  bzaAuthChangePassword,
  bzaAuthLogin,
  bzaAuthLogout,
  bzaAuthMe,
  bzaAuthRefresh
...
export function clearBzaSession(): void {
  ...
  bzaSession.loaded = false;
  bzaSession.message = "";
}

async function refreshBzaSession(expectedEpoch: number): Promise<void> {
  const result = await bzaAuthRefresh<{ operator?: BzaOperator }>();
  if (sessionEpoch !== expectedEpoch) {
    throw new Error("이미 종료된 BZA 세션의 refresh 응답입니다.");
  }
  if (!result.operator?.loginId) {
    throw new Error("BFF refresh 후 서버 세션 주체를 확인할 수 없습니다.");
  }
  bzaSession.operator = result.operator;
  bzaSession.loaded = true;
}

configureCpfBffSessionRecovery({
  epoch: () => sessionEpoch,
  refresh: refreshBzaSession,
  clear: expectedEpoch => {
    if (sessionEpoch === expectedEpoch) clearBzaSession();
  }
});
...
export async function loginBza(loginId: string, password: string): Promise<void> {
  ...
  try {
    const result = await bzaAuthLogin<{ operator?: BzaOperator }>({ data: { loginId, password, operationId } });
...
export async function loadBzaOperator(): Promise<void> { bzaSession.operator = await bzaAuthMe<BzaOperator>(); bzaSession.loaded = true; }
export async function restoreBzaSession(): Promise<void> { try { await loadBzaOperator(); } catch { clearBzaSession(); } }
export async function logoutBza(): Promise<void> { try { await bzaAuthLogout(); } finally { clearBzaSession(); } }
export async function changeBzaPassword(currentPassword: string, newPassword: string, newPasswordConfirm: string): Promise<void> {
  await bzaAuthChangePassword({ data: { currentPassword, newPassword, newPasswordConfirm } });
  clearBzaSession();
}
