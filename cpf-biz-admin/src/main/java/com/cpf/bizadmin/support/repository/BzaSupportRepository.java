...
public class BzaSupportRepository {
    ...
    public Map<String, Object> dashboard(String loginId) {
        return jdbc().queryForMap(sql.required("support-repository-dashboard-01"), new MapSqlParameterSource("loginId", loginId));
    }
    ...
    public long insertNotification(Map<String, ?> values) {
        ...
        jdbc().update(sql.required("support-repository-insert-notification-01"), new MapSqlParameterSource(values), keyHolder, new String[]{"notification_id"});
    ...
    public int markNotificationRead(long notificationId, String loginId, String requestUser) {
        return jdbc().update(sql.required("support-repository-mark-notification-read-01"), new MapSqlParameterSource()
    ...
    public int markAllNotificationsRead(String loginId,String requestUser) {
        return jdbc().update(sql.required("support-repository-mark-all-notifications-read-01"),new MapSqlParameterSource().addValue("loginId",loginId).addValue("requestUser",requestUser));
    }

    public List<Map<String, Object>> findAttachments(String groupId) {
        return jdbc().queryForList(sql.required("support-repository-find-attachments-01"), new MapSqlParameterSource("groupId", groupId));
    }

    public Optional<Map<String, Object>> findAttachment(long attachmentId) {
        return jdbc().queryForList(sql.required("support-repository-find-attachment-01"), new MapSqlParameterSource("attachmentId", attachmentId)).stream().findFirst();
    }

    public long insertAttachment(Map<String, ?> values) {
        KeyHolder keyHolder = new GeneratedKeyHolder();
        jdbc().update(sql.required("support-repository-insert-attachment-01"), new MapSqlParameterSource(values), keyHolder, new String[]{"attachment_id"});
    ...
    public int updateAttachmentSecurity(long attachmentId,String scanStatus,String dataClassification,java.time.Instant retentionUntil,String quarantineYn,String useYn,String requestUser) {
        return jdbc().update(sql.required("support-repository-update-attachment-security-01"),new MapSqlParameterSource().addValue("attachmentId",attachmentId).addValue("scanStatus",scanStatus).addValue("classification",dataClassification)
    ...
    public List<Map<String, Object>> findSavedSearches(String loginId, String screenCode) {
        return jdbc().queryForList(sql.required("support-repository-find-saved-searches-01"), new MapSqlParameterSource()
                .addValue("loginId", loginId)
    ...
    public Optional<Map<String, Object>> findSavedSearch(String loginId, String screenCode, String searchName) {
        return jdbc().queryForList(sql.required("support-repository-find-saved-search-01"), new MapSqlParameterSource()
                .addValue("loginId", loginId)
                .addValue("screenCode", screenCode)
    ...
    public int disableSavedSearch(long savedSearchId, String loginId, String requestUser) {
        return jdbc().update(sql.required("support-repository-disable-saved-search-01"), new MapSqlParameterSource()
    ...
    public List<Map<String, Object>> findRolePermissions(List<String> roleCodes) {
        return jdbc().queryForList(sql.required("support-repository-find-role-permissions-01"), new MapSqlParameterSource("roleCodes", roleCodes));
    }
