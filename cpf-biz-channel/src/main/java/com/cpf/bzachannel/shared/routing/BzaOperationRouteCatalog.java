package com.cpf.bzachannel.shared.routing;

import com.cpf.bzachannel.shared.error.UnsupportedBzaOperationException;
...
@Component
public final class BzaOperationRouteCatalog {
    private final List<Route> routes;

    public BzaOperationRouteCatalog() {
        try (var stream = BzaOperationRouteCatalog.class.getResourceAsStream("/bza-routes.tsv")) {
            if (stream == null) throw new IllegalStateException("bza-routes.tsv missing");
            ...
            try (var reader = new BufferedReader(new InputStreamReader(stream, StandardCharsets.UTF_8))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (parts.length != 3) throw new IllegalStateException("invalid BZA route catalog row: " + line);
            ...
            if (routes.isEmpty()) throw new IllegalStateException("BZA route catalog is empty");
        } catch (java.io.IOException e) {
            throw new IllegalStateException("failed to load BZA route catalog", e);
        }
    }
    ...
    public Route require(String method, String path) {
        return routes.stream().filter(r -> r.method().equalsIgnoreCase(method) && r.pattern().matcher(path).matches())
                .findFirst().orElseThrow(() -> new UnsupportedBzaOperationException(method + " " + path));
    }
    ...
    public record Route(String method, String template, String operationId, Pattern pattern, int staticSegments) {
        static Route of(String method, String template, String operationId) {
            ...
            if (!template.startsWith("/api/bza/")) throw new IllegalArgumentException("route outside BZA public boundary");
