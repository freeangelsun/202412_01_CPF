...
public class CmnCalendarService extends CmnBaseService implements CmnBusinessCalendar {
    ...
    CmnCalendarService(CmnCalendarStore store){this(store,CmnCalendarChangePublisher.noop(),false);}
    CmnCalendarService(CmnCalendarStore store,CmnCalendarChangePublisher changePublisher,boolean productMode){this.store=Objects.requireNonNull(store);this.changePublisher=Objects.requireNonNull(changePublisher);this.productMode=productMode;}
    ...
    private void publishChange(CmnCalendarChangeEvent event){
        if(productMode){
            // Product mode publisher is a durable cmnDB outbox adapter. Propagate failure so the surrounding
