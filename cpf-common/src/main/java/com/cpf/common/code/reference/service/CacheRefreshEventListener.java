...
public class CacheRefreshEventListener {
    ...
    public CacheRefreshEventListener(
        ...
        this(mapper, code, message, response, config, Clock.systemUTC());
    }
    ...
    public CacheRefreshEventListener(
        ...
        this.mapper = mapper; this.code = code; this.message = message;
        this.response = response; this.config = config; this.clock = java.util.Objects.requireNonNull(clock, "clock");
    }
    ...
    public void initialize() {
        ...
        try {
            ...
            if (checkpoint == null) {
                ...
                refreshAll();
                lastFullRefreshAt = clock.instant();
            ...
            lastSuccessfulPollAt = clock.instant();
    ...
    private void refreshCache(String name) {
        switch (name) {
            ...
            case "businessCalendar", "commonTemplate" -> {
                // Calendar and Template providers read canonical cpfDB common store on every request and do not
