...
public class DefaultCpfResponseCodeResolver implements CpfResponseCodeResolver {
    ...
    public CpfResolvedResponse resolve(
        ...
        return new CpfResolvedResponse(
                defaultHttpStatus(errorDefinition.category()),
    ...
    private int defaultHttpStatus(CpfErrorDefinition.Category category) {
        if (category == null) return 500;
        return switch (category) {
            case VALIDATION -> 400;
            case AUTHENTICATION -> 401;
            case AUTHORIZATION -> 403;
            case NOT_FOUND -> 404;
            case CONFLICT, BUSINESS -> 409;
            case RATE_LIMIT -> 429;
            case EXTERNAL, INFRASTRUCTURE -> 503;
            case INTERNAL -> 500;
        };
    }
