...
public final class CpfReconciliationWorker implements CpfReconciliationRuntimeStatus {
    ...
    private CpfReconciliationProbePort.ProbeResult executeProbe(
        ...
        String correlationSeed = record.transactionId() == null || record.transactionId().isBlank()
                ? record.unknownId()
                : record.transactionId();
        String transactionId = "reconciliation-" + sha256(correlationSeed).substring(0, 32);
    ...
    private static String safe(String value) {
        return SensitiveDataMasker.mask(value == null ? "" : value, 300);
    }
