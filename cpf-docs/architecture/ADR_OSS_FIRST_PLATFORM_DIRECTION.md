...
CPF는 검증된 permissive OSS를 Primary로 사용하되 제품 고유 책임을 명확히 소유한다.
...
현재 제품 Gateway는 SCG Server Web MVC 하나다.
...
- exactly-once는 DB side effect까지 증명되지 않으면 제품 보장 금지
