...
External WAS는 정본 목표지만 현재 Source에 WAR Packaging과 Servlet Initializer Consumer가 없다. Tomcat Image만 준비하는 것은 Runtime 완료 근거가 되지 않으므로 기본 환경에 넣지 않고 Source Packaging Gap으로 추적한다. Frontend 독립 Web Server도 공식 제품과 배포 계약이 확정된 뒤 추가한다.
