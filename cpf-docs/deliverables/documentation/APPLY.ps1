...
$tmp=Join-Path $env:TEMP ('cpf-doc-2154-'+[guid]::NewGuid().ToString('N'));New-Item -ItemType Directory -Path $tmp -Force|Out-Null
try{
  ...
  Get-Content -LiteralPath $m -Encoding UTF8|ForEach-Object{
    ...
    $p=[IO.Path]::GetFullPath((Join-Path $root ($r.Replace('/',[string]$sep))))
  ...
  Write-Host "[CPF][DOC] APPLIED=2.15.4 ZIP=$ZipPath"
