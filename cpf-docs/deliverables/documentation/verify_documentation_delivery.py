...
run([sys.executable,str(H/'validators/validate_architecture_visual_semantics.py')])
run([sys.executable,str(H/'validators/validate_pdf_openability.py')])
...
if si.get('harnessVersion')!='2.13.0': raise SystemExit('source identity harness mismatch')
...
if pm.get('harnessVersion')!='2.13.0': raise SystemExit('package manifest harness mismatch')
...
print(f'DOCX={len(docx)} PDF={len(pdf)} MANIFEST_FILES={len(pm.get("files",[]))} HARNESS=2.13.0')
