...
| 포함 범위 | 코어, 공통, `cpf-starters/` 제품 컨테이너, 7개 선택 스타터, 업무 영역, ADM, BZA, 게이트웨이, 배치, DB, 화면, 빌드 |
...
| 책임 | 실제 경로 | 형태 | 핵심 역할 |
|---|---|---|---|
| Core | `cpf-core` | 독립 라이브러리 | 구현 중립 공개 API·SPI와 현재 최소 실행 기반 |
| Common | `cpf-common` | 독립 라이브러리 | 고객 업무 공통 정책과 Core 확장 |
| 스타터 제품군 | `cpf-starters/` | 고정 제품 컨테이너 | 기능별 스타터 소스코드의 물리적 루트 |
| 보안 스타터 | `cpf-starters/security` | `:cpf-starter-security` | ADM/BZA Session JDBC·CSRF·Vault 조립 |
| Kafka 스타터 | `cpf-starters/messaging-kafka` | `:cpf-starter-messaging-kafka` | `CpfBrokerClient`·Bridge의 Kafka 연결 |
| 캐시 스타터 | `cpf-starters/cache` | `:cpf-starter-cache` | LOCAL·Caffeine·Redis 캐시 활성화 |
| 관측 스타터 | `cpf-starters/observability` | `:cpf-starter-observability` | Micrometer Observation·OTel 연결 |
| 복원력 스타터 | `cpf-starters/resilience` | `:cpf-starter-resilience` | CircuitBreaker 기반 실행 어댑터 |
| 기능 플래그 스타터 | `cpf-starters/featureflag` | `:cpf-starter-featureflag` | OpenFeature 클라이언트 연결 |
| 비밀값 스타터 | `cpf-starters/secret` | `:cpf-starter-secret` | 승인된 `CpfSecretProvider` Registry |
| Generated Domain | `cpf-member` | 실행형 JAR·WAR | 생성기 산출 구조의 Golden Reference |
| ADM/BZA/Gateway/Reference | 각 루트 경로 | 제품 실행 모듈 | 운영·업무관리·진입·교육 책임 |
| Batch | `cpf-batch/*` | 계약·실행 제품군 | Spring Batch·Scheduler·Worker·Center-Cut |
| 빌드 규칙 | `cpf-tools/build/gradle-plugin` | 포함 빌드 | 공통 빌드·검증 규칙 |
| Platform BOM | `cpf-tools/build/platform-bom` | 포함 빌드 | CPF·Spring 버전 정렬, 스타터 자동 제약 |

`cpf-starters/`는 Gradle 프로젝트가 아니며 별도 `build.gradle`이나 `check` 작업을 갖지 않습니다.
실제 프로젝트 등록 정본은 `settings.gradle`의 7개 `include`와 `projectDir` 연결입니다.

<p align="center">
  <img src="png/CPF스타터프로젝트관리.png" width="96%" alt="cpf-starters 제품 컨테이너와 7개 기능별 Gradle 프로젝트 구조" />
</p>

### 4.1.1 현재 실제 사용처와 책임 경계

| 스타터 | 확인된 실제 제품 소비자 | 현재 구조 판단 |
|---|---|---|
| 보안 | ADM, BZA | 제품 연결 존재. Session/Vault 정책과 범용 보안 스타터 세분화 필요 |
| 관측 | Gateway, Batch Execution Runtime | 제품 연결 존재. 실제 수집기·마스킹·Backpressure 검증 필요 |
| 복원력 | Gateway, Batch Execution Runtime | 제품 연결 존재. 시간 예산·멱등성·결과 미확정 대사 검증 필요 |
| Kafka | 주요 제품 빌드 직접 소비 미확인 | 구현과 게시 설정은 있으나 Consumer 연결 재확인 |
| 캐시 | 주요 제품 빌드 직접 소비 미확인 | `cpf-common` 역전이와 Caffeine·Redis 동시 전이 구조 재검토 |
| 기능 플래그 | 실제 제품 소비자 미확인 | Provider·감사·Secure Override 전 부분 구현 |
| 비밀값 | 실제 제품 소비자 미확인 | 고객 Provider·Rotation·상태 점검 전 부분 구현 |

제품 고유 조합은 범용 프로필로 숨기지 않습니다. 예를 들어 Gateway와 Batch Execution Runtime은
관측·복원력 스타터를 각 제품 빌드에서 직접 선택하며, ADM/BZA는 보안 스타터를 직접 선택합니다.

### 4.1.2 경량 코어와 선택 스타터 목표 구조

<p align="center">
  <img src="png/CPF경량코어의존구조.png" width="96%" alt="현재 Core와 목표 Leaf Starter 의존 방향" />
</p>

```text
비 Spring 계약 소비자 ───────→ cpf-core
제품·Generated Domain ──────→ cpf-core / cpf-common
                            └→ 필요한 :cpf-starter-* Leaf만 선택

:cpf-starter-* ─────────────→ cpf-core 공개 API·SPI
                            └→ 승인된 외부 기술 라이브러리
```

허용 방향은 제품·업무 → 스타터 → Core입니다. 스타터가 ADM·BZA·Gateway·Batch·Generated Domain을
참조하거나 Core가 선택 스타터를 역참조하면 구조 실패입니다.

현재 Core에는 MyBatis, AspectJ, Validation, Commons Compress, Springdoc UI·Scalar가
`implementation`으로 남아 있고 WebFlux·RestClient·Batch·OTel은 컴파일 경계에 존재합니다.
Gateway 공통 계약 일부가 Core에서 제거된 것은 경량화 진척이지만 목표 달성은 아닙니다.

### 4.1.3 Leaf·Profile·Aggregate·BOM 선택 계층

| 계층 | 아키텍처 책임 | 현재 상태 |
|---|---|---|
| Leaf Starter | 단일 기술 기능의 구현·자동 설정·시험·게시 정본 | 7개 부분 구현 |
| Capability Profile | 생성기·빌드가 Leaf 목록으로 해석하고 결과를 잠금 | 정책 후보, 미구현 |
| Aggregate Starter | 검증된 Leaf 조합을 하나의 의존성으로 전이 | 미구현 |
| Platform BOM | 버전·호환성 정렬만 수행 | 구현됨 |

Profile은 실행 코드를 소유하지 않고 생성 결과에 해석된 Leaf 목록을 남겨야 합니다. Aggregate는
고유 Bean·자동 설정·DB를 소유하지 않으며 승인된 Leaf 의존성만 전이합니다. `all`, `full`,
`everything` 형태의 Mega Starter는 금지합니다.

현재 `create-domain.ps1`, 중앙 Domain 계약, `cpf-member/build.gradle`에는 Profile·
`resolvedStarters`·Profile Version Lock이 없으므로 Generated Domain은 아직 직접 실행 기술을
선언합니다.

### 4.1.4 상호 배타 선택과 충돌 처리

- 메시징: `NONE | KAFKA | RABBITMQ(공식 승인 시)`
- 캐시: `NONE | CAFFEINE | REDIS`
- 보안: `NONE | SESSION_JDBC | RESOURCE_SERVER`
- 영속성: `NONE | MYBATIS | 승인된 다른 Provider`

Profile이나 Aggregate가 상호 배타 Provider를 동시에 포함하면 생성·빌드 단계에서 거부해야 합니다.
공존이 필요한 전환 시나리오는 별도 요구사항·책임 주체·실행 시험·운영 절차가 있어야 합니다.
...
```text
제품·Generated Domain    → 필요한 :cpf-starter-* Leaf만 선택
...
- `cpf-starters/`는 스타터 목록·등록·통합 검증을 관리하고 고객 실행 경로에는 필요한 `:cpf-starter-*`만 선택하게 합니다.
...
- `cpf-starters/`를 통해 7개 스타터의 모든 구현 의존성을 고객 모듈에 일괄 전파
...
### 22.5 경량 코어·스타터 전환 위험

| 위험 | 현재 근거 | 통제 |
|---|---|---|
| Core 선택 실행 의존성 잔존 | MyBatis·AspectJ·Validation·Compress·Springdoc가 `implementation` | 공개 계약·실제 사용처·시험을 함께 이관하고 이전 기본 경로 제거 |
| 캐시 역방향 전이 | 캐시 스타터가 `cpf-common`을 `api`로 전이 | 예외 승인 또는 Cache Runtime·Provider Leaf 분리 |
| Provider 묶음 비대화 | 캐시 스타터가 Caffeine·Redis를 동시에 공개 의존성으로 포함 | `cache-caffeine`, `cache-redis` 후보를 Consumer·Footprint로 검증 |
| 자동 설정 무조건 활성화 | 비밀값 Registry는 Provider 0개이면 기동 실패 | 실제 Consumer가 명시적으로 선택하고 Provider·상태 점검을 함께 제공 |
| 구현만 있고 Consumer 없음 | Kafka·기능 플래그·비밀값의 주요 제품 소비자 미확인 | Consumer 연결 전 GA 금지 |
| Profile 이름만 남음 | 생성기·Manifest에 해석된 Leaf 목록·잠금 없음 | Profile 구현 시 결과 Leaf 목록·버전·SBOM 기록 |
| 제품 고유 조합 은닉 | Batch/Gateway/ADM/BZA 조합을 범용 Profile로 만들 위험 | 제품 빌드 또는 제품 전용 규칙에서 관리 |
| BOM 오해 | BOM 등록만으로 실행 기술이 들어온다고 오해 | BOM은 제약만 제공하고 실행 의존성 0을 시험 |
| 정본 정책 SHA 노후화 | Starter 정책 머리말이 이전 분석 SHA를 가리킴 | 본문보다 최신 Source를 우선하고 정책 헤더 갱신 필요 |
