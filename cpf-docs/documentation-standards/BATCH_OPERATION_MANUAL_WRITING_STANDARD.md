...
## 현재 공식 문서 체계 우선 규칙 (2026-08-16)

이 문서의 과거 절에 배치 운영 내용을 `04 운영자 매뉴얼` 내부로 병합하는 권고가 있더라도 현재 공식 7종 체계에서는 **`05 배치 운영 가이드`가 독립 Owner**다. 현재 사용자 문서 체계가 우선하며 역할은 다음과 같이 유지한다.

- `03 배치 개발자 가이드`: Job/Step을 어떻게 설계하고 개발·테스트하는가
- `05 배치 운영 가이드`: 만들어진 Batch를 어떻게 실행·중지·재시작·재처리·대사하는가
- `04 운영자 매뉴얼`: 일반 Runtime/ADM/BZA 운영과 공통 운영 판단

새 문서를 추가하지 않고 위 7종 Owner 경계 안에서 내용을 보강한다.
