...
# 레이아웃·페이지·표 최종 품질 Gate (2026-08-16)

공식 02~07 문서는 DOCX 구조 검사만으로 완료하지 않고 실제 DOCX/PDF 렌더 결과를 전 페이지 확인한다.

- 표지와 전체 목차를 제외한 본문에서 내용 없는 빈 페이지를 허용하지 않는다.
- 새 대장/대메뉴 시작이 아닌데 하단의 절반 이상을 비우기 위해 강제 Page Break를 사용하지 않는다.
- 제목은 다음 내용과 함께 이동시키되, `전체 목차로` 같은 Navigation 문구가 다음 표·문단 전체를 다음 페이지로 밀지 않게 한다.
- 문서 좌우 여백은 가독성과 출력 안전 범위 안에서 본문 폭을 충분히 확보한다.
- 표는 한두 글자 때문에 의미 없이 두 줄이 되는 셀을 줄이도록 컬럼 폭을 개별 최적화한다. 짧은 `No/상세/상태` 컬럼은 좁게, API·용도·옵션·참고 컬럼은 넓게 배분한다.
- 표현을 짧게 바꾸면 한 줄로 정리되는 경우 의미를 보존하는 범위에서 문구를 압축한다. 정확한 API·Property·Class 명칭은 임의 축약하지 않는다.
- 그림은 박스·텍스트·화살표가 카드 경계를 침범하거나 서로 겹치지 않아야 하며, PDF 100% 배율에서 주요 텍스트를 읽을 수 있어야 한다.
- 최종 시각 Gate는 `blank page / collision / clipping / overflow / orphan heading / avoidable wrap / broken glyph = 0`을 목표로 한다.
