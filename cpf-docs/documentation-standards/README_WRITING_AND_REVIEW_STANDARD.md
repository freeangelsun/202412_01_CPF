...
# 59. 사람과 AI가 함께 읽는 Repository 첫 화면 기준 (2026-08-16)

README 최상단은 사람뿐 아니라 Repository를 1차 분석하는 AI가 CPF의 목적과 사용 경계를 잘못 추론하지 않도록 다음 6개 사실을 명시적으로 유지한다.

1. CPF가 무엇인가
2. 어떤 문제를 해결하는가
3. Spring Boot와 어떤 관계인가
4. 개발자가 실제 쓰는 Golden Path
5. 어떤 규모·구성의 시스템에 적합한가
6. 5분 안에 실행하는 방법

이 6개는 README 상단의 Canonical First Impression이다. 단, 바로 뒤의 `구조 한눈에 보기`에서 같은 내용을 문장만 바꿔 반복하지 않는다. 상단 6개는 **정의와 계약**, Quick Overview는 **구조 지도**, 상세 Architecture는 **구성요소와 경계**를 담당한다.

AI가 저수준 클래스를 Golden Path로 오인하지 않도록 일반 업무 개발은 Public Starter/Public API를 사용하며 Advanced/SPI/Internal은 별도 사용 수준이라는 경계를 함께 명시한다. 구현되지 않은 기능, 계획 상태, 미검증 Runtime 품질을 현재 지원 사실처럼 단정하지 않는다.
