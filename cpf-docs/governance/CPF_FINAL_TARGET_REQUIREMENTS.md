...
> Canonical Requirement Count: **208개**
...
- `CPF_REQUIREMENT_MASTER.csv`, Scenario/Execution Sequence, Control Register 같은 대규모 실행 Dataset은 **208 Canonical Requirement에서 파생된 관리 자료**이며 상위 정본이 아니다. Canonical 변경 후 QA/중앙 관리 Pipeline으로 재생성·정합화해야 하고, 재생성 전 stale Dataset을 근거로 Final Target을 되돌리지 않는다.
