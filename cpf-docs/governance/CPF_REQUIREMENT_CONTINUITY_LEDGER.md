...
물리 파일은 exact Delete Manifest로 정리한다. Git history 보존에 의존하지 않으며 삭제 전 현재도 필요한 Requirement/Decision/검증 조건을 Canonical/Current 문서에 흡수한다.


# Current Architecture Supersession — 2026-08-10 / 10_1

기준 SHA `f4eb3518a98bbdb2ef8582a8709281d8627bcc6a`에서 기존 개발 결과를 다시 검토한 뒤 다음 정책을 **선택지 없이 Current Target으로 확정**한다. 이전 Requirement의 기능 목표는 유지하되 Owner/Path/DB/Starter 구조가 아래와 충돌하면 아래 정책이 우선한다.

| Supersession ID | 기존 | Current Target |
|---|---|---|
| SUP-NXT2-001 | `cpf-common` 독립 고객공통 Product | `cpf-starters/common` CPF Common Product Capability; 고객 특화는 `<customer>-common` |
| SUP-NXT2-002 | `cmnDB/admDB/batDB` 독립 physical DB | CPF 소유 관계형 상태를 `CPF_PLATFORM_DB(cpfDB)`로 통합, Prefix/Metadata Owner 유지 |
| SUP-NXT2-003 | Generated Domain별 `mbrDB/...` | Customer Business DB 통합 + Domain Table Prefix |
| SUP-NXT2-004 | `cpf-member`을 CPF Product Module로 취급 | `cpf-member/`는 Generated Customer Project Root naming으로만 허용; Public BOM/Publication 제외 |
| SUP-NXT2-005 | `cpf-starter-profile-*` 공개명 | `cpf-starter-<profile>` 직관적 공개명 |
| SUP-NXT2-006 | Capability별 `*/core` | `cpf-core` 외 core module 금지 |
| SUP-NXT2-007 | Error enum/source 확장 중심 | Core fallback semantics + DB dynamic Response/Error/Message Catalog |
| SUP-NXT2-008 | Redis next-cycle 신규개발 | 10_1 선구현 승계 + 미흡/Live/Valkey regression closure |

QA 미통과 항목의 기능 Acceptance는 취소하지 않는다. 본 Supersession은 기능을 약화하지 않고 Ownership/Developer Experience/DB topology를 currentize한다.
