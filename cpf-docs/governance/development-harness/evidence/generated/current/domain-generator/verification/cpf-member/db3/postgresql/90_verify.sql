...
BEGIN
    SELECT COUNT(*) INTO v_actual
     ...
     WHERE table_schema = current_schema()
       ...
       AND LEFT(LOWER(table_name), LENGTH(LOWER('MBR_'))) = LOWER('MBR_');
    ...
    SELECT COUNT(*) INTO v_actual FROM information_schema.columns
     WHERE table_schema = current_schema() AND LOWER(table_name) = LOWER('MBR_sample_item');
    IF v_actual <> 14 THEN
        RAISE EXCEPTION 'CPF generated domain sample column count mismatch';
    END IF;
    SELECT COUNT(*) INTO v_actual FROM information_schema.columns
     WHERE table_schema = current_schema() AND LOWER(table_name) = LOWER('MBR_sample_item_idem');
    IF v_actual <> 8 THEN
    ...
    SELECT COUNT(*) INTO v_actual
     ...
     WHERE n.nspname = current_schema()
       AND LOWER(t.relname) IN (LOWER('MBR_sample_item'), LOWER('MBR_sample_item_idem'))
       AND c.oid IS NULL
       AND i.indisvalid;
    SELECT COUNT(*) INTO v_matched
      FROM pg_indexes
     WHERE schemaname = current_schema()
       AND LOWER(tablename || '.' || indexname) IN (LOWER('MBR_sample_item.ix_MBR_sample_item_idem'), LOWER('MBR_sample_item.ix_MBR_sample_item_status'), LOWER('MBR_sample_item.ix_MBR_sample_item_tx'), LOWER('MBR_sample_item_idem.ix_MBR_sample_idem_item'), LOWER('MBR_sample_item_idem.ix_MBR_sample_idem_tx'));
    ...
    SELECT COUNT(*) INTO v_matched
      ...
      JOIN pg_namespace n ON n.oid = t.relnamespace
     WHERE n.nspname = current_schema()
       AND LOWER(t.relname || '.' || c.conname || '.' || c.contype) IN (LOWER('MBR_sample_item.PK_MBR_sample_item.p'), LOWER('MBR_sample_item.uk_MBR_sample_item_key.u'), LOWER('MBR_sample_item.ck_MBR_sample_item_status.c'), LOWER('MBR_sample_item.ck_MBR_sample_item_deleted.c'), LOWER('MBR_sample_item_idem.PK_MBR_sample_item_idem.p'), LOWER('MBR_sample_item_idem.fk_MBR_sample_idem_item.f'), LOWER('MBR_sample_item_idem.ck_MBR_sample_idem_op.c'), LOWER('MBR_sample_item_idem.ck_MBR_sample_idem_deleted.c'));
