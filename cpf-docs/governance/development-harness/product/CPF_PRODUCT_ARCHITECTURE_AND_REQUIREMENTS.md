...
Public release tooling은 staging/validation/`VERIFIED`까지 수행한다. `git add`/index staging/commit/push는 사용자 최종 검토·승인 이전에 수행하지 않는다. `cpf-release/`의 `work/`/`logs/`/`open-git/`은 local-generated transient이지만, catalog-classified Current Verified `binary-repository/`와 `reports/`는 사용자 승인으로 Private CPF master에 보존할 수 있다. Runtime executable JAR은 exact Git LFS scope, 일반 metadata/source는 regular Git이다.
...
Public Runtime Target Catalog에서 `provision=binary`로 선언된 executable product runtime은 Development Master와 Open Git에서 모두 **Git LFS**로 track한다. LFS 대상은 catalog-derived exact `.gitattributes` path뿐이며 전역 `*.jar`/repository glob/size threshold를 사용하지 않는다. Library/starter JAR, POM, checksum, manifest, SBOM, report와 일반 Source는 regular Git이다. Fresh Build → Test/Runtime → checksum/SBOM/manifest → promotion → LFS currentization 순서를 지키며, Fresh consumer는 clone/LFS materialization 후 runtime JAR SHA와 package manifest를 대조하고 실제 실행한다. `GIT_LFS_NOT_AVAILABLE`, `LFS_OBJECT_NOT_MATERIALIZED`, `LFS_DOWNLOAD_FAILED`, `LFS_HASH_MISMATCH`, `RELEASE_MANIFEST_MISMATCH`, `RUNTIME_ARTIFACT_INVALID`은 모두 fail-closed다.
...
- Private Repository Root의 Release delivery root는 `cpf-release/`이다. `work/`, `logs/`, `open-git/`만 transient generated output으로 Git/Product Source Identity에서 제외하며, 검증 후 승격되는 binary metadata/report 및 LFS runtime artifact를 root 전체 ignore로 숨기지 않는다.
- Release 시작 시 이전 **managed candidate**를 안전하게 제거하고 Clean isolated workspace에서 새 Release를 생성하여 **Current Release 한 본만 유지**한다. 이전 Release Binary/JAR/POM/Generated Source/ZIP/Handover/RERUN Evidence/날짜별 Copy/`_OLD`/`_BACKUP`/V1/V2 history를 누적하지 않는다.
...
- **Private CPF Repository/master는 Release Source이며 catalog-classified Current Verified Result의 사용자 승인 Git 반영 위치가 될 수 있으나, Release Tool의 Git write target은 아니다.** `cpf release open-git`과 그 하위 Tool은 Private CPF Repository에서 `git add`, `git commit`, `git push`, branch/tag/reset/restore/stash/clean 등 Git write/history 변경을 수행하지 않는다. Private Git은 Repository Root, Current Branch, HEAD SHA, Working Tree 상태와 provenance를 읽기 위한 read-only 조회만 허용한다. Working Tree 변경이 존재해도 이를 임의 Reset/Restore/Clean하지 않으며 Current Local Source Identity를 기준으로 Release를 생성한다.
- `cpf-release/`는 **Open Git 전달 전용 local-generated delivery root**다. `/cpf-release/` 전체를 Private master `.gitignore`로 제외하지 않으며 `work/`, `logs/`, `open-git/`만 transient로 제외한다. Release 정의/정책/Template/Tool의 정본을 `cpf-release/`에 두지 않고, verified result는 Fresh Release 입력으로 사용하지 않는다.
...
- **Open Git Commit/Push는 필수 Release Gate가 모두 PASS한 뒤 사용자가 `cpf-release/open-git` 결과를 직접 검토하고 Open Git Repository에 수행한다.** CPF 개발 master Commit/Push와 Open Git Commit/Push는 별도 작업이다. Private master에는 catalog-classified `binary-repository/`/`reports/` Current Verified Result만 별도 사용자 승인으로 반영할 수 있고 transient `work/`/`logs/`/`open-git/`은 반영하지 않는다.
...
| `REL-OPEN-GIT-GIT-BOUNDARY` | cpf-tools release/open-git + unified CLI release namespace | `cpf-release/work`/`logs`/`open-git`은 transient local delivery workspace이고 Private master에는 catalog-classified Current Verified `binary-repository`/`reports`만 사용자 승인으로 보존할 수 있다. Runtime executable은 exact Git LFS scope, metadata/source는 regular Git이다. `cpf release open-git`과 Tool/CLI/DevGPT/Codex는 Private master와 Open Git fresh clone 모두에서 사용자 승인 전 add/index staging/commit/push/branch/tag/reset/restore/stash/clean을 수행하지 않는다. Tool은 read-only status/diff/provenance와 Release 검증까지만 수행해 `VERIFIED`를 만들고, 사용자가 결과 확인 후 필요한 각 repository에 직접 commit/push한다. Private Working Tree가 dirty여도 임의 정리하지 않고 Current Source Identity를 기록한다. | transient tracked=0 + verified result path role allowlist, exact LFS attribute/materialization/manifest correlation, private/Open-Git write-command negative, branch/HEAD/status/sourceIdentity capture, GENERATED→VERIFIED→USER_REVIEWED→GIT_COMMITTED→GIT_PUSHED state separation, wrong remote/dirty clone negative, user Git command generation-only, automatic add/commit/push=0 Evidence |
