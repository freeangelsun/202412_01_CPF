# CPF Development Harness Core Execution Policy

이 파일은 기존 개발 기본 지침을 **Harness 내부 비협상 실행정책으로 완전 흡수**한 것이다. 별도 개발 기본 지침 전달을 요구하지 않는다. 하위 정책·도구·Agent 지침이 이 문서보다 약한 완료 기준을 정의하면 하위 정의가 무효다.

## Harness 추가 비협상 규칙

1. **Hardcoding 금지**: Requirement 수, Work Item 수, Profile, Vendor, Header, Module Owner, Status Enum, Gate Count를 Validator/Script 코드에 숫자·문자열로 박아 맞추지 않는다. Canonical Registry/Policy JSON/Source discovery에서 읽는다. 제품 계약상 고정된 이름도 Registry 한 곳에서 선언하고 Consumer가 읽는다.
2. **미구현·부분구현·검증불가 완료 금지**: Interface/DTO/Mock/Sample/Swagger/문서/정적 Gate만으로 완료하지 않는다. 실제 Consumer·호출경로·실패·복구·운영·DB·Config·Frontend·Test·Runtime·Evidence까지 대상 Requirement가 요구하는 전체 범위를 닫아야 한다.
3. **검증 완료 근거 의무**: `완료/PASS/CLOSED`에는 실행 명령, 환경, 시작·종료시각, Exit Code, Source Identity, Evidence 경로, 실제 관측 결과, 영향도·회귀 검토, 완료 사유가 모두 있어야 한다. 하나라도 빠지면 `VERIFICATION_PENDING` 또는 `BLOCKED_EXTERNAL`이다.
4. **최대강도 Test 요청**: 로컬/외부 환경 문제로 사용자 또는 다른 Agent에게 Test를 요청할 때도 smoke로 축소하지 않는다. 대상 기능의 Fresh 초기화부터 E2E·오류·복구·재실행·회귀까지 최고 강도 명령을 Windows/Linux 가능한 형태로 제공한다.
5. **Standalone Runtime 계약**: 독립 실행 프로세스는 Windows PowerShell과 Linux shell의 start/run/stop/status/verify 진입점이 모두 있어야 하고 동일 기능·옵션·Exit Code 의미를 유지한다. 한 OS만 있으면 완료가 아니다.
6. **Profile 계약**: Runtime Application은 `local/dev/stg/test/prod` 전체 Profile 세트를 관리한다. `application.yml`은 공통값만, 환경 차이는 profile 파일에 둔다. Profile 누락·비밀값 Source 저장·환경값 하드코딩은 FAIL이다.
7. **YAML 설명 계약**: 사람이 수정하는 YAML의 실제 설정 항목은 용도·단위·허용값·기본/미지정 의미·보안 주의가 필요한 경우 이를 설명하는 한글 주석을 인접하게 둔다. 값만 나열된 설정은 완료가 아니다.
8. **Java 문서화 계약**: Public API/SPI/Annotation/Configuration 및 중요 Runtime Source의 공개/보호 type·method는 JavaDoc 생성에 충분한 설명을 제공한다. 파라미터·반환·예외·thread/transaction/security/side effect가 계약에 영향을 주면 JavaDoc에 포함한다. 핵심 분기·복구·동시성·보안 의도에는 한국어 주석을 둔다.
9. **UTF-8 단일 표준**: Source/Config/SQL/Script/Markdown/CSV/JSON/YAML은 UTF-8을 기본으로 한다. BOM 허용 여부는 파일 유형 정책에 따른다. mojibake·invalid UTF-8·제어문자·NFC duplicate를 Fail-Closed 처리한다.
10. **Current-only Harness**: `development-harness/` 현행본 하나만 유지한다. v1/v2, old, backup, history, session, checkpoint, rerun 같은 Harness 복제본을 Repository에 남기지 않는다. Harness 변경은 현행 파일을 갱신하고 Manifest/Evidence를 재생성한다.
11. **세부 리뷰 의무**: 사용자가 개발/검수/완료 리뷰를 요청하면 Requirement/WP/세부 Work Item 각각을 원요구→영향 Source/Consumer→변경→Side Effect→Test/Runtime→Evidence→완료/미완료 사유 순서로 1:1 리뷰한다. 상위 요약만으로 대체할 수 없다.
12. **제품 완성도**: 코드가 동작하는 것만으로 완료하지 않는다. 사용성, 발견 가능성, 오류 메시지, 접근성, 운영성, 보안, 성능, 설치/업그레이드/롤백, 문서, 배포, 관찰 가능성, 장애복구, 고객 개발 DX까지 완성품 관점에서 Acceptance에 포함한다.

---

## 기존 개발 기본 지침 — Harness 내부 흡수본
...
- 각 항목은 요구사항이 축소·왜곡되지 않도록 충분히 상세하게 작성하며 최소 다음을 추적 가능하게 관리해 주세요.
  **ID / 원 Requirement·Steering / Root Cause / 영향 범위 / 우선순위 / 변경 Source·Consumer / 개발·보완 내용 / Test·Verifier / Runtime / Evidence / 상태 / Source Identity / 미검증·BLOCKED\_EXTERNAL / 재실행 조건**
...
- 필수 항목에 **FAIL / SKIP / NOT\_EXECUTED / UNKNOWN / Runtime 미실행 / Evidence·Source Identity 불일치**가 있으면 전체 PASS로 처리하지 말아 주세요.
...
- 인수인계에는 최소 다음을 포함해 주세요.
  **기준 Source Identity / 현재 정본 / 전체 개발 목록 및 상태 / CLOSED·진행 중·BLOCKED\_EXTERNAL·미검증 항목 / 주요 Root Cause / 완료 개발 내용 / 남은 작업 / 재실행 조건 / 주요 검증 결과 / Canonical Final Gate 상태 / Delete Manifest 상태**
...
## 8. 외부 개발 에이전트 LONG-TURN MODE 및 병행 세션 고정 규칙
...
**본 절은 1~7절의 개발·Closure·최종 완료 기준을 변경하거나 완화하지 않으며, 외부 Coding Agent의 실행 방식과 병행 세션 비간섭 원칙만 추가로 규정합니다.**
