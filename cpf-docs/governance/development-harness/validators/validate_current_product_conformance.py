...
out={'sourceZipSha256':c['sourceIdentity']['zipSha256'],'baselineProductContentSha256':c['sourceIdentity']['baselineProductContentSha256'],'findingCount':len(findings),'untrackedCount':len(untracked),'findings':findings,'status':'PASS' if not findings else ('UNTRACKED_GAPS' if untracked else 'TRACKED_GAPS')}
