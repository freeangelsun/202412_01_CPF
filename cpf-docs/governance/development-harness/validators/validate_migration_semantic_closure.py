...
if set(dd)!=set(mp): errs.append(f'DELETE_MANIFEST_SET_DRIFT missing={len(set(mp)-set(dd))} extra={len(set(dd)-set(mp))}')
...
for old,r in mp.items():
 np=ROOT/r['new_path']
 if not np.is_file(): errs.append('MIGRATION_REPLACEMENT_MISSING '+old+' -> '+r['new_path'])
 ...
 if protected:
  ...
  if d.get('delete_eligible')!='false' or d.get('lifecycle')!='PROTECTED_RETAIN' or d.get('precondition')!='PROTECTED_PATH_RETAIN': errs.append('PROTECTED_DELETE_MANIFEST_BAD '+old)
 else:
 ...
 if op.is_file():
  actual=sha(op)
  if d.get('expected_sha256')!=actual: errs.append('DELETE_EXPECTED_SHA_DRIFT '+old)
  if r.get('observed_current_sha256')!=actual: errs.append('MIGRATION_OBSERVED_SHA_DRIFT '+old)
 else:
...
eligible=sum(1 for r in dm if r.get('delete_eligible')=='true')
retained=sum(1 for r in dm if r.get('delete_eligible')!='true')
