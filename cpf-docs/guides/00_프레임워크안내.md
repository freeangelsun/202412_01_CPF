...
![CPF 전체 제품 Architecture](../assets/guides/cpf-platform-architecture.svg)

그림의 왼쪽에서 오른쪽으로 사용 채널, Gateway·관리 Plane, 업무 실행 Plane, 플랫폼 기반을 읽는다. 모든 영역은 `transactionId`, 멱등성, 권한, 승인, 감사, Timeout, 대사 계약으로 연결된다.

### 역할별 공식 매뉴얼 지도

![역할별 공식 매뉴얼 지도](../assets/guides/cpf-role-guide-map.svg)

README에서 제품 구조를 확인한 뒤 실제 업무는 역할별 매뉴얼로 이동한다. 한 기능이 여러 역할에 걸치면 개발 문서에서 구현하고 운영 문서에서 배포·조치·복구를 확인한다.

### 표준 요청과 결과 확정

![표준 요청 상태 확정 Lifecycle](../assets/guides/cpf-request-lifecycle.svg)

응답 수신은 업무 결과 확정과 같지 않다. Timeout·응답 유실이 발생하면 Owner 원장과 상대 시스템 상태를 조회해 결과를 확정한다.
...
| 실행 | 온라인, Kafka, File, 외부 연계, Spring Batch | 업무 처리를 수행하지 않는 단순 계약 호출과 Owner 우회 |
| 운영 | ADM Query·Command, 승인, 감사, 복구 | ADM의 Owner DB 직접 갱신 |
| 선택 제품 | BZA, Gateway | 미선택 시스템의 필수 Dependency |
| 공급 | Local·Remote·Offline Artifact, DB Lifecycle | 정의된 Vendor·Version·Topology 범위를 벗어난 임의 조합 |
...
Spring Batch가 Job·Step·Metadata·Checkpoint·Restart 정본이다. CPF는 승인·Idempotency·Fencing·실행 링크·운영 Control을 확장한다. Canonical Request Hash, `idempotencyScope`, Plan Checksum, Job별 `CPF_BATCH_EXECUTION_EPOCH`, 상태 전이와 실행 링크를 사용해 단일·다중 인스턴스 실행을 통제한다.
...
CPF의 Included Build는 다음 Source를 사용한다.
...
Build 계약 적용 순서는 다음과 같다.

1. `settings.gradle`, `cpf-tools/build/gradle-plugin/build.gradle`, Plugin 구현 Class와 `cpf-member/build.gradle`의 Plugin ID를 대조한다.
2. BOM·Plugin Publication 좌표를 Root Artifact Federation의 Canonical Coordinate와 맞춘다.
3. `settings.gradle`, Plugin Marker, Generator Template, Golden Domain, Publication·Verification Script에 같은 ID·좌표를 사용한다.
4. 별도 `GRADLE_USER_HOME`과 빈 CPF Local Repository에서 `help`, `projects`, Included Build `check`, Golden Domain Plugin Resolution, Root Build를 순서대로 실행한다.
5. exact SHA, 명령 출력, Artifact Hash와 Publication Manifest를 같은 Build 기록으로 보관한다.
...
| 기능군 | Owner Module·정본 | 실제 Consumer·접근 위치 | 제품 제공 범위 | 주 사용 매뉴얼 |
|---|---|---|---|---|
| 공개 계약·Context·오류·Paging | `cpf-core` Public API·SPI | 생성 Domain, ADM, Batch, Gateway, BZA | 표준 요청·응답·Context·오류·Paging 계약 | 01, 03, 91 |
| 선택형 공통 기능 | `cpf-common` | Calendar·Code·Message·Archive Consumer | 영업일·코드·메시지·Archive 공통 기능 | 01, 04, 05 |
| Security Starter | `cpf-starters/security` | ADM·BZA BFF, 업무 Domain | 인증·인가·Context·Data Scope·Masking·Audit 연계 | 01, 03, 04, 05, 90 |
| Kafka Starter | `cpf-starters/messaging-kafka` | 비동기 업무·Batch Remote Worker | Outbox·Inbox·ACK·DLT·Replay·중복 제어 | 01, 02, 05 |
| Cache Starter | `cpf-starters/cache` | Domain·ADM Cache 운영 | Cache 조회·Evict·정책·관측·운영 조치 | 01, 04, 05 |
| Observability·Resilience | `cpf-starters/observability`, `cpf-starters/resilience` | Online·Gateway·Batch | Log·Metric·Trace, Timeout·Retry·Circuit·Bulkhead | 01, 04, 05, 91 |
| ADM Control Plane | `cpf-admin` Backend·Frontend | 개발자·운영자·승인자 | 59개 Route, Query·Command·Permission·Approval·Audit·Recovery | 03, 04 |
| BZA | `cpf-biz-admin` Backend·Frontend | 조직·사용자·권한·결재 담당자 | 조직·직원·사용자·Role·Permission·Data Scope·결재·첨부·감사 | 90 |
| Gateway | `cpf-gateway` | 외부/내부 요청 Data Plane, ADM Control Plane | Route·Target·보안·Timeout·Retry·Attempt Ledger·Snapshot·LKG | 91 |
| Spring Batch 제품 | `cpf-batch/*` | Batch 개발자·운영자·Agent | Job·Step·Scheduler·Worker·Agent·Center-Cut·Reconciliation | 02, 04, 05 |
| Generator | `cpf-tools/generator/create-domain.ps1` | 신규 Domain 개발자 | Domain·API·DB·Batch·Messaging·Security·UI 산출물 생성 | 01 |
| DB Lifecycle | `cpf-tools/db/canonical`, `vendor/*/migration`, `rollback`, `runtime` | 설치·Upgrade·Runtime Query | MariaDB·PostgreSQL·Oracle 설치·Migration·Rollback·Drift·Runtime Query | 01, 02, 05, 90 |
| Build·Artifact Federation | Root Gradle, `cpf-tools/build/*` | Root·Standalone Domain·CI | BOM·Convention Plugin·Local·Remote·Offline Artifact·Checksum | 01, 05 |
...
- [ ] Test 명령·환경·결과·Artifact Hash와 Evidence를 같은 기록으로 남겼다.

---

## 제3부. CPF 전체 기능 활용 안내

> 이 부는 CPF를 처음 접한 사용자가 기능을 빠짐없이 찾고, 자신의 역할 매뉴얼로 이동하며, 기능 간 상태와 복구 관계를 이해하도록 구성한다. 기준 Source SHA는 `23babb9140b90e501d6ac715e7b77f55b66198a5`다.
...
## 제5부. 역할별 CPF 전체 활용 지도

## 46. 개발자 업무 흐름

```mermaid
flowchart LR
  A[Baseline·Toolchain] --> B[Generator Dry Run]
  B --> C[API·Application·Domain·Persistence]
  C --> D[3DB Migration]
  D --> E[Local·Remote·Kafka·File]
  E --> F[Security·Audit]
  F --> G[Test·Fault]
  G --> H[OpenAPI·ADM 확인]
  H --> I[배포 인계]
...
`01_개발자매뉴얼.md`의 워크북을 따라 신규 업무 기능의 설계·구현·시험·복구·운영 인계를 수행한다.

## 47. Batch 개발자·운영자 업무 흐름

```mermaid
flowchart LR
  A[Job·Step 설계] --> B[Parameter·Metadata·Checkpoint]
  B --> C[Scheduler·Partition·Worker]
  C --> D[Dry Run·Approval]
  D --> E[Start·Monitor]
  E --> F{결과}
  F -->|Stop·Failure| G[Restart·Reprocess]
  F -->|Unknown| H[Reconcile]
  F -->|Success| I[합계·Audit]
  G --> I
  H --> I
```

`02_배치개발매뉴얼.md`와 `04_ADM운영자매뉴얼.md`를 함께 사용한다.

## 48. 플랫폼 운영자 업무 흐름

```mermaid
flowchart LR
  A[Artifact Trust] --> B[계정·Directory]
  B --> C[Secret·Certificate]
  C --> D[DB·Kafka]
  D --> E[Config·Property]
  E --> F[기동·Health]
  F --> G[배포·관측]
  G --> H[Backup·Restore·DR]
```

`05_플랫폼운영매뉴얼.md`는 ADM 화면이 없거나 장애가 난 상황에서도 설치·기동·복구를 수행하도록 구성한다.

## 49. 공식 매뉴얼 선택표

| 수행하려는 업무 | 읽을 문서 |
|---|---|
| CPF 구조·Module·Topology 이해 | `00_프레임워크안내.md` |
| 신규 업무 Domain·API·DB·Kafka·File 개발 | `01_개발자매뉴얼.md` |
| Spring Batch·Scheduler·Worker·Center-Cut | `02_배치개발매뉴얼.md` |
| ADM Backend·Frontend 기능 개발 | `03_ADM개발자매뉴얼.md` |
| ADM 전체 Route 조회·조치·대사 | `04_ADM운영자매뉴얼.md` |
| 설치·Property·DB·Kafka·배포·Backup·DR | `05_플랫폼운영매뉴얼.md` |
| BZA 조직·권한·결재·첨부·감사 | `90_BZA매뉴얼.md` |
| Gateway Route·보안·게시·복구 | `91_Gateway매뉴얼.md` |

## 50. 기능 간 공통 식별자

- `transactionId`: 온라인·비동기·외부 연계의 업무 실행 ID
- `traceId`, `segmentId`, `attempt`: 호출 구조와 재시도
- `operationId`: 운영·비동기 변경 조치
- `idempotencyKey`, `requestHash`: 중복 요청과 요청 불변성
- `approvalId`, `reason`: 위험 조치 승인·감사
- `jobExecutionId`, `cpfExecutionId`: Spring Batch와 CPF Control 연결
- `routeId`, `snapshotVersion`, `checksum`: Gateway 게시·적용 상태
- `expectedVersion`: 동시 변경 방지
...
변경 업무는 Owner 상태, Version·Checksum, Ledger, Audit, Log·Metric·Trace와 업무 합계를 확인한 뒤 종료한다. `PARTIAL`과 `UNKNOWN_RESULT`는 성공으로 합치지 않는다.


---

## 제6부. 처음 접하는 사용자를 위한 전체 기능 이해 과정

### 52. 90분 제품 Orientation

| 시간 | 확인 대상 | 사용자가 답할 수 있어야 하는 질문 |
|---:|---|---|
| 0~15분 | README와 이 문서 | CPF가 공통 Library와 다른 점은 무엇인가 |
| 15~30분 | Module Ownership | 기능 변경 시 어느 Module과 팀이 Owner인가 |
| 30~45분 | 실행 문맥 | transactionId·segment·attempt·idempotency가 어떻게 연결되는가 |
| 45~60분 | Topology | Local·Remote·MSA·Modular Monolith에서 무엇이 같아야 하는가 |
| 60~75분 | 운영 기능 | ADM·BZA·Gateway·Batch가 각각 어떤 업무를 담당하는가 |
| 75~90분 | 오류·복구 | Timeout·응답 유실·부분 실패 시 무엇으로 결과를 확정하는가 |

Orientation 종료 시에는 다음 경로를 Source에서 직접 찾는다.
...
1. **누가 소유하는가**: `cpf-core`, `cpf-common`, ADM, BZA, Batch, Gateway, Generated Domain 중 Owner를 정한다.
2. **누가 소비하는가**: Controller, Starter AutoConfiguration, Frontend Page, Worker, Script, SQL Consumer를 확인한다.
3. **어떤 원장이 결과를 보유하는가**: 업무 Table, Operation Ledger, Outbox/Inbox, Spring Batch Metadata, Attempt Ledger를 찾는다.
4. **어떻게 복구하는가**: Retry, Restart, Reprocess, Reconcile, Compensation, Rollback 중 허용 동작을 확인한다.

### 54. 제품 전체 기능 연결표

| 기능 | 개발 문서 | 운영 문서 | 핵심 식별자 | 결과 원장 |
|---|---|---|---|---|
| 온라인 업무 API | 01 | 04·05 | transactionId, requestHash | 업무 Table·Audit |
| Local·Remote 호출 | 01·03 | 04·05 | segmentId, attempt | Owner Operation |
| Kafka 비동기 | 01 | 04·05 | messageId, idempotencyKey | Outbox·Inbox·DLQ |
| File·Attachment | 01 | 04·90 | attachmentId, checksum | Metadata·Object/File Store |
| Batch | 02 | 04·05 | cpfExecutionId, jobExecutionId | CPF 실행 원장·Spring Metadata |
| ADM | 03 | 04 | operationId, approvalId | Owner 상태·Audit |
| BZA | 90 | 90·05 | organizationId, userId, policyVersion | BZA 업무 원장·Audit |
| Gateway | 91·03 | 04·91 | routeId, snapshotVersion, attemptId | Snapshot·ACK/NACK·Attempt Ledger |

### 55. 독립 수행 확인 시나리오

다음 질문에 Source 검색 없이 문서 위치를 답할 수 있어야 한다.

- 신규 업무영역을 생성하고 Oracle·PostgreSQL·MariaDB Migration을 어디서 확인하는가.
- 응답 유실이 발생한 상태 변경 API를 어떤 식별자로 대사하는가.
- Batch 실행을 중단한 뒤 Restart와 Reprocess를 어떻게 구분하는가.
- ADM 위험 조치에서 Permission·Reason·Approval·Expected Version을 어디서 확인하는가.
- BZA 조직 변경이 결재 경로와 Data Scope에 미치는 영향을 어떻게 확인하는가.
- Gateway Route 게시 중 일부 Instance만 NACK일 때 어떤 상태를 비교하는가.
- DB Restore 뒤 Kafka Offset과 Batch Metadata를 어떤 순서로 대사하는가.

한 질문이라도 답할 수 없으면 관련 역할 매뉴얼의 기능 카드와 실무 Workbook을 다시 수행한다.

---

## 제7부. 기능을 놓치지 않는 제품 활용 완결성

![기능 개발과 운영 인계](../assets/guides/deep-dive/cpf-feature-delivery-evidence.svg)

### 58. 기능 하나의 문서 완결성

기능을 찾았다는 것은 Class나 메뉴 이름을 확인했다는 뜻이 아니다. 다음 연결을 모두 찾을 수 있어야 한다.

1. 제품 목적과 비범위
2. Owner Module과 실제 Consumer
3. Public API·SPI·Internal 경계
4. Source·SQL·Config·Frontend·Script·Test 위치
5. 정상 흐름과 상태 변화
6. 오류·Timeout·동시성·응답 유실·부분 실패
7. Retry·Restart·Reprocess·Reconcile·Compensation·Rollback
8. Permission·Data Scope·Masking·Reason·Approval·Audit
9. Log·Metric·Trace와 ADM 확인
10. 배포·Upgrade·Backup·Restore·DR 영향

### 59. 역할별 독립 수행 기준

- 개발자는 01·02·03을 사용해 기능 Source와 Test를 만들고 04·05에 운영 인계를 남긴다.
- 운영자는 04·05·90·91을 사용해 화면·설정·배포·복구 업무를 수행한다.
- 신규 담당자는 각 메뉴 장의 독립 수행 Checklist를 통과한 뒤 권한을 부여받는다.
- 메뉴 설명과 실제 화면이 다르면 화면만 따르지 않고 Release SHA의 Source·OpenAPI·Generated Client와 문서를 함께 수정한다.

---

## 52. 매뉴얼 사실 수준 표시 규칙

매뉴얼의 표·그림·절차는 다음 세 층을 구분한다.

| 층 | 의미 | 작성 규칙 |
|---|---|---|
| Source Fact | 기준 Commit의 실제 Route·Page·Class·Property·SQL·API | exact path·식별자를 적고 임의 이름을 만들지 않는다. |
| Operational Interpretation | Source Fact를 사용한 조회·판정·복구 방법 | Owner 상태·Version·Audit·합계를 종료 기준으로 적는다. |
| Environment Record | 고객 환경의 Host·계정·Secret Ref·Artifact Hash·실행 결과 | Repository 문서에 Secret을 넣지 않고 환경 기록으로 관리한다. |

ADM·BZA 메뉴별 계약의 기계 판독 정본은 다음 보조 자산이다.

- `cpf-docs/assets/guides/contracts/adm-operator-menu-contracts.json`
- `cpf-docs/assets/guides/contracts/adm-developer-route-contracts.json`
- `cpf-docs/assets/guides/contracts/bza-menu-contracts.json`

이 JSON은 별도 사용자 매뉴얼이 아니라 Route·Field·Column·Action Drift를 검사하기 위한 문서 자산이다.
