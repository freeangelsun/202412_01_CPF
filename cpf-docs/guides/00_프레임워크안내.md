...
## 15. 시간·데이터 품질·암호화·Webhook 운영 계약

![Integration Closure 운영 흐름](../assets/manuals/cpf-integration-closure-operations.svg)

CPF는 업무 기능의 마지막 운영 공백을 다음 네 계약으로 연결한다.

| 고객 결과 | CPF 계약 | 운영 완료 판정 |
|---|---|---|
| 모든 Instance가 같은 시간 판단을 사용 | `CpfTimeOperations`, `CpfTimeSnapshot`, `CpfDeadline` | UTC·업무 Zone·monotonic 값과 `estimatedSkew`, `healthy`가 허용 범위 |
| 오류 데이터를 정상 처리에서 격리 | Rule Version·Validation·Quarantine·Correction·Replay | `QUARANTINED → CORRECTED → REPLAYED`, Version·Audit 일치 |
| 개인정보를 분류별로 암호화·가림 | `CpfFieldEncryptionOperations`, `CpfCryptoPolicy` | Algorithm·Provider·Key strength 정책 통과, Decrypt/Rekey Actor·Reason Audit |
| 외부 시스템에 후속 Event 전달 | Endpoint·Delivery Ledger·Idempotency·Signed Header·DLQ | Target 2xx, 결과 불명 대사, Version 기반 Replay와 Audit |

이 기능은 각각 독립 Library가 아니라 온라인·Batch·BZA·Gateway 업무가 운영 종료 조건을 만들 때 사용하는 공통 계약이다. 개발 절차는 01, ADM 연결은 03·04, 설치·관측·복구는 05를 따른다.
