...
UTC와 고객 업무 Zone을 함께 기록하고, Timeout 계산은 wall clock 대신 monotonic time을 사용한다. `CpfTimeSnapshot`은 `utc`, `businessZone`, `businessTime`, `monotonicNanos`, `estimatedSkew`, `healthy`를 반환한다.

### 16.2 개발 절차

1. 업무 Zone을 IANA ID로 결정한다. 한국 영업일 기준은 `Asia/Seoul`을 사용한다.
2. 허용 NTP 편차를 `Duration`으로 정한다.
3. 처리 시작에서 `snapshot(zone, maximumAllowedSkew)`를 호출하고 `healthy=false`면 시간 의존 Write를 차단한다.
4. 외부 호출 Timeout은 `deadline(Duration)`으로 생성한다.
5. DB에는 UTC 기준 시각과 업무일자를 분리 저장한다.
6. Test에서는 `DeterministicCpfTimeOperations`로 경계 시각·만료·Clock rollback을 재현한다.

정상 결과는 Trace의 UTC, 업무 DB 일자, Scheduler 기준시각과 ADM `admIntegrationTimeHealth` 결과가 일치하는 것이다.

## 17. 데이터 품질 Rule·격리·정정·재검증

### 17.1 선택 기준

입력 오류를 단순 400으로 끝내지 않고 후속 정정과 감사가 필요한 대량 수신·파일·기관 연계에서 사용한다. Rule은 `ACTIVE` 상태와 증가하는 Version을 사용하며 Reference 표현식은 `NOT_BLANK`, `REGEX:`, `MIN_LENGTH:`, `MAX_LENGTH:`다.

### 17.2 단계별 구현

1. `CpfDataQualityRule`에 Rule ID·Version·Field·Expression·Severity·State를 정의한다.
2. `register(rule, actorId, reason)`으로 등록하고 이전 Version보다 큰지 확인한다.
3. `validate(recordId, record)`를 업무 반영 전 호출한다.
4. ERROR 또는 CRITICAL 위반이면 정상 원장 반영을 중단하고 `QUARANTINED` Item ID를 기록한다.
5. 운영자가 원본·위반·Version을 확인해 `correct(..., expectedVersion, corrected, actorId, reason, approved)`를 실행한다.
6. 승인된 정정은 `CORRECTED`와 증가한 Version을 반환한다.
7. `replay(quarantineId, actorId, reason)`가 통과하면 `REPLAYED`로 종료한다.
8. 응답 유실 시 Quarantine ID·Version·Audit를 조회한 뒤 같은 정정을 Blind Retry하지 않는다.

### 17.3 Test와 운영 인계

정상·Warning·Error·Critical, Rule Version 역전, 승인 누락, Version Conflict, 정정 후 재검증 실패를 Test한다. 운영팀에는 Rule ID, Owner, 원본 보존기간, 정정 승인자, Replay 후 업무 원장 반영 방법을 인계한다.

## 18. 필드 암호화·가림·키 교체

### 18.1 설계값

Field마다 분류(`CpfFieldClassification`), 검색 가능 여부, Key Version, Masking 규칙을 결정한다. `CpfCryptoPolicy`는 허용 Algorithm·Provider, 폐기 Algorithm, 최소 Key Bit, PQC 준비 여부를 검증한다. 최소 Key Bit는 128 미만을 허용하지 않는다.

### 18.2 개발 절차

1. DTO·Entity Field에 분류를 지정한다.
2. 저장 전 `encrypt(fieldName, value, classification, searchable)`를 호출한다.
3. 검색이 필요한 Field만 Search Token을 별도 저장한다.
4. API·Log·Audit 출력에는 `mask` 결과를 사용한다.
5. 복호화는 `decrypt(..., actorId, reason)`으로만 수행하고 Actor·Reason을 감사에 남긴다.
6. Key 교체는 `rekey(..., targetKeyVersion, actorId, reason)`을 사용한다.
7. 일부 Row 교체 실패 시 이전 Key Version과 새 Version을 함께 읽을 수 있는 기간을 유지하고 실패 Row를 대사한다.

암호문·Search Token·Masked Preview가 서로 다른 목적의 값임을 유지하며, 평문을 Log·Trace·오류 응답에 기록하지 않는다.

## 19. 외부 Webhook Callback

### 19.1 업무 결과와 선택

고객 외부 시스템에 상태 변경 Event를 Callback할 때 사용한다. 내부 비동기 후속 처리는 Broker를 우선하고, 외부 HTTP Endpoint가 명시적으로 Callback을 요구할 때 Webhook을 선택한다.

### 19.2 개발 절차

1. Endpoint ID, Callback URI, Secret Version, 허용 Event Type, Enabled를 등록한다.
2. Event ID·Event Type·Payload·Idempotency Key로 Delivery를 `enqueue`한다.
3. Worker가 `deliverNext`를 호출해 Signed Header와 Payload를 전송한다.
4. 2xx이면서 `resultUnknown=false`인 경우만 성공이다.
5. Timeout·응답 유실은 UNKNOWN으로 기록하고 Target Event ID로 대사한다.
6. Retry 한도 초과는 DLQ로 전이한다.
7. 운영 Replay는 Delivery ID·Expected Version·Actor·Reason을 사용한다.
8. 같은 Endpoint와 Idempotency Key의 중복 Delivery를 생성하지 않는다.

### 19.3 Test

2xx, 4xx, 5xx, Timeout, TLS 실패, 중복 Key, Secret Version 교체, UNKNOWN 후 Target 확인, DLQ Replay와 Rate Limit을 재현한다. 운영팀에는 Endpoint Owner·허용 Event·Secret 교체·Retry/DLQ 정책·Target 대사 Query를 인계한다.
