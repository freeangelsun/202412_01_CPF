...
## 21. Batch UNKNOWN 승인 대사 원장

`V103__batch_unknown_reconciliation.sql`은 `bat_reconciliation_audit`를 생성한다. Batch 개발자는 UNKNOWN 조치를 단순 상태 UPDATE로 구현하지 않고 다음 정보를 하나의 승인 기록으로 남긴다.

| Column | 의미 |
|---|---|
| `request_id` | 운영 요청 식별자 |
| `entity_type`, `entity_key` | Scheduler Trigger·Remote Message·Execution 대상 |
| `from_status`, `to_status` | 승인된 상태 전이 |
| `requester_id`, `approver_id` | 요청자·승인자 분리 |
| `reason_text` | 조치 근거 |
| `idempotency_key` | 같은 조치 Replay 식별자 |
| `expected_attempt`, `expected_version` | 동시성 조건 |

요청자와 승인자가 같으면 DB Check에서 거부한다. 응답 유실 시 `idempotency_key`로 Audit를 먼저 조회하고, Scheduler·Remote Message Ledger와 Target 결과를 대사한 뒤 다음 Attempt를 결정한다.
