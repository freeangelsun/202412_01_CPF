...
배치 제품은 실행 역할마다 필요한 스타터가 다르다. 최신 `build.gradle`의 직접 의존 관계는 다음과 같다.

| 배치 실행 모듈 | 직접 사용하는 공개 스타터 | 이유 |
|---|---|---|
| 제어 서버 | `security`, `observability`, `messaging-kafka` | 제어 API 보안, 명령·감사 관측, Kafka 원격 명령 |
| 실행 Runtime | `resilience`, `observability` | 실행 제어의 시간 제한·회복성과 관측 |
| 스케줄러 | `observability`, `messaging-kafka` | 트리거·실행 요청 관측과 원격 전달 |
| 작업자 | `messaging-kafka`, `resilience`, `observability`, `secret` | 원격 작업, 시간 제한·복구, 관측, 승인된 비밀정보 참조 |
| 센터컷 실행기 | `messaging-kafka`, `resilience`, `observability` | 대상 분할·원격 실행·복구·관측 |
| 호스트 에이전트 | 없음 | 제품 고유의 승인된 프로세스 실행 계약을 직접 소유 |

논리 프로젝트명은 `:cpf-starter-*`이며 복수형 물리 폴더명을 Gradle 논리 경로로 사용하지 않는다. Spring Batch 메타데이터·작업·단계·체크포인트·작업자 임대 잠금·펜싱·센터컷 대상 원장은 배치 제품이 소유한다. 스타터는 기술 연결과 자동 설정만 제공한다.

현재 `execution-runtime`은 회복성·관측 스타터를 사용하지만 Kafka·Spring Integration 라이브러리를 직접 선언하는 부분도 있다. 운영자는 “Kafka를 사용한다”는 사실만으로 모든 배치 모듈이 같은 메시징 스타터 구성을 사용한다고 가정하지 않고 각 실행 파일의 실제 의존성을 확인한다.

### 최신 실행 Source 기준

센터컷과 작업자 실행 구현은 다음 Source를 기준으로 확인한다.

| 영역 | 현재 Owner Source | 개발·장애 시험에서 확인할 상태 |
|---|---|---|
| 센터컷 상태 | `cpf-batch/center-cut-runner/src/main/java/com/cpf/batch/centercut/runner/SpringBatchCenterCutRuntimeState.java` | 대상·파티션·진행 상태·실패 상태 보존 |
| 센터컷 실행 | `cpf-batch/center-cut-runner/src/main/java/com/cpf/batch/centercut/runner/SpringBatchCenterCutStepHandler.java` | Spring Batch 단계 경계, 임대 잠금 상실, 실패 대상 재처리 |
| 작업자 상태 | `cpf-batch/worker/src/main/java/com/cpf/batch/worker/SpringBatchWorkerRuntimeState.java` | 작업자 소유권·상태 전이·재시작 |
| 작업자 처리 | `cpf-batch/worker/src/main/java/com/cpf/batch/worker/SpringBatchWorkerStepHandler.java` | 단계 입력·출력·오류·펜싱 확인 |
| 실행 추적 | `cpf-batch/worker/src/main/java/com/cpf/batch/worker/WorkerExecutionTracker.java` | 실행 건·시도·완료·실패 추적 |
| 스케줄 동기화 | `cpf-batch/scheduler/src/main/java/com/cpf/batch/scheduler/BatchProjectionScheduleSynchronizer.java` | 정의·버전·트리거 동기화와 누락 방지 |
| 주 스케줄 구성 | `cpf-batch/scheduler/src/main/java/com/cpf/batch/scheduler/DbSchedulerPrimaryConfiguration.java` | db-scheduler 기동·클레임·오작동 처리 |

이전 `CenterCutRuntime`, `WorkerRuntime`, `BatchRuntimeProjectionRepository` 이름을 신규 개발·장애 분석의 정본으로 사용하지 않는다.
