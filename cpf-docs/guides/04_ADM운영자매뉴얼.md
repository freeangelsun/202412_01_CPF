...
## 최신 Source 기준 운영 주의사항

현재 Repository에는 7개 공개 스타터가 있지만 ADM 본체가 직접 소비하는 스타터는 `cpf-starter-security`다. 나머지 스타터는 게이트웨이·배치·고객 업무 등 각 실행 모듈에서 사용하며 ADM은 배포·설정·상태·로그·지표를 조회한다.

- Gradle 논리 프로젝트: `:cpf-starter-security` 형식
- Repository 물리 경로: `cpf-starters/security` 형식
- 기능 묶음 별칭: 현재 루트 빌드에 미등록이므로 운영 화면에서 실제 활성 기능처럼 취급하지 않음
- 플랫폼 BOM: 버전 정렬만 하며 스타터 활성 여부를 뜻하지 않음
- 스타터 자체: 독립 프로세스가 아니므로 별도 시작·중지 대상이 아님

ADM 화면은 생성된 경로·조치 계약과 생성 API 클라이언트를 사용한다. 승인 화면과 분석 센터에서 구조화 결과가 표시되더라도 작업 성공 여부는 Owner 응답·버전·대상 결과·감사로 판정한다.
...
스타터는 포함된 실행 모듈 안에서 동작한다. 운영자는 “스타터 서비스”를 찾지 않고 배포 명세와 실행 모듈의 유효 설정·준비 상태·로그·지표를 확인한다.

| 스타터 | 최신 Source의 대표 직접 소비 | 운영 신호 | 장애 시 첫 판단 |
|---|---|---|---|
| `security` | ADM·BZA·배치 제어 서버 | 세션 DB, 로그인·로그아웃, 쿠키·CSRF, 허용 출처, 준비 상태 | 자격증명 문제인지 세션 저장소·키·시간 동기 문제인지 분리 |
| `messaging-kafka` | 배치 제어·스케줄러·작업자·센터컷 | 발행·수신 원장, 지연, 재시도, 격리 큐 | 작업 ID·메시지 ID로 실제 처리 여부를 먼저 대사 |
| `cache` | 제품 직접 소비는 배포 명세에서 확인 | 제공자, 적중·미적중, TTL, 무효화, 원본 조회 | 오래된 값이면 캐시를 업무 정본으로 사용했는지 확인 |
| `observability` | 게이트웨이와 주요 배치 실행 모듈 | 내보내기 큐, 드롭, 샘플링, 수집기 연결 | 관측 누락과 업무 실패를 분리 |
| `resilience` | 게이트웨이·배치 실행·작업자·센터컷 | 시간 제한, 회로 상태, 재시도, 격벽 포화 | 전체 시간 예산과 외부 효과의 결과 미확정 여부 확인 |
| `featureflag` | 제품 소비는 배포 명세에서 확인 | 제공자 연결, 평가 실패, 기본값, 새로 고침 | 잘못된 기본값이 보안·업무 정책을 열지 않는지 확인 |
| `secret` | 배치 작업자 | 제공자 연결, 참조 해석, 회전, 만료, 준비 상태 | 원문 노출 없이 이전·현재 버전과 소비 권한 확인 |

다중 인스턴스에서 버전·설정·활성 조건이 다르면 해당 인스턴스를 신규 요청에서 제외하고 다음을 비교한다.

1. 실행 파일 검사합과 CPF 플랫폼 버전
2. 포함된 공개 스타터와 각 버전
3. 프로필·환경변수의 유효값
4. 자동 설정 활성·비활성 이유
5. 외부 기반 연결과 준비 상태
6. 같은 대표 요청의 결과·작업 ID·추적·감사

현재 구현되지 않은 기능 프로필이나 통합 스타터 이름을 운영 원장에 사용하지 않는다. 이후 도입되면 프로필 이름뿐 아니라 최종 해석된 공개 스타터 목록과 버전을 함께 기록한다.
