...
최신 루트 `settings.gradle`은 물리 컨테이너 `cpf-starters/` 아래의 7개 프로젝트를 다음 논리 이름으로 등록한다.

![CPF 스타터의 빌드·소비·운영 경계](png/cpf-r10-starters-project-map.png)

| 논리 프로젝트 | 물리 경로 | 게시 이름 | 최신 Source에서 확인한 대표 직접 소비 |
|---|---|---|---|
| `:cpf-starter-security` | `cpf-starters/security` | `cpf-starter-security` | ADM, BZA, 배치 제어 서버 |
| `:cpf-starter-messaging-kafka` | `cpf-starters/messaging-kafka` | `cpf-starter-messaging-kafka` | 배치 제어·스케줄러·작업자·센터컷 |
| `:cpf-starter-cache` | `cpf-starters/cache` | `cpf-starter-cache` | 제품별 배포 명세에서 확인 |
| `:cpf-starter-observability` | `cpf-starters/observability` | `cpf-starter-observability` | 게이트웨이와 주요 배치 실행 모듈 |
| `:cpf-starter-resilience` | `cpf-starters/resilience` | `cpf-starter-resilience` | 게이트웨이·배치 실행·작업자·센터컷 |
| `:cpf-starter-featureflag` | `cpf-starters/featureflag` | `cpf-starter-featureflag` | 제품별 배포 명세에서 확인 |
| `:cpf-starter-secret` | `cpf-starters/secret` | `cpf-starter-secret` | 배치 작업자 |

스타터는 라이브러리 배포 파일이며 독립 프로세스·포트·서비스 계정을 만들지 않는다. 실제 운영 대상은 스타터를 포함한 ADM·BZA·게이트웨이·배치·고객 업무 실행 파일이다.

### 정식 프로젝트와 빌드 소유 경로

최신 Repository의 소유 경로는 다음과 같다.

| 목적 | 정본 경로·논리 이름 |
|---|---|
| 스타터 Source | `cpf-starters/*` |
| 스타터 Gradle 논리 프로젝트 | `:cpf-starter-*` |
| Gradle 규칙 | `cpf-tools/build/gradle-plugin` Included Build |
| 플랫폼 BOM | `cpf-tools/build/platform-bom` Included Build |
| 로컬 통합 Runtime Source | `cpf-tools/runtime/cpf-local-runtime` |
| 로컬 배치 Runtime Source | `cpf-tools/runtime/cpf-local-batch-runtime` |
| 생성 도구 | `cpf-tools/generator/create-domain.ps1` |
| 이전 생성 경로 | `cpf-tools/scripts/create-domain.ps1` 호환 실행기 |

루트에 `cpf-gradle-plugins`, `cpf-platform-bom`, 로컬 Runtime 복사본을 다시 만들지 않는다.
...
`projects` 출력이 `:cpf-starter-*` 형식인지 확인한다. 복수형 물리 폴더 이름을 Gradle 논리 경로로 옮기지 않는다.

### 현재 자동 설정·외부 기반

| 스타터 | 확인된 주요 의존성·자동 설정 | 운영 준비 항목 |
|---|---|---|
| `security` | Spring Security, Spring Session JDBC, 서버 세션 보안 자동 설정 | DataSource, 세션 테이블, 자격증명 암호화 키, 쿠키, 허용 출처, CSRF, `cpf.security.session.enabled` |
| `messaging-kafka` | Spring Kafka와 CPF 브로커 연결 | Broker, 주제, ACL, 직렬화, 아웃박스·인박스, 재시도·격리 큐 |
| `cache` | Spring Cache, Caffeine, Redis가 현재 함께 전이 | 선택 제공자, 비선택 제공자 비활성, TTL·무효화, 직렬화·호환성, 메모리·연결 용량 |
| `observability` | Micrometer Observation, OTel bridge, OTLP exporter | Collector, Endpoint, 인증, 샘플링, 민감정보 가림, 내보내기 큐·역압 |
| `resilience` | Spring Cloud CircuitBreaker, Resilience4j | 시간 예산, 회로·격벽, 재시도 가능 오류, 멱등성, 결과 미확정 복구 |
| `featureflag` | OpenFeature SDK 연결 | 실제 제공자, 기본값, 평가 문맥, 새로 고침, 감사, 제공자 장애 정책 |
| `secret` | CPF 비밀정보 제공자 자동 구성 경계 | 실제 저장소 제공자, 참조 규칙, 최소 권한, 회전, 캐시, 준비 상태 |

### 실제 소비 관계와 불일치 검사

운영팀은 문서의 대표 소비만 믿지 않고 Commit마다 다음을 수집한다.

```powershell
.\gradlew.bat :cpf-admin:dependencies --configuration runtimeClasspath
.\gradlew.bat :cpf-biz-admin:dependencies --configuration runtimeClasspath
.\gradlew.bat :cpf-gateway:dependencies --configuration runtimeClasspath
.\gradlew.bat :cpf-batch:control-server:dependencies --configuration runtimeClasspath
.\gradlew.bat :cpf-batch:worker:dependencies --configuration runtimeClasspath
```
...
- 직접·전이 스타터와 각 버전
- 선택 사유와 사용하지 않는 기능
- 외부 기반·설정 접두어·비밀정보 참조
- 자동 설정 활성 조건과 준비 상태
- 최종 실행 파일·POM·Gradle 메타데이터·잠금·SBOM 포함 결과
- 장애·우회·대사·되돌리기 절차

현재 `cpf-reference`와 생성 기준 도메인 `cpf-member`는 스타터를 직접 선언하지 않는다. 생성 도구의 기능 옵션도 스타터 의존성을 자동으로 추가하지 않는다. 따라서 고객 생성 도메인은 기능 코드를 생성한 뒤 01 개발자 매뉴얼의 선택표에 따라 필요한 공개 스타터를 수동 등록하고 그 결과를 명세에 남긴다.

### 기능 프로필·통합 스타터·BOM

| 계층 | 현재 상태 | 운영 기준 |
|---|---|---|
| 공개 스타터 | 프로젝트 등록 확인 | 실제 기능·게시·검증 단위 |
| 기능 프로필 | 정책상 후보 | 생성 도구나 중앙 빌드에 해석 구현과 잠금이 생기기 전 사용 금지 |
| 통합 스타터 | 승인 후 선택 제공 | 하나의 의존성으로 검증된 공개 스타터만 전이하며 자체 업무 로직을 갖지 않음 |
| 플랫폼 BOM | 사용 | 버전·호환성만 정렬하고 기능은 활성화하지 않음 |

`DOMAIN_WEB_API`, `DOMAIN_EVENT_KAFKA` 같은 이름은 정책상 후보일 뿐 현재 고객이 복사할 Gradle 별칭이 아니다. 기능 프로필이나 통합 스타터가 구현되면 실제 게시 좌표, 최종 펼침 목록, 프로필 버전, 생성 도구 결과, 미선택 제거 시험과 되돌리기가 확인된 뒤 매뉴얼에 사용 가능한 값으로 올린다.

### 설정·기동·부분 적용 판정

1. 소비 모듈의 실행 파일에서 포함 스타터와 버전을 확인한다.
2. 프로필별 유효 설정·비밀정보 참조·재기동 필요 여부를 기록한다.
3. 기동 로그에서 자동 설정 활성·비활성 이유를 확인한다.
4. 준비 상태에서 외부 기반과 필수 설정 누락을 확인한다.
5. 대표 업무를 실행해 결과·작업 ID·로그·지표·추적·감사를 연결한다.
6. 다중 인스턴스의 검사합·스타터 버전·설정·활성 결과가 다르면 실패 인스턴스를 신규 요청에서 제외한다.
7. 이미 처리된 요청은 멱등 키·메시지 원장·세션 감사·대상 상태로 결과를 확정하며 전체를 일괄 재실행하지 않는다.

### 하위 프로젝트 세분화·업그레이드·되돌리기

1. 기존 공개 스타터의 직접·전이 소비, 자동 설정, 설정 키와 데이터 형식을 수집한다.
2. 새 프로젝트가 공개 스타터인지 내부 세부 구현인지 구분한다.
3. 내부 세부 구현은 기존 공개 스타터가 조립하게 하고 고객 도메인 의존성을 바꾸지 않는다.
4. 공개 경계가 바뀌면 BOM·잠금·게시 메타데이터·SBOM·생성 도구·공식 매뉴얼을 함께 갱신한다.
5. 한 인스턴스에 먼저 배포해 정상·장애·복구를 확인한 뒤 확대한다.
6. Kafka 스키마·캐시 직렬화·세션 DB·비밀정보 참조가 바뀌면 JAR만 되돌리지 않고 데이터 호환 절차를 먼저 확인한다.
7. 이전 버전 제거 전 POM·Gradle 메타데이터·실행 파일·SBOM과 Repository 참조에서 소비가 0인지 확인한다.
8. 되돌리기는 이전 BOM·잠금·실행 파일·설정·필요한 데이터 형식을 함께 복원한다.
