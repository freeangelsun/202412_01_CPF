...
최신 `cpf-gateway/build.gradle`에서 게이트웨이는 `cpf-starter-resilience`와 `cpf-starter-observability`를 직접 소비한다.

| 스타터 | 게이트웨이에서 사용하는 범위 | 게이트웨이가 직접 소유하는 범위 |
|---|---|---|
| `resilience` | 회로 차단기·격벽·시간 제한 실행 연결 | 경로별 정책, 전체 시간 예산, 멱등성, 결과 미확정 대사와 시도 원장 |
| `observability` | Micrometer·OTel·OTLP 연결 | 경로·대상·시도 ID의 업무 속성, 민감정보 가림, 운영 판정 |

게이트웨이는 `security`, `messaging-kafka`, `cache` 스타터를 직접 소비하지 않는다. 다음을 혼동하지 않는다.

- 게이트웨이의 HMAC·Audience·Nonce·본문 해시·SSRF·TLS·대상 허용 목록은 게이트웨이 소유 보안이다.
- 경로 초안·검증·승인·게시·ACK/NACK·부분 적용·최근 정상본·시도 원장은 게이트웨이가 소유한다.
- 후단 고객 서비스의 브라우저 세션, Kafka 업무 이벤트, 조회 캐시는 후단 서비스가 필요한 공개 스타터를 개별 선택한다.
- 플랫폼 BOM만 추가해도 후단 스타터나 게이트웨이 기능이 활성화되지는 않는다.
- 현재 루트에 기능 묶음 별칭은 구현돼 있지 않으므로 문서상 후보 이름을 의존성에 사용하지 않는다.

운영 인계에는 게이트웨이 실행 파일의 `resilience`·`observability` 버전과 후단 서비스의 스타터 목록을 분리해 기록한다. 장애 분석 시 게이트웨이 시도 ID와 후단 작업 ID를 연결해 회로 차단·전달 실패·후단 처리·응답 유실을 구분한다.
