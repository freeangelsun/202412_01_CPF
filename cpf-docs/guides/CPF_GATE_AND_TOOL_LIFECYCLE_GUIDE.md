...
## 13. ADM/BZA Data Safety Gate

`cpf-tools/scripts/check-admin-data-safety.ps1`

- 분류: `DEV_ONLY`, `CI_RELEASE`
- Runtime 제품 배포: 제외
- 목적: ADM/BZA fail-closed, 상태/PII/SQL Ownership, V61 lifecycle 정적 회귀 방지
- 옵션: `-RootPath`; 생략 시 Repository Root 자동 계산
- Side Effect: 없음(Read-only)
- 실패 시: Source/DB/Migration/Query Resource를 올바른 Owner에서 수정한 뒤 재실행
- 실제 DB/Browser 검증을 대체하지 않음

상세 운영/개발 계약은 `CPF_ADMIN_DATA_SAFETY_GUIDE.md`를 정본으로 사용한다.
