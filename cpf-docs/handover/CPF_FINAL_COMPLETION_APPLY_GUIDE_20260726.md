...
이 패키지는 CPF Repository Root에 작업용 보조 파일을 생성하지 않는다.

패키지 적용/삭제 메타데이터는 `cpf-tools/scripts/config`,
패키지 검증 결과는 `cpf-docs/evidence`,
Build 지원 Unit은 `cpf-tools/build` 아래에 둔다.

Repository Root에는 기존 제품 Root 파일과 공식 Module/Directory만 유지한다.
