...
## 12. Historical Flyway 보호

적용된 Migration은 파일명, 내용과 Checksum을 변경하지 않습니다. 광범위 Rename으로 Historical SQL이 바뀌었는지 원본 Commit과 SHA-256을 대조합니다.

Pre-release Re-baseline은 다음 조건을 모두 만족할 때만 수행합니다.

- 사용자 명시 승인
- 적용 고객/운영 DB 없음 확인
- 모든 개발 DB 폐기
- 기존 Migration/Checksum 처리 원칙 기록
- Empty Install, Upgrade path와 rollback 대안
- Source/Guide/Evidence 일치

## 13. 물리명 변경

Schema, Table, Column, Index, Constraint, Seed, Mapper, Repository, Config, UI, Generator와 Monitoring을 함께 변경합니다. 단순 문자열 치환을 금지하고 compatibility view/dual-write/data copy/consumer switch 필요성을 판단합니다.

## 14. 문서와 Evidence

Migration Guide는 Installation Guide와 중복하지 않습니다. 설치는 빈 환경, Migration은 기존 Version에서 목표 Version으로의 호환성과 데이터 보존을 다룹니다.
