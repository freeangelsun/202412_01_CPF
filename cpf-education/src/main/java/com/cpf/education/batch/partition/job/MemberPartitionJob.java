package com.cpf.education.batch.partition.job;
...
import com.cpf.education.batch.partition.service.MemberPartitionJobService;
...
/** Partition 교육 예제의 Job 역할과 CPF 표준 사용 경계를 보여줍니다. */
public class MemberPartitionJob {
    private final MemberPartitionJobService service;
    /** Partition 예제의 Job 의존성을 주입해 표준 실행 경계를 구성합니다. */
    public MemberPartitionJob(MemberPartitionJobService service) { this.service = service; }

    @CpfBatchStep(value = "partition-worker", order = 1)
    /** Partition 예제의 run 단계를 실행하고 실제 처리 책임은 분리된 역할로 위임합니다. */
