...
public class SettlementStepTransactionJobService {
 ...
 @CpfTransactional(propagation=Propagation.REQUIRES_NEW) public BatchStepResult stepB(BatchStepCommand command){boolean fail=Boolean.parseBoolean(String.valueOf(command.jobParameters().getOrDefault("failStepB","false")));return fail?new BatchStepResult(BatchStepResult.Status.FAILED,"STEP_B_FAILED","Step A 결과는 유지되고 B만 재시작 대상입니다.",1,0,0,Map.of("stepA","COMPLETED")):BatchStepResult.completed("step B committed",1,1,Map.of("stepB","COMPLETED"));}
}
