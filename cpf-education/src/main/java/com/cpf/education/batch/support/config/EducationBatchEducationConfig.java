...
/**
 * EDU 배치 교육용 Job 설정입니다.
 *
 * <p>로컬 교육 환경에서는 {@link EducationBatchRepositoryConfig}가 CPF Platform DB의 BATCH_* 테이블을
...
public class EducationBatchEducationConfig {
    private static final Logger log = LoggerFactory.getLogger(EducationBatchEducationConfig.class);

    @Bean
    @CpfBatchJob(id = "BEDUAA0003", name = "EDUTasklet교육배치", ownerDomain = "EDU")
    /** cpfEduTaskletJob 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    @Bean
    /** cpfEduTaskletStep 작업을 CPF 표준 계약에 따라 수행한다. */
    public Step cpfEduTaskletStep(
            JobRepository jobRepository,
            @Qualifier("educationBatchPlatformTransactionManager") PlatformTransactionManager transactionManager) {
    ...
    @Bean
    @CpfBatchJob(id = "BEDUAA0001", name = "EDUChunk교육배치", ownerDomain = "EDU")
    /** cpfEduChunkJob 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    @Bean
    /** cpfEduChunkStep 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    @Bean
    @CpfBatchJob(id = "BEDUAA0002", name = "EDU재시도교육배치", ownerDomain = "EDU")
    /** cpfEduRetryJob 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    @Bean
    /** cpfEduRetryStep 작업을 CPF 표준 계약에 따라 수행한다. */
