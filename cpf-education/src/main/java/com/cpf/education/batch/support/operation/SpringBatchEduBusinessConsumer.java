package com.cpf.education.batch.support.operation;
...
/** SpringBatchEduBusinessConsumer 타입의 역할과 책임을 정의하며 CPF 계약 경계를 명확히 유지한다. */
public final class SpringBatchEduBusinessConsumer implements EduBusinessConsumer {
    private final ApplicationContext context; private final JobOperator operator;
    public SpringBatchEduBusinessConsumer(ApplicationContext context,JobOperator operator){this.context=context;this.operator=operator;}
    ...
    @Override public EduBusinessConsumerResult invoke(EduConsumerBinding b,EduExecutionCommand c,long fence){
        try{
            ...
            for(var e:c.payload().entrySet())p.addString("edu."+e.getKey(),String.valueOf(e.getValue()));JobExecution x=operator.start(job,p.toJobParameters());BatchStatus status=x.getStatus();if(status==BatchStatus.FAILED||status==BatchStatus.ABANDONED)throw new IllegalStateException("Spring Batch execution failed status="+status+" id="+x.getId());
