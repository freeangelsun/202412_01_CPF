package com.cpf.education.batch.support.runtime;
...
/**
 * Durable worker shared by the 30 optional cpf-education Batch jobs.
 *
 * <p>The worker owns only EDU data. It never reads or writes generated-domain tables.
 * Batch execution, checkpoint and target outcome are persisted in the removable
 * {@code CPF_EDU_BAT_*} pack, while the final education business state is stored in
 ...
 */
/** EduBatchScenarioWorker 타입의 역할과 책임을 정의하며 CPF 계약 경계를 명확히 유지한다. */
public final class EduBatchScenarioWorker {
    ...
    public int execute(String requirementId,
        ...
        try {
            ...
            int claimed = jdbc.update(
                    "update CPF_EDU_BAT_JOB_EXECUTION set STATE=?,FENCING_TOKEN=?," +
            ...
            int businessWrites = writeEducationBusinessRecord(
            ...
            jdbc.update(
                    "update CPF_EDU_BAT_JOB_EXECUTION set STATE=?,RESULT_JSON=?,UPDATED_AT=?,COMPLETED_AT=? " +
                            "where JOB_EXECUTION_KEY=? and FENCING_TOKEN=?",
        ...
        // 트랜잭션·재시도·복구 경계의 의미를 보존해 부분 실패에서도 일관성을 유지한다.
            ...
            throw new IllegalStateException("EDU Batch worker failed: " + requirementId, e);
        }
    }
    ...
    private void createExecutionIfAbsent(String executionKey,
        ...
        try {
            jdbc.update(
                    "insert into CPF_EDU_BAT_JOB_EXECUTION " +
    ...
    private int writeEducationBusinessRecord(String requirementId,
                                             String businessKey,
                                             String dataScope,
                                             long fencingToken,
                                             String payloadJson,
        ...
        if (updated == 0) {
            ...
            // 트랜잭션·재시도·복구 경계의 의미를 보존해 부분 실패에서도 일관성을 유지한다.
                ...
                int retried = jdbc.update(
                        ...
                        requirementId, businessKey, dataScope, fencingToken);
                if (retried != 1) {
                    throw new IllegalStateException("Education business record fencing conflict");
                }
                return retried;
            }
        }
    ...
    private void saveCheckpoint(String executionKey,
                                ...
                                Instant now) {
        int updated = jdbc.update(
                "update CPF_EDU_BAT_CHECKPOINT set CHECKPOINT_VERSION=?,STATE=?,CHECKPOINT_JSON=?,UPDATED_AT=? " +
        ...
        if (updated == 0) {
            try {
                jdbc.update(
                        "insert into CPF_EDU_BAT_CHECKPOINT " +
    ...
    private void saveTargetResult(String targetId,
                                  ...
                                  Instant now) {
        int updated = jdbc.update(
                "update CPF_EDU_BAT_TARGET_RESULT set STATE=?,ATTEMPT_COUNT=ATTEMPT_COUNT+1," +
        ...
        if (updated == 0) {
            try {
                jdbc.update(
                        "insert into CPF_EDU_BAT_TARGET_RESULT " +
