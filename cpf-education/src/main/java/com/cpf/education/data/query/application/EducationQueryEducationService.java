 ...
 */
@Service
public class EducationQueryEducationService extends com.cpf.education.base.EducationBaseService {
    private final EducationQueryEducationRepository repository;

    public EducationQueryEducationService(EducationQueryEducationRepository repository) {
     ...
     */
    @CpfTx(id = "EDU-ATIONSERVICE-01", name = "EducationQueryEducationServiceTx1", ownerDomain = "EDU", transactionManager = "educationReferenceFixtureTransactionManager", readOnly = true)
    public EducationQueryEducationItem getItem(Long itemId) {
     ...
     */
    @CpfTx(id = "EDU-ATIONSERVICE-02", name = "EducationQueryEducationServiceTx2", ownerDomain = "EDU", transactionManager = "educationReferenceFixtureTransactionManager", readOnly = true)
     ...
     */
    @CpfTx(id = "EDU-ATIONSERVICE-03", name = "EducationQueryEducationServiceTx3", ownerDomain = "EDU", transactionManager = "educationReferenceFixtureTransactionManager", readOnly = true)
    public EducationQueryPageResponse<EducationQueryEducationItem> findOffsetPage(
            String keyword,
            String statusCode,
            String sort,
            int page,
        ...
        List<EducationQueryEducationItem> items = repository.findOffsetPageItems(
                keyword,
                statusCode,
                sort,
                normalizedPage,
        ...
        return new EducationQueryPageResponse<>(items, normalizedPage, normalizedSize, total, hasNext);
    }
     ...
     */
    @CpfTx(id = "EDU-ATIONSERVICE-04", name = "EducationQueryEducationServiceTx4", ownerDomain = "EDU", transactionManager = "educationReferenceFixtureTransactionManager", readOnly = true)
    public EducationQueryKeysetResponse<EducationQueryEducationItem> findKeysetPage(Long cursorId, int size) {
        int normalizedSize = repository.normalizeSize(size);
        List<EducationQueryEducationItem> page = repository.findKeysetPageItems(cursorId, normalizedSize);
        boolean hasNext = page.size() > normalizedSize;
        List<EducationQueryEducationItem> items = hasNext ? page.subList(0, normalizedSize) : page;
        Long nextCursorId = items.isEmpty() ? cursorId : items.get(items.size() - 1).itemId();
        return new EducationQueryKeysetResponse<>(items, nextCursorId, hasNext);
    }
}
