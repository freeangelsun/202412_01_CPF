 ...
 */
@RestController
@RequestMapping({"/api/education/transactions", "/education/edu/transactions"})
@Tag(name = "EDU Education Composite Transaction", description = "복합 거래 trace 샘플 안내 API")
public class EducationCompositeTransactionEducationController extends com.cpf.education.base.EducationBaseController {

    @GetMapping("/composite-sample")
    @CpfOnlineTransaction(id = "OEDUAA0043", name = "EDUCompositeTransactionSample", ownerDomain="EDU")
    @Operation(
            operationId = "refCompositeTransactionEducationCompositeSample",
            summary = "복합 거래 추적 샘플 안내",
            description = "EDU에서 CPF Service Call Engine으로 중립 시뮬레이터를 호출하고 ADM에서 타임라인을 확인하는 절차를 안내합니다.")
    /** compositeSample 작업을 CPF 표준 계약에 따라 수행한다. */
    public ResponseEntity<Map<String, Object>> compositeSample() {
        return ResponseEntity.ok(Map.of(
                ...
                "patterns", List.of(
                        "EDU Service Call Engine 샘플에서 EDU 자체 중립 endpoint를 호출",
