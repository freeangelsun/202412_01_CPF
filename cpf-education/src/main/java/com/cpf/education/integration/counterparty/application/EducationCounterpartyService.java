...
/** Deterministic EDU-owned external counterparty simulator with durable idempotency/reconciliation. */
/** EducationCounterpartyService 타입의 역할과 책임을 정의하며 CPF 계약 경계를 명확히 유지한다. */
public final class EducationCounterpartyService {
    private static final Set<Integer> ALLOWED=Set.of(200,202,409,429,503);
    private final EducationCounterpartyStore store; private final Clock clock;
    public EducationCounterpartyService(EducationCounterpartyStore store,Clock clock){this.store=Objects.requireNonNull(store);this.clock=Objects.requireNonNull(clock);}
    public EducationCounterpartyResult exchange(String family,String scenario,String requirementId,String idempotencyKey,String traceId,Map<String,Object> request){
        ...
        var received=new EducationCounterpartyExchange(requestId,requirementId,idempotencyKey,requestHash,businessKey,family,scenario,"RECEIVED",0,Map.of(),1,traceId,now,now,null);
        ...
        var completed=new EducationCounterpartyExchange(requestId,requirementId,idempotencyKey,requestHash,businessKey,family,scenario,state,responseStatus,response,1,traceId,now,clock.instant(),terminal(state)?clock.instant():null);
        store.update(completed);
        return new EducationCounterpartyResult(responseStatus,false,response);
    }
    private EducationCounterpartyResult replay(EducationCounterpartyExchange old,String requestHash){
        if(!old.requestHash().equals(requestHash))return new EducationCounterpartyResult(409,true,Map.of("state","IDEMPOTENCY_CONFLICT","counterpartyRequestId",old.requestId()));
        int status=old.responseStatus()==0?202:old.responseStatus();return new EducationCounterpartyResult(status,true,old.response());
    }
