package com.cpf.education.online.centercut;
...
@CpfRestController
@RequestMapping("/edu/online/settlement-cut")
public class SettlementCutController {
    private final CpfCenterCutOperations centerCut;

    public SettlementCutController(CpfCenterCutOperations centerCut) {
        this.centerCut = centerCut;
    }

    @PostMapping
    @CpfPreAuthorize("hasAuthority('CENTER_CUT_EXECUTE')")
    @Operation(operationId = "EDU_CENTER_CUT_REQUEST", summary = "Center-Cut 대응 온라인 거래")
    @CpfOnlineTransaction(
            operationId = "EDU_CENTER_CUT_REQUEST",
