...
@CpfService public class SettlementCutService { private final CpfCenterCutOperations centerCut; public SettlementCutService(CpfCenterCutOperations c){this.centerCut=c;}
 ...
 public CenterCutView launch(SettlementCutCommand c)throws Exception{Map<String,Object> accepted=centerCut.launch(new CenterCutExecutionRequest("EDU-BATCH-05",c.idempotencyKey(),c.parameters(),"1",c.tpsLimit(),c.concurrencyLimit(),c.requestedBy(),"교육 Center-Cut 실행"));String id=String.valueOf(accepted.getOrDefault("executionId",CpfContexts.currentSegmentId()));return new CenterCutView(id,CpfContexts.transactionId(),accepted,centerCut.status(id));}}
