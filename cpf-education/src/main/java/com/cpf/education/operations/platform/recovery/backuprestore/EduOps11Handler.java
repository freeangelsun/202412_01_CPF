 ...
 */
public final class EduOps11Handler extends AbstractEduCapabilityHandler {
    private static final List<String> BUSINESS_STATES = List.of("BACKING_UP", "VERIFIED", "RESTORING", "RECONCILED");
    private static final List<String> EXCEPTION_SCENARIOS = List.of("불완전 backup", "key 누락", "WAL/archive gap", "다른 app version", "상태 변경이 있는 경우 DB Commit 전 실패와 Commit 후 응답 유실을 구분한다.", "같은 요청의 중복·동시 실행·순서 역전·늦은 응답을 포함한다.", "Retry 가능한 오류와 즉시 중단해야 하는 업무 오류를 구분한다.", "대량·다중 대상 기능에서 부분 성공이 발생하면 성공 대상을 다시 실행하지 않고 실패 대상만 Reprocess·Reconcile한다.", "복구·보상이 필요한 기능은 복구 실패와 보상 실패도 별도 상태·Attempt·Audit로 남긴다.");
    private static final List<String> REQUIRED_VERIFICATION = List.of("Restore drill", "checksum", "control totals", "secret dependency", "입력 Validation·권한·Data Scope·Masking Negative Test", "해당 기능에 적용되는 중복·동시성·Timeout·응답 유실·부분 실패 Test", "해당 기능의 Failure Injection 후 필요한 Retry·Restart·Reprocess·Reconcile·Rollback Test", "Oracle·PostgreSQL·MariaDB 또는 해당 Infra의 의미 동일성 Test", "Log·Metric·Trace·Audit에 Secret·개인정보 원문이 없는지 검사");

    public EduOps11Handler() {
        super(new EduCapabilityDefinition(
                "EDU-OPS-11", "Backup·Restore·시점 복구·대사", EduCapabilityKind.OPS, "cpf-education",
                "CPF_PLATFORM_OPERATOR", List.of("backupSetId", "targetTime", "restoreTarget", "endpointAlias", "institutionRequestId", "recoveryPoint"), List.of(EduWorkflowStep.VALIDATE, EduWorkflowStep.AUTHORIZE, EduWorkflowStep.SCOPE, EduWorkflowStep.DEDUPE, EduWorkflowStep.VERSION_CHECK, EduWorkflowStep.CHECKSUM, EduWorkflowStep.PROTECT, EduWorkflowStep.MUTATE, EduWorkflowStep.COMMIT, EduWorkflowStep.OUTBOX, EduWorkflowStep.EXTERNAL_SEND, EduWorkflowStep.ACK, EduWorkflowStep.RECONCILE, EduWorkflowStep.AUDIT, EduWorkflowStep.OBSERVE),
    ...
    @Override public String implementationPackage() { return "com.cpf.education.operations.platform.recovery.backuprestore"; }
    ...
    @Override public List<String> targetKeys(EduExecutionCommand command) {
        return List.of("edu-ops-11" + "-" + command.businessKey());
    }

    @Override public EduConsumerBinding consumerBinding() {
        return new EduConsumerBinding(
                "EDU-OPS-11", EduConsumerType.PROCESS, "cpf-education",
                "cpf-education/src/main/scripts/edu/invoke-education.ps1", "RECONCILED", "cpf-education allowlisted platform operation simulation contract",
                "POST /api/education/edu-capabilities/EDU-OPS-11/executions", "cpf.repository-root", 600, List.of("backupSetId", "targetTime", "restoreTarget", "endpointAlias", "institutionRequestId", "recoveryPoint"));
    }

    @Override public Map<String,Object> buildBusinessResult(EduExecutionCommand command,long fencingToken) {
        Map<String,Object> result = new LinkedHashMap<>(super.buildBusinessResult(command,fencingToken));
        result.put("scenarioTitle", "Backup·Restore·시점 복구·대사");
