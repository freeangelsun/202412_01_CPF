...
import com.cpf.education.operations.runtime.persistence.EduOperationRepository;
 ...
 */
/** EduExecutionService 타입의 역할과 책임을 정의하며 CPF 계약 경계를 명확히 유지한다. */
public final class EduExecutionService {
    ...
    /** EduExecutionService 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    /** reconcile 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    /** acknowledgeExternal 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    private static String destination(EduCapabilityDefinition definition) {
        return switch (definition.kind()) {
            case OPERATIONS -> "cpf.education.operations.edu.operation";
            case BACKOFFICE -> "cpf.education.backoffice.edu.operation";
            case GATEWAY -> "cpf.education.gateway.edu.operation";
            case BATCH -> "cpf.education.batch.edu.operation";
            case OPS -> "cpf.education.ops.edu.operation";
            default -> "cpf.education.edu.operation";
        };
    }
