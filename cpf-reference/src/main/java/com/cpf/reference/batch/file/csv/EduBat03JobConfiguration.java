...
import java.util.*;
/** EDU-BAT-03 실제 Spring Batch Job 진입점. 고객 Job은 cpf-batch Runtime에 적치하지 않습니다. */
@Configuration(proxyBeanMethods=false)
@ConditionalOnProperty(name="cpf.reference.features.batch.enabled",havingValue="true",matchIfMissing=true)
public class EduBat03JobConfiguration {
 @Bean("eduBat03Job") Job eduBat03Job(JobRepository repository, Step eduBat03Step) {
  return new JobBuilder("EDU-BAT-03.job", repository).start(eduBat03Step).build();
 }
 @Bean("eduBat03Step") Step eduBat03Step(JobRepository repository, PlatformTransactionManager transactionManager, EduBatchScenarioWorker worker) {
  return new StepBuilder("EDU-BAT-03.step", repository).tasklet((contribution, chunkContext) -> {
   var p=chunkContext.getStepContext().getJobParameters(); Map<String,Object> payload=new LinkedHashMap<>();
   for(String field:List.of("fileId", "fileHash", "businessDate", "fileName", "contentLength", "checksum")) { Object value=p.get("edu."+field); if(value!=null)payload.put(field,value); }
   String businessKey=String.valueOf(p.getOrDefault("cpf.businessKey", "EDU-BAT-03-"+UUID.randomUUID()));
