...
/**
 * 온라인 A→B→C(/D) reference flow의 실제 HTTP Channel adapter입니다.
 * {@code cpf-reference-online-abcd} profile에서만 노출되어 제품 업무 API와 혼동되지 않습니다.
 */
@RestController
@Profile({"cpf-reference-online-abcd","cpf-reference-online-abcd-jdbc"})
@RequestMapping("/reference/online/abcd")
public final class OnlineAbcdReferenceHttpController {
     ...
     */
    public OnlineAbcdReferenceHttpController(
    ...
    /**
     * 인가 ingress에서 확정된 transactionId를 reference domain chain으로 전달합니다.
     ...
     */
    @PostMapping
    public OnlineAbcdReferenceFlow.Result execute(@RequestBody OnlineAbcdReferenceFlow.Request request) {
