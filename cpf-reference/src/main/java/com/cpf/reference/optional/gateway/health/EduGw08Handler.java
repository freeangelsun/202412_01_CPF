...
public final class EduGw08Handler extends AbstractEduCapabilityHandler {
    ...
    @Override protected void validateBusinessInput(EduExecutionCommand command) {
        super.validateBusinessInput(command);
                rejectPrivateNetworkLiteral(command, "target");
    }
    ...
    @Override public EduConsumerBinding consumerBinding() {
        return new EduConsumerBinding(
                "EDU-GW-08", EduConsumerType.REFERENCE_GATEWAY, "cpf-reference",
                "CPF_EDU_BUSINESS_RECORD", "REJECTED", "cpf-reference REF Gateway simulator + refDB contract",
                "POST /api/reference/edu-capabilities/EDU-GW-08/executions", "cpf.reference.features.gateway.enabled", 60, List.of("targetUrl", "allowlistVersion", "tlsPolicy"));
    }
    @Override public Map<String,Object> buildBusinessResult(EduExecutionCommand command,long fencingToken) {
        Map<String,Object> result = new LinkedHashMap<>(super.buildBusinessResult(command,fencingToken));
        result.put("scenarioTitle", "SSRF Allowlist·DNS Rebinding·내부망 차단");
