...
public final class EduGw01Handler extends AbstractEduCapabilityHandler {
    ...
    @Override protected void validateBusinessInput(EduExecutionCommand command) {
        super.validateBusinessInput(command);
                requireLongRange(command, "weight", 0L, 100L);
    }
    ...
    @Override public EduConsumerBinding consumerBinding() {
        return new EduConsumerBinding(
                "EDU-GW-01", EduConsumerType.REFERENCE_GATEWAY, "cpf-reference",
                "CPF_EDU_BUSINESS_RECORD", "ROLLED_BACK", "cpf-reference REF Gateway simulator + refDB contract",
                "POST /api/reference/edu-capabilities/EDU-GW-01/executions", "cpf.reference.features.gateway.enabled", 60, List.of("serverGroupId", "weight", "routeId", "targetGroup"));
    }
