...
public final class EduOps01Handler extends AbstractEduCapabilityHandler {
    ...
    @Override protected void validateBusinessInput(EduExecutionCommand command) {
        ...
        requireSafePath(command, "artifactPath");
        requireSafePath(command, "installDir");
    ...
    @Override public EduConsumerBinding consumerBinding() {
        return new EduConsumerBinding(
                "EDU-OPS-01", EduConsumerType.PROCESS, "cpf-reference",
                "cpf-reference/src/main/scripts/edu/invoke-reference-edu.ps1", "READY", "cpf-reference allowlisted platform operation simulation contract",
                "POST /api/reference/edu-capabilities/EDU-OPS-01/executions", "cpf.repository-root", 600, List.of("artifactPath", "manifest", "installDir", "serviceAccount", "fileName", "contentLength", "checksum"));
    }
    @Override public Map<String,Object> buildBusinessResult(EduExecutionCommand command,long fencingToken) {
        Map<String,Object> result = new LinkedHashMap<>(super.buildBusinessResult(command,fencingToken));
        result.put("scenarioTitle", "신규 환경 설치·Artifact·Checksum 검증");
