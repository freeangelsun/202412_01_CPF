...
@Service
public final class CmnCpfMessageService implements CpfMessageService {
    ...
    CmnCpfMessageService(CmnErrorCatalogStore store, CmnMessageArgumentPolicy argumentPolicy) {
        this(store, argumentPolicy, Clock.systemUTC());
    }

    @org.springframework.beans.factory.annotation.Autowired
    CmnCpfMessageService(CmnErrorCatalogStore store, CmnMessageArgumentPolicy argumentPolicy, Clock clock) {
        this.store = store; this.argumentPolicy = argumentPolicy; this.clock = java.util.Objects.requireNonNull(clock, "clock");
    }

    @Override
    public String resolve(String messageCode, Locale locale, Map<String, Object> arguments) {
