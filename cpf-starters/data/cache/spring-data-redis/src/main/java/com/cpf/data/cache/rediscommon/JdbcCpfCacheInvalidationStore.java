...
public final class JdbcCpfCacheInvalidationStore implements CpfCacheInvalidationPort {
    ...
    @Override
    public long version(String consumerId, String tenantId, String namespace, String cacheKey) {
        String consumer = requiredConsumer(consumerId);
        String tenant = requiredSubject(tenantId, "tenantId");
        String ns = requiredSubject(namespace, "namespace");
        String key = normalizedCacheKey(cacheKey);
        List<Long> values = jdbc.query(
                "SELECT LAST_VERSION FROM CPF_CACHE_INVALIDATION_VERSION "
                        + "WHERE CONSUMER_ID = ? AND TENANT_ID = ? AND NAMESPACE_NAME = ? AND CACHE_KEY_VALUE = ?",
                (rs, row) -> rs.getLong(1), consumer, tenant, ns, key);
        return values.isEmpty() ? -1L : values.get(0);
    }

    @Override
    public void advanceVersion(
            String consumerId, String tenantId, String namespace, String cacheKey, long version) {
        if (version < 0) {
        ...
        String key = normalizedCacheKey(cacheKey);
        int updated = jdbc.update(
                "UPDATE CPF_CACHE_INVALIDATION_VERSION SET LAST_VERSION = ?, UPDATED_AT = CURRENT_TIMESTAMP "
                        + "WHERE CONSUMER_ID = ? AND TENANT_ID = ? AND NAMESPACE_NAME = ? "
                        + "AND CACHE_KEY_VALUE = ? AND LAST_VERSION < ?",
                version, consumer, tenant, ns, key, version);
        if (updated > 0 || version(consumer, tenant, ns, key) >= version) {
            return;
        }
        try {
            jdbc.update(
                    "INSERT INTO CPF_CACHE_INVALIDATION_VERSION "
                            + "(CONSUMER_ID, TENANT_ID, NAMESPACE_NAME, CACHE_KEY_VALUE, LAST_VERSION, UPDATED_AT) "
                            + "VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)",
                    consumer, tenant, ns, key, version);
        } catch (DataIntegrityViolationException race) {
            jdbc.update(
                    "UPDATE CPF_CACHE_INVALIDATION_VERSION SET LAST_VERSION = ?, UPDATED_AT = CURRENT_TIMESTAMP "
                            + "WHERE CONSUMER_ID = ? AND TENANT_ID = ? AND NAMESPACE_NAME = ? "
    ...
    private static String normalizedCacheKey(String cacheKey) {
        return CpfCacheInvalidationSubjectKey.encode(cacheKey);
    }

    private static String requiredSubject(String value, String name) {
