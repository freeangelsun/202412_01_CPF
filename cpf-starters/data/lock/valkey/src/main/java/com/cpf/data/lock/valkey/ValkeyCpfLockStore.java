...
public final class ValkeyCpfLockStore implements CpfLockStore {
 ...
 @Override public UpdateResult update(String key,UnaryOperator<StoredLock> transition){String rk=lockKey(key);for(int attempt=1;attempt<=maxRetries;attempt++){UpdateResult result=redis.execute(new SessionCallback<UpdateResult>(){@Override public <K,V> UpdateResult execute(RedisOperations<K,V> operations){@SuppressWarnings("unchecked") RedisOperations<String,String> stringOps=(RedisOperations<String,String>)(RedisOperations<?,?>)operations;stringOps.watch(rk);StoredLock before=decode(key,stringOps.opsForHash().entries(rk));StoredLock after=transition.apply(before);if(after==before||java.util.Objects.equals(after,before)){stringOps.unwatch();return new UpdateResult(before,after);}stringOps.multi();if(after==null)stringOps.delete(rk);else stringOps.opsForHash().putAll(rk,encode(after));List<Object> exec=stringOps.exec();return exec==null?null:new UpdateResult(before,after);}});if(result!=null)return result;}throw new IllegalStateException("Valkey lock CAS conflict after "+maxRetries+" retries");}
