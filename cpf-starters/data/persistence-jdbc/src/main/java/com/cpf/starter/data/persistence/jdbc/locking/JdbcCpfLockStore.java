...
import com.cpf.core.api.locking.CpfLockManager.State;
...
/** CPF_RUNTIME_LOCK 테이블의 JDBC 저장소입니다. */
public final class JdbcCpfLockStore implements CpfJdbcLockManager.Store {
    ...
    @Override public Optional<CpfJdbcLockManager.LockRow> findForUpdate(String key){
        List<CpfJdbcLockManager.LockRow> rows=jdbc.query("SELECT "+COLUMNS+" FROM CPF_RUNTIME_LOCK WHERE LOCK_KEY=? FOR UPDATE",JdbcCpfLockStore::map,key);
        return rows.stream().findFirst();
    }
    @Override public Optional<CpfJdbcLockManager.LockRow> find(String key){
        List<CpfJdbcLockManager.LockRow> rows=jdbc.query("SELECT "+COLUMNS+" FROM CPF_RUNTIME_LOCK WHERE LOCK_KEY=?",JdbcCpfLockStore::map,key);
        return rows.stream().findFirst();
    }
    @Override public List<CpfJdbcLockManager.LockRow> list(int limit){
        return jdbc.query(con -> {
            PreparedStatement ps=con.prepareStatement("SELECT "+COLUMNS+" FROM CPF_RUNTIME_LOCK ORDER BY UPDATED_AT,LOCK_KEY");
            ps.setMaxRows(limit); return ps;
        },JdbcCpfLockStore::map);
    }
    @Override public void insert(CpfJdbcLockManager.LockRow r){
        jdbc.update("INSERT INTO CPF_RUNTIME_LOCK ("+COLUMNS+") VALUES (?,?,?,?,?,?,?,?,?,?,?)",
    ...
    @Override public int update(CpfJdbcLockManager.LockRow r,long expectedVersion){
        return jdbc.update("UPDATE CPF_RUNTIME_LOCK SET OWNER_ID=?,REQUEST_ID=?,FENCING_TOKEN=?,OWNER_EPOCH=?,VERSION_NO=?,ACQUIRED_AT=?,LEASE_UNTIL=?,LOCK_STATE=?,LAST_REASON=?,UPDATED_AT=? WHERE LOCK_KEY=? AND VERSION_NO=?",
