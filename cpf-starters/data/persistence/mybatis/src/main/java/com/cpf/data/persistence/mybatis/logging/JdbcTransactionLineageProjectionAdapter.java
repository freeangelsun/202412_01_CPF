...
public final class JdbcTransactionLineageProjectionAdapter implements CpfTransactionLineageProjectionPort {
    ...
    @Override public void upsert(CpfTransactionLineageRecord r) {
        ...
        if (changed > 0) return;
        try {
            jdbc.update("INSERT INTO cpf_transaction_lineage (lineage_id,transaction_id,segment_id,parent_segment_id,attempt_no,trace_id,span_id,request_id,idempotency_key,tenant_id,system_code,actor_id_masked,instance_id,was_id,agent_id,worker_id,remote_system,operation_id,message_id,consumer_group,dlq_id,batch_job_instance_id,batch_job_execution_id,batch_step_execution_id,partition_id,file_id,source_type,source_ref_id,lifecycle_state,failure_stage,unknown_yn,reconcile_state,occurred_at,freshness_at,payload_hash,archived_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
                    r.lineageId(),r.transactionId(),r.segmentId(),r.parentSegmentId(),r.attemptNo(),r.traceId(),r.spanId(),r.requestId(),r.idempotencyKey(),r.tenantId(),r.systemCode(),r.actorIdMasked(),r.instanceId(),r.wasId(),r.agentId(),r.workerId(),r.remoteSystem(),r.operationId(),r.messageId(),r.consumerGroup(),r.dlqId(),r.batchJobInstanceId(),r.batchJobExecutionId(),r.batchStepExecutionId(),r.partitionId(),r.fileId(),r.sourceType(),r.sourceRefId(),r.lifecycleState(),r.failureStage(),r.unknown()?"Y":"N",r.reconcileState(),ts(r.occurredAt()),ts(r.freshnessAt()),r.payloadHash(),ts(r.archivedAt()));
