...
public enum CpfDatabaseVendor {
    ...
    /** accepts 작업을 CPF 표준 계약에 따라 수행한다. */
    ...
    public static CpfDatabaseVendor fromDatabaseProductName(String productName) {
        String normalized = requireText(productName, "databaseProductName").toLowerCase(Locale.ROOT);
        return Arrays.stream(values())
                .filter(vendor -> normalized.contains(vendor.id))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("지원하지 않는 DB product name입니다: " + productName));
    }

    /** fromDriverClassName 작업을 CPF 표준 계약에 따라 수행한다. */
    public static CpfDatabaseVendor fromDriverClassName(String driverClassName) {
        String normalized = requireText(driverClassName, "driverClassName").toLowerCase(Locale.ROOT);
        return Arrays.stream(values())
                .filter(vendor -> normalized.equals(vendor.driverClassName.toLowerCase(Locale.ROOT))
                        || normalized.contains(vendor.id))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("지원하지 않는 DB driver입니다: " + driverClassName));
    }
