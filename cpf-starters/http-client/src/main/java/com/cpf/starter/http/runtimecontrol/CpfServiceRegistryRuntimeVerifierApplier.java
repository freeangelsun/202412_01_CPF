    ...
    public CpfRuntimeApplyResult apply(CpfRuntimeDelivery delivery) {
        try {
            boolean matched = switch (changeType) {
                case "SERVICE_ROUTE" -> verifyRoute(CpfRuntimePayloadReader.asMap(delivery.payload()));
                case "CIRCUIT" -> verifyCircuit(CpfRuntimePayloadReader.asMap(delivery.payload()));
                case "MAINTENANCE" -> verifyMaintenance(CpfRuntimePayloadReader.asMap(delivery.payload()));
                default -> false;
            };
            return matched
