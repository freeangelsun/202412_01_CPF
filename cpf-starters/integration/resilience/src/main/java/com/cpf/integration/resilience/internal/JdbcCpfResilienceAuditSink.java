...
public final class JdbcCpfResilienceAuditSink implements CpfResilienceAuditSink {
    ...
    @Override public void record(String eventType,String operationId,String actorId,String reason,Map<String,String> attrs,Instant at){
        ...
        jdbc.update("insert into OPS_RESILIENCE_AUDIT(audit_id,event_type,operation_id,actor_id,reason_code,sanitized_attributes,occurred_at) values(?,?,?,?,?,?,?)",UUID.randomUUID().toString(),eventType,operationId,actorId,truncate(reason,256),sanitized.toString(),java.sql.Timestamp.from(at));
    }
