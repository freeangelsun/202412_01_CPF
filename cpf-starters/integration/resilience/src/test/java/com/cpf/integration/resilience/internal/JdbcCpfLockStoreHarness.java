...
import com.cpf.data.lock.api.CpfLockManager;import com.cpf.data.lock.api.CpfLockManagers;import java.time.*;import java.util.concurrent.*;import java.util.concurrent.atomic.AtomicInteger;import org.springframework.jdbc.core.JdbcTemplate;import org.springframework.transaction.support.TransactionTemplate;
public final class JdbcCpfLockStoreHarness{
 public static void main(String[]args)throws Exception{
  JdbcCpfLockStore store=new JdbcCpfLockStore(new JdbcTemplate(),new TransactionTemplate()); Clock clock=Clock.fixed(Instant.parse("2026-08-05T00:00:00Z"),ZoneOffset.UTC); CpfLockManager manager=CpfLockManagers.create(store,null,clock);
