...
public final class IbmMqCpfBrokerBridgeAdapter implements CpfBrokerBridgePort, AutoCloseable {
    ...
    @Override public CpfBrokerBridgeResult publish(String destination,String key,Object payload,Map<String,String> additionalHeaders){
        ...
        catch(RuntimeException e){throw new IllegalStateException("IBM MQ publish result is UNKNOWN; reconcile before retry",e);}
    ...
    @Override public void subscribe(String destination,CpfBrokerBridgeHandler handler){String d=required(destination,"destination");if(!d.equals(properties.getDestination()))throw new IllegalArgumentException("IBM MQ subscription destination mismatch");Objects.requireNonNull(handler,"handler");String group=resolveGroup(d,handler.consumerGroup());String existing=consumerGroups.putIfAbsent(d,group);if(existing!=null&&!existing.equals(group))throw new IllegalStateException("IBM MQ destination already bound to different CPF consumerGroup: "+d);handlers.computeIfAbsent(d,x->new CopyOnWriteArrayList<>()).add(handler);containers.computeIfAbsent(d,this::start);}
    private DefaultMessageListenerContainer start(String d){var c=factory.createListenerContainer();c.setDestinationName(d);c.setMessageListener((MessageListener)this::consume);c.afterPropertiesSet();c.start();return c;}
    private void consume(Message provider){try{byte[] body=body(provider);if(body.length>properties.getMaxPayloadBytes())throw new IllegalArgumentException("IBM MQ bridge consumer payload exceeds maximum size");var message=mapper.readValue(body,CpfBrokerBridgeMessage.class);String pd=provider.getStringProperty("cpfBridgeDestination");if(pd!=null&&!pd.equals(message.destination()))throw new SecurityException("IBM MQ destination metadata mismatch");Map<String,String> wire=read(provider,message.headers());if(provider.getJMSCorrelationID()!=null&&!provider.getJMSCorrelationID().equals(wire.get(CpfMessageHeaderNames.TRANSACTION_ID)))throw new SecurityException("IBM MQ correlation/context mismatch");int attempt=provider.propertyExists("JMSXDeliveryCount")?Math.max(1,provider.getIntProperty("JMSXDeliveryCount")):1;String group=consumerGroups.get(message.destination());if(group==null||group.isBlank())throw new IllegalStateException("IBM MQ CPF consumerGroup is not registered");var bundle=contextSupport.extractInbound("IBM_MQ",message.key(),message.destination(),null,group,null,null,attempt,provider.getJMSRedelivered(),null,null,wire,null);var targets=handlers.getOrDefault(message.destination(),new CopyOnWriteArrayList<>());if(targets.isEmpty())throw new IllegalStateException("No IBM MQ bridge consumer registered");contextSupport.consume(bundle,()->targets.forEach(h->h.handle(message)));provider.acknowledge();}catch(Exception e){throw new IllegalStateException("IBM MQ bridge consumer rejected message for redelivery/DLQ",e);}}
