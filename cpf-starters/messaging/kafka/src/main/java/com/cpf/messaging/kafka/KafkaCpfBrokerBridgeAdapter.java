...
public final class KafkaCpfBrokerBridgeAdapter implements CpfBrokerBridgePort, AutoCloseable {
    ...
    @Override public List<CpfBrokerBridgeMessage> findRecent(String destination,int limit){String topic=hasText(destination)?destination:null;int bounded=Math.max(1,Math.min(limit<=0?50:limit,RECENT_LIMIT));return recent.stream().filter(m->topic==null||topic.equals(m.destination())).sorted(Comparator.comparing(value -> value.createdAt()).reversed()).limit(bounded).toList();}
