...
/** CpfBrokerClientRouter는 메시징 신뢰성 경계에서 중복 방지·재시도·결과불명 복구 책임을 명확히 수행합니다. */
public final class CpfBrokerClientRouter implements CpfBrokerClient {
    ...
    /** CpfBrokerClientRouter 작업을 CPF 메시징 신뢰성 정책과 상태 전이 규칙에 따라 수행합니다. */
    ...
    public CpfBrokerPublishResult enqueue(CpfBrokerPublishRequest request) {
        return defaultBinding.client().enqueue(CpfBrokerHeaderPolicy.validatedRequest(request));
    }
    ...
    public CpfBrokerPublishResult enqueue(String binding, CpfBrokerPublishRequest request) {
        ...
        return client.client().enqueue(CpfBrokerHeaderPolicy.validatedRequest(request));
    }
