...
public class JdbcCpfBrokerReliabilityRepository
    ...
    /** JdbcCpfBrokerReliabilityRepository 작업을 CPF 메시징 신뢰성 정책과 상태 전이 규칙에 따라 수행합니다. */
    ...
    // 메시지 상태 전이를 단일 트랜잭션으로 묶어 부분 저장과 중복 처리를 방지합니다.
    @Transactional(transactionManager = "cpfTransactionManager")
    /** replay 작업을 CPF 메시징 신뢰성 정책과 상태 전이 규칙에 따라 수행합니다. */
    ...
    // 메시지 상태 전이를 단일 트랜잭션으로 묶어 부분 저장과 중복 처리를 방지합니다.
    @Transactional(transactionManager = "cpfTransactionManager")
    /** replayRange 작업을 CPF 메시징 신뢰성 정책과 상태 전이 규칙에 따라 수행합니다. */
