...
public final class JdbcCpfFeatureFlagAuditSink implements CpfFeatureFlagAuditSink{
 ...
 public void record(String event,String key,String actor,String reason,Map<String,String>a,Instant at){var s=new TreeMap<String,String>();if(a!=null)a.forEach((k,v)->{String x=k==null?"":k.toLowerCase(Locale.ROOT);if(!(x.contains("token")||x.contains("password")||x.contains("secret")||x.contains("value")))s.put(k,v==null?null:v.substring(0,Math.min(512,v.length())));});jdbc.update("insert into OPS_FEATURE_FLAG_AUDIT(audit_id,event_type,flag_key,actor_id,reason_code,sanitized_attributes,occurred_at) values(?,?,?,?,?,?,?)",UUID.randomUUID().toString(),event,key,actor,reason,s.toString(),java.sql.Timestamp.from(at));}
}
