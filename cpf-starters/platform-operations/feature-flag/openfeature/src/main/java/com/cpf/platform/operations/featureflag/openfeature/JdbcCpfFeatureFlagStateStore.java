...
public final class JdbcCpfFeatureFlagStateStore implements CpfFeatureFlagStateStore {
 ...
 @Override public Optional<CpfFeatureFlagResult<CpfFeatureFlagValue>> findEffective(String key,Instant now){
  var kills=jdbc.query("select revision from OPS_FEATURE_FLAG_KILL_SWITCH where flag_key=? and enabled_flag='Y'",(r,n)->r.getLong(1),key);
 ...
 @Override public String requestOverride(String key,CpfFeatureFlagValue value,Instant expires,String requester,String reason){
  ...
  try{jdbc.update("insert into OPS_FEATURE_FLAG_OVERRIDE_REQUEST(request_id,flag_key,value_type,value_text,expires_at,requester_id,request_reason,request_status,active_flag_key) values(?,?,?,?,?,?,?,'PENDING',?)",id,key,valueType,valueText,java.sql.Timestamp.from(expires),requester,reason,key);return id;}
  catch(DuplicateKeyException e){
   var existing=jdbc.query("select request_id,value_type,value_text,expires_at,requester_id,request_reason from OPS_FEATURE_FLAG_OVERRIDE_REQUEST where active_flag_key=? and request_status='PENDING'",(r,n)->new Pending(r.getString(1),r.getString(2),r.getString(3),r.getTimestamp(4).toInstant(),r.getString(5),r.getString(6)),key);
 ...
 @Override public CpfFeatureFlagResult<CpfFeatureFlagValue> approveOverride(String id,String approver,String reason,Instant now){return tx.execute(st->{
  var rows=jdbc.query("select flag_key,value_type,value_text,expires_at,requester_id,request_status,approver_id from OPS_FEATURE_FLAG_OVERRIDE_REQUEST where request_id=? for update",(r,n)->new Req(r.getString(1),r.getString(2),r.getString(3),r.getTimestamp(4).toInstant(),r.getString(5),r.getString(6),r.getString(7)),id);
  ...
  long rev=nextRevision();jdbc.update("update OPS_FEATURE_FLAG_OVERRIDE set override_status='SUPERSEDED',active_flag_key=null where flag_key=? and override_status='ACTIVE'",q.key);jdbc.update("insert into OPS_FEATURE_FLAG_OVERRIDE(override_id,flag_key,value_type,value_text,expires_at,override_status,revision,active_flag_key) values(?,?,?,?,?,'ACTIVE',?,?)",id,q.key,q.type,q.value,java.sql.Timestamp.from(q.expires),rev,q.key);int n=jdbc.update("update OPS_FEATURE_FLAG_OVERRIDE_REQUEST set request_status='APPROVED',approver_id=?,approval_reason=?,active_flag_key=null where request_id=? and request_status='PENDING'",approver,reason,id);if(n!=1)throw new IllegalStateException("approval conflict");bumpRevision(rev);return new CpfFeatureFlagResult<>(q.key,decode(q.type,q.value),"OVERRIDE","APPROVED_OVERRIDE",CpfFeatureFlagResult.Source.SECURE_OVERRIDE,rev,now);});}
 @Override public void revokeOverride(String id,String operator,String reason,Instant now){tx.executeWithoutResult(st->{var rows=jdbc.query("select override_status from OPS_FEATURE_FLAG_OVERRIDE where override_id=? for update",(r,n)->r.getString(1),id);if(rows.size()!=1)throw new IllegalArgumentException("override not found");String status=rows.getFirst();if("REVOKED".equals(status))return;if(!"ACTIVE".equals(status))throw new IllegalStateException("override is not active: "+status);long rev=nextRevision();int n=jdbc.update("update OPS_FEATURE_FLAG_OVERRIDE set override_status='REVOKED',active_flag_key=null,revision=? where override_id=? and override_status='ACTIVE'",rev,id);if(n!=1)throw new IllegalStateException("override revoke conflict");bumpRevision(rev);});}
 @Override public void setKillSwitch(String key,boolean enabled,String operator,String reason,Instant now){tx.executeWithoutResult(st->{lockRevision();var rows=jdbc.query("select enabled_flag from OPS_FEATURE_FLAG_KILL_SWITCH where flag_key=? for update",(r,n)->r.getString(1),key);String desired=enabled?"Y":"N";if(rows.size()==1&&desired.equals(rows.getFirst()))return;long rev=nextRevision();if(rows.isEmpty())jdbc.update("insert into OPS_FEATURE_FLAG_KILL_SWITCH(flag_key,enabled_flag,revision,updated_by,updated_at) values(?,?,?,?,?)",key,desired,rev,operator,java.sql.Timestamp.from(now));else{int n=jdbc.update("update OPS_FEATURE_FLAG_KILL_SWITCH set enabled_flag=?,revision=?,updated_by=?,updated_at=? where flag_key=?",desired,rev,operator,java.sql.Timestamp.from(now),key);if(n!=1)throw new IllegalStateException("kill switch update conflict");}bumpRevision(rev);});}
 private CpfFeatureFlagResult<CpfFeatureFlagValue> approvedResult(String id,Req q,Instant now){var rows=jdbc.query("select revision from OPS_FEATURE_FLAG_OVERRIDE where override_id=?",(r,n)->r.getLong(1),id);if(rows.size()!=1)throw new IllegalStateException("approved override state is missing: "+id);return new CpfFeatureFlagResult<>(q.key,decode(q.type,q.value),"OVERRIDE","APPROVED_OVERRIDE",CpfFeatureFlagResult.Source.SECURE_OVERRIDE,rows.getFirst(),now);}
 private void lockRevision(){Long v=jdbc.queryForObject("select revision from OPS_FEATURE_FLAG_REVISION where singleton_id=1 for update",Long.class);if(v==null)throw new IllegalStateException("feature flag revision row is missing");}
 @Override public long revision(){Long v=jdbc.queryForObject("select revision from OPS_FEATURE_FLAG_REVISION where singleton_id=1",Long.class);return v==null?0:v;}
 private long nextRevision(){int n=jdbc.update("update OPS_FEATURE_FLAG_REVISION set revision=revision+1 where singleton_id=1");if(n!=1)throw new IllegalStateException("feature flag revision row is missing");return revision();}
