...
final class CpfOtlpTelemetryAdapter implements CpfTelemetry {
 ...
 private static SpanKind toKind(String k){if(k==null)return SpanKind.INTERNAL;try{return SpanKind.valueOf(k.trim().toUpperCase(java.util.Locale.ROOT));}catch(Exception e){return SpanKind.INTERNAL;}}
 private static boolean sensitive(String k){String n=k.toLowerCase(java.util.Locale.ROOT).replace("-","").replace("_","");return n.contains("password")||n.contains("secret")||n.contains("token")||n.contains("authorization")||n.contains("cookie")||n.contains("account")||n.contains("memberno")||n.contains("customerno");}
}
