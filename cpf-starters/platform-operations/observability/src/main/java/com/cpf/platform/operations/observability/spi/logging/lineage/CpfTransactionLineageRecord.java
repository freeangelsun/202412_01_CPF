 ...
 * @param systemCode 현재 처리 Logical System
 * @param targetSystemCode 원격 Target Logical System
 * @param currentChannel 현재 처리 Runtime Channel(선택 Context)
 * @param actorIdMasked 마스킹된 actor 식별자
 * @param instanceId 처리 instance 식별자
 * @param wasId WAS 식별자
 * @param agentId agent 식별자
 * @param workerId worker 식별자
 * @param targetChannel 원격 Target Channel(선택 Context)
...
public record CpfTransactionLineageRecord(
        ...
        String targetSystemCode, String currentChannel, String actorIdMasked, String instanceId, String wasId, String agentId, String workerId, String targetChannel,
    ...
    public static CpfTransactionLineageRecord fromSegment(TransactionSegmentRecord source, boolean terminal) {
        ...
        return new CpfTransactionLineageRecord(sha256("SEGMENT|" + seg + "|" + attempt + "|" + phase + "|" + occurred),
                ...
                firstNonBlank(source.getSystemCode(), source.getOriginalSystemCode()),
                firstNonBlank(source.getTargetSystemCode(), firstNonBlank(source.getTargetModuleCode(), source.getExternalInstitutionCode())),
                firstNonBlank(source.getCurrentChannel(), source.getOriginalChannel()), source.getOperatorIdMasked(),
                source.getSelectedInstanceId(), null, null, null, source.getTargetChannel(),
