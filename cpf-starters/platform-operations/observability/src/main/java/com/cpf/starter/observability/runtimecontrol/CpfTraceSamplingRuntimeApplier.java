    ...
    public CpfRuntimeApplyResult apply(CpfRuntimeDelivery delivery) {
        try {
            ...
            double defaultRate = decimal(
                    CpfRuntimePayloadReader.value(delivery.payload(), "defaultRate"),
                    1.0d);
            boolean alwaysSampleErrors = bool(
                    CpfRuntimePayloadReader.value(delivery.payload(), "alwaysSampleErrors"),
                    true);
            Map<String, Double> moduleRates = rates(
                    CpfRuntimePayloadReader.value(delivery.payload(), "moduleRates"));
            Map<String, Double> transactionRates = rates(
                    CpfRuntimePayloadReader.value(delivery.payload(), "businessTransactionRates"));
