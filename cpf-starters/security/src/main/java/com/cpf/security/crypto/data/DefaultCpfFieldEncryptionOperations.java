package com.cpf.security.crypto.data;
...
public final class DefaultCpfFieldEncryptionOperations implements CpfFieldEncryptionOperations {
    ...
    @Override public CpfEncryptedField encrypt(String fieldName,String value,CpfFieldClassification classification,boolean searchable){
        ...
        return new CpfEncryptedField(classification, token, crypto.activeKeyVersion(),
                toCoreEnvelope(crypto.encrypt(value.getBytes(StandardCharsets.UTF_8), aad)),
                mask(value, classification));
    }
    @Override public String decrypt(String fieldName,CpfEncryptedField field,String actorId,String reason){
        require(actorId,"actorId"); require(reason,"reason"); try { String v=new String(crypto.decrypt(toSecurityEnvelope(field.envelope()),aad(fieldName,field.classification())),StandardCharsets.UTF_8); audit.add(new AuditEvent(Instant.now(),actorId,"REVEAL",fieldName,reason,field.keyVersion(),true)); return v; }
    ...
    @Override public CpfEncryptedField rekey(String fieldName,CpfEncryptedField field,String targetKeyVersion,String actorId,String reason){
        require(actorId,"actorId"); require(reason,"reason"); CpfRekeyResult result=crypto.rekey(toSecurityEnvelope(field.envelope()),aad(fieldName,field.classification()),targetKeyVersion);
        audit.add(new AuditEvent(Instant.now(),actorId,"REKEY",fieldName,reason,targetKeyVersion,true));
        return new CpfEncryptedField(field.classification(), field.searchableToken(), targetKeyVersion,
                toCoreEnvelope(result.ciphertext()), field.maskedPreview());
    }
    ...
    private static com.cpf.core.api.security.crypto.CpfEnvelopeCiphertext toCoreEnvelope(
            CpfEnvelopeCiphertext value) {
        return new com.cpf.core.api.security.crypto.CpfEnvelopeCiphertext(
                value.algorithm(), value.provider(), value.keyVersion(), value.encryptedDataKey(),
                value.encryptedDataKeyNonce(), value.payloadNonce(), value.ciphertext(), value.aadHash());
    }

    private static CpfEnvelopeCiphertext toSecurityEnvelope(
            com.cpf.core.api.security.crypto.CpfEnvelopeCiphertext value) {
        return new CpfEnvelopeCiphertext(
                value.algorithm(), value.provider(), value.keyVersion(), value.encryptedDataKey(),
                value.encryptedDataKeyNonce(), value.payloadNonce(), value.ciphertext(), value.aadHash());
    }
}
