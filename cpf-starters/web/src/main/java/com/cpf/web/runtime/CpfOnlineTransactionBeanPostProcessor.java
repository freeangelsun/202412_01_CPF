...
/** @CpfOnlineTransaction 선언 오류와 중복 거래 ID를 Application startup에서 차단합니다. */
public final class CpfOnlineTransactionBeanPostProcessor implements BeanPostProcessor {
    ...
    private void register(CpfOnlineTransaction rule, String source) {
        ...
        String previous = transactionIds.putIfAbsent(rule.id(), source);
        if (previous != null && !previous.equals(source)) {
            throw new IllegalStateException("CPF_ONLINE_TX_DUPLICATE_ID:" + rule.id() + ":" + previous + ":" + source);
        }
    }
}
