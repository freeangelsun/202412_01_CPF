...
& pwsh -NoProfile -File (Join-Path $Root 'cpf-tools/verification/tools/verify-local-artifact-propagation.ps1') -Root $Root -LocalRepository $LocalRepository -RequireManifest | Out-Host
...
if (-not $SkipDbVendorPacks) {
    ...
    $selector = Join-Path $Root 'cpf-tools/db/tools/select-db-vendor-resources.ps1'
...
$packGate = Join-Path $Root 'cpf-tools/db/verification/check-offline-db-resource-pack.ps1'
