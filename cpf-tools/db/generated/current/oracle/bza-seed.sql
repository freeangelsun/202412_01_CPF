...
USING (SELECT 'bza-admin' AS admin_login_id, '업무 관리자 샘플' AS admin_name, NULL AS password_hash, 'BZA_MANAGER' AS role_code, 'Y' AS use_yn, 'N' AS lock_yn, 0 AS login_fail_count, 'Y' AS password_change_required_yn, NULL AS password_expire_at, NULL AS last_login_at, 'SYSTEM' AS created_by, 'SYSTEM' AS updated_by FROM dual) src
...
MERGE INTO BZA_ROLE tgt
USING (SELECT 'BZA_MANAGER' AS role_code, '업무 관리자' AS role_name, 'Y' AS write_allowed_yn, 'ALL' AS data_scope, 'Y' AS use_yn, 'SYSTEM' AS created_by, 'SYSTEM' AS updated_by FROM dual) src
ON (tgt.role_code=src.role_code)
...
USING (SELECT admin_user_id AS admin_user_id, 'BZA_MANAGER' AS role_code, CURRENT_TIMESTAMP(3) AS valid_from, NULL AS valid_to, 'Y' AS primary_yn, 'CPF_TEST_SEED' AS grant_reason, 'CPF-TEST-BZA-ROLE-MANAGER-0001' AS operation_id, 'SYSTEM' AS created_by, 'SYSTEM' AS updated_by FROM BZA_ADMIN_USER
WHERE admin_login_id = 'bza-admin') src
...
USING (SELECT 'bza-admin' AS owner_login_id, 'APPROVAL' AS screen_code, '진행 중 결재' AS search_name, '{"approvalStatus":"IN_REVIEW"}' AS criteria_json, 'N' AS shared_yn, 'Y' AS use_yn, 'SYSTEM' AS created_by, 'SYSTEM' AS updated_by FROM dual) src
