SELECT 'WORKER' lease_type, TO_CHAR(execution_id) target_id, worker_id owner_id,
       lease_status state, lease_until, fencing_token
FROM bat_execution_lease
WHERE lease_status <> 'RELEASED'
UNION ALL
SELECT 'CENTER_CUT', TO_CHAR(center_cut_item_id), runner_id, claim_status, lease_until, fencing_token
