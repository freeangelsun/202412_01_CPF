...
-- ============================================================================
-- cpf-tools/db/source/mariadb/55_cmn_seed_data.sql
-- ============================================================================
...
-- ============================================================================
-- cpf-tools/db/source/mariadb/58_reference_external_edu_seed.sql
-- ============================================================================
-- REF EDU external-integration simulator seed.
-- Fixed EXS domain/database is intentionally not used.
...
INSERT INTO cpf_service_routing_policy (
    ...
    updated_at = CURRENT_TIMESTAMP(3);
-- ============================================================================
-- cpf-tools/db/source/mariadb/58_reference_runtime_seed.sql
-- ============================================================================
-- CPF 선택형 Reference/Local Runtime Seed입니다.
-- 제품 필수 Seed가 아니며 EDU/로컬 Runtime/Service Registry 샘플을 명시적으로 사용할 때만 적용합니다.
-- 실제 고객 배포에서는 환경별 Registry/Instance/Endpoint를 운영 설정으로 등록해야 합니다.

USE batDB;

-- Local Batch/EDU Runtime fixtures -----------------------------------------
...
INSERT INTO batDB.bat_execution (
    ...
    worker_id, transaction_id, start_time, end_time,
...
-- 의존하지 않도록 제거했습니다. 운영/EDU runtime에서 실제 이벤트로 검증합니다.

-- Reference Service Registry / Center-Cut fixtures -------------------------
USE batDB;
...
-- BZA 조직/직급/직책/결재 Optional Sample -----------------------------------
-- 고객 실제 기준정보가 아니라 fresh-install 기능 확인용 중립 샘플입니다.
USE bzaDB;
...
INSERT INTO bza_organization (
    ...
    ('SAMPLE_ROOT', NULL, '샘플 본부', 'COMPANY', 10, CURRENT_TIMESTAMP(3), NULL, 'Y', 'SYSTEM', 'SYSTEM'),
...
ON DUPLICATE KEY UPDATE
    position_name = VALUES(position_name), rank_order = VALUES(rank_order),
...
ON DUPLICATE KEY UPDATE
    job_title_name = VALUES(job_title_name), manager_yn = VALUES(manager_yn),
    use_yn = VALUES(use_yn), updated_by = VALUES(updated_by), updated_at = CURRENT_TIMESTAMP(3);
...
INSERT INTO bza_employee (
    ...
    ('SAMPLE0001', NULL, 'SAMPLE_DEV', '샘플 결재자', 'SAMPLE_P2', 'SAMPLE_MANAGER', NULL,
     'ACTIVE', CURRENT_DATE, NULL, NULL, NULL, 'Y', 'SYSTEM', 'SYSTEM'),
    ('SAMPLE0002', NULL, 'SAMPLE_DEV', '샘플 요청자', 'SAMPLE_P1', 'SAMPLE_MEMBER', 'SAMPLE0001',
     'ACTIVE', CURRENT_DATE, NULL, NULL, NULL, 'Y', 'SYSTEM', 'SYSTEM')
ON DUPLICATE KEY UPDATE
    organization_code = VALUES(organization_code), employee_name = VALUES(employee_name),
    position_code = VALUES(position_code), job_title_code = VALUES(job_title_code),
    manager_employee_no = VALUES(manager_employee_no), employment_status = VALUES(employment_status),
    use_yn = VALUES(use_yn), updated_by = VALUES(updated_by), updated_at = CURRENT_TIMESTAMP(3);
...
INSERT INTO bza_organization_responsibility (
    organization_code, responsibility_type, employee_no, effective_from, effective_to,
    priority_no, use_yn, created_by, updated_by
)
...
ON DUPLICATE KEY UPDATE
    policy_name = VALUES(policy_name), enabled_yn = VALUES(enabled_yn),
    self_approval_allowed_yn = VALUES(self_approval_allowed_yn),
    description = VALUES(description), updated_by = VALUES(updated_by), updated_at = CURRENT_TIMESTAMP(3);
...
ON DUPLICATE KEY UPDATE
    step_type = VALUES(step_type), target_type = VALUES(target_type), target_code = VALUES(target_code),
...
-- ============================================================================
-- cpf-tools/db/source/mariadb/59_adm_local_seed.sql
-- ============================================================================
