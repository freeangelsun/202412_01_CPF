...
class VendorRendererTest(unittest.TestCase):
    ...
    def test_role_counts_and_vendor_dirs(self):
        ...
        roles=('CPF_PLATFORM_DB','CUSTOMER_BUSINESS_DB')
    ...
    def test_official_vendor_contract_and_current_db4_projection(self):
        ...
        self.assertNotIn('MYSQL(',vendor_source); self.assertNotIn('MSSQL(',vendor_source)
        for vendor in OFFICIAL:
            pack=DB/'generated/current'/vendor
            manifest=json.loads((pack/'manifest.json').read_text(encoding='utf-8-sig'))
            self.assertFalse(any(name.startswith('reference-fixture-') for name in manifest['artifacts']),vendor)
            self.assertFalse(any(pack.glob('reference-fixture-*.sql')),vendor)
            self.assertTrue((pack/'cpf-platform-schema.sql').is_file(),vendor)
            self.assertTrue((pack/'backoffice-schema.sql').is_file(),vendor)
