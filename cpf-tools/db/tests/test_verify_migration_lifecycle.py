...
def write_sql_with_checksum(pack: Path, name: str, body: str = "select 1;\n") -> None:
    ...
    with manifest.open("w", encoding="utf-8", newline="\n") as handle:
        handle.write(previous + f"{digest} *{name}\n")
...
class Fixture:
    def __init__(self, root: Path) -> None:
        ...
        runtime = root / "cpf-tools/db/verification"
        static = root / "cpf-tools/db/verification"
        generator = root / "cpf-tools/generator/tools"
        write_text(runtime / "runtime.ps1", "Write-Output 'fixture'\n")
        write_text(static / "static.ps1", "Write-Output 'fixture'\n")
        write_text(db / "verify_migration_lifecycle.py", "# fixture verifier\n")
        write_text(generator / "initialize-domain-database.ps1", "Write-Output 'fixture'\n")
        ...
        for vendor in VENDORS:
            ...
            pack = {
                ...
                "generatedDomainContract": {
                    ...
                    "databaseBootstrapScript": "cpf-tools/generator/tools/initialize-domain-database.ps1",
                },
...
class VerifyMigrationLifecycleTest(unittest.TestCase):
    ...
    def test_required_static_gates_must_consume_verifier(self):
        ...
        contract["requiredStaticGates"] = ["cpf-tools/db/verification/static.ps1"]
    ...
    def test_multiple_rollbacks_for_same_pack_version_fail(self):
        ...
        write_sql(root / "U1__baseline.sql", "drop table other;\n")
        with self.assertRaisesRegex(MOD.ContractError, "multiple exact rollback artifacts"):
            self.verify()
...
class RepositoryIntegrationContractTest(unittest.TestCase):
    def test_existing_gradle_db_readiness_consumer_invokes_lifecycle_verifier(self):
        ...
        gate = repo_root / "cpf-tools/db/verification/check-official-db-vendor-readiness.ps1"
        body = gate.read_text(encoding="utf-8-sig")
        for token in (
            "cpf-tools/db/verify_migration_lifecycle.py",
            "cpf-tools/db/verification/verify-cpf-db-vendor-manifest.py",
            "cpf-tools/db/verification/verify-cpf-db-lifecycle-contract.py",
    ...
    def test_runtime_matrix_is_source_bound_vendor3_and_fail_closed(self):
        repo_root = Path(__file__).resolve().parents[3]
        matrix = (repo_root / "cpf-tools/db/verification/invoke-cpf-db-runtime-matrix.ps1").read_text(encoding="utf-8-sig")
        for token in (
            ...
            "Upgrade",
            "RollbackReapply",
            "UNVERIFIED_EXTERNAL_RUNTIME",
            "lifecycleStatus-eq'SUCCEEDED'",
            "lifecycleStatus-eq'PLANNED'",
            "-ClientAdapter $ClientAdapter",
            "RequireRuntime",
            "db-runtime-matrix.json",
        ):
    ...
    def test_pitr_result_is_exact_sha_bound(self):
        repo_root = Path(__file__).resolve().parents[3]
        script = (repo_root / "cpf-tools/db/tools/invoke-cpf-pitr-restore.ps1").read_text(encoding="utf-8-sig")
        self.assertIn("sourceSha=(Get-CpfGitHeadOrUnknown $rootPath)", script)
        self.assertIn("schemaVersion=2", script)

    def test_vendor_lifecycle_delegates_credential_transport_to_official_consumers(self):
        repo_root = Path(__file__).resolve().parents[3]
        script = (repo_root / "cpf-tools/db/tools/run-db-vendor-lifecycle.ps1").read_text(encoding="utf-8-sig")
        self.assertIn("initialize-cpf-database.ps1", script)
        self.assertIn("invoke-platform-database-migration.ps1", script)
        self.assertIn("invoke-db-lifecycle-docker-client.ps1", script)
        self.assertIn("sanitized = $true", script)
        for forbidden in (
            "CPF_ORACLE_PASSWORD",
...
class RuntimeEvidenceHardeningTest(unittest.TestCase):
    ...
    def load_script_module(name: str):
        repo_root = Path(__file__).resolve().parents[3]
        path = repo_root / "cpf-tools/db/verification" / name
    ...
    def test_runtime_approval_wrappers_bind_clean_head_and_approver(self):
        ...
        for rel in paths:
            body = (repo_root / rel).read_text(encoding="utf-8-sig")
            for token in (
