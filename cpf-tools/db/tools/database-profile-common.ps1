...
$script:CpfDatabaseVendorManifestPath = [IO.Path]::GetFullPath(
    (Join-Path $PSScriptRoot "../vendor-pack-manifest.json"))
...
function Get-CpfDomainDatabaseProfile {
    ...
    if (-not (Test-Path -LiteralPath $Path -PathType Leaf)) {
        throw "Generated Domain DB Profile 파일이 없습니다: $Path"
    }
    $profile = Get-Content -LiteralPath $Path -Raw -Encoding UTF8 | ConvertFrom-Json -Depth 50
    if ([int]$profile.profileVersion -ne 1) {
        throw "지원하지 않는 Generated Domain DB Profile version입니다: $($profile.profileVersion)"
    }
    if ($null -eq $profile.database) { throw "Generated Domain DB Profile database가 없습니다." }
    $vendor = Assert-CpfSupportedDatabaseVendor ([string]$profile.database.vendor)
    if ([string]::IsNullOrWhiteSpace([string]$profile.environment)) { throw "Generated Domain DB Profile environment가 없습니다." }
    if ([string]::IsNullOrWhiteSpace([string]$profile.database.host)) { throw "Generated Domain DB Profile database.host가 없습니다." }
    if ([int]$profile.database.port -lt 1 -or [int]$profile.database.port -gt 65535) { throw "Generated Domain DB Profile database.port 범위가 올바르지 않습니다." }
    Assert-CpfDbIdentifier ([string]$profile.database.databaseName) "databaseName"
    if (-not [string]::IsNullOrWhiteSpace([string]$profile.database.schemaName)) { Assert-CpfDbIdentifier ([string]$profile.database.schemaName) "schemaName" }
    if ($null -eq $profile.database.migration) { throw "Generated Domain DB Profile database.migration이 없습니다." }
    Assert-CpfDbUsername ([string]$profile.database.migration.username) "migration.username"
    if ($null -eq $profile.database.migration.password) { throw "Generated Domain DB Profile migration.password env/fallbackEnv 계약이 없습니다." }
    $profile.database.vendor = $vendor
    return $profile
}
