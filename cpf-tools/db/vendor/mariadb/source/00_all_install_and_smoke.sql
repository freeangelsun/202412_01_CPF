...
('GATEWAY_APPLY_STATUS','GATEWAY_DASHBOARD','Gateway 적용 상태·이력','/adm#gateway-apply-status',308,'Y','SYSTEM','SYSTEM') ON DUPLICATE KEY UPDATE parent_menu_id = VALUES(parent_menu_id), menu_name = VALUES(menu_name), menu_path = VALUES(menu_path), sort_order = VALUES(sort_order), use_yn = VALUES(use_yn), updated_by = VALUES(updated_by), updated_at = VALUES(updated_at);
...
('GATEWAY_TEST_CONTROL','GATEWAY_HEALTH','CONTROL','Gateway 연결시험 취소·재검증','POST','/adm/api/gateway-registry/connection-test-operations/*/**',80,'Y','SYSTEM','SYSTEM') ON DUPLICATE KEY UPDATE menu_id = VALUES(menu_id), action_code = VALUES(action_code), button_name = VALUES(button_name), http_method = VALUES(http_method), api_pattern = VALUES(api_pattern), sort_order = VALUES(sort_order), use_yn = VALUES(use_yn), updated_by = VALUES(updated_by), updated_at = VALUES(updated_at);
...
('ADM_VIEWER','GATEWAY_APPLY_STATUS','Y','N','N','SYSTEM','SYSTEM') ON DUPLICATE KEY UPDATE read_yn = VALUES(read_yn), write_yn = VALUES(write_yn), delete_yn = VALUES(delete_yn), updated_by = VALUES(updated_by), updated_at = VALUES(updated_at);
...
('API_GATEWAY_TEST_CONTROL','GATEWAY','POST','/adm/api/gateway-registry/connection-test-operations/*/**','Gateway 연결시험 취소·재검증','CONTROL','GATEWAY_HEALTH','GATEWAY_TEST_CONTROL','Y','SYSTEM','SYSTEM') ON DUPLICATE KEY UPDATE api_group_code = VALUES(api_group_code), http_method = VALUES(http_method), api_path = VALUES(api_path), api_name = VALUES(api_name), permission_code = VALUES(permission_code), menu_id = VALUES(menu_id), button_id = VALUES(button_id), use_yn = VALUES(use_yn), updated_by = VALUES(updated_by), updated_at = VALUES(updated_at);
...
('ADM_VIEWER','GATEWAY_TEST_CONTROL','N','SYSTEM','SYSTEM') ON DUPLICATE KEY UPDATE allow_yn = VALUES(allow_yn), updated_by = VALUES(updated_by), updated_at = VALUES(updated_at);
...
('ADM_VIEWER','API_GATEWAY_TEST_CONTROL','N','SYSTEM','SYSTEM') ON DUPLICATE KEY UPDATE allow_yn = VALUES(allow_yn), updated_by = VALUES(updated_by), updated_at = VALUES(updated_at);
-- ============================================================================
