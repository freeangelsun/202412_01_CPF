...
-- MBW baseline is regenerated from the current canonical MBW schema section.
-- Do not hand-edit this migration independently from the canonical DB source after GA baseline is frozen.
...
CREATE TABLE IF NOT EXISTS mbw_admin_user (
    ...
    role_code VARCHAR(50) NOT NULL COMMENT '호환용 기본 역할 코드; 실제 권한은 mbw_user_role 다중 매핑을 정본으로 사용',
    ...
    UNIQUE KEY uk_mbw_admin_user_login (admin_login_id),
...
CREATE TABLE IF NOT EXISTS mbw_login_history (
    ...
    CONSTRAINT fk_mbw_login_history_user
        FOREIGN KEY (admin_user_id) REFERENCES mbw_admin_user(admin_user_id)
        ON DELETE SET NULL
...
CREATE TABLE IF NOT EXISTS mbw_refresh_token (
    ...
    UNIQUE KEY uk_mbw_refresh_token_hash (refresh_token_hash),
    INDEX ix_mbw_refresh_token_user (admin_user_id, revoked_yn, expire_at),
    CONSTRAINT fk_mbw_refresh_token_user
        FOREIGN KEY (admin_user_id) REFERENCES mbw_admin_user(admin_user_id)
        ON DELETE CASCADE
...
CREATE TABLE IF NOT EXISTS mbw_menu (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시',
    PRIMARY KEY (menu_id),
    UNIQUE KEY uk_mbw_menu_code (menu_code)
...
CREATE TABLE IF NOT EXISTS mbw_role (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '수정일시',
    PRIMARY KEY (role_id),
    UNIQUE KEY uk_mbw_role_code (role_code)
...
CREATE TABLE IF NOT EXISTS mbw_user_role (
    ...
    CONSTRAINT fk_mbw_user_role_user FOREIGN KEY (admin_user_id)
        REFERENCES mbw_admin_user(admin_user_id) ON DELETE CASCADE,
    CONSTRAINT fk_mbw_user_role_role FOREIGN KEY (role_code)
        REFERENCES mbw_role(role_code),
    CONSTRAINT ck_mbw_user_role_primary CHECK (primary_yn IN ('Y','N')),
    CONSTRAINT ck_mbw_user_role_effective CHECK (
        valid_to IS NULL OR valid_from IS NULL OR valid_to > valid_from
    )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 사용자 다중 역할 매핑';
...
CREATE TABLE IF NOT EXISTS mbw_permission (
    ...
    UNIQUE KEY uk_mbw_permission (role_code, menu_code, button_code),
...
CREATE TABLE IF NOT EXISTS mbw_organization (
    ...
    UNIQUE KEY uk_mbw_organization_code (organization_code),
    ...
    CONSTRAINT ck_mbw_organization_effective CHECK (
        effective_to IS NULL OR effective_from IS NULL OR effective_to > effective_from
    )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 조직';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 직급 기준정보';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 직책 기준정보';
...
CREATE TABLE IF NOT EXISTS mbw_employee (
    ...
    UNIQUE KEY uk_mbw_employee_no (employee_no),
    UNIQUE KEY uk_mbw_employee_admin_user (admin_user_id),
    INDEX ix_mbw_employee_organization (organization_code, employment_status),
    CONSTRAINT fk_mbw_employee_admin_user FOREIGN KEY (admin_user_id)
        REFERENCES mbw_admin_user(admin_user_id) ON DELETE SET NULL,
    CONSTRAINT fk_mbw_employee_organization FOREIGN KEY (organization_code)
        REFERENCES mbw_organization(organization_code),
    CONSTRAINT fk_mbw_employee_position FOREIGN KEY (position_code)
        REFERENCES mbw_position(position_code) ON DELETE SET NULL,
    CONSTRAINT fk_mbw_employee_job_title FOREIGN KEY (job_title_code)
        REFERENCES mbw_job_title(job_title_code) ON DELETE SET NULL,
    CONSTRAINT ck_mbw_employee_use CHECK (use_yn IN ('Y','N')),
    CONSTRAINT ck_mbw_employee_employment_period CHECK (
        leave_date IS NULL OR join_date IS NULL OR leave_date >= join_date
    )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 직원 프로필';
...
CREATE TABLE IF NOT EXISTS mbw_employee_assignment (
    ...
    CONSTRAINT fk_mbw_employee_assignment_employee FOREIGN KEY (employee_no)
        REFERENCES mbw_employee(employee_no) ON DELETE CASCADE,
    CONSTRAINT fk_mbw_employee_assignment_org FOREIGN KEY (organization_code)
        REFERENCES mbw_organization(organization_code),
    CONSTRAINT fk_mbw_employee_assignment_position FOREIGN KEY (position_code)
        REFERENCES mbw_position(position_code) ON DELETE SET NULL,
    CONSTRAINT fk_mbw_employee_assignment_job_title FOREIGN KEY (job_title_code)
        REFERENCES mbw_job_title(job_title_code) ON DELETE SET NULL,
    CONSTRAINT ck_mbw_employee_assignment_type CHECK (
    ...
    CONSTRAINT ck_mbw_employee_assignment_effective CHECK (
        effective_to IS NULL OR effective_to > effective_from
    )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 직원 유효기간 기반 조직/직급/직책 Assignment';
...
CREATE TABLE IF NOT EXISTS mbw_organization_responsibility (
    ...
    CONSTRAINT fk_mbw_org_responsibility_org FOREIGN KEY (organization_code)
        REFERENCES mbw_organization(organization_code),
    CONSTRAINT fk_mbw_org_responsibility_employee FOREIGN KEY (employee_no)
        REFERENCES mbw_employee(employee_no),
    CONSTRAINT ck_mbw_org_responsibility_type CHECK (
    ...
    CONSTRAINT ck_mbw_org_responsibility_effective CHECK (
        effective_to IS NULL OR effective_to > effective_from
    )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 조직 책임자/대행/결재 책임자 유효기간 모델';
...
CREATE TABLE IF NOT EXISTS mbw_notification (
    ...
    recipient_login_id VARCHAR(100) NOT NULL COMMENT '수신 MBW 로그인 ID',
...
CREATE TABLE IF NOT EXISTS mbw_attachment (
    ...
    UNIQUE KEY uk_mbw_attachment_storage_key (storage_key),
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 첨부파일 메타';
...
CREATE TABLE IF NOT EXISTS mbw_saved_search (
    ...
    UNIQUE KEY uk_mbw_saved_search_owner (owner_login_id, screen_code, search_name),
    INDEX ix_mbw_saved_search_screen (screen_code, shared_yn, use_yn)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 저장 검색';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 다운로드 감사';
...
CREATE TABLE IF NOT EXISTS mbw_approval_policy (
    ...
    CONSTRAINT ck_mbw_approval_policy_flags CHECK (
        enabled_yn IN ('Y','N') AND self_approval_allowed_yn IN ('Y','N')
    ),
    CONSTRAINT ck_mbw_approval_policy_effective CHECK (
        effective_to IS NULL OR effective_to > effective_from
    )
...
CREATE TABLE IF NOT EXISTS mbw_approval_policy_step (
    ...
    CONSTRAINT fk_mbw_approval_policy_step_policy FOREIGN KEY (policy_code, policy_version)
        REFERENCES mbw_approval_policy(policy_code, policy_version) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_mbw_approval_policy_step_target CHECK (
    ...
    CONSTRAINT ck_mbw_approval_policy_step_required CHECK (
        required_yn IN ('Y','N')
        AND (
...
CREATE TABLE IF NOT EXISTS mbw_approval_document (
    ...
    UNIQUE KEY uk_mbw_approval_document_no (approval_no),
    UNIQUE KEY uk_mbw_approval_document_idempotency (request_idempotency_key),
    ...
    CONSTRAINT fk_mbw_approval_document_policy FOREIGN KEY (policy_code, policy_version)
        REFERENCES mbw_approval_policy(policy_code, policy_version),
    CONSTRAINT fk_mbw_approval_document_resubmit FOREIGN KEY (resubmitted_from_approval_id)
        REFERENCES mbw_approval_document(approval_id),
    CONSTRAINT ck_mbw_approval_document_policy_pair CHECK (
        (policy_code IS NULL AND policy_version IS NULL)
        OR (policy_code IS NOT NULL AND policy_version IS NOT NULL)
    ),
    CONSTRAINT ck_mbw_approval_document_status CHECK (
...
CREATE TABLE IF NOT EXISTS mbw_approval_line (
    ...
    UNIQUE KEY uk_mbw_approval_line (approval_id, step_no, target_type, target_code),
    INDEX ix_mbw_approval_line_approver (approver_employee_no, decision_status),
    CONSTRAINT fk_mbw_approval_line_document FOREIGN KEY (approval_id)
        REFERENCES mbw_approval_document(approval_id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_mbw_approval_line_target CHECK (
        target_type IN ('EMPLOYEE','ROLE','ORGANIZATION','ORG_MANAGER','POSITION')
    ),
    CONSTRAINT ck_mbw_approval_line_rule CHECK (decision_rule IN ('ALL','ANY','N_OF_M')),
    CONSTRAINT ck_mbw_approval_line_required CHECK (
        required_yn IN ('Y','N')
        AND (
            (decision_rule = 'N_OF_M' AND required_count IS NOT NULL AND required_count > 0)
            OR (decision_rule <> 'N_OF_M' AND required_count IS NULL)
    ...
    CONSTRAINT ck_mbw_approval_line_status CHECK (
...
CREATE TABLE IF NOT EXISTS mbw_approval_participant (
    ...
    UNIQUE KEY uk_mbw_approval_participant (approval_line_id, approver_employee_no),
    UNIQUE KEY uk_mbw_approval_participant_idem (idempotency_key),
    INDEX ix_mbw_approval_participant_inbox (approver_employee_no, decision_status, approval_id),
    CONSTRAINT fk_mbw_approval_participant_document FOREIGN KEY (approval_id)
        REFERENCES mbw_approval_document(approval_id) ON DELETE CASCADE,
    CONSTRAINT fk_mbw_approval_participant_line FOREIGN KEY (approval_line_id)
        REFERENCES mbw_approval_line(approval_line_id) ON DELETE CASCADE,
    ...
    CONSTRAINT ck_mbw_approval_participant_status CHECK (
        decision_status IN ('WAITING','APPROVED','AGREED','REJECTED','SKIPPED')
    )
...
CREATE TABLE IF NOT EXISTS mbw_approval_delegation (
    ...
    CONSTRAINT fk_mbw_approval_delegation_from FOREIGN KEY (delegator_employee_no)
        REFERENCES mbw_employee(employee_no),
    CONSTRAINT fk_mbw_approval_delegation_to FOREIGN KEY (delegate_employee_no)
        REFERENCES mbw_employee(employee_no),
...
CREATE TABLE IF NOT EXISTS mbw_approval_history (
    ...
    UNIQUE KEY uk_mbw_approval_history_idempotency (idempotency_key),
    INDEX ix_mbw_approval_history_document (approval_id, created_at),
    CONSTRAINT fk_mbw_approval_history_document FOREIGN KEY (approval_id)
        REFERENCES mbw_approval_document(approval_id)
...
CREATE TABLE IF NOT EXISTS mbw_project_setting (
    ...
    UNIQUE KEY uk_mbw_project_setting_key (setting_key)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='MBW 프로젝트 설정';
