...
CREATE TABLE IF NOT EXISTS mbw_login_operation (
    ...
    CONSTRAINT fk_mbw_login_operation_user FOREIGN KEY (admin_user_id)
        REFERENCES mbw_admin_user(admin_user_id) ON DELETE CASCADE,
    CONSTRAINT ck_mbw_login_operation_status CHECK (operation_status IN ('PROCESSING','SUCCESS'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

ALTER TABLE mbw_refresh_token
    ADD COLUMN login_operation_id VARCHAR(100) NULL AFTER transaction_id,
    ADD INDEX ix_mbw_refresh_token_login_operation (login_operation_id, revoked_yn);
