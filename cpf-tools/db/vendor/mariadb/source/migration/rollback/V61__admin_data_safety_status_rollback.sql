...
USE backofficeDB;
ALTER TABLE mbw_employee DROP CONSTRAINT IF EXISTS ck_mbw_employee_status;
ALTER TABLE mbw_employee
    ADD CONSTRAINT ck_mbw_employee_status CHECK (employment_status IN ('ACTIVE','EMPLOYED','ON_LEAVE','SECONDMENT','DISPATCHED','RETIRED','TERMINATED'));
ALTER TABLE mbw_admin_user DROP CONSTRAINT IF EXISTS ck_mbw_admin_user_status;
ALTER TABLE mbw_admin_user DROP INDEX IF EXISTS ix_mbw_admin_user_status;
ALTER TABLE mbw_admin_user
    MODIFY COLUMN role_code VARCHAR(50) NOT NULL COMMENT '호환용 기본 역할 코드; 실제 권한은 mbw_user_role 다중 매핑 정본',
