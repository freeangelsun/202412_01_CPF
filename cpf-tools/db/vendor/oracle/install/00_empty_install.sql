...
CREATE TABLE BAT_RECONCILIATION_AUDIT (
    reconciliation_audit_id NUMBER(19) GENERATED BY DEFAULT ON NULL AS IDENTITY NOT NULL,
    request_id VARCHAR2(100 CHAR) NOT NULL,
    entity_type VARCHAR2(40 CHAR) NOT NULL,
    entity_key VARCHAR2(300 CHAR) NOT NULL,
    from_status VARCHAR2(40 CHAR) NOT NULL,
    to_status VARCHAR2(40 CHAR) NOT NULL,
    requester_id VARCHAR2(120 CHAR) NOT NULL,
    approver_id VARCHAR2(120 CHAR) NOT NULL,
    reason_text VARCHAR2(1000 CHAR) NOT NULL,
    idempotency_key VARCHAR2(200 CHAR) NOT NULL,
...
COMMENT ON TABLE BAT_RECONCILIATION_AUDIT IS 'Immutable BAT UNKNOWN-result reconciliation approval audit';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.reconciliation_audit_id IS 'Reconciliation audit sequence';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.request_id IS 'Approved reconciliation request identifier';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.entity_type IS 'Reconciled entity type';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.entity_key IS 'Canonical reconciled entity identity';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.from_status IS 'State observed before reconciliation';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.to_status IS 'State established by reconciliation';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.requester_id IS 'Verified reconciliation requester';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.approver_id IS 'Verified independent approver';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.reason_text IS 'Required operator reason';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.idempotency_key IS 'Single reconciliation outcome key';
COMMENT ON COLUMN BAT_RECONCILIATION_AUDIT.expected_attempt IS 'Expected trigger execution attempt';
...
CREATE TABLE CPF_MASKING_POLICY_COMMAND (
    command_id_hash CHAR(64 CHAR) NOT NULL,
    command_hash CHAR(64 CHAR) NOT NULL,
    result_version NUMBER(19) NOT NULL,
    result_sensitive_keys_csv VARCHAR2(4000 CHAR) NOT NULL,
    result_max_length NUMBER(10) NOT NULL,
    result_mask_bearer_flag CHAR(1 CHAR) DEFAULT 'Y' NOT NULL,
    result_value_rules_csv VARCHAR2(1000 CHAR) NOT NULL,
    result_updated_at TIMESTAMP(6) NOT NULL,
    result_updated_by VARCHAR2(128 CHAR) NOT NULL,
    result_reason VARCHAR2(1000 CHAR),
...
COMMENT ON TABLE CPF_MASKING_POLICY_COMMAND IS '마스킹 정책 명령 중복 제거 원장';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.command_id_hash IS '명령 식별자 SHA-256(원문 미보관)';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.command_hash IS '명령 본문 SHA-256';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_version IS '명령 결과 정책 버전';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_sensitive_keys_csv IS '결과 민감 키 목록(CSV)';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_max_length IS '결과 최대 길이';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_mask_bearer_flag IS '결과 Bearer 마스킹 여부';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_value_rules_csv IS '결과 값 규칙 목록(CSV)';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_updated_at IS '결과 반영 시각';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_updated_by IS '결과 반영 운영자';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.result_reason IS '결과 사유';
COMMENT ON COLUMN CPF_MASKING_POLICY_COMMAND.recorded_at IS '명령 기록 시각(TTL 정리 기준)';

CREATE TABLE CPF_MASKING_POLICY_HEAD (
    singleton_id NUMBER(10) NOT NULL,
    active_version NUMBER(19) NOT NULL,
    CONSTRAINT pk_CPF_MASKING_POLICY_HEAD PRIMARY KEY (singleton_id)
);
COMMENT ON TABLE CPF_MASKING_POLICY_HEAD IS '현재 활성 마스킹 정책 버전 포인터';
COMMENT ON COLUMN CPF_MASKING_POLICY_HEAD.singleton_id IS '단일 head 행 식별자(항상 1)';
COMMENT ON COLUMN CPF_MASKING_POLICY_HEAD.active_version IS '현재 활성 마스킹 정책 버전';
...
COMMENT ON TABLE CPF_MASKING_POLICY_SHARD IS '마스킹 정책 변경을 직렬화하는 제어 shard';
COMMENT ON COLUMN CPF_MASKING_POLICY_SHARD.shard_id IS '마스킹 정책 제어 shard 식별자(단일 shard=0)';

CREATE TABLE CPF_MASKING_POLICY_VERSION (
    policy_version NUMBER(19) NOT NULL,
    sensitive_keys_csv VARCHAR2(4000 CHAR) NOT NULL,
    max_length NUMBER(10) NOT NULL,
    mask_bearer_flag CHAR(1 CHAR) DEFAULT 'Y' NOT NULL,
    value_rules_csv VARCHAR2(1000 CHAR) NOT NULL,
    updated_at TIMESTAMP(6) NOT NULL,
...
COMMENT ON TABLE CPF_MASKING_POLICY_VERSION IS '마스킹 정책 버전 이력';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.policy_version IS '마스킹 정책 버전';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.sensitive_keys_csv IS '운영자가 지정한 민감 키 목록(CSV)';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.max_length IS '마스킹 후 최대 길이';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.mask_bearer_flag IS 'Bearer 토큰 마스킹 여부';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.value_rules_csv IS '운영자가 선택한 값 규칙 목록(CSV)';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.updated_at IS '정책 반영 시각';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.updated_by IS '정책을 변경한 운영자';
COMMENT ON COLUMN CPF_MASKING_POLICY_VERSION.update_reason IS '변경 사유';
...
CREATE TABLE CPF_TRANSACTION_SEGMENT (
    ...
    transaction_id CHAR(34 CHAR) NOT NULL,
    execution_id VARCHAR2(160 CHAR),
...
COMMENT ON TABLE SPRING_SESSION IS '서버 세션 저장소(Spring Session JDBC 표준 규격)';
COMMENT ON COLUMN SPRING_SESSION.PRIMARY_ID IS '세션 기본 식별자';
COMMENT ON COLUMN SPRING_SESSION.SESSION_ID IS '세션 ID';
COMMENT ON COLUMN SPRING_SESSION.CREATION_TIME IS '생성 시각(epoch millis)';
COMMENT ON COLUMN SPRING_SESSION.LAST_ACCESS_TIME IS '마지막 접근 시각(epoch millis)';
COMMENT ON COLUMN SPRING_SESSION.MAX_INACTIVE_INTERVAL IS '최대 비활성 유지 시간(초)';
COMMENT ON COLUMN SPRING_SESSION.EXPIRY_TIME IS '만료 시각(epoch millis)';
COMMENT ON COLUMN SPRING_SESSION.PRINCIPAL_NAME IS '세션 주체 이름';
...
COMMENT ON TABLE SPRING_SESSION_ATTRIBUTES IS '서버 세션 속성 저장소(Spring Session JDBC 표준 규격)';
COMMENT ON COLUMN SPRING_SESSION_ATTRIBUTES.SESSION_PRIMARY_ID IS '세션 기본 식별자';
COMMENT ON COLUMN SPRING_SESSION_ATTRIBUTES.ATTRIBUTE_NAME IS '세션 속성 이름';
COMMENT ON COLUMN SPRING_SESSION_ATTRIBUTES.ATTRIBUTE_BYTES IS '직렬화된 세션 속성 값';
...
CREATE TABLE BAT_EXECUTION_LEASE (
    ...
    CONSTRAINT fk_bat_execution_lease_worker FOREIGN KEY (worker_id) REFERENCES BAT_WORKER (worker_id)
);
...
CREATE OR REPLACE TRIGGER trg_touch_MBW_APPROVAL_EXECUTION BEFORE UPDATE ON MBW_APPROVAL_EXECUTION FOR EACH ROW BEGIN :NEW.updated_at := CURRENT_TIMESTAMP; END;
/
