...
CREATE OR REPLACE FUNCTION cpf_touch_ADM_APPROVAL_CAPABILITY_NONCE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_ADM_APPROVAL_CAPABILITY_NONCE ON ADM_APPROVAL_CAPABILITY_NONCE;
CREATE TRIGGER trg_cpf_touch_ADM_APPROVAL_CAPABILITY_NONCE BEFORE UPDATE ON ADM_APPROVAL_CAPABILITY_NONCE FOR EACH ROW EXECUTE FUNCTION cpf_touch_ADM_APPROVAL_CAPABILITY_NONCE();
...
CREATE OR REPLACE FUNCTION cpf_touch_ADM_APPROVAL_POLICY_LOCK() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_ADM_APPROVAL_POLICY_LOCK ON ADM_APPROVAL_POLICY_LOCK;
CREATE TRIGGER trg_cpf_touch_ADM_APPROVAL_POLICY_LOCK BEFORE UPDATE ON ADM_APPROVAL_POLICY_LOCK FOR EACH ROW EXECUTE FUNCTION cpf_touch_ADM_APPROVAL_POLICY_LOCK();
...
CREATE OR REPLACE FUNCTION cpf_touch_BAT_EXECUTION_CONTROL() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_BAT_EXECUTION_CONTROL ON BAT_EXECUTION_CONTROL;
CREATE TRIGGER trg_cpf_touch_BAT_EXECUTION_CONTROL BEFORE UPDATE ON BAT_EXECUTION_CONTROL FOR EACH ROW EXECUTE FUNCTION cpf_touch_BAT_EXECUTION_CONTROL();
...
CREATE OR REPLACE FUNCTION cpf_touch_BAT_EXECUTION_EPOCH() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_BAT_EXECUTION_EPOCH ON BAT_EXECUTION_EPOCH;
CREATE TRIGGER trg_cpf_touch_BAT_EXECUTION_EPOCH BEFORE UPDATE ON BAT_EXECUTION_EPOCH FOR EACH ROW EXECUTE FUNCTION cpf_touch_BAT_EXECUTION_EPOCH();
...
CREATE TABLE BAT_RECONCILIATION_AUDIT (
    reconciliation_audit_id BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
...
CREATE OR REPLACE FUNCTION cpf_touch_CMN_CACHE_REFRESH_CHECKPOINT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_CMN_CACHE_REFRESH_CHECKPOINT ON CMN_CACHE_REFRESH_CHECKPOINT;
CREATE TRIGGER trg_cpf_touch_CMN_CACHE_REFRESH_CHECKPOINT BEFORE UPDATE ON CMN_CACHE_REFRESH_CHECKPOINT FOR EACH ROW EXECUTE FUNCTION cpf_touch_CMN_CACHE_REFRESH_CHECKPOINT();
...
CREATE OR REPLACE FUNCTION cpf_touch_CMN_CACHE_REFRESH_EVENT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_CMN_CACHE_REFRESH_EVENT ON CMN_CACHE_REFRESH_EVENT;
CREATE TRIGGER trg_cpf_touch_CMN_CACHE_REFRESH_EVENT BEFORE UPDATE ON CMN_CACHE_REFRESH_EVENT FOR EACH ROW EXECUTE FUNCTION cpf_touch_CMN_CACHE_REFRESH_EVENT();
...
CREATE OR REPLACE FUNCTION cpf_touch_CMN_CODE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_CMN_CODE ON CMN_CODE;
CREATE TRIGGER trg_cpf_touch_CMN_CODE BEFORE UPDATE ON CMN_CODE FOR EACH ROW EXECUTE FUNCTION cpf_touch_CMN_CODE();
...
CREATE TABLE CMN_MESSAGE (
    ...
    escape_html_yn CHAR(1) NOT NULL DEFAULT 'Y',
    mask_arguments_yn CHAR(1) NOT NULL DEFAULT 'Y',
    effective_from TIMESTAMP,
...
CREATE OR REPLACE FUNCTION cpf_touch_CMN_MESSAGE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_CMN_MESSAGE ON CMN_MESSAGE;
CREATE TRIGGER trg_cpf_touch_CMN_MESSAGE BEFORE UPDATE ON CMN_MESSAGE FOR EACH ROW EXECUTE FUNCTION cpf_touch_CMN_MESSAGE();
...
CREATE OR REPLACE FUNCTION cpf_touch_CMN_PARAMETER() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_CMN_PARAMETER ON CMN_PARAMETER;
CREATE TRIGGER trg_cpf_touch_CMN_PARAMETER BEFORE UPDATE ON CMN_PARAMETER FOR EACH ROW EXECUTE FUNCTION cpf_touch_CMN_PARAMETER();
...
CREATE TABLE CMN_RESPONSE_CODE (
    ...
    retry_disposition VARCHAR(16) NOT NULL DEFAULT 'NEVER',
    exposure VARCHAR(32) NOT NULL DEFAULT 'SAFE_MESSAGE_ONLY',
    effective_from TIMESTAMP,
    effective_to TIMESTAMP,
    catalog_version BIGINT NOT NULL DEFAULT 1,
...
CREATE OR REPLACE FUNCTION cpf_touch_CMN_RESPONSE_CODE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_CMN_RESPONSE_CODE ON CMN_RESPONSE_CODE;
CREATE TRIGGER trg_cpf_touch_CMN_RESPONSE_CODE BEFORE UPDATE ON CMN_RESPONSE_CODE FOR EACH ROW EXECUTE FUNCTION cpf_touch_CMN_RESPONSE_CODE();
...
CREATE OR REPLACE FUNCTION cpf_touch_CMN_TEMPLATE_DEFINITION() RETURNS trigger AS $$ BEGIN NEW.UPDATED_AT = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_CMN_TEMPLATE_DEFINITION ON CMN_TEMPLATE_DEFINITION;
CREATE TRIGGER trg_cpf_touch_CMN_TEMPLATE_DEFINITION BEFORE UPDATE ON CMN_TEMPLATE_DEFINITION FOR EACH ROW EXECUTE FUNCTION cpf_touch_CMN_TEMPLATE_DEFINITION();
...
CREATE TABLE CPF_BROKER_INBOX (
    ...
    message_id VARCHAR(120) NOT NULL,
    consumer_identity VARCHAR(120) NOT NULL DEFAULT 'default',
...
CREATE TABLE CPF_MASKING_POLICY_VERSION (
    ...
    mask_bearer_flag CHAR(1) NOT NULL DEFAULT 'Y',
...
CREATE TABLE CPF_TRANSACTION_LINEAGE (
    ...
    target_system_code VARCHAR(128),
...
CREATE TABLE CPF_TRANSACTION_LOG (
    ...
    CALLER_SYSTEM_CODE VARCHAR(120),
    TARGET_SYSTEM_CODE VARCHAR(32),
    ORIGINAL_SYSTEM_CODE VARCHAR(20),
    SYSTEM_CODE VARCHAR(20),
    CALLER_CHANNEL VARCHAR(120),
    TARGET_CHANNEL VARCHAR(32),
    TARGET_OPERATION_ID VARCHAR(160),
...
CREATE TABLE CPF_TRANSACTION_SEGMENT (
    ...
    caller_system_code VARCHAR(100),
    target_system_code VARCHAR(32),
    current_channel VARCHAR(30),
    original_channel VARCHAR(30),
    client_id VARCHAR(100),
    caller_channel VARCHAR(100),
    target_channel VARCHAR(32),
    target_operation_id VARCHAR(160),
...
CREATE OR REPLACE FUNCTION cpf_touch_GW_SERVER_GROUP() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_GW_SERVER_GROUP ON GW_SERVER_GROUP;
CREATE TRIGGER trg_cpf_touch_GW_SERVER_GROUP BEFORE UPDATE ON GW_SERVER_GROUP FOR EACH ROW EXECUTE FUNCTION cpf_touch_GW_SERVER_GROUP();
...
CREATE OR REPLACE FUNCTION cpf_touch_GW_SPOOL_CHECKPOINT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_GW_SPOOL_CHECKPOINT ON GW_SPOOL_CHECKPOINT;
CREATE TRIGGER trg_cpf_touch_GW_SPOOL_CHECKPOINT BEFORE UPDATE ON GW_SPOOL_CHECKPOINT FOR EACH ROW EXECUTE FUNCTION cpf_touch_GW_SPOOL_CHECKPOINT();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_CHANNEL_EXECUTION_POLICY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_CHANNEL_EXECUTION_POLICY ON OPS_CHANNEL_EXECUTION_POLICY;
CREATE TRIGGER trg_cpf_touch_OPS_CHANNEL_EXECUTION_POLICY BEFORE UPDATE ON OPS_CHANNEL_EXECUTION_POLICY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_CHANNEL_EXECUTION_POLICY();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_CHANNEL_POLICY_VERSION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_CHANNEL_POLICY_VERSION ON OPS_CHANNEL_POLICY_VERSION;
CREATE TRIGGER trg_cpf_touch_OPS_CHANNEL_POLICY_VERSION BEFORE UPDATE ON OPS_CHANNEL_POLICY_VERSION FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_CHANNEL_POLICY_VERSION();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_CHANNEL_REGISTRY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_CHANNEL_REGISTRY ON OPS_CHANNEL_REGISTRY;
CREATE TRIGGER trg_cpf_touch_OPS_CHANNEL_REGISTRY BEFORE UPDATE ON OPS_CHANNEL_REGISTRY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_CHANNEL_REGISTRY();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_CONTROL_OPERATION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_CONTROL_OPERATION ON OPS_CONTROL_OPERATION;
CREATE TRIGGER trg_cpf_touch_OPS_CONTROL_OPERATION BEFORE UPDATE ON OPS_CONTROL_OPERATION FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_CONTROL_OPERATION();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_LOG_POLICY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_LOG_POLICY ON OPS_LOG_POLICY;
CREATE TRIGGER trg_cpf_touch_OPS_LOG_POLICY BEFORE UPDATE ON OPS_LOG_POLICY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_LOG_POLICY();
...
CREATE TABLE OPS_MANAGED_SERVER (
    ...
    management_identity VARCHAR(200),
    ...
    registered_by VARCHAR(100) NOT NULL DEFAULT 'CPF',
    row_version BIGINT NOT NULL DEFAULT 0,
    created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_by VARCHAR(100) NOT NULL DEFAULT 'CPF',
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_MANAGED_SERVER() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_MANAGED_SERVER ON OPS_MANAGED_SERVER;
CREATE TRIGGER trg_cpf_touch_OPS_MANAGED_SERVER BEFORE UPDATE ON OPS_MANAGED_SERVER FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_MANAGED_SERVER();
...
CREATE TABLE OPS_RESILIENCE_POLICY (
    ...
    idempotent_flag CHAR(1) NOT NULL DEFAULT 'N',
    reconcile_flag CHAR(1) NOT NULL DEFAULT 'N',
...
CREATE TABLE OPS_RETENTION_CONTROL_AUDIT (
    ...
    expected_version BIGINT,
    result_state VARCHAR(20) NOT NULL DEFAULT 'SUCCEEDED',
    created_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE OPS_RETENTION_POLICY (
    ...
    action_name VARCHAR(16) NOT NULL DEFAULT 'KEEP',
    retention_days INTEGER NOT NULL DEFAULT 90,
    schedule_expression VARCHAR(100),
    maintenance_start VARCHAR(8),
    maintenance_end VARCHAR(8),
    enabled_yn CHAR(1) NOT NULL DEFAULT 'Y',
    paused_yn CHAR(1) NOT NULL DEFAULT 'N',
    legal_hold_yn CHAR(1) NOT NULL DEFAULT 'N',
    chunk_size INTEGER NOT NULL DEFAULT 1000,
    throttle_millis BIGINT NOT NULL DEFAULT 0,
    max_rows_per_run BIGINT NOT NULL DEFAULT 100000,
    max_runtime_seconds BIGINT NOT NULL DEFAULT 300,
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_CONTROLLER_LEASE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_CONTROLLER_LEASE ON OPS_RUNTIME_CONTROLLER_LEASE;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_CONTROLLER_LEASE BEFORE UPDATE ON OPS_RUNTIME_CONTROLLER_LEASE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_CONTROLLER_LEASE();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_INSTANCE_GROUP() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_INSTANCE_GROUP ON OPS_RUNTIME_INSTANCE_GROUP;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_INSTANCE_GROUP BEFORE UPDATE ON OPS_RUNTIME_INSTANCE_GROUP FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_INSTANCE_GROUP();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_RATE_BUCKET() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_RATE_BUCKET ON OPS_RUNTIME_RATE_BUCKET;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_RATE_BUCKET BEFORE UPDATE ON OPS_RUNTIME_RATE_BUCKET FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_RATE_BUCKET();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_VERSION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_VERSION ON OPS_RUNTIME_VERSION;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_VERSION BEFORE UPDATE ON OPS_RUNTIME_VERSION FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_VERSION();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SCHEMA_INSTALLATION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SCHEMA_INSTALLATION ON OPS_SCHEMA_INSTALLATION;
CREATE TRIGGER trg_cpf_touch_OPS_SCHEMA_INSTALLATION BEFORE UPDATE ON OPS_SCHEMA_INSTALLATION FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SCHEMA_INSTALLATION();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SERVICE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SERVICE ON OPS_SERVICE;
CREATE TRIGGER trg_cpf_touch_OPS_SERVICE BEFORE UPDATE ON OPS_SERVICE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SERVICE();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SYSTEM_REGISTRY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SYSTEM_REGISTRY ON OPS_SYSTEM_REGISTRY;
CREATE TRIGGER trg_cpf_touch_OPS_SYSTEM_REGISTRY BEFORE UPDATE ON OPS_SYSTEM_REGISTRY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SYSTEM_REGISTRY();

CREATE TABLE OPS_TRANSACTION_SUBJECT (
    transaction_id VARCHAR(128) NOT NULL,
    subject_role VARCHAR(32) NOT NULL DEFAULT 'ACTOR',
    subject_type VARCHAR(32) NOT NULL,
    subject_search_key VARCHAR(64) NOT NULL,
    subject_masked_value VARCHAR(256) NOT NULL,
    source_type VARCHAR(40) NOT NULL,
    trust_level VARCHAR(20) NOT NULL DEFAULT 'CLAIMED',
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_TRANSACTION_SUBJECT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_TRANSACTION_SUBJECT ON OPS_TRANSACTION_SUBJECT;
CREATE TRIGGER trg_cpf_touch_OPS_TRANSACTION_SUBJECT BEFORE UPDATE ON OPS_TRANSACTION_SUBJECT FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_TRANSACTION_SUBJECT();
...
CREATE OR REPLACE FUNCTION cpf_touch_SEC_JWT_KEY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_SEC_JWT_KEY ON SEC_JWT_KEY;
CREATE TRIGGER trg_cpf_touch_SEC_JWT_KEY BEFORE UPDATE ON SEC_JWT_KEY FOR EACH ROW EXECUTE FUNCTION cpf_touch_SEC_JWT_KEY();
...
CREATE OR REPLACE FUNCTION cpf_touch_SEC_TOKEN_AUDIT_LOG() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_SEC_TOKEN_AUDIT_LOG ON SEC_TOKEN_AUDIT_LOG;
CREATE TRIGGER trg_cpf_touch_SEC_TOKEN_AUDIT_LOG BEFORE UPDATE ON SEC_TOKEN_AUDIT_LOG FOR EACH ROW EXECUTE FUNCTION cpf_touch_SEC_TOKEN_AUDIT_LOG();
...
CREATE OR REPLACE FUNCTION cpf_touch_BAT_EXECUTION_LINK() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_BAT_EXECUTION_LINK ON BAT_EXECUTION_LINK;
CREATE TRIGGER trg_cpf_touch_BAT_EXECUTION_LINK BEFORE UPDATE ON BAT_EXECUTION_LINK FOR EACH ROW EXECUTE FUNCTION cpf_touch_BAT_EXECUTION_LINK();
...
CREATE TABLE GW_BINDING (
    ...
    updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
    row_version BIGINT NOT NULL DEFAULT 1,
    binding_key_hash CHAR(64) NOT NULL,
...
CREATE OR REPLACE FUNCTION cpf_touch_GW_BINDING() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_GW_BINDING ON GW_BINDING;
CREATE TRIGGER trg_cpf_touch_GW_BINDING BEFORE UPDATE ON GW_BINDING FOR EACH ROW EXECUTE FUNCTION cpf_touch_GW_BINDING();
...
CREATE TABLE GW_RATE_LIMIT_REQUEST (
    ...
    limiting_index INTEGER NOT NULL DEFAULT -1,
...
CREATE OR REPLACE FUNCTION cpf_touch_GW_SERVER_GROUP_MEMBER() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_GW_SERVER_GROUP_MEMBER ON GW_SERVER_GROUP_MEMBER;
CREATE TRIGGER trg_cpf_touch_GW_SERVER_GROUP_MEMBER BEFORE UPDATE ON GW_SERVER_GROUP_MEMBER FOR EACH ROW EXECUTE FUNCTION cpf_touch_GW_SERVER_GROUP_MEMBER();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_LOG_POLICY_OVERRIDE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_LOG_POLICY_OVERRIDE ON OPS_LOG_POLICY_OVERRIDE;
CREATE TRIGGER trg_cpf_touch_OPS_LOG_POLICY_OVERRIDE BEFORE UPDATE ON OPS_LOG_POLICY_OVERRIDE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_LOG_POLICY_OVERRIDE();
...
CREATE TABLE OPS_OPERATION_CATALOG (
    ...
    openapi_operation_id VARCHAR(160),
    source_fingerprint VARCHAR(64),
    discovery_status VARCHAR(30) NOT NULL DEFAULT 'ACTIVE',
    first_seen_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    last_seen_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,
    last_instance_id VARCHAR(160),
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_OPERATION_CATALOG() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_OPERATION_CATALOG ON OPS_OPERATION_CATALOG;
CREATE TRIGGER trg_cpf_touch_OPS_OPERATION_CATALOG BEFORE UPDATE ON OPS_OPERATION_CATALOG FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_OPERATION_CATALOG();
...
CREATE TABLE OPS_RETENTION_RUN (
    ...
    compressed_count BIGINT NOT NULL DEFAULT 0,
    freed_bytes BIGINT NOT NULL DEFAULT 0,
    pause_requested_yn CHAR(1) NOT NULL DEFAULT 'N',
    control_actor_id VARCHAR(100),
    control_reason VARCHAR(500),
    error_code VARCHAR(100),
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_CHANGE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_CHANGE ON OPS_RUNTIME_CHANGE;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_CHANGE BEFORE UPDATE ON OPS_RUNTIME_CHANGE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_CHANGE();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_POLICY_DELIVERY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_POLICY_DELIVERY ON OPS_RUNTIME_POLICY_DELIVERY;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_POLICY_DELIVERY BEFORE UPDATE ON OPS_RUNTIME_POLICY_DELIVERY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_POLICY_DELIVERY();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SERVICE_ENDPOINT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SERVICE_ENDPOINT ON OPS_SERVICE_ENDPOINT;
CREATE TRIGGER trg_cpf_touch_OPS_SERVICE_ENDPOINT BEFORE UPDATE ON OPS_SERVICE_ENDPOINT FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SERVICE_ENDPOINT();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SYSTEM_DOMAIN_ACCESS() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SYSTEM_DOMAIN_ACCESS ON OPS_SYSTEM_DOMAIN_ACCESS;
CREATE TRIGGER trg_cpf_touch_OPS_SYSTEM_DOMAIN_ACCESS BEFORE UPDATE ON OPS_SYSTEM_DOMAIN_ACCESS FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SYSTEM_DOMAIN_ACCESS();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_LOG_POLICY_AUDIT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_LOG_POLICY_AUDIT ON OPS_LOG_POLICY_AUDIT;
CREATE TRIGGER trg_cpf_touch_OPS_LOG_POLICY_AUDIT BEFORE UPDATE ON OPS_LOG_POLICY_AUDIT FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_LOG_POLICY_AUDIT();
...
CREATE TABLE OPS_OPERATION_CALLER_POLICY (
    ...
    allowed_yn CHAR(1) NOT NULL DEFAULT 'N',
    policy_version BIGINT NOT NULL DEFAULT 1,
    seed_source VARCHAR(80),
    seed_revision VARCHAR(120),
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_OPERATION_CALLER_POLICY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_OPERATION_CALLER_POLICY ON OPS_OPERATION_CALLER_POLICY;
CREATE TRIGGER trg_cpf_touch_OPS_OPERATION_CALLER_POLICY BEFORE UPDATE ON OPS_OPERATION_CALLER_POLICY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_OPERATION_CALLER_POLICY();
...
CREATE TABLE OPS_OPERATION_DISCOVERY_INSTANCE (
    ...
    artifact_commit VARCHAR(120),
    discovered_yn CHAR(1) NOT NULL DEFAULT 'Y',
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_OPERATION_DISCOVERY_INSTANCE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_OPERATION_DISCOVERY_INSTANCE ON OPS_OPERATION_DISCOVERY_INSTANCE;
CREATE TRIGGER trg_cpf_touch_OPS_OPERATION_DISCOVERY_INSTANCE BEFORE UPDATE ON OPS_OPERATION_DISCOVERY_INSTANCE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_OPERATION_DISCOVERY_INSTANCE();

CREATE TABLE OPS_OPERATION_POLICY (
    operation_id VARCHAR(160) NOT NULL,
    enabled_yn CHAR(1) NOT NULL DEFAULT 'Y',
    all_callers_yn CHAR(1) NOT NULL DEFAULT 'N',
    channel_policy_required_yn CHAR(1) NOT NULL DEFAULT 'N',
    policy_version BIGINT NOT NULL DEFAULT 1,
    seed_source VARCHAR(80),
    seed_revision VARCHAR(120),
    seeded_at TIMESTAMP(3),
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_OPERATION_POLICY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_OPERATION_POLICY ON OPS_OPERATION_POLICY;
CREATE TRIGGER trg_cpf_touch_OPS_OPERATION_POLICY BEFORE UPDATE ON OPS_OPERATION_POLICY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_OPERATION_POLICY();
...
CREATE TABLE OPS_SERVICE_INSTANCE (
    ...
    application_role VARCHAR(80),
    runtime_hostname VARCHAR(200),
    process_id VARCHAR(80),
    java_version VARCHAR(80),
    cpf_version VARCHAR(80),
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SERVICE_INSTANCE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SERVICE_INSTANCE ON OPS_SERVICE_INSTANCE;
CREATE TRIGGER trg_cpf_touch_OPS_SERVICE_INSTANCE BEFORE UPDATE ON OPS_SERVICE_INSTANCE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SERVICE_INSTANCE();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SERVICE_ROUTING_POLICY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SERVICE_ROUTING_POLICY ON OPS_SERVICE_ROUTING_POLICY;
CREATE TRIGGER trg_cpf_touch_OPS_SERVICE_ROUTING_POLICY BEFORE UPDATE ON OPS_SERVICE_ROUTING_POLICY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SERVICE_ROUTING_POLICY();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_DELIVERY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_DELIVERY ON OPS_RUNTIME_DELIVERY;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_DELIVERY BEFORE UPDATE ON OPS_RUNTIME_DELIVERY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_DELIVERY();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_GROUP_MEMBER() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_GROUP_MEMBER ON OPS_RUNTIME_GROUP_MEMBER;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_GROUP_MEMBER BEFORE UPDATE ON OPS_RUNTIME_GROUP_MEMBER FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_GROUP_MEMBER();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_INSTANCE_FEATURE_STATE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_INSTANCE_FEATURE_STATE ON OPS_RUNTIME_INSTANCE_FEATURE_STATE;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_INSTANCE_FEATURE_STATE BEFORE UPDATE ON OPS_RUNTIME_INSTANCE_FEATURE_STATE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_INSTANCE_FEATURE_STATE();
...
CREATE TABLE OPS_RUNTIME_INSTANCE_STATE (
    ...
    desired_runtime_state VARCHAR(32) NOT NULL DEFAULT 'RUNNING',
    actual_runtime_state VARCHAR(32) NOT NULL DEFAULT 'UNKNOWN',
    control_row_version BIGINT NOT NULL DEFAULT 0,
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_RUNTIME_INSTANCE_STATE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_RUNTIME_INSTANCE_STATE ON OPS_RUNTIME_INSTANCE_STATE;
CREATE TRIGGER trg_cpf_touch_OPS_RUNTIME_INSTANCE_STATE BEFORE UPDATE ON OPS_RUNTIME_INSTANCE_STATE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_RUNTIME_INSTANCE_STATE();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SERVICE_CALL_HISTORY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SERVICE_CALL_HISTORY ON OPS_SERVICE_CALL_HISTORY;
CREATE TRIGGER trg_cpf_touch_OPS_SERVICE_CALL_HISTORY BEFORE UPDATE ON OPS_SERVICE_CALL_HISTORY FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SERVICE_CALL_HISTORY();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SERVICE_CIRCUIT_STATE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SERVICE_CIRCUIT_STATE ON OPS_SERVICE_CIRCUIT_STATE;
CREATE TRIGGER trg_cpf_touch_OPS_SERVICE_CIRCUIT_STATE BEFORE UPDATE ON OPS_SERVICE_CIRCUIT_STATE FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SERVICE_CIRCUIT_STATE();
...
CREATE OR REPLACE FUNCTION cpf_touch_OPS_SERVICE_HEALTH_STATUS() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_OPS_SERVICE_HEALTH_STATUS ON OPS_SERVICE_HEALTH_STATUS;
CREATE TRIGGER trg_cpf_touch_OPS_SERVICE_HEALTH_STATUS BEFORE UPDATE ON OPS_SERVICE_HEALTH_STATUS FOR EACH ROW EXECUTE FUNCTION cpf_touch_OPS_SERVICE_HEALTH_STATUS();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_ADMIN_USER() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_ADMIN_USER ON MBW_ADMIN_USER;
CREATE TRIGGER trg_cpf_touch_MBW_ADMIN_USER BEFORE UPDATE ON MBW_ADMIN_USER FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_ADMIN_USER();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_POLICY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_POLICY ON MBW_APPROVAL_POLICY;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_POLICY BEFORE UPDATE ON MBW_APPROVAL_POLICY FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_POLICY();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_ATTACHMENT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_ATTACHMENT ON MBW_ATTACHMENT;
CREATE TRIGGER trg_cpf_touch_MBW_ATTACHMENT BEFORE UPDATE ON MBW_ATTACHMENT FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_ATTACHMENT();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_AUDIT_CHAIN_LOCK() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_AUDIT_CHAIN_LOCK ON MBW_AUDIT_CHAIN_LOCK;
CREATE TRIGGER trg_cpf_touch_MBW_AUDIT_CHAIN_LOCK BEFORE UPDATE ON MBW_AUDIT_CHAIN_LOCK FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_AUDIT_CHAIN_LOCK();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_BUSINESS_AUDIT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_BUSINESS_AUDIT ON MBW_BUSINESS_AUDIT;
CREATE TRIGGER trg_cpf_touch_MBW_BUSINESS_AUDIT BEFORE UPDATE ON MBW_BUSINESS_AUDIT FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_BUSINESS_AUDIT();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_DOWNLOAD_AUDIT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_DOWNLOAD_AUDIT ON MBW_DOWNLOAD_AUDIT;
CREATE TRIGGER trg_cpf_touch_MBW_DOWNLOAD_AUDIT BEFORE UPDATE ON MBW_DOWNLOAD_AUDIT FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_DOWNLOAD_AUDIT();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_JOB_TITLE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_JOB_TITLE ON MBW_JOB_TITLE;
CREATE TRIGGER trg_cpf_touch_MBW_JOB_TITLE BEFORE UPDATE ON MBW_JOB_TITLE FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_JOB_TITLE();
...
CREATE TABLE MBW_MENU (
    ...
    module_code VARCHAR(20) NOT NULL DEFAULT 'MBW',
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_MENU() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_MENU ON MBW_MENU;
CREATE TRIGGER trg_cpf_touch_MBW_MENU BEFORE UPDATE ON MBW_MENU FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_MENU();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_NOTIFICATION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_NOTIFICATION ON MBW_NOTIFICATION;
CREATE TRIGGER trg_cpf_touch_MBW_NOTIFICATION BEFORE UPDATE ON MBW_NOTIFICATION FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_NOTIFICATION();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_ORGANIZATION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_ORGANIZATION ON MBW_ORGANIZATION;
CREATE TRIGGER trg_cpf_touch_MBW_ORGANIZATION BEFORE UPDATE ON MBW_ORGANIZATION FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_ORGANIZATION();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_PERMISSION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_PERMISSION ON MBW_PERMISSION;
CREATE TRIGGER trg_cpf_touch_MBW_PERMISSION BEFORE UPDATE ON MBW_PERMISSION FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_PERMISSION();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_POSITION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_POSITION ON MBW_POSITION;
CREATE TRIGGER trg_cpf_touch_MBW_POSITION BEFORE UPDATE ON MBW_POSITION FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_POSITION();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_PROJECT_SETTING() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_PROJECT_SETTING ON MBW_PROJECT_SETTING;
CREATE TRIGGER trg_cpf_touch_MBW_PROJECT_SETTING BEFORE UPDATE ON MBW_PROJECT_SETTING FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_PROJECT_SETTING();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_ROLE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_ROLE ON MBW_ROLE;
CREATE TRIGGER trg_cpf_touch_MBW_ROLE BEFORE UPDATE ON MBW_ROLE FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_ROLE();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_SAVED_SEARCH() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_SAVED_SEARCH ON MBW_SAVED_SEARCH;
CREATE TRIGGER trg_cpf_touch_MBW_SAVED_SEARCH BEFORE UPDATE ON MBW_SAVED_SEARCH FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_SAVED_SEARCH();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_DOCUMENT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_DOCUMENT ON MBW_APPROVAL_DOCUMENT;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_DOCUMENT BEFORE UPDATE ON MBW_APPROVAL_DOCUMENT FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_DOCUMENT();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_POLICY_STEP() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_POLICY_STEP ON MBW_APPROVAL_POLICY_STEP;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_POLICY_STEP BEFORE UPDATE ON MBW_APPROVAL_POLICY_STEP FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_POLICY_STEP();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_EMPLOYEE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_EMPLOYEE ON MBW_EMPLOYEE;
CREATE TRIGGER trg_cpf_touch_MBW_EMPLOYEE BEFORE UPDATE ON MBW_EMPLOYEE FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_EMPLOYEE();

CREATE TABLE MBW_LOGIN_HISTORY (
    login_history_id BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    admin_user_id BIGINT,
    login_domain VARCHAR(30) NOT NULL DEFAULT 'MBW',
    admin_login_id VARCHAR(80) NOT NULL,
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_LOGIN_HISTORY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_LOGIN_HISTORY ON MBW_LOGIN_HISTORY;
CREATE TRIGGER trg_cpf_touch_MBW_LOGIN_HISTORY BEFORE UPDATE ON MBW_LOGIN_HISTORY FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_LOGIN_HISTORY();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_LOGIN_OPERATION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_LOGIN_OPERATION ON MBW_LOGIN_OPERATION;
CREATE TRIGGER trg_cpf_touch_MBW_LOGIN_OPERATION BEFORE UPDATE ON MBW_LOGIN_OPERATION FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_LOGIN_OPERATION();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_REFRESH_TOKEN() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_REFRESH_TOKEN ON MBW_REFRESH_TOKEN;
CREATE TRIGGER trg_cpf_touch_MBW_REFRESH_TOKEN BEFORE UPDATE ON MBW_REFRESH_TOKEN FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_REFRESH_TOKEN();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_USER_ROLE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_USER_ROLE ON MBW_USER_ROLE;
CREATE TRIGGER trg_cpf_touch_MBW_USER_ROLE BEFORE UPDATE ON MBW_USER_ROLE FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_USER_ROLE();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_DELEGATION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_DELEGATION ON MBW_APPROVAL_DELEGATION;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_DELEGATION BEFORE UPDATE ON MBW_APPROVAL_DELEGATION FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_DELEGATION();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_EXECUTION() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_EXECUTION ON MBW_APPROVAL_EXECUTION;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_EXECUTION BEFORE UPDATE ON MBW_APPROVAL_EXECUTION FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_EXECUTION();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_HISTORY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_HISTORY ON MBW_APPROVAL_HISTORY;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_HISTORY BEFORE UPDATE ON MBW_APPROVAL_HISTORY FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_HISTORY();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_LINE() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_LINE ON MBW_APPROVAL_LINE;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_LINE BEFORE UPDATE ON MBW_APPROVAL_LINE FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_LINE();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_EMPLOYEE_ASSIGNMENT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_EMPLOYEE_ASSIGNMENT ON MBW_EMPLOYEE_ASSIGNMENT;
CREATE TRIGGER trg_cpf_touch_MBW_EMPLOYEE_ASSIGNMENT BEFORE UPDATE ON MBW_EMPLOYEE_ASSIGNMENT FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_EMPLOYEE_ASSIGNMENT();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_ORGANIZATION_RESPONSIBILITY() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_ORGANIZATION_RESPONSIBILITY ON MBW_ORGANIZATION_RESPONSIBILITY;
CREATE TRIGGER trg_cpf_touch_MBW_ORGANIZATION_RESPONSIBILITY BEFORE UPDATE ON MBW_ORGANIZATION_RESPONSIBILITY FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_ORGANIZATION_RESPONSIBILITY();
...
CREATE OR REPLACE FUNCTION cpf_touch_MBW_APPROVAL_PARTICIPANT() RETURNS trigger AS $$ BEGIN NEW.updated_at = CURRENT_TIMESTAMP; RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS trg_cpf_touch_MBW_APPROVAL_PARTICIPANT ON MBW_APPROVAL_PARTICIPANT;
CREATE TRIGGER trg_cpf_touch_MBW_APPROVAL_PARTICIPANT BEFORE UPDATE ON MBW_APPROVAL_PARTICIPANT FOR EACH ROW EXECUTE FUNCTION cpf_touch_MBW_APPROVAL_PARTICIPANT();
