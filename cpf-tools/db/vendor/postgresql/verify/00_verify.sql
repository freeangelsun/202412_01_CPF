...
SELECT 'cpfDB.table_count' AS check_name,
       CASE WHEN COUNT(*) = 205 THEN 1 ELSE 0 END AS passed
...
SELECT 'bat_spring_batch_6_sequence_contract' AS check_name,
       CASE WHEN
           ...
           (SELECT COUNT(*) FROM information_schema.sequences
             WHERE sequence_schema = current_schema()
               AND UPPER(sequence_name) IN ('BAT_SB_JOB_INSTANCE_SEQ', 'BAT_SB_JOB_EXECUTION_SEQ', 'BAT_SB_STEP_EXECUTION_SEQ')) = 3
           AND
           (SELECT COUNT(*) FROM information_schema.sequences
             WHERE sequence_schema = current_schema()
               AND UPPER(sequence_name) IN ('BATCH_JOB_EXECUTION_SEQ', 'BATCH_JOB_INSTANCE_SEQ', 'BATCH_JOB_SEQ', 'BATCH_STEP_EXECUTION_SEQ')) = 0
       THEN 1 ELSE 0 END AS passed;
