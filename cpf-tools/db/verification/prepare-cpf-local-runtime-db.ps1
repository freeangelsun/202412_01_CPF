...
$platformDb="cpf_verify_${VerifierRunId}_runtime";$bzaDb="cpf_verify_${VerifierRunId}_bza"
$platformMig="cpfv_${VerifierRunId}_pm";$platformRun="cpfv_${VerifierRunId}_pr";$bzaMig="cpfv_${VerifierRunId}_bm";$bzaRun="cpfv_${VerifierRunId}_br"
foreach($prop in @($profile.modules.PSObject.Properties)){
    ...
    $m.databaseName=if($isBza){$bzaDb}else{$platformDb};$m.schemaName=$m.databaseName
    $m.admin.username='root';$m.admin.userHost='%';$m.admin.password=[pscustomobject]@{env='CPF_ADMIN_PASSWORD'}
    $m.migration.username=if($isBza){$bzaMig}else{$platformMig};$m.migration.userHost='%';$m.migration.password=[pscustomobject]@{env='CPF_LOCAL_RUNTIME_DB_MIGRATION_PASSWORD'}
    $m.runtime.username=if($isBza){$bzaRun}else{$platformRun};$m.runtime.userHost='%';$m.runtime.password=[pscustomobject]@{env='CPF_LOCAL_RUNTIME_DB_PASSWORD'}
...
$result=[ordered]@{schemaVersion=1;runId=$VerifierRunId;vendor='mariadb';host='127.0.0.1';port=3306;platformDatabase=$platformDb;platformRuntimeUser=$platformRun;bzaDatabase=$bzaDb;bzaRuntimeUser=$bzaRun;profile='profile.json';status='PASS';sanitized=$true}
