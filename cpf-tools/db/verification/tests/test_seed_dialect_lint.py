...
P=ROOT/'cpf-tools/db/verification/verify-cpf-seed-dialect.py'
...
class SeedDialectLintTest(unittest.TestCase):
 def test_existing_official_vendor_product_and_test_bundles_pass(self):
  for v in M.OFFICIAL:
   paths=M.seed_paths(ROOT,v)
   self.assertTrue(paths)
   for path in paths:
    self.assertTrue(path.is_file(),path)
    self.assertEqual([],M.lint(v,path.read_text(encoding='utf-8-sig')),f'{v}:{path}')

 def test_runtime_query_fragment_omits_client_terminator_but_remains_dialect_checked(self):
  self.assertEqual([],M.lint('mariadb',"SELECT * FROM CPF_SAMPLE",require_terminal=False))
  self.assertTrue(M.lint('mariadb',"SELECT * FROM t ON CONFLICT DO NOTHING",require_terminal=False))
