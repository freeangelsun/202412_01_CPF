...
function Invoke-Migration([string]$Vendor, [string]$Profile, [string]$Direction, [int]$Version) {
    ...
    Invoke-Step $Vendor "$Direction-v$Version-dry-run" {
        & (Join-Path $rootPath 'cpf-tools/db/tools/invoke-platform-database-migration.ps1') `
...
Invoke-Step 'all' 'current-migration-checksums' { & (Join-Path $rootPath 'cpf-tools/db/verification/check-migration-checksums.ps1') -Root $rootPath }
Invoke-Step 'all' 'canonical-db-lifecycle-contract' { & (Join-Path $rootPath 'cpf-tools/db/verification/check-canonical-db-lifecycle-contract.ps1') -Root $rootPath }
Invoke-Step 'all' 'vendor-static-token-parity' { & python (Join-Path $rootPath 'cpf-tools/db/verification/verify-cpf-db-vendor-static-token-parity.py') --root $rootPath --json-report (Join-Path $out 'static-token-parity.json') }

foreach ($vendor in $profiles.Keys) {
    $profile = $profiles[$vendor]; $upgradeProfile = $upgradeProfiles[$vendor]
    $baselineInit = Join-Path $baselineRoot 'cpf-tools/db/tools/initialize-cpf-database.ps1'
    ...
    Invoke-Step $vendor 'runtime-query-contract' {
        if ($vendor -eq 'mariadb') {
            & (Join-Path $rootPath 'cpf-tools/runtime/tools/smoke-platform-runtime-query-packs-mariadb.ps1') -Root $rootPath -ProfilePath $profile -EvidencePath (Join-Path $out "$vendor-runtime-query.json")
        } else {
            & (Join-Path $rootPath 'cpf-tools/db/verification/smoke-platform-runtime-query-packs-official-db.ps1') -Root $rootPath -ProfilePath $profile -Vendor $vendor -EvidencePath (Join-Path $out "$vendor-runtime-query.json")
        }
    }
    Invoke-Step $vendor 'schema-drift' { & (Join-Path $rootPath 'cpf-tools/db/verification/check-database-schema-drift.ps1') -Root $rootPath }
    ...
    Invoke-Step $vendor 'forward-reapply-runtime-query' {
        if ($vendor -eq 'mariadb') {
            & (Join-Path $rootPath 'cpf-tools/runtime/tools/smoke-platform-runtime-query-packs-mariadb.ps1') -Root $rootPath -ProfilePath $profile -EvidencePath (Join-Path $out "$vendor-reapply-runtime-query.json")
        } else {
            & (Join-Path $rootPath 'cpf-tools/db/verification/smoke-platform-runtime-query-packs-official-db.ps1') -Root $rootPath -ProfilePath $profile -Vendor $vendor -EvidencePath (Join-Path $out "$vendor-reapply-runtime-query.json")
        }
    }
}
