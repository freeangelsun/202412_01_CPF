...
def validate(root: Path, metadata_only: bool) -> dict:
    ...
    for vendor in OFFICIAL:
        ...
        required_paths = {
            ...
            "generatedCurrent": entry.get("generatedCurrent"),
            "generatedDomainTemplate": entry.get("generatedDomainTemplate"),
            "historicalMigration": entry.get("historicalMigration"),
            "historicalRollback": entry.get("historicalRollback"),
            "runtimeDialectRoot": entry.get("runtimeDialectRoot"),
        }
        ...
        for label, raw in required_paths.items():
            ...
            if label in {"domainTemplateRoot", "generatedDomainTemplate"}:
                expected_prefix = PurePosixPath("cpf-tools", "db", "generated", "domain-template", vendor)
            elif label == "generatedCurrent":
                expected_prefix = PurePosixPath("cpf-tools", "db", "generated", "current", vendor)
            elif label == "runtimeDialectRoot":
                expected_prefix = PurePosixPath("cpf-starters", "data", "persistence", "src", "main", "resources", "cpf-generated-domain-dialect", vendor)
            else:
                expected_prefix = PurePosixPath("cpf-tools", "db", "vendor", vendor)
            if tuple(prefix.parts[:len(expected_prefix.parts)]) != tuple(expected_prefix.parts):
            ...
            if not metadata_only:
                if label in {"vendorRoot", "runtimeRoot", "domainTemplateRoot", "historicalMigration", "historicalRollback", "runtimeDialectRoot"}:
