...
& pwsh -NoProfile -File (Join-Path $cpfRoot "cpf-env.ps1") -Action up -Target external
...
'@.Replace("__SFTP_USER__", $sftpUser)
    ...
    if ($StopAfter) {
        & pwsh -NoProfile -File (Join-Path $cpfRoot "cpf-env.ps1") -Action stop -Target external
    }
}
