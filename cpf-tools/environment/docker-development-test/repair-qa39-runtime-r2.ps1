...
$verify=@("-NoProfile","-File",(Join-Path $cpfRoot "verify-qa39-runtime.ps1"),"-DockerRoot",$DockerRoot,"-RepoRoot",$RepoRoot,"-RequireStopped");if($IncludeIbmMq){$verify+="-IncludeIbmMq"};& pwsh @verify;if($LASTEXITCODE -ne 0){throw "수정 후 정지 상태 검증 실패(exit=$LASTEXITCODE)"}
