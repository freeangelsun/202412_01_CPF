...
while (($#)); do
  case "$1" in
    ...
    --definition) args+=(-DefinitionPath "$2"); shift 2 ;;
    --database-vendor) args+=(-DatabaseVendor "$2"); shift 2 ;;
    --profile) args+=(-ProfilePath "$2"); shift 2 ;;
...
exec pwsh -NoProfile -File "$script_dir/../../cpf-tools/generator/tools/initialize-domain-database.ps1" "${args[@]}"
