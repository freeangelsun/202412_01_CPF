...
# Nested Starter layout(data/*, profiles/*, security/* ...) 전체를 실제 Gradle project 기준으로 탐색합니다.
$starterModules = @(Get-ChildItem -LiteralPath $starterRoot -Recurse -File -Filter 'build.gradle' |
    Sort-Object FullName |
    ForEach-Object {
        $projectDir = $_.Directory.FullName
        $relative = $projectDir.Substring($Root.Length + 1).Replace("\", "/")
        if ($relative -notmatch '^cpf-starters/.+') {
            throw "Starter project escaped canonical root: $relative"
        }
        $relative
...
function Get-StructuralPathRule {
    ...
    if ($path -match '^cpf-education/src/(main|test)/java/com/cpf/education/(controller|dto|service|repository|mapper|facade|operation)/') {
        return 'EDU_CAPABILITY_SLICE_REQUIRED'
    }
    ...
    foreach ($file in (Get-JavaFiles $module)) {
        ...
        if ($businessModules -contains $module) {
            ...
            if (Test-Text $text "https?://") {
                if ($relativePath -match "^cpf-education/src/main/java/com/cpf/education/" -and $text -match "CPF-ARCH-ALLOW-DIRECT-URL:\s*EDU_ONLY") {
                    continue
                }
...
$regressionFixtures = @(
    @{ path = 'cpf-education/src/main/java/com/cpf/education/controller/BadEducationCrudController.java'; rule = 'EDU_CAPABILITY_SLICE_REQUIRED' },
    ...
    @{ path = 'cpf-batch/control-plane/src/test/java/com/cpf/batch/control/edu/BadControlEducationSample.java'; rule = 'BAT_RUNTIME_NO_EDU_SOURCE' }
)
