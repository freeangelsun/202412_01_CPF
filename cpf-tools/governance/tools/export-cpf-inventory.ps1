...
function Get-Owner {
    ...
    if (@("CPF", "CMN", "MBR", "EDU", "ADM", "BZA", "BAT", "ACC", "EXS", "GWY") -contains $first) {
        return $first
    }
    return "ROOT"
}
...
$requestSha = (Get-FileHash -LiteralPath (Join-Path $Root "cpf-docs/governance/development-harness/current/CPF_CURRENT_WORK_REQUEST.md") -Algorithm SHA256).Hash.ToLowerInvariant()
$inventory = [ordered]@{
