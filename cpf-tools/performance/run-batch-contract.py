#!/usr/bin/env python3
"""Live batch semantic probe for launch -> observe -> restart/reconcile contract."""
from __future__ import annotations
import json, os, sys, urllib.request, uuid
from urllib.parse import urlparse

def fail(msg: str) -> int:
    print(json.dumps({'status':'FAIL','reason':msg},ensure_ascii=False)); return 2

def main() -> int:
    url=os.environ.get('CPF_PERF_BATCH_PROBE_URL','').strip()
    if not url: return fail('CPF_PERF_BATCH_PROBE_URL is required')
    parsed=urlparse(url)
    if parsed.scheme not in {'http','https'} or not parsed.hostname: return fail('batch probe URL must be http/https')
    if parsed.scheme!='https' and parsed.hostname not in {'127.0.0.1','localhost','::1'}: return fail('non-local batch probe must use https')
    request_id=str(uuid.uuid4())
    body=json.dumps({'requestId':request_id,'scenario':'launch-observe-restart-reconcile'}).encode()
    ...
    try:
        with urllib.request.urlopen(req,timeout=float(os.environ.get('CPF_PERF_BATCH_TIMEOUT_SECONDS','120'))) as res:
    ...
    required=('launched','observed','restartSafe','reconciled')
    ...
    print(json.dumps({'status':'PASS','requestId':request_id,'checks':list(required)},ensure_ascii=False)); return 0
