...
class WorkPackageSourceReviewTest(unittest.TestCase):
    def test_positive_exact_file_and_gate_mapping(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            (source / "cpf-tools/verification/tools").mkdir(parents=True, exist_ok=True)
            (source / "cpf-tools/verification/tools/verify-cpf-frontend-consumer-closure.py").write_text("print('ok')", encoding="utf-8")
            (source / "cpf-tools/verification/tools/verify-cpf-operator-trust-boundary.py").write_text("print('ok')", encoding="utf-8")
