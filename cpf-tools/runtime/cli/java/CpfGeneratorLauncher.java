...
public final class CpfGeneratorLauncher {
    ...
    /** Java 기반 CPF CLI에서 동일 Generator client를 직접 호출하기 위한 canonical entrypoint. */
    public static int run(String[] args) throws Exception {
        ...
        ProcessBuilder pb=new ProcessBuilder(cmd).directory(root.toFile()).inheritIO();Process p=pb.start();if(!p.waitFor(1800,TimeUnit.SECONDS)){p.destroyForcibly();throw new IllegalStateException("Generator timeout");}
        return p.exitValue();
    }
