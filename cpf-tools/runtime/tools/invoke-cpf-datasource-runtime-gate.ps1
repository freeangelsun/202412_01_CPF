...
$args = @(
    'cpf-tools/runtime/tools/verify-cpf-datasource-runtime-evidence.py',
...
if([string]::IsNullOrWhiteSpace($OutputPath)){$OutputPath=Join-Path $root ("build/reports/cpf-db/datasource-runtime-$Vendor.sanitized.json")}elseif(-not[IO.Path]::IsPathRooted($OutputPath)){$OutputPath=Join-Path $root $OutputPath}
