...
try {
    ...
    $result.centerCutApi.hasTransactionId = @($targets | Where-Object { $_.transactionId }).Count -gt 0
    $result.centerCutApi.hasTransactionSegmentId = @($targets | Where-Object { $_.transactionSegmentId }).Count -gt 0
