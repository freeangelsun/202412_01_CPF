...
function Invoke-AdmBootJarBuild {
    ...
    $result.process.buildBeforeRun = [ordered]@{
        requested = $true
        command = ".\gradlew.bat :apps:admin:bootJar --offline --no-daemon --console=plain"
        log = $buildLog
    }

    $buildOutput = & $gradle ":apps:admin:bootJar" "--offline" "--no-daemon" "--console=plain" 2>&1
...
try {
    if ($BuildBeforeRun) {
        ...
        $result.process.buildBeforeRun = [ordered]@{
            requested = $false
            reason = "Pass -BuildBeforeRun to build :apps:admin:bootJar before starting ADM smoke."
        }
    }
    ...
    if ($null -ne $initialHealth) {
        ...
        if ($null -ne $bootJar) {
            ...
            $startedProcess = Start-Process `
                -FilePath $gradle `
                -ArgumentList @("--offline", ":apps:admin:bootRun") `
                -WorkingDirectory $Root `
                -RedirectStandardOutput $stdoutLog `
                -RedirectStandardError $stderrLog `
                -WindowStyle Hidden `
    ...
    & (Join-Path $Root "cpf-tools/verification/openapi/smoke-openapi.ps1") `
        -Root $Root `
        -Modules @("ADM") `
    ...
    if ($IncludePermissionWriteSmoke) {
        ...
        $result.permissionWriteApi.reason = "Run cpf-tools/runtime/tools/smoke-adm-permission-runtime.ps1 or pass -IncludePermissionWriteSmoke to execute write/filter checks."
    }
    ...
    $result.centerCutApi.hasTransactionId = $hasTransactionId
    $result.centerCutApi.hasTransactionSegmentId = $hasTransactionSegmentId
    $result.centerCutApi.hasFailureReason = $hasFailureReason
    $result.centerCutApi.rawPayloadExposed = $false
    & (Join-Path $Root "cpf-tools/runtime/tools/smoke-adm-center-cut-runtime.ps1") `
        -AdmBaseUrl $AdmBaseUrl `
    ...
    & (Join-Path $Root "cpf-tools/runtime/tools/smoke-transaction-meta-runtime.ps1") `
        -AdmBaseUrl $AdmBaseUrl `
