...
$result=[ordered]@{startedAt=(Get-Date).ToString('o');status='FAILED';clientAdapter=$null;registration=@{};distribution=@{};drain=@{};crashUnknown=@{};mismatch=@{}}
...
function Start-Worker([string]$WorkerId,[string]$InstanceId,[int]$ServerPort){
 ...
 $psi.Environment['BAT_DATABASE_URL']="jdbc:mariadb://${HostName}:${Port}/batDB";$psi.Environment['BAT_DATABASE_USERNAME']=$Username;$psi.Environment['BAT_DATABASE_PASSWORD']=$Password;$psi.Environment['CPF_RUNTIME_INSTANCE_ID']=$InstanceId;$psi.Environment['CPF_WAS_ID']=$InstanceId;$psi.Environment['CPF_BAT_WORKER_ID']=$WorkerId;$psi.Environment['CPF_BAT_WORKER_VERSION']='runtime-smoke-v1';$psi.Environment['CPF_BAT_WORKER_CAPABILITIES']='CPF_BAT_DIAGNOSTIC';$psi.Environment['CPF_BATCH_CONTROL_BASE_URL']='http://127.0.0.1:65534';$psi.Environment['CPF_LOG_ROOT']=(Join-Path $runtimeDir 'logs')
 $p=[Diagnostics.Process]::new();$p.StartInfo=$psi;[void]$p.Start();$processes[$WorkerId]=$p;return $p
}
try{
 if([string]::IsNullOrWhiteSpace($Password)){throw 'CPF_DB_ROOT_PASSWORD or CPF_ADMIN_PASSWORD is required'};$script:MariaClient=Resolve-Client;$result.clientAdapter=$script:MariaClient.mode
 [void](Invoke-Sql @"
USE batDB;
