...
if ([string]::IsNullOrWhiteSpace($ProfilePath)) { $ProfilePath = Join-Path $Root 'cpf-tools/db/config/database-install.default.json' }
...
& (Join-Path $Root 'cpf-tools/db/verification/check-query-contract-integrity.ps1') -Root $Root | Out-Null
