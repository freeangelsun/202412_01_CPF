...
spec=importlib.util.spec_from_file_location('ds',ROOT/'cpf-tools/runtime/tools/verify-cpf-datasource-runtime-evidence.py');mod=importlib.util.module_from_spec(spec);spec.loader.exec_module(mod)
H='a'*64

def fixture():
 return {'vendor':'postgresql','sourceSha':'b'*40,'operationId':'op-1','operator':'maker','approvedBy':'checker','reason':'scheduled datasource runtime validation','sanitized':True,'status':'PASS','startedAt':'2026-08-15T00:00:00Z','endedAt':'2026-08-15T00:00:01Z',
 'routing':{'writeTarget':'PRIMARY','readTarget':'REPLICA','readOnly':True,'consistency':'BOUNDED_STALENESS','replicaLagMs':100,'freshnessEvidence':'replica-lag-probe','decisionAuditId':'route-1'},
 'multiDataSource':{'owner':'cpf-common','resourceCount':1,'writeOperation':False,'crossOwnerWrite':False},
 'connectionPool':{'maxPoolSize':20,'instanceCount':2,'databaseConnectionBudget':100,'reservedConnections':20,'connectionTimeoutMs':3000,'validationTimeoutMs':1000,'maxLifetimeMs':1800000},
...
class Test(unittest.TestCase):
 ...
 def test_consumer_has_sod_hash_and_vendor_guards(self):
  t=(ROOT/'cpf-tools/runtime/tools/invoke-cpf-datasource-runtime-gate.ps1').read_text(encoding='utf-8')
