...
def test_frontend_workflow_harness_targets_adm_and_external_bza_reference_sources():
    root = Path(__file__).resolve().parents[4]
    harness = root / "cpf-tools/verification/frontend-workflow-runtime/harness.cjs"
    text = harness.read_text(encoding="utf-8")
    assert "cpf-biz-frontend/src/features/employees/model/employeeModel.js" in text
...
def test_runtime_launcher_uses_current_reference_harness():
    ...
    assert "cpf-biz-frontend" in text
