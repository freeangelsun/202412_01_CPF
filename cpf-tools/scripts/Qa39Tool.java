...
public final class Qa39Tool {
    ...
    public static void main(String[] args) {
        try {
            ...
            switch (mode) {
                ...
                case "source-closure" -> tool.verifySourceClosure(options.get("evidence-output"), options.get("source-head"));
    ...
    private void verifyBuildContract() throws Exception {
        ...
        Path publicBomPath = root.resolve("cpf-tools/build/platform-bom/public-bom/build.gradle");
        String publicBomBuild = Files.isRegularFile(publicBomPath) ? Files.readString(publicBomPath, UTF8) : bomBuild;
        ...
        if (!(publicBomBuild.contains("artifactId = 'cpf-platform-bom'") || publicBomBuild.contains("artifactId='cpf-platform-bom'"))) fail("BUILD-CONTRACT", "canonical BOM artifact mismatch");
        if (Files.isRegularFile(publicBomPath)) {
            String bomSettings = Files.readString(requireFile(root.resolve("cpf-tools/build/platform-bom/settings.gradle"), "missing split BOM settings"), UTF8);
            if (!bomSettings.contains("include 'public-bom', 'internal-bom'")) fail("BUILD-CONTRACT", "split BOM settings must include public-bom and internal-bom");
            String internalBomBuild = Files.readString(requireFile(root.resolve("cpf-tools/build/platform-bom/internal-bom/build.gradle"), "missing internal BOM build"), UTF8);
            if (!(internalBomBuild.contains("artifactId = 'cpf-internal-platform-bom'") || internalBomBuild.contains("artifactId='cpf-internal-platform-bom'"))) fail("BUILD-CONTRACT", "internal BOM artifact mismatch");
        }
    ...
    private void verifyNetworkIdentity() throws Exception {
        ...
        Map<String,List<String>> required = Map.ofEntries(
            ...
            Map.entry("gateway-client",List.of("NoConnectionReuseStrategy.INSTANCE",".disableRedirectHandling()",".disableAutomaticRetries()")),
        ...
        List<String> failed = new ArrayList<>(); for (Map.Entry<String,List<String>> e:required.entrySet()) for(String t:e.getValue()) {
            String body=b.get(e.getKey());
            if(e.getKey().equals("gateway-resolver")&&t.equals("mixed private/public DNS response denied")) {
                String normalized=body.toLowerCase(Locale.ROOT);
                if(!(normalized.contains("mixed")&&normalized.contains("private/public")
                        && normalized.contains("dns")&&normalized.contains("response denied"))) {
                    failed.add(e.getKey()+":"+t);
                }
            } else if(!body.contains(t)) failed.add(e.getKey()+":"+t);
        }
        String gatewayClient=b.get("gateway-client");
        boolean pinnedDnsResolver=(gatewayClient.contains(".setDnsResolver(CpfGatewayPinnedAddressContext::resolve)")
                || (gatewayClient.contains(".setDnsResolver(new DnsResolver()")
                    && gatewayClient.contains("CpfGatewayPinnedAddressContext.resolve(host)")));
        if(!pinnedDnsResolver)failed.add("gateway-client:pinned DNS resolver");
        if(gatewayClient.contains("SimpleClientHttpRequestFactory"))failed.add("gateway-client:forbidden:SimpleClientHttpRequestFactory");
    ...
    private void verifyRuntimeContracts() throws Exception {
        ...
        for(Map.Entry<String,List<String>>e:checks.entrySet()) {
            String body=Files.readString(requireFile(root.resolve(e.getKey()),"missing: "+e.getKey()),UTF8);
            List<String> required=e.getValue();
            if(e.getKey().endsWith("SchedulerDispatchService.java")) {
                List<String> base=required.stream().filter(t->!t.equals("reconcile")).toList();
                requireTokens(body,"RUNTIME-CONTRACT",e.getKey(),base);
                boolean reconcilePath=body.contains("reconcile") || body.contains("재조정")
                        || (body.contains("dispatchPending()")&&body.contains("scheduler-trigger-mark-unknown"));
                if(!reconcilePath)fail("RUNTIME-CONTRACT",e.getKey()+" missing scheduler reconciliation path");
            } else {
                requireTokens(body,"RUNTIME-CONTRACT",e.getKey(),required);
            }
        }
    ...
    private void verifySourceClosure(String evidenceOutput, String sourceHead) throws Exception {
        String head;
        if(sourceHead!=null&&!sourceHead.isBlank()) {
            head=sourceHead.trim();
            if(!head.matches("[0-9a-f]{40}"))fail("SOURCE-CLOSURE","--source-head must be an exact lowercase 40-char SHA");
        } else {
            head=run("git","-C",root.toString(),"rev-parse","HEAD").trim();
            if(!head.matches("[0-9a-f]{40}"))fail("SOURCE-CLOSURE","exact Git SHA required");
            String dirty=run("git","-C",root.toString(),"status","--porcelain=v1","--untracked-files=all").trim();
            if(!dirty.isBlank())fail("SOURCE-CLOSURE","release source closure requires clean Working Tree");
        }
    ...
    private void verifySupplyChain(Map<String,String> options) throws Exception {
        ...
        Set<String>expectedProjects=Files.isRegularFile(settings)?gradleProjectPaths(Files.readString(settings,UTF8)):Set.of();expectedProjects=expectedProjects.stream().filter(project->!project.startsWith("cpf-tools/verification/")).collect(Collectors.toCollection(LinkedHashSet::new));List<Map<String,Object>>artifacts=catalog.get("artifacts") instanceof List<?>?asMapList(catalog.get("artifacts")):new ArrayList<>();String catalogSchema=s(catalog.get("schemaVersion"));Set<String>supportedCatalogSchemas=Set.of("1","1.0","1.0.0","2.0.0");if(!supportedCatalogSchemas.contains(catalogSchema)||artifacts.isEmpty())failures.add("unsupported or empty final artifact catalog schemaVersion="+catalogSchema);if("2.0.0".equals(catalogSchema)){if(!"CPF-FINAL-ARTIFACT-CATALOG".equals(s(catalog.get("catalogId"))))failures.add("artifact catalog v2 catalogId mismatch");if(!"GIT_HEAD_RUNTIME".equals(s(catalog.get("baselinePolicy")))||!"RUNTIME_GIT_HEAD".equals(s(catalog.get("baselineSha"))))failures.add("artifact catalog v2 must use runtime Git HEAD baseline policy");if(!"cpf-tools/generator/contracts/cpf-starter-catalog.json".equals(s(catalog.get("canonicalStarterCatalog"))))failures.add("artifact catalog v2 canonical starter catalog path mismatch");Object surface=catalog.get("publicSelectionSurface");if(!(surface instanceof Map<?,?>m)||!Boolean.FALSE.equals(m.get("directInternalArtifactSelection")))failures.add("artifact catalog v2 must forbid direct internal artifact selection");}if(!strSet(catalog.get("officialDatabaseVendors")).stream().map(x->x.toLowerCase(Locale.ROOT)).collect(Collectors.toSet()).equals(Set.of("oracle","postgresql","mariadb")))failures.add("artifact catalog official DB vendors must be Oracle/PostgreSQL/MariaDB only");Set<String>ids=new HashSet<>(),owners=new HashSet<>();for(Map<String,Object>a:artifacts){String id=s(a.get("artifactId")),owner=s(a.get("ownerPath")).replaceAll("^/+|/+$",""),pattern=s(a.get("outputPattern"));if(id.isBlank()||!ids.add(id))failures.add("duplicate/blank artifactId: "+id);if(!relativeSafe(owner)||!relativeSafe(pattern))failures.add("unsafe artifact path/pattern: "+id);if(s(a.get("producer")).isBlank()||s(a.get("consumer")).isBlank())failures.add("artifact producer/consumer missing: "+id);if(!(a.get("requiredAttestations") instanceof List<?>att)&&false){}else if(!(a.get("requiredAttestations") instanceof List<?>att)||!att.contains("sha256"))failures.add("artifact sha256 attestation missing: "+id);owners.add(owner);}if(!owners.containsAll(expectedProjects))failures.add("included Gradle projects missing from artifact catalog: "+difference(expectedProjects,owners));Set<String>dbPacks=artifacts.stream().filter(a->"database-pack".equals(a.get("kind"))).map(a->Paths.get(s(a.get("ownerPath"))).getFileName().toString()).collect(Collectors.toSet());if(!dbPacks.equals(Set.of("oracle","postgresql","mariadb")))failures.add("database artifact packs must be exactly Oracle/PostgreSQL/MariaDB: "+dbPacks);validateEnvironment(envTemplate,null,false,failures);
    ...
    private static void validateEnvironment(Map<String,Object>d,String expected,boolean release,List<String>f){if(number(d.get("schemaVersion"))!=1)f.add("environment manifest schemaVersion must be 1");if(!Boolean.TRUE.equals(d.get("sanitized")))f.add("environment manifest must declare sanitized=true");String sha=s(d.get("sourceSha"));if(!sha.matches("[0-9a-f]{40}"))f.add("environment manifest sourceSha must be 40-char SHA");else if(expected!=null&&!expected.equals(sha))f.add("environment manifest SHA mismatch");Set<String>names=new HashSet<>();Map<String,String>versions=new HashMap<>();if(d.get("tools") instanceof List<?>l)for(Object x:l)if(x instanceof Map<?,?>m){names.add(s(m.get("name")));versions.put(s(m.get("name")),s(m.get("version")));}else f.add("environment manifest tool row invalid");for(String r:Set.of("java","gradle-wrapper","node","npm","python","powershell"))if(!names.contains(r))f.add("environment manifest tool missing: "+r);if(release&&(!versions.getOrDefault("java","").startsWith("25")||!versions.getOrDefault("node","").startsWith("22")))f.add("release environment requires Java 25 and Node 22");Set<String>db=new HashSet<>();if(d.get("databases") instanceof List<?>l)for(Object x:l)if(x instanceof Map<?,?>m){db.add(s(m.get("vendor")).toLowerCase(Locale.ROOT));if(release&&!Boolean.TRUE.equals(m.get("available")))f.add("release database unavailable: "+m.get("vendor"));}if(!db.equals(Set.of("oracle","postgresql","mariadb")))f.add("environment database vendors must be exactly official three");Set<String>br=new HashSet<>();if(d.get("browsers") instanceof List<?>l)for(Object x:l)if(x instanceof Map<?,?>m){br.add(s(m.get("name")).toLowerCase(Locale.ROOT));if(release&&!Boolean.TRUE.equals(m.get("available")))f.add("release browser unavailable: "+m.get("name"));}if(!br.equals(Set.of("chromium","firefox","webkit")))f.add("environment browsers must be chromium/firefox/webkit");}
