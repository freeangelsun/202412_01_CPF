...
param(
    ...
    [Alias('Host')][string]$DatabaseHost='127.0.0.1', [int]$Port=3306, [string]$User='root',
...
# MariaDB 배포판에 따라 호환 mysqldump 실행 파일만 제공될 수 있습니다.
...
$args=@('--single-transaction','--routines','--events','--triggers','--hex-blob','--host', $DatabaseHost,'--port',"$Port",'--user',$User,$Database)
