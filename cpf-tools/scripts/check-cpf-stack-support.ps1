...
.EXAMPLE
pwsh -File .\cpf-tools\scripts\check-cpf-stack-support.ps1
.EXAMPLE
pwsh -File .\cpf-tools\scripts\check-cpf-stack-support.ps1 -RequireSupported
#>
param(
...
$generator = Get-Content -LiteralPath (Join-Path $Root 'cpf-tools/generator/create-domain.ps1') -Raw -Encoding UTF8
...
$buildFiles = @(Get-ChildItem -LiteralPath $Root -Recurse -File -Filter 'build.gradle' | Where-Object {
    $_.FullName -notmatch '[\\/](build|\.gradle|local-domains)[\\/]'
})
...
if (-not $generator.Contains('$springBootVersion') -or -not $generator.Contains('$dependencyManagementVersion')) {
    throw 'Generator does not use canonical stack versions.'
}
if (-not $exporter.Contains('cpf-stack.properties')) { throw 'Standalone exporter does not carry canonical stack properties.' }
