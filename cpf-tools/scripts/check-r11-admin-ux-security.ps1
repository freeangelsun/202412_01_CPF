...
@(
 ...
 'cpf-admin/frontend/src/stores/admConsoleStore.ts',
 'cpf-admin/frontend/src/app/useAdmConsolePage.ts',
...
if(Test-Path $admMixin){Fail 'Legacy ADM options-API mixin remains.'}
$admStore=Join-Path $Root 'cpf-admin/frontend/src/stores/admConsoleStore.ts'
if(Test-Path $admStore){
  $text=Get-Content $admStore -Raw
  ...
  elseif($text -notmatch '!state\.permissionsLoaded\s*\|\|\s*!state\.authorizedMenus\.length\)\s*return\s*\[\]'){Fail 'ADM does not fail closed while permissions are unloaded or explicitly empty.'}
  else{Pass 'ADM menu authorization is fail-closed after permission loading'}
  if($text -notmatch 'this\.permissionsLoaded\s*=\s*Array\.isArray\(data\.menus\)'){Fail 'ADM /auth/me success path does not mark permissions loaded from the server contract.'}
  else{Pass 'ADM auth/me permission loading state'}
}
$admComposable=Join-Path $Root 'cpf-admin/frontend/src/app/useAdmConsolePage.ts'
$admApp=Join-Path $Root 'cpf-admin/frontend/src/App.vue'
if((Test-Path $admComposable) -and (Test-Path $admApp)){
  $composable=Get-Content $admComposable -Raw; $app=Get-Content $admApp -Raw
  if($composable -notmatch 'useAdmConsoleStore' -or $app -notmatch 'useAdmConsolePage\(\)'){Fail 'ADM Composition/Pinia console wiring is missing.'}
  else{Pass 'ADM Composition/Pinia console wiring'}
}
