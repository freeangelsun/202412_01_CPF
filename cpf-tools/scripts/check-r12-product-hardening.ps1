...
$g=Read 'cpf-tools/generator/create-domain.ps1';Must 'Generator cpf-common dependency' ($g.Contains("implementation project(':cpf-common')"));Must 'Generator no core.common' (-not$g.Contains('com.cpf.core.common.'));Must 'Generator memory production blocked' ($g.Contains('@Profile({"local", "test", "edu"})'))
...
$mix=Read 'cpf-admin/frontend/src/app/admConsoleMixin.ts';Must 'Deleted frontend feature imports absent' (-not($mix-match 'features/(access|observability|platform|reference)/methods'))
