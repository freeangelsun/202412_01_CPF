...
$runner = Join-Path $Root "cpf-tools/scripts/invoke-reference-database-lifecycle.ps1"
$liveRunner = Join-Path $Root "cpf-tools/scripts/invoke-reference-live-idempotency-conflict.ps1"
$contractVerifier = Join-Path $Root "cpf-tools/scripts/verify-reference-db-lifecycle-contract.py"
$verifyGenerator = Join-Path $Root "cpf-tools/generator/generate-reference-db-verify.py"
$defaultProfilePath = Join-Path $Root "cpf-tools/config/database-install.default.json"
