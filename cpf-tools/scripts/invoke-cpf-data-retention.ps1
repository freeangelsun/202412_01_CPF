...
try{
    $generateArgs=@((Join-Path $rootPath 'cpf-tools/scripts/generate-cpf-data-retention-sql.py'),'--root',$rootPath,'--vendor',$Vendor,'--policy-id',$PolicyId,'--legal-hold-manifest',$holdPath,'--cutoff-utc',$CutoffUtc.ToUniversalTime().ToString('o'),'--operator',$Operator,'--reason',$Reason,'--output',$sql)
