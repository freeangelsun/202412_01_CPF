...
$result=[ordered]@{schemaVersion=1;operation='PITR';mode=if($Execute){'EXECUTE'}else{'PLAN'};status=if($Execute){'APPLYING'}else{'PLANNED'};vendor=$vendor;database=$database;targetTimeUtc=$target.ToString('o');planSha256=$planHash;operator=$Operator;reason=$Reason;approvalReference=$ApprovalReference;startedAt=(Get-Date).ToUniversalTime().ToString('o');finishedAt='';reconcileRequired=$false;failureCode='';outputSha256='';sanitized=$true}
