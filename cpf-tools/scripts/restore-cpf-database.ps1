...
$rootPath=(Resolve-Path $Root).Path; $plan=Get-Content (Join-Path $rootPath 'cpf-tools/config/database-source-plan.json') -Raw|ConvertFrom-Json
...
$tool=Get-Command mariadb -ErrorAction SilentlyContinue; if(-not $tool){$tool=Get-Command mysql -ErrorAction SilentlyContinue}; if(-not $tool){throw 'mariadb/mysql client를 찾을 수 없습니다.'}
