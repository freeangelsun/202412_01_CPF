...
function Run-Sql([string]$File){
  ...
  switch($Vendor){
    ...
    'oracle' {
      ...
      try {
        @("whenever sqlerror exit sql.sqlcode rollback","set echo on feedback on serveroutput on","connect $user/\"$password\"@$connect","@$($filePath.Replace('\\','/'))","exit success") | Set-Content -LiteralPath $driver -Encoding utf8
  ...
  if($LASTEXITCODE -ne 0){throw "SQL 실행 실패 vendor=$Vendor mode=$Mode file=$filePath exit=$LASTEXITCODE"}
}
