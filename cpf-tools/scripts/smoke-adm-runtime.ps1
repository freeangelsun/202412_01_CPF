...
try {
    ...
    if ($IncludePermissionWriteSmoke) {
        ...
        $result.permissionWriteApi.reason = "Run cpf-tools/scripts/smoke-adm-permission-runtime.ps1 or pass -IncludePermissionWriteSmoke to execute write/filter checks."
    }
    ...
    & (Join-Path $Root "cpf-tools/scripts/smoke-adm-center-cut-runtime.ps1") `
        -AdmBaseUrl $AdmBaseUrl `
    ...
    & (Join-Path $Root "cpf-tools/scripts/smoke-transaction-meta-runtime.ps1") `
        -AdmBaseUrl $AdmBaseUrl `
    ...
    & (Join-Path $Root "cpf-tools/scripts/smoke-adm-ui.ps1") -Root $Root
