        ...
        if ($case.database -eq 'Y') {
            ...
            foreach ($databaseOperation in @('bootstrap', 'migration', 'verify')) {
                [void](Invoke-PwshScript `
                    -ScriptPath (Join-Path $Root 'cpf-tools/scripts/initialize-domain-database.ps1') `
                    -Arguments @(
                        '-Root', $sandbox,
...
"@
    ...
    foreach ($case in $matrix) {
        [void](Invoke-PwshScript `
            -ScriptPath (Join-Path $Root 'cpf-tools/scripts/verify-domain.ps1') `
            -Arguments @(
                '-Root', $sandbox,
                '-DomainName', $case.domain,
                '-SystemCode', $case.code,
    ...
    [void](Invoke-PwshScript `
        -ScriptPath (Join-Path $Root 'cpf-tools/scripts/remove-domain.ps1') `
        -Arguments @(
            '-Root', $sandbox,
