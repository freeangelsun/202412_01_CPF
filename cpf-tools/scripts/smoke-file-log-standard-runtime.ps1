...
. (Join-Path $Root "cpf-tools/scripts/runtime-diagnostics.ps1")
...
try {
    ...
    $result.files += Test-LogFile -Module "REF" -LogType "transaction" -TransactionId $tx -Required $runtimeVerified
    $result.files += Test-LogFile -Module "REF" -LogType "integration" -TransactionId $tx -Required $false
