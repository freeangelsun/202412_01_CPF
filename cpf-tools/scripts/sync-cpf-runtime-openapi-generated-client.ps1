...
Step 'explicit-operation-id-source-contract' {
    & (Join-Path $rootPath 'cpf-tools/scripts/ensure-explicit-openapi-operation-ids.ps1') -Root $rootPath -ResultDir $operationEvidenceDir
}
Step 'runtime-openapi-export-and-controller-coverage' {
    & (Join-Path $rootPath 'cpf-tools/scripts/export-cpf-backend-openapi.ps1') -Module $Module -BaseUrl $BaseUrl -Root $rootPath -Output $canonical -EvidenceOutput $exportEvidence
}
