...
class BackupReplicaContractTest(unittest.TestCase):
 def test_contract_requires_verified_cross_region_replica_before_primary_purge(self):
  replica=json.loads((ROOT/'cpf-tools/db/cpf-backup-lifecycle-contract.json').read_text())['crossRegionReplication']
 ...
 def test_backup_manifest_declares_primary_region_without_duplicate_replica_ledger(self):
  text=(SCRIPTS/'backup-cpf-database.ps1').read_text();self.assertIn('primaryRegion=$Region',text);self.assertNotIn('replicas=@()',text)
 def test_replica_registration_and_copy_share_hash_protected_sidecar_contract(self):
  register=(SCRIPTS/'register-cpf-backup-replica.ps1').read_text();copy=(SCRIPTS/'replicate-cpf-backup-artifact.ps1').read_text()
 ...
 def test_retention_requires_replica_reviewed_plan_sod_and_reconcile(self):
  text=(SCRIPTS/'invoke-cpf-backup-retention.ps1').read_text()
