...
SCRIPT = Path(__file__).resolve().parents[1] / "verify-cpf-batch-execution-fencing.py"
...
def test_recover_without_fencing_fails(tmp_path: Path) -> None:
    ...
    rel = "cpf-batch/execution-runtime/src/main/java/com/cpf/batch/execution/CpfSpringBatchExecutionControl.java"
    path = root / rel
    text = path.read_text()
...
def test_stable_remote_owner_reintroduction_fails(tmp_path: Path) -> None:
    root = fixture(tmp_path)
    rel = "cpf-batch/execution-runtime/src/main/java/com/cpf/batch/execution/CpfBatchKafkaInboundBridge.java"
    path = root / rel
