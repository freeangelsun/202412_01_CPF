...
COMMAND = ROOT / "cpf-core/src/main/java/com/cpf/core/api/batch/CpfBatchRiskCommand.java"
COORDINATOR = ROOT / "cpf-batch/control-server/src/main/java/com/cpf/batch/control/compat/CpfBatchRiskCommandCoordinator.java"
LEDGER = ROOT / "cpf-batch/control-server/src/main/java/com/cpf/batch/control/compat/JdbcBatchRiskCommandLedger.java"
...
def test_java21_batch_risk_command_runtime_harness() -> None:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        write(src / "com/cpf/core/api/batch/CpfBatchRiskCommand.java", COMMAND.read_text(encoding="utf-8"))
...
public class JdbcBatchRiskCommandLedger {
 ...
 public void complete(CpfBatchRiskCommand c,String p){completed++; FakeStore.put(p, com.cpf.core.api.data.CpfDataRow.of("value","ok"));}
...
public class RiskHarness { static int checks;
 ...
 public static void main(String[]a){
  ...
  l.next=new JdbcBatchRiskCommandLedger.Decision(JdbcBatchRiskCommandLedger.Kind.CREATED,null,null,null);try{x.executeRow(c,()->{throw new org.springframework.dao.QueryTimeoutException("timeout");});throw new AssertionError();}catch(org.springframework.dao.QueryTimeoutException expected){ok(l.unknown==1,"transient becomes unknown");}
...
def test_schema_and_java_ledger_contract_are_closed() -> None:
    ...
    assert '"name": "bat_operation_request"' in canonical
