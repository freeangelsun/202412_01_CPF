...
SCRIPT = Path(__file__).parents[1] / "verify-cpf-batch-ghost-safety.py"
...
class BatchGhostSafetyTest(unittest.TestCase):
    ...
    def test_missing_exact_mutation_rejected(self):
        root = self.fixture()
        p = root / "cpf-batch/control-server/src/main/java/com/cpf/batch/control/compat/BatchOperationsCompatibilityService.java"
        p.write_text(p.read_text(encoding="utf-8").replace(
            'requireSingleMutation(changed, "ghost lock release", jobId);', ""), encoding="utf-8")
        with self.assertRaises(ValueError):
            module.verify(root)
