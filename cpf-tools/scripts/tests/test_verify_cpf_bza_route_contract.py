...
SCRIPT=Path(__file__).parents[1]/'verify-cpf-bza-route-contract.py';spec=importlib.util.spec_from_file_location('bza_route',SCRIPT);module=importlib.util.module_from_spec(spec);assert spec.loader;spec.loader.exec_module(module)
class BzaRouteContractTest(unittest.TestCase):
    def test_real_overlay(self):self.assertEqual(26,module.validate(Path(__file__).parents[3])['routes'])
