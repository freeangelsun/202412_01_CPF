...
SCRIPT=Path(__file__).resolve().parents[1]/'verify-cpf-db-lifecycle-contract.py';spec=importlib.util.spec_from_file_location('db_contract',SCRIPT);module=importlib.util.module_from_spec(spec);assert spec and spec.loader;spec.loader.exec_module(module)
class DbLifecycleTest(unittest.TestCase):
 def fixture(self):
  ...
  for vendor in module.OFFICIAL:
   ...
   p.write_text(json.dumps({'vendor':vendor,'status':'완료','runtimeVerification':'미검증','migrationLocationPattern':migration,'rollbackLocationPattern':rollback}), encoding="utf-8")
   lifecycle={key:f'cpf-tools/db/vendor/{vendor}/{key}' for key in module.LIFECYCLE_KEYS};lifecycle['rollback']=f'cpf-tools/db/vendor/{vendor}/rollback'
  ...
  contract={'officialVendors':list(module.OFFICIAL),'orderedStages':module.STAGES,'statusModel':{'development':'완료','runtimeVerification':'미검증'},'requiredStaticGates':[],'runtimeExecutor':'x.ps1','vendorContracts':contracts}
 ...
 def test_valid_contract(self):
  ...
  try:self.assertEqual((3,7),module.validate(root,False))
 ...
 def test_rollback_drift_rejected(self):
  temp,root,_,_=self.fixture();p=root/'cpf-tools/db/vendor/oracle/pack.json';d=json.loads(p.read_text(encoding="utf-8"));d['rollbackLocationPattern']='cpf-tools/db/vendor/oracle/migration/rollback/{logicalDatabase}';p.write_text(json.dumps(d), encoding="utf-8")
  try:
   with self.assertRaises(module.DbContractError):module.validate(root,False)
  finally:temp.cleanup()
