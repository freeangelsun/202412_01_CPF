...
MODULE_PATH = Path(__file__).resolve().parents[1] / "verify-cpf-db-vendor-manifest.py"
spec = importlib.util.spec_from_file_location("db_vendor_gate", MODULE_PATH)
module = importlib.util.module_from_spec(spec)
assert spec and spec.loader
spec.loader.exec_module(module)

SOURCE_MANIFEST = Path(__file__).resolve().parents[2] / "db/vendor-pack-manifest.json"
...
class DbVendorManifestTest(unittest.TestCase):
    ...
    def _materialize_paths(self) -> None:
        ...
        for vendor, entry in data["vendors"].items():
            for key in ("vendorRoot", "runtimeRoot", "domainTemplateRoot"):
    ...
    def test_valid_exact_three_vendor_manifest_passes(self) -> None:
        ...
        self.assertEqual(36, result["checked_path_count"])
