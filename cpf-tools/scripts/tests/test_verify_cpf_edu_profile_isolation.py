...
S=Path(__file__).resolve().parents[1]/'verify-cpf-edu-profile-isolation.py';spec=importlib.util.spec_from_file_location('iso',S);m=importlib.util.module_from_spec(spec);spec.loader.exec_module(m)
class EduIsolationTest(unittest.TestCase):
 def fixture(self):
  ...
  f=r/m.REQUIRED_FIXTURE;f.write_text('CPF_EDU_TASKLET_JOB CPF_EDU_TASKLET_DAILY CPF_REF_CENTER_CUT_SAMPLE_JOB', encoding="utf-8")
  return t,r,p,f
 def test_valid(self):
