...
SCRIPT=Path(__file__).resolve().parents[1]/"verify-cpf-final-plan.py"
...
class FinalPlanNegativeTest(unittest.TestCase):
    ...
    def test_known_command_missing_required_flag_rejected(self):
        data=self.base()
        data["commands"][0].update({"path":"cpf-tools/scripts/verify-cpf-openapi-controller-coverage.py","args":["--root","{root}"]})
        with self.assertRaises(module.PlanError):module.validate(Path("."),data,False)
