...
REQ_RE=re.compile(r"^CPF-FR-\d{6}$"); SC_RE=re.compile(r"^CPF-SC-\d{6}$")
def read(path):
...
def main()->int:
    ...
    try:
        ...
        if any(not REQ_RE.fullmatch(x) for x in req_ids) or len(set(req_ids))!=len(req_ids): raise ValueError("invalid/duplicate requirement IDs")
        if any(not SC_RE.fullmatch(x) for x in sc_ids) or len(set(sc_ids))!=len(sc_ids): raise ValueError("invalid/duplicate scenario IDs")
        req_set=set(req_ids)
        for row in req:
            ...
            if not linked.issubset(set(sc_ids)): raise ValueError(f"unknown scenario link {row.get('requirement_id')}")
