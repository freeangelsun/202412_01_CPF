...
DB_SCRIPTS = [
    "backup-cpf-database.ps1",
    "restore-cpf-database.ps1",
    "set-cpf-backup-legal-hold.ps1",
    "invoke-cpf-backup-retention.ps1",
    "replicate-cpf-backup-artifact.ps1",
    "verify-dr-restore.ps1",
    "invoke-cpf-pitr-restore.ps1",
    "prepare-cpf-test-data.ps1",
    "invoke-official-db-vendor-sql.ps1",
    "invoke-platform-database-migration.ps1",
    "run-db-vendor-lifecycle.ps1",
    "cpf-backup-lifecycle-common.ps1",
    "invoke-cpf-data-retention.ps1",
    "invoke-cpf-db-performance-gate.ps1",
    "invoke-cpf-datasource-runtime-gate.ps1",
    "invoke-cpf-data-observability-gate.ps1",
    "invoke-cpf-db-operability-gate.ps1",
]
...
def verify(root: Path) -> tuple[list[StaticFailure], dict]:
    ...
    for name in DB_SCRIPTS:
        ...
        for pattern in forbidden_argument_patterns:
            ...
            if match and not any(marker in text for marker in secret_exceptions.get(name, [])):
                failures.append(StaticFailure(name, f"possible secret-bearing process argument: {match.group(0)[:120]}"))
