...
def main()->int:
 parser=argparse.ArgumentParser();parser.add_argument('--root',type=Path,default=Path.cwd());parser.add_argument('--matrix',default='cpf-docs/quality/CPF_20260801_INTEGRATED_RESULT_MATRIX.csv');parser.add_argument('--expected-sha')
 args=parser.parse_args();root=args.root.resolve();verified,documents=validate_matrix(root,root/args.matrix,args.expected_sha);print(f'[PASS] CPF evidence semantics verifiedRows={verified} documents={documents} bulkIdForbidden=true');return 0
