...
def _load_support_bundle(root: Path):
    path = root / 'cpf-tools/scripts/generate-cpf-support-bundle.py'
    if not path.is_file():
...
def main() -> int:
    ...
    profile_path = args.profile or (root / 'cpf-tools/runtime-alternatives/cpf-runtime-alternative-profile.json')
    try:
