...
def verify(root: Path, expected_sha: str | None = None, require_clean: bool = False) -> dict:
    ...
    if not mirror_path.is_file():
        raise GateError(f"catalog mirror missing: {mirror_path.relative_to(root)}")
    if catalog_path.read_bytes() != mirror_path.read_bytes():
        raise GateError(
            "starter catalog mirror drift: cpf-tools/config/cpf-starter-catalog.json "
            "must be byte-identical to cpf-tools/generator/contracts/cpf-starter-catalog.json"
        )
    ...
    if catalog.get("baselinePolicy") != "GIT_HEAD_RUNTIME" or catalog.get("baselineSha") != "RUNTIME_GIT_HEAD":
        findings.append(
            "catalog must use runtime Git HEAD policy; stale static baseline forbidden"
        )
    ...
    for index, module in enumerate(modules):
        ...
        if visibility == "public" and role != "profile":
            findings.append(f"{owner}: only profile modules may be public")
        ...
        if package:
            ...
            if not declarations and role != "profile":
                findings.append(
    ...
    if declared_public_profile_artifacts != expected_public_profile_artifacts:
        findings.append(
            ...
            f"expected={sorted(expected_public_profile_artifacts)} "
            f"actual={sorted(declared_public_profile_artifacts)}"
        )
    ...
    for slot, providers in provider_slots.items():
        ...
        for provider, binding in providers.items():
            ...
            if project not in active_projects:
                findings.append(
                    f"providerSlots.{slot}.{provider}: unknown projectPath {project}"
                )
            artifact = project[1:] if isinstance(project, str) and project.startswith(":") else ""
            expected_coordinate = f"com.cpf.starter:{artifact}" if artifact else ""
            if coordinate != expected_coordinate:
                findings.append(
                    f"providerSlots.{slot}.{provider}: coordinate mismatch "
