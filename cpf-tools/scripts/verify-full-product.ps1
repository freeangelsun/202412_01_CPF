...
try{
    ...
    foreach($gate in @(
        @('Work context','cpf-tools/scripts/check-work-context.ps1'),
        @('R10 product standard','cpf-tools/scripts/check-r10-product-standard.ps1'),
        @('R10 cleanup','cpf-tools/scripts/check-r10-cleanup.ps1'),
        @('Frontend route/local asset','cpf-tools/scripts/check-frontend-route-targets.ps1'),
        @('Source documentation/OpenAPI','cpf-tools/scripts/check-source-documentation-standard.ps1'),
    ...
    if($WithBrowser){
        foreach($script in @('cpf-tools/scripts/smoke-adm-ui.ps1','cpf-tools/scripts/smoke-bza-ui.ps1')){
    ...
    foreach($post in @(
        @('Post-run cleanup','cpf-tools/scripts/check-r10-cleanup.ps1'),
        @('Post-run repository hygiene','cpf-tools/scripts/check-repository-hygiene.ps1')
    )){
