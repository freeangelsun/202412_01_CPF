...
try{
 ...
 $rules=Join-Path $rootPath 'cpf-tools/release/supply-chain/ort/evaluator.rules.kts';if(-not(Test-Path $rules)){throw'ORT evaluator rules missing'}
 ...
 foreach($artifact in $ArtifactPaths){
  ...
  Run"License $safe"$python @((Join-Path $rootPath 'cpf-tools/supply-chain/tools/verify-cpf-supply-chain.py'),'--root',$rootPath,'--sbom',$sbom)
 ...
 $e=[ordered]@{schemaVersion=3;evidenceId='CPF-SUPPLY-CHAIN';sourceSha=$sourceSha;resultSha=$sourceSha;sourceDirty=$false;startedAt=$started.ToString('o');finishedAt=[DateTimeOffset]::UtcNow.ToString('o');exitCode=0;requirements=@('CPF-SUPPLY-CHAIN');tools=@{ort=(& $ort version 2>&1|Out-String).Trim();syft=(& $syft version 2>&1|Out-String).Trim();grype=(& $grype version 2>&1|Out-String).Trim()};ortAnalyzerSha256=FileHash $analyzer.FullName;ortEvaluationSha256=FileHash $evaluation.FullName;artifacts=$records;sanitized=$true;releaseEligible=$true}
