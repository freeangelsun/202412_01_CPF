...
MODULE = Path(__file__).resolve().parents[4] / "cpf-tools/verification/tools/verify-cpf-adm-route-source-consumers.py"
...
class RouteSourceConsumerUnitTest(unittest.TestCase):
    ...
    def test_global_route_workbench_missing_actor_boundary_is_rejected(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            api.write_text(api.read_text(encoding="utf-8").replace("assertNoClientActor({});", "void 0;"), encoding="utf-8")
