...
SCRIPT = Path(__file__).resolve().parents[4] / "cpf-tools/verification/tools/verify-cpf-bat-operation-ledger-lifecycle.py"
...
"""
        ...
        fresh_roles = "role = 'Provision'; role = 'EmptyInstall'; role = 'ProductSeed'; role = 'Verify'"
        if wrong_order:
            fresh_roles = "role = 'ProductSeed'; role = 'Provision'; role = 'EmptyInstall'; role = 'Verify'"
        lifecycle = f"""
# vendor-pack-manifest.json / checksums.sha256
function Get-CpfMigrationCatalog {{}}
$initializeTool = 'initialize-cpf-database.ps1'
$migrationTool = 'invoke-platform-database-migration.ps1'
$ExpectedLifecyclePlanSha256 = ''
{fresh_roles}
stage = 'Rollback'
stage = 'Reapply'
"""
        script = root / "cpf-tools/db/tools/run-db-vendor-lifecycle.ps1"; script.parent.mkdir(parents=True); script.write_text(lifecycle, encoding="utf-8")
    ...
    def test_wrong_manifest_fresh_order_fails(self):
