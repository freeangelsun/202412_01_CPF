...
def complete_fixture(repo: Path, module) -> None:
    for path in module.PUBLIC_API:
        write(repo, path)
    write(repo, module.SHARED_RUNTIME[0], "class X implements CpfCache, CpfDistributedLockPort {}")
    ...
    write(repo, module.REDIS_PROVIDER[0], "new RedisCpfCache CpfCacheInvalidationCoordinator JdbcCpfCacheInvalidationStore")
    write(repo, module.REDIS_PROVIDER[1], "class X extends SpringDataRedisCpfCache {}")
    write(repo, module.REDIS_PROVIDER[2])
    write(repo, module.VALKEY_PROVIDER[0], "new ValkeyCpfCache CpfCacheInvalidationCoordinator JdbcCpfCacheInvalidationStore")
    write(repo, module.VALKEY_PROVIDER[1], "class X extends SpringDataRedisCpfCache {}")
