...
SCRIPT=Path(__file__).resolve().parents[4] / "cpf-tools/verification/tools/verify-cpf-calendar-contract.py"
...
"jdbc":'''class X { boolean actorAwareMutations(){return true;} void x(){ Object x=actor,actor; required(operatorId); } void y(){try{}catch(DuplicateKeyException e){String s="CREATE_CONFLICT";}} }''',
...
"insertSql":'''INSERT INTO cmn_business_calendar_day (calendar_id,business_date,business_day_yn,day_type,institution_code,reason,version_no,created_by,updated_by,updated_at) VALUES (?,?,?,?,?,?,1,?,?,CURRENT_TIMESTAMP)''',
"updateSql":'''UPDATE cmn_business_calendar_day SET version_no=version_no+1,updated_by=? WHERE calendar_id=? AND business_date=? AND version_no=?''',
"deleteSql":'''DELETE FROM cmn_business_calendar_day WHERE calendar_id=? AND business_date=? AND version_no=?''',
"frontend":'''const canWrite=computed(()=>writable.value&&permission.value.writeAllowed);const canDelete=computed(()=>writable.value&&permission.value.deleteAllowed);if(e.status===409){} const operationForm=ref({}); async function resolveBusinessDate(){return admCalendarResolveDate(`/resolve?date=${x}`)}'''}
PATHS=m.FILES
