...
SCRIPT=Path(__file__).resolve().parents[4]/'cpf-tools/db/verification/verify-cpf-db-vendor-semantic-parity.py'
def load():
...
class TestDbVendorSemanticParity(unittest.TestCase):
 def fixture(self,bad_dialect=False,missing_column=False,empty_rollback=False,bad_checksum=False):
  ...
  g=root/'cpf-tools/db/generator/generate-official-db-vendor-source.ps1';g.parent.mkdir(parents=True);g.write_text("'^LONGBLOB$','BYTEA' '^MEDIUMTEXT$','TEXT' $u -eq 'LONGBLOB' $u -eq 'MEDIUMTEXT'",encoding='utf-8')
  return td,root
