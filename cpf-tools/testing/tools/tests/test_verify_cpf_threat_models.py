...
SCRIPT = Path(__file__).resolve().parents[4] / "cpf-tools/verification/tools/verify-cpf-threat-models.py"
...
def model():
    return {
        ...
        "scope_paths": ["cpf-tools/security/**"],
        ...
        "mitigations": [{"id": "M1", "control": "input validation", "evidence_paths": ["cpf-tools/security/tool.py"]}],
        "threats": [{
...
def setup_repo(tmp_path: Path, value=None):
    ...
    scripts = repo / "cpf-tools/security"; manifests.mkdir(parents=True); scripts.mkdir(parents=True, exist_ok=True)
...
def test_valid_model_and_changed_path_coverage(tmp_path: Path):
    ...
    result = module.verify(repo, manifests, ["cpf-tools/security/tool.py"], now=NOW)
