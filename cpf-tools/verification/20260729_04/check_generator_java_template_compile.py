...
src=(root/'cpf-tools/generator/create-domain.ps1').read_text(encoding='utf-8')
...
for name in blocks:
 ...
 p=out/'src'/Path(*m.group(1).split('.'))/(c.group(1)+'.java');p.parent.mkdir(parents=True,exist_ok=True);p.write_text(t)
...
for rel,t in extra.items(): p=out/'src'/rel;p.parent.mkdir(parents=True,exist_ok=True);p.write_text(t)
