...
public final class BatchRuntimeControllerHarness {
 ...
 static Map<String,Object> validPlan(){return Map.of("idempotencyKey","P1","reason","deploy","approvalRequestId","A1","expectedVersion",0L);}
 public static void main(String[] args){
   BatchRuntimeControlClient c=new BatchRuntimeControlClient(); BatchRuntimeControlController ctl=new BatchRuntimeControlController(c);
   ...
   status(ctl.command("session-admin",Map.of("commandId","C1","idempotencyKey","I1","commandType","STOP","reason","r","approvalRequestId","A1","approvedBy","boss","targetIds",List.of("T1"),"payload",nested())),202);noAlias(c.last,true);
   ...
   status(ctl.command("session-admin",Map.of()),400);
   status(ctl.plan(" ",validPlan()),400);
