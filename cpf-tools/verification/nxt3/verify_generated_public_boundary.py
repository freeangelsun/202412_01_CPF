...
def main()->int:
    ...
    # Generated Domain은 Online 업무 Source만 직접 생성한다. Batch는 별도 Public Capability/Profile이다.
    for root_name in ('cpf-member','cpf-external'):
        ...
        forbidden_modules=[x for x in ('batch','domain','jobpack') if (root/root_name/x).is_dir() and any(y.is_file() for y in (root/root_name/x).rglob('*'))]
        add(f'{root_name}-generated-batch-surface-zero',not forbidden_modules,forbidden_modules)
