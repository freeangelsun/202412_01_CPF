...
BASE_ROOT_DIRS={'.git','.github','.gradle','.pytest_cache','.vscode','cpf-admin','cpf-batch','cpf-biz-admin','cpf-core','cpf-docs','cpf-gateway','cpf-education','cpf-starters','cpf-tools','deploy','gradle'}
...
def main()->int:
    ...
    for physical,logical in EXPECTED_GENERATED.items():
        ...
        expected={'online'}; dirs=domain_surface_dirs(p); add(f'{physical}-minimal-ia',dirs==expected,{'expected':sorted(expected),'actual':sorted(dirs)})
