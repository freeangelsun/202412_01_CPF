...
BASE_ROOT_DIRS={'.git','.github','.gradle','.vscode','cpf-admin','cpf-batch','cpf-biz-admin','cpf-core','cpf-docs','cpf-gateway','cpf-education','cpf-starters','cpf-tools','deploy','gradle'}
...
def main()->int:
    ...
    for physical,logical in EXPECTED_GENERATED.items():
        ...
        expected={'online'}; dirs={x.name for x in p.iterdir() if x.is_dir() and any(y.is_file() for y in x.rglob('*'))} if p.is_dir() else set(); add(f'{physical}-minimal-ia',dirs==expected,{'expected':sorted(expected),'actual':sorted(dirs)})
