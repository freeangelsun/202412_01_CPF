...
def main()->int:
    ...
    for physical,logical in EXPECTED_GENERATED.items():
        p=root/physical; definition=p/'cpf-domain.yaml'; add(f'{physical}-physical-root',p.is_dir(),str(p)); add(f'{physical}-logical-domain-name',domain_name(definition)==logical,str(definition)); add(f'{physical}-customer-metadata-zero',not (p/'.cpf').exists(),str(p/'.cpf'))
        definition_text=definition.read_text(encoding='utf-8',errors='replace'); batch_selected='batch: true' in definition_text.lower(); expected={'online'}|({'batch'} if batch_selected else set()); dirs={x.name for x in p.iterdir() if x.is_dir() and x.name not in EPHEMERAL_DIRS and any(y.is_file() for y in x.rglob('*'))} if p.is_dir() else set(); add(f'{physical}-minimal-ia',dirs==expected,{'expected':sorted(expected),'actual':sorted(dirs)})
