...
& .\gradlew @props :cpf-data-transaction-jta:runXaCrashHarness -PcpfXaHarnessMode=prepare-kill
if($LASTEXITCODE -ne 73){ throw "prepare-kill expected process exit 73 but was $LASTEXITCODE" }
& .\gradlew @props :cpf-data-transaction-jta:runXaCrashHarness -PcpfXaHarnessMode=recover
