...
.DESCRIPTION
  ...
  PII masked/raw 경계, Backoffice Query Contract, MariaDB V61~V63 독립 Immutable 이력을 검증합니다.
...
$backofficeEmployee = 'cpf-backoffice/online/src/main/java/com/cpf/backoffice/online/management/service/BackofficeManagementService.java'
$backofficeAudit = 'cpf-backoffice/online/src/main/java/com/cpf/backoffice/online/audit/service/BackofficeBusinessAuditService.java'
...
$backofficeFrontend = 'cpf-backoffice-web/frontend/src/features/employees/components/EmployeeChangeForm.vue'
...
Require-Contains $admService '@CpfTransactional\(transactionManager\s*=\s*"admTransactionManager"\)' 'ADM 변경 메서드 CPF Transaction 경계가 없습니다.'
...
Require-Contains $backofficeEmployee 'EMPLOYED' 'Backoffice EMPLOYED 안전 기본값이 없습니다.'
Require-Contains $backofficeEmployee 'BackofficeEmploymentStatus\.parse' 'Backoffice 재직상태 Catalog 검증이 없습니다.'
Require-Contains $backofficeEmployee 'CpfSensitiveData\.maskEmail' 'Backoffice 기본 이메일 Projection이 마스킹되지 않았습니다.'
Require-Contains $backofficeEmployee 'EMPLOYEE_PII_RAW_VIEW' 'Backoffice 원문 조회 감사 이벤트가 없습니다.'
Require-Contains $backofficeAudit '\[MASKED\]' 'Backoffice 감사 Snapshot PII 마스킹이 없습니다.'
Require-Contains $backofficeAudit '\[REDACTED\]' 'Backoffice 감사 Snapshot Secret 제거가 없습니다.'
...
# Backoffice Java inline SQL 금지.
$backofficeJava = Join-Path $RootPath 'cpf-backoffice/online/src/main/java'
...
foreach ($java in Get-ChildItem -LiteralPath $backofficeJava -Recurse -Filter '*.java') {
    $content = Get-Content -LiteralPath $java.FullName -Raw -Encoding UTF8
    if ([regex]::IsMatch($content, $sqlPattern)) { throw "Backoffice Java inline SQL이 남아 있습니다: $($java.FullName)" }
}
...
Require-Contains 'cpf-backoffice/online/src/main/java/com/cpf/backoffice/online/management/controller/BackofficeManagementController.java' '@PostMapping\("/employees/\{employeeNo\}/contacts/raw"\)' 'Backoffice PII raw 조회가 audited POST가 아닙니다.'
Require-NotContains $admFrontend 'window\.prompt|contacts/raw\?reason=' 'ADM Raw PII가 prompt 또는 URL query reason을 사용합니다.'
Require-NotContains $backofficeFrontend 'contacts/raw\?reason=|reason=\$\{encodeURIComponent' 'Backoffice PII 조회 사유가 URL query string에 남아 있습니다.'
...
# V61~V63 Source compatibility archive와 central executable history는 각각
# 이미 published 된 독립 Immutable 이력이다. 전용 gate가 각 manifest/SHA,
# current MBW와 historical BZA identity, exact rollback을 모두 검증한다.
& (Join-Path $PSScriptRoot 'check-data-safety-schema-contract.ps1') -Root $RootPath
...
Require-Contains 'cpf-tools/db/vendor/mariadb/source/10_cpf_schema.sql' 'PENDING_ACTIVATION' 'ADM fresh schema safe default가 없습니다.'
Require-Contains 'cpf-tools/db/vendor/mariadb/source/40_business_modules_schema.sql' 'EMPLOYED' 'Backoffice fresh schema employment default가 없습니다.'
