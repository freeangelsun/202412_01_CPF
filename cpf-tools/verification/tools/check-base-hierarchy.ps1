...
foreach ($module in $moduleRules.Keys) {
    if (-not $baseParentRules.Contains($module)) {
        $baseParentRules[$module] = [ordered]@{
            controller = "com.cpf.starter.profile.webapi.api.CpfControllerExtension"
            service = "com.cpf.foundation.api.CpfServiceExtension"
        }
    }
}
...
foreach ($module in $moduleRules.Keys) {
    ...
    foreach ($kind in @("controller", "service")) {
        ...
        $expectedParent = $baseParentRules[$module][$kind]
        if ($null -eq $baseClass -or $null -eq $expectedParent) { continue }
        $baseFile = Get-ChildItem -LiteralPath $sourceRoot -Recurse -File -Filter "$baseClass.java" -ErrorAction SilentlyContinue | Select-Object -First 1
        if ($null -eq $baseFile) {
            $failures.Add([ordered]@{ module=$module; path=$sourceRoot; reason="$baseClass source가 없습니다." })
            continue
        }
        $baseText = [System.IO.File]::ReadAllText($baseFile.FullName, [System.Text.Encoding]::UTF8)
        $expectedSimple = $expectedParent.Substring($expectedParent.LastIndexOf('.') + 1)
        $parentPattern = '\bextends\s+' + [regex]::Escape($expectedSimple) + '\b'
        $importPattern = '(?m)^\s*import\s+' + [regex]::Escape($expectedParent) + '\s*;'
        if ($baseText -notmatch $parentPattern -or $baseText -notmatch $importPattern) {
            $failures.Add([ordered]@{ module=$module; path=($baseFile.FullName.Substring($Root.Length + 1).Replace("\", "/")); reason="$baseClass 가 표준 Owner Extension $expectedParent 를 직접 확장하지 않습니다." })
        }
    }
