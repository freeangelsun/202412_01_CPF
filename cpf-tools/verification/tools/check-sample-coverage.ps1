...
$forbidden = @(
    Get-ChildItem -LiteralPath $Root -Recurse -File -Filter "*EducationSample.java" |
        Where-Object {
            ...
            $_.FullName -notmatch '[\\/]cpf-education[\\/]src[\\/]main[\\/]java[\\/]com[\\/]cpf[\\/]reference[\\/]'
        }
)
foreach ($file in $forbidden) {
    $failures.Add("EDU 외 범용 EDU Source: $($file.FullName.Substring($Root.Length + 1))") | Out-Null
}
...
$matrixSources = @(
    [regex]::Matches(
        $matrixText,
        '\|\s*(cpf-education/src/main/java/[^|]+EducationSample\.java)\s*\|') |
