...
function Get-WorktreeFingerprint {
    ...
    # HEAD-relative binary diff captures tracked/staged source changes. The one
    # tracked live checkpoint is excluded so its mandatory update cannot invalidate
    # an otherwise reusable PASS. Every untracked, non-ignored file remains hashed.
...
if ([string]::IsNullOrWhiteSpace($LedgerRoot)) {
    $LedgerRoot = Join-Path $repositoryRoot "build/codex-onepass/$($gitHead.Substring(0, 8))"
}
