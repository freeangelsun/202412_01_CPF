...
$targets = @(
    $profile.modules.PSObject.Properties |
        ...
        Where-Object { $_.enabled -and $_.logicalDatabase -eq "referenceFixture" }
)
if ($targets.Count -ne 1) { throw "Profile에는 enabled REFERENCE_FIXTURE owner가 정확히 하나 있어야 합니다." }
$target = $targets[0]
...
try {
    if ($RequireRun) {
        ...
        $psi.Environment["CPF_EDU_LIVE_DB_TEST"] = "true"
        $psi.Environment["CPF_EDU_LIVE_JDBC_URL"] = $jdbcUrl
        $psi.Environment["CPF_EDU_LIVE_DB_USERNAME"] = [string]$target.runtimeUsername
        $psi.Environment["CPF_EDU_LIVE_DB_PASSWORD"] = [string]$target.runtimePassword
        $psi.Environment["CPF_EDU_LIVE_RESULT_PATH"] = $testResultPath
