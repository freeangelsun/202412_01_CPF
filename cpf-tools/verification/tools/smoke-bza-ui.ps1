param(
  ...
  [string] $ResultDir = "",
  [string] $FrontendRoot = "",
  [string] $BzaFrontendUrl = $(if ([string]::IsNullOrWhiteSpace($env:CPF_BZA_FRONTEND_URL)) { 'http://127.0.0.1:5173/' } else { $env:CPF_BZA_FRONTEND_URL }),
  [switch] $BrowserClick,
...
if([string]::IsNullOrWhiteSpace($FrontendRoot)){$FrontendRoot=Join-Path $Root 'cpf-biz-frontend'}elseif(-not[IO.Path]::IsPathRooted($FrontendRoot)){$FrontendRoot=Join-Path $Root $FrontendRoot}
$required=@(
  'src/App.vue','src/router/index.ts','src/shared/api/channelHttpClient.ts','src/generated/bza-api.ts',
...
$missing=@($required|?{-not(Test-Path $_ -PathType Leaf)}); if($missing){throw "BZA reference UI source incomplete: $($missing -join ', ')"}
...
$generated=Get-Content (Join-Path $FrontendRoot 'src/generated/bza-api.ts') -Raw -Encoding UTF8
$markers=[ordered]@{
  referenceSurface=$app.Contains('CPF BZA') -and $app.Contains('Reference Backoffice')
  fourFeatureRoutes=(@("path: '/'","path: '/employees'","path: '/approvals'","path: '/authorization'")|?{$router -notlike "*$_*"}).Count -eq 0
  channelOnly=$transport.Contains('VITE_BZA_CHANNEL_BASE_URL') -and $transport.Contains("credentials: 'include'")
...
$result=[ordered]@{schemaVersion=3;checkedAt=[DateTimeOffset]::Now.ToString('o');status=$(if($failed.Count){'FAILED'}else{'PASS'});frontendRoot=$FrontendRoot;frontendUrl=$BzaFrontendUrl;markers=$markers;browser='NOT_REQUESTED';sanitized=$true}
$resultPath=Join-Path $ResultDir 'bza-ui-runtime-result.sanitized.json'
if($failed.Count){$result.failed=$failed;[IO.File]::WriteAllText($resultPath,($result|ConvertTo-Json -Depth 10),$Utf8);throw "BZA reference UI static contract failed: $($failed -join ', ')"}
if($BrowserClick){
  $npm=Get-Command npm -ErrorAction SilentlyContinue
  if(-not $npm){$result.browser='SKIPPED_NPM_UNAVAILABLE';if($RequireBrowserClick){$result.status='FAILED'}}
  else{
    $runner=Join-Path $Root 'build/runtime-smoke/bza-reference-playwright';New-Item -ItemType Directory -Force -Path $runner|Out-Null
    $spec=Join-Path $runner 'bza-reference.spec.js'
    @'
const { test, expect } = require('@playwright/test');
test('BZA reference navigation', async ({page}) => {
 const base=process.env.CPF_BZA_FRONTEND_URL || 'http://127.0.0.1:5173/';
 await page.goto(base,{waitUntil:'domcontentloaded'});
 await expect(page.getByRole('heading',{name:'CPF BZA'})).toBeVisible();
...
'@ | Set-Content -LiteralPath $spec -Encoding UTF8
    ...
    $env:CPF_BZA_FRONTEND_URL=$BzaFrontendUrl
...
if($result.status -ne 'PASS'){throw "BZA reference UI smoke failed: $resultPath"}
Write-Host "BZA reference UI smoke PASS: $resultPath"
