...
def read_action_bodies(frontend: Path) -> dict[str, tuple[Path, str]]:
    ...
    """
    candidates = sorted(p for p in (set((frontend / "src/app/methods").glob("*.ts")) | set((frontend / "src/features").glob("*/methods.ts"))) if not p.name.endswith(".test.ts"))
    ...
    for path in candidates:
        ...
        for index, match in enumerate(matches):
            ...
            if name in {"return", "if", "for", "while", "switch", "catch", "function"}:
                continue
...
def validate_global_route_workbench(frontend: Path) -> list[str]:
    ...
    api_contracts = {
        ...
        "actor trust boundary": r'(?:actorKey\(key\)|assertNoClientActor\()',
