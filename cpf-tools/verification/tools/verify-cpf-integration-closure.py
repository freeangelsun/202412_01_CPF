...
for v in ('oracle','postgresql','mariadb'):
 ...
 pack=json.loads((b/'pack.json').read_text(encoding="utf-8"));
...
catalog=json.loads((root/'cpf-tools/generator/contracts/cpf-starter-catalog.json').read_text(encoding="utf-8")); webhook=[m for m in catalog['modules'] if m.get('projectPath')==':internal:integration:webhook']
settings=(root/'settings.gradle').read_text(encoding="utf-8"); route=(root/'cpf-admin/frontend/src/app/methods/routeClosureMethods.ts').read_text(encoding="utf-8"); generated=(root/'cpf-admin/frontend/src/generated/cpf-api.ts').read_text(encoding="utf-8"); facade=(root/'cpf-admin/frontend/src/features/integration-closure/integrationClosureApi.ts').read_text(encoding="utf-8"); app_methods=(root/'cpf-admin/frontend/src/app/methods/integrationClosureMethods.ts').read_text(encoding="utf-8");
