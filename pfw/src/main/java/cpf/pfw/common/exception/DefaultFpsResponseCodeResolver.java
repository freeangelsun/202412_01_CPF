 ...
 */
public class DefaultFpsResponseCodeResolver implements FpsResponseCodeResolver {
    @Override
    public FpsResolvedResponse resolve(
            ...
            String detail) {
        String resolvedCode = hasText(responseCode) ? responseCode : FpsErrorCode.INTERNAL_SERVER_ERROR.getStatusCode();
        boolean failure = resolvedCode.startsWith("E");
        String messageCode = failure ? FpsErrorCode.INTERNAL_SERVER_ERROR.getMessageCode() : defaultMessageCode(resolvedCode);
    ...
    public FpsResolvedResponse resolve(
        ...
        String externalMessage = FpsMessageFormatter.format(errorDefinition.getDefaultExternalMessage(), arguments);
        String internalMessage = FpsMessageFormatter.format(errorDefinition.getDefaultInternalMessage(), arguments);
