...
import fps.pfw.common.exception.DefaultFpsMessageResolver;
...
import fps.pfw.common.workflow.FpsWorkflowMetadata;
import fps.pfw.common.workflow.FpsWorkflowStatus;
...
public class LoggingAspect {
    ...
    @Around("execution(* fps..*Controller.*(..))")
    public Object logTransaction(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            ...
            int responseCode = resolveResponseCode(result, 200);
            String response = SensitiveDataMasker.mask(String.valueOf(result));

            logger.info(
                    "Transaction completed. transactionId={}, traceId={}, spanId={}, moduleId={}, sequenceNo={}, workflowStatus={}, compensationYn={}, responseCode={}, durationMs={}",
                    transactionId,
                    traceId,
                    spanId,
                    moduleId,
            ...
            logDynamic(
                    dynamicLogLevelRule,
                    "Dynamic transaction diagnostic completed. transactionId={}, businessTransactionId={}, responseCode={}, durationMs={}, response={}",
                    transactionId,
                    businessTransactionId,
                    responseCode,
                    durationMs,
            ...
            String errorMessage = SensitiveDataMasker.mask(errorMetadata.internalMessage());
            ...
            logDynamic(
                    dynamicLogLevelRule,
                    "Dynamic transaction diagnostic failed. transactionId={}, businessTransactionId={}, responseCode={}, durationMs={}, errorCode={}, error={}",
                    transactionId,
                    businessTransactionId,
                    resolveErrorResponseCode(ex),
                    durationMs,
    ...
    private int resolveResponseCode(Object result, int defaultCode) {
    ...
    private ErrorMetadata resolveErrorMetadata(Throwable ex, Locale locale) {
        if (ex instanceof FpsException fpsException && fpsException.getErrorCode() != null) {
            FpsResolvedMessage resolvedMessage = messageResolver.resolve(fpsException.getErrorCode(), locale);
            String externalTemplate = firstText(
                    fpsException.getExternalMessage(),
                    resolvedMessage.externalMessage(),
                    fpsException.getErrorCode().getDefaultExternalMessage());
            String internalTemplate = firstText(
                    fpsException.getInternalMessage(),
                    firstText(fpsException.getDetail(), resolvedMessage.internalMessage()),
                    fpsException.getErrorCode().getDefaultInternalMessage());
            return new ErrorMetadata(
                    fpsException.getErrorCode().getMessageCode(),
                    FpsMessageFormatter.format(externalTemplate, fpsException.getMessageArguments()),
                    FpsMessageFormatter.format(internalTemplate, fpsException.getMessageArguments()));
        }
