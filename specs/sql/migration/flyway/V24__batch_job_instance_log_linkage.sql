-- BAT JobInstance 로그와 PFW 배치 실행 이력을 상호 추적하기 위한 확장 컬럼입니다.
...
CREATE INDEX IF NOT EXISTS ix_pfw_batch_execution_job_instance
    ON pfw_batch_execution (spring_batch_job_instance_id, business_date);

CREATE INDEX IF NOT EXISTS ix_pfw_batch_execution_parent_transaction
    ON pfw_batch_execution (parent_transaction_global_id);

CREATE INDEX IF NOT EXISTS ix_pfw_batch_execution_segment
    ON pfw_batch_execution (transaction_segment_id, parent_segment_id);
