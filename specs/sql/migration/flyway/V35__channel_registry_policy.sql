-- PFW 통합 채널 레지스트리와 표준 실행별 최초·호출 채널 정책을 추가합니다.
...
CREATE TABLE IF NOT EXISTS pfw_channel_execution_policy (
    ...
    INDEX ix_pfw_channel_execution_policy_lookup (standard_execution_id, original_channel_code, caller_channel_code, request_type, active_yn),
    INDEX ix_pfw_channel_execution_policy_effective (active_yn, effective_from, effective_to),
    CONSTRAINT fk_pfw_channel_execution_policy_original FOREIGN KEY (original_channel_code) REFERENCES pfw_channel_registry(channel_code),
    CONSTRAINT fk_pfw_channel_execution_policy_caller FOREIGN KEY (caller_channel_code) REFERENCES pfw_channel_registry(channel_code),
    CONSTRAINT ck_pfw_channel_execution_policy_execution CHECK (standard_execution_id = '*' OR standard_execution_id REGEXP '^[OSB][A-Z]{3}[A-Z0-9]{2}[0-9]{4}$'),
    ...
    CONSTRAINT ck_pfw_channel_execution_policy_period CHECK (effective_from IS NULL OR effective_to IS NULL OR effective_from <= effective_to)
...
) VALUES ('PFW.DEFAULT', '*', 'ANY', 'ANY', '*', 'Y', 'N', 'N', 0, NULL, NULL, 'Y', 0, 'SYSTEM', 'SYSTEM')
ON DUPLICATE KEY UPDATE
