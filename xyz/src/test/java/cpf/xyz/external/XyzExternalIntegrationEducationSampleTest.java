...
class XyzExternalIntegrationEducationSampleTest {
    ...
    void 업무입력만전달하고복원력정책은중앙기본값을사용한다() {
        ...
        when(webClient.get(eq("XYZ-EXTERNAL-SIMULATOR"), any(Function.class), eq(Map.class)))
                .thenReturn(Map.of("status", 200));

        Map<String, Object> result = new XyzExternalIntegrationEducationSample(webClient)
                .call("ORDER-1", 200, 0);

        verify(webClient).get(eq("XYZ-EXTERNAL-SIMULATOR"), uriFunction.capture(), eq(Map.class));
